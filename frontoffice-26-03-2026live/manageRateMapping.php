...
if ($_REQUEST['action'] == 'change') {
    ...
    if ($_REQUEST['activeId'] != '') {
        ...
        checkUserLevelPermission($_SESSION['userLevel'], TBL_RATE_MAPPING, 'activate');

        $statusId = addslashes(encryptor('decrypt', $_REQUEST['activeId']));

        $statusSql = "	UPDATE `" . TBL_RATE_MAPPING . "`
        ...
        checkUserLevelPermission($_SESSION['userLevel'], TBL_RATE_MAPPING, 'deactivate');
    ...
    checkUserLevelPermission($_SESSION['userLevel'], TBL_RATE_MAPPING, 'delete');

    $delSql = "DELETE FROM `" . TBL_RATE_MAPPING . "` WHERE `id` = '" . addslashes(encryptor('decrypt', $_REQUEST['delId'])) . "'";

    $sqlDelUserLevel = selectRow(TBL_RATE_MAPPING, " WHERE `id` = '" . addslashes(encryptor('decrypt', $_REQUEST['delId'])) . "'");
...
if ($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])) {
    ...
    $delSql = "DELETE FROM `" . TBL_RATE_MAPPING . "` WHERE `id` IN (" . addslashes($deleteIds) . ")";
...
$sql = " SELECT * FROM `" . TBL_RATE_MAPPING . "` WHERE `id_shop` = '" . addslashes($_SESSION['shop']) . "'";

if ($_REQUEST['rate_id'] != '') {

    $sql .= " AND `rate_id` = '" . addslashes($_REQUEST['rate_id']) . "%'";

}
if ($_REQUEST['hotel_id'] != '') {

    $sql .= " AND `id_hotel` = '" . addslashes($_REQUEST['hotel_id']) . "%'";
        ...
        <div class="box box-default">
            ...
            <div class="box-header with-border">
                ...
                <div class="btn-group  pull-right">
                    <a type="button" class="btn btn-success" href="editRateMapping.php"> Add Rate Mapping</a>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                $resCat = selectSql(TBL_RATE_PLAN, " where status='1' and `id_shop` = '" . addslashes($_SESSION['shop']) . "'", ' ORDER BY `id`');
...
<tr>
  ...
  <td><?php echo selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".$row->id_hotel."'");   ?>, <?php echo selectColumn(TBL_HOTELS,'city'," WHERE `id` = '".$row->id_hotel."'");   ?></td>
  <td><?php echo selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = '".$row->id_room."'");   ?></td>


  

  <td><?php echo selectColumn(TBL_RATE_PLAN,'name'," WHERE `id` = '".$row->rate_id."'");   ?></td>

  <td><?=$row->booking_engine_id;?></td>				 

  <td><?=$row->status=='1'?'<span onclick="location.href=\'manageRateMapping.php?inactiveId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageRateMapping.php?activeId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>			 

  <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editRateMapping.php?eId=<?=encryptor('encrypt',$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="../images/delete.gif" style="cursor:pointer;" title="Delete" onClick="if(confirm('Are you sure that you want to delete this record <?=$row->name;?>?')){window.location.href='manageRateMapping.php?delId=<?=encryptor('encrypt',$row->id)?>&action=delete&page=<?=$_REQUEST['page']?>';}"/></td>
