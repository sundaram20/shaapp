...
if (!isset($_REQUEST["datefilter"]) && !isset($_REQUEST["search_name"]) && !isset($_REQUEST["status"])) {
...
if (isset($_REQUEST["datefilter"]) && $_REQUEST["datefilter"] != "") {
...
if (isset($_REQUEST["search_name"]) && $_REQUEST["search_name"] != "") {
  $searchDocumentType = " AND `mdoc_no` LIKE '%" . addslashes(trim($_REQUEST["search_name"])) . "%'";
}

if (isset($_REQUEST["folio_no"]) && $_REQUEST["folio_no"] != "") {
  $searchfolio_no = " AND `mdoc_no` LIKE '%" . addslashes(trim($_REQUEST["folio_no"])) . "%'";
}
...
echo $SQL="SELECT * from fo_folio WHERE id!=0 ". $statuscase . $dateCondition . $searchDocumentType . $searchfolio_no ." order by id desc";



//$SQL="SELECT * from fo_folio WHERE id!=0 and folio_status='1' and doc_no>0". $statuscase . $dateCondition . $searchDocumentType ." order by id desc";
//echo $SQL;
...
$i=1;
          ...
          <div class="box-body">
            <div class="row">
              <div class="col-md-2 col-sm-6 col-xs-6">
                  ...
                  <input type="text" name="folio_no" id="folio_no" value="<?php echo trim($_REQUEST['folio_no']);?>" class="form-control" />
                </div>
				  </div> 
				     <div class="col-md-2 col-sm-6 col-xs-6">
				  <div class="form-group">
 ...
 //$resCat = selectSql("pos_purch_details WHERE qty-adj_qty>0 AND id_pos_purch  IN  (SELECT id FROM pos_purch WHERE pos_bill_type= '1')");
                    ...
                    <tr>
					 ...
					 <?php 
                      ...
                      <td><?php echo   selectColumn(FO_BILL,'mdoc_no'," WHERE `id` = '".$row->id_fo_bill."'"); ?></td> 
                      <?php
                   ...
                   //  echo 'id_mst_guest','WHERE id_fo_folio_to="'.$row->id_fo_folio_to.'"';
                      	...
                      	$SQL2 = "select *  from ".TBL_GUEST." where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and  `id` = '".addslashes($row->id_mst_guest)."' ";
                      	//echo $SQL;
 ...
 $id_resevation	=  selectColumn(FO_BILL,'id_reservations'," WHERE `id` = '".$row->id_fo_bill."'");
		?>
                       <td><?php echo  $GuestName; ?></td> 
                       <td><?php echo $checkin_date= date('d-m-Y',strtotime(selectColumn(FO_RESERVATIONS,'checkin','WHERE id="'.$id_resevation.'"')));//date('d-m-Y',strtotime($row->date_created));  ?></td> 
                       <td><?php echo $checkout_date=  date('d-m-Y',strtotime(selectColumn(FO_RESERVATIONS,'checkout','WHERE id="'.$id_resevation.'"')))//date('d-m-Y',strtotime($row->checkout_date));  ?></td> 

                      <td> 

                      <?php
                      $frontbillprint = selectColumn('mst_shops','folio_url'," WHERE `id` = '".addslashes($_SESSION['shop'])."'");
                      if($frontbillprint==''){
                     ...
                     <a target="_blank" href="<?= $frontbillprint ?>?idfobill=<?=encryptor(encrypt, $row->id_fo_bill);?>&id_folio=<?=encryptor(encrypt, $row->id);?>&id_mst_room_no_allocation=<?=encryptor(encrypt, $id_room);?>&submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>" ><img src="../images/preview.png" style="cursor:pointer;height:20px;" title="Page Preview "  /></a>&nbsp;&nbsp;
