...
function frontofficeSalesRegisterReport($Date,$id_outlet,$id_shift,$objPHPExcel,$connNew,$id_shop,$cronSet,$pdfName){
...
if ($payment_remarks == 'UPI') {
            $payment_remarks = 'UPI';//selectColumn(MST_COMPANY, 'name', "WHERE id='{$rowPay->id_company}' AND id_shop='{$id_shop}'").'- UPI';
...
foreach ($ArrayOfFoBill as $billData){
	...
	$reservation_query_sql ="SELECT 
    ROUND((tax_per_day_per_room / tariff_price_per_day_per_room * 100), 2) AS tax_percent,
    COUNT(*) AS rows_count,
    SUM(tax_per_day_per_room) AS total_tax,
    SUM(tariff_price_per_day_per_room) AS total_tariff, fo_reservations_details.*
...
GROUP BY 
    ROUND((tax_per_day_per_room / tariff_price_per_day_per_room * 100), 2)
ORDER BY 
    tax_percent;
";
	...
	$reservation_query = mysqli_query($connNew, $reservation_query_sql);
...
while ($reservation = mysqli_fetch_object($reservation_query)) {
    ...
    $percentage =$reservation->tax_percent;//round(($reservation->total_tax ?? 0) / ($reservation->total_tariff ?? 0) * 100);
    ...
    $reservation_percentage[] = $percentage;
				...
				$taxMethod_sgst='Tariff Sales'.$percentage;	
	...
	$SalesRegisterArray['Sales Register'][$id_fo_folio_to][$matchedMdocNo][$reservation_id_mst_charges_sales_local][$taxMethod_sgst]['outlet_name']='Front Office';
				...
				$Account_Name = 'Output SGST @ '.($percentage / 2).'%' ;//.$reservation->total_tax.'=='.$reservation->total_tariff ;
...
while ($reservation = mysqli_fetch_object($reservation_query)) {
	...
	$SalesRegisterArray['Sales Register'][$id_fo_folio_to][$matchedMdocNo][$reservation_id_mst_charges_sales_local][$taxMethod_sgst]['outlet_name']='Front Office';
				...
				$percentage_sgst	=round($percentage > 0 ? ($percentage / 2) : 0);
				$percentage_cgst	=round($percentage > 0 ? ($percentage / 2) : 0);
				$taxMethod_sgst='charges_sgst'.($percentage / 2);	
				$reservation_id_mst_charges_sales_local=$id_reservations;//'charges_sgst_1';						
				$Account_Name = 'Output SGST @ '.customRound($percentage / 2).'%';
	...
	$SalesRegisterArray['Sales Register'][$id_fo_folio_to][$matchedMdocNo][$reservation_id_mst_charges_sales_local][$taxMethod_sgst]['outlet_name']='Front Office';
				...
				$percentage_cgst	=round($percentage > 0 ? ($percentage / 2) : 0);
				$taxMethod_cgst='charges_cgst'.($percentage / 2);	
				$reservation_id_mst_charges_sales_local=$id_reservations;//'charges_cgst_1';						
				$Account_Name = 'Output CGST @ '.customRound($percentage / 2).'%';
				$tax_per_day_per_room_sgst	=($reservation->tax_per_day_per_room ?? 0) / 2;
...
$keys = array_keys($array, $searchObject);
	...
	$SalesRegisterArray['Sales Register'][$id_fo_folio_to][$matchedMdocNo][$RecordsSalesReport->id_mst_charges_sales_local][$taxMethod_sales_local]['cramount']=$RecordsSalesReport->sub_item_amount;
