...
$sql = " SELECT `fo_reservations`.*, `fo_reservations_details`.id as id_reservations_details ,`fo_reservations_details`.id_mst_room_types ,`fo_reservations_details`.id_fo_rate_plan,
  ...
  ,fo_reservations.net_booking_amount ,fo_reservations.res_internal_remarks   FROM `fo_reservations`  
 ...
 FROM `fo_reservations_details`  where   fo_reservations_details.id_fo_reservations='.$row->id.'". $condSum;
		...
		//foreach($datewise_array as $checkinDatearr){			
			...
			if($hk_statusCondtion==0){
				...
				$hk_statusCondtionValue=0;
					...
					$datawisearrayFinal[$checkinDatearr][$row->id_fo_reservations]["sub_adults"]+=$row->adults*$row->room_quantity;
					
					$datawisearrayFinal[$checkinDatearr][$row->id_fo_reservations]["sub_total_products"]+=$row->room_quantity;     
	...
	$datawisearrayFinal[$checkinDatearr][$row->id_fo_reservations]['BookingDeatils'][$row->id_mst_room_types][$FHstatusGroup][$row->adults]["tariff_price_per_day_per_room"]+=$row->tariff_price_per_day_per_room+$row->tax_per_day_per_room;
			...
			$hkno_showoff=$row->no_showoff;
					...
					$BookingStatusArray[$checkinDatearr]['Plan List'][$planname]['PlanCount'] +=$row->room_quantity;
					...
					$BookingperiodArray[$periodRangeHeading][$planname]['PlanCount'] +=$row->room_quantity;
					...
					foreach($dateData as $dateData1){
						...
						$content .= '<tr style="font-weight:bold;">
                            <td style="text-align: center;background-color:#eff4e3" >'.$Sno++.'</td>
							  <td style="text-align: center;background-color:#eff4e3">'.$dateData1['booking_no'].'</td>
                            <td style="text-align: center;background-color:#eff4e3">'.$dateData1['reference'].'</td>
                            ...
                            <td style="text-align: center;background-color:#eff4e3;">'.$dateData1['sub_adults'].'</td>
                            <td style="text-align: center;background-color:#eff4e3">'.$dateData1['sub_total_products'].'</td>
...
//debugdata($order_data);
 ...
 <td style="text-align: center;"></td>
                           <td></td>
                            ...
                            <td  style="">Nights ('.$order_data['no_of_days'].')</td>
                            
                            <td  style="">Adults ('.$order_data['adults'].')</td>
                            <td  >Rooms ('.$order_data['total_products'].')</td>
                            <td></td>
                           ...
                           <td></td> 
                             ...
                             <td  colspan="5"><b>Tariff: INR </b>'.$order_data['tariff_price_per_day_per_room'];
						
								
								}
					}
					} 
					  $content .= ' <br/><b> Total Payable amount: INR </b>'.$dateData1['net_booking_amount'];
                         
                          if($dateData1['res_internal_remarks']!=''){ 
                        $contentSpecialInstructions= ' <br/><b> Special Instructions: </b>'.$dateData1['res_internal_remarks'];    
						  }else{
