...
function frontofficeSalesRegisterReportTally2($Date,$id_outlet,$id_shift,$objPHPExcel,$connNew,$id_shop,$cronSet,$pdfName){
	//echo $id_shift;die;
	global $connNew;
	global $objPHPExcel;
...
while ($reservation = mysqli_fetch_object($reservation_query)) {
    ...
    $reservation_percentage[] = $percentage;
				...
				$Account_Name = 'ROOM TARIFF INCOME 5%';
...
while ($reservation = mysqli_fetch_object($reservation_query)) {
	...
	$reservation->id_fo_bill =$k;
				...
				$Account_Name = selectColumn(TBL_CHARGES,'name','WHERE id="'.$reservation->id_mst_charges.'"  ');
