...
//Popup Table Show  
 ...
 $sql = "SELECT * FROM inv_items WHERE $where"; 
                        ...
                        $numRows= $db->num_rows(); 
                        //Table Section
                        $table = '<br><h4 style="padding-left:38%;">'.$companyname.'</h4><p  style="padding-left:30%;"><b>Stock Ledger</b> From '.date('d-m-Y' , strtotime(addslashes($minimum_date))). ' To '.date('d-m-Y' , strtotime(addslashes($maximum_date))).'</p>';
                        $i=1;                                           
                        ...
                        $mpdf->Output('../pdf/Stock_Ledger.pdf','F');
                      
                        //Excel Sheet Work 
                         $file_pointer = fopen("../excel/Stock_Ledger.csv","w");
...
?> 
<a href="../excel/Stock_Ledger.csv"><button class="btn btn-danger btn-sm btn-labeled" id="download" type="button">
