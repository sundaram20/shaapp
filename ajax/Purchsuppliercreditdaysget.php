...
//echo "SELECT inv_purch.*, inv_purch_details.bal_qty FROM inv_purch LEFT JOIN inv_purch_details ON inv_purch_details.id_inv_purch = inv_purch.id WHERE inv_purch.id_shop = '".addslashes($_SESSION['shop'])."' AND inv_purch.id_mst_party_supplier = '".$id_mst_party_supplier."' AND inv_purch.doc_type='4' AND inv_purch_details.bal_qty>0 ";	
		
		
 //   $sql2 = " SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `id_mst_party_supplier`='".$id_mst_party_supplier."' AND `doc_type`='4' ";


$sql2 = "SELECT inv_purch.*, inv_purch_details.bal_qty FROM inv_purch LEFT JOIN inv_purch_details ON inv_purch_details.id_inv_purch = inv_purch.id WHERE inv_purch.id_shop = '".addslashes($_SESSION['shop'])."' AND inv_purch.id_mst_party_supplier = '".$id_mst_party_supplier."' AND inv_purch.doc_type='4' AND inv_purch_details.bal_qty>0 GROUP BY inv_purch_details.id_inv_purch";
