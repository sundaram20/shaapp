...
//echo "SELECT inv_po.id as id_po, inv_po_details.* FROM  inv_po, inv_po_details WHERE $where";
 $sql = "SELECT inv_po.id as id_po, inv_po_details.* FROM  inv_po, inv_po_details WHERE $where";
	...
	$sql1 = "SELECT * FROM ".TBL_INV_OTHERS_CHARGES." WHERE id_inv_po IN (".$id.") ";
		...
		$ot_type = rtrim($ot_type1,',');							
	                			...
	                			$pono  =  selectColumn('inv_po','doc_no'," WHERE `id` = '".addslashes($row22->id_inv_po)."'");
	                			$podate1  =  selectColumn('inv_po','doc_date'," WHERE `id` = '".addslashes($row22->id_inv_po)."'");
		                    	...
		                    	if($row22->id_mst_attributes_store == ''){
		                    		//Item Table
		                    		//echo "select id_mst_attributes_store from inv_items where `id_shop` = '".addslashes($_SESSION['shop'])."' AND `id` = '".addslashes($row22->id_inv_items)."'";
