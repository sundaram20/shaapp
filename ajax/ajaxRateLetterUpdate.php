...
if($_POST['Save']){		
	...
	$inclusionDetail = json_encode(array_combine($_POST['inclusion_id'],$_POST['inclusion_detail']));
		...
		if(($_POST['Save'] == 'Add') && empty($_POST['id'])){//add
			...
			$checkExisting = executeSql("SELECT * FROM `".TBL_RATE."` as a join `".TBL_RATE_DETAILS."` as b where a.company_id='".addslashes($_POST['company_id'])."' and a.rate_level_id='".addslashes($_POST['new_rate_level_id'])."' and a.seasonId='".addslashes($_POST['seasonId'])."' and a.market='".addslashes($_POST['market'])."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND a.id=b.rate_id AND b.rate_plan_id='".addslashes($_POST['rate_plan_id'][$i])."'");
...
$checkRatePlanExisting = executeSql("SELECT * FROM `".TBL_RATE."` as a where a.company_id='".addslashes($_POST['company_id'])."' and a.rate_level_id='".addslashes($_POST['new_rate_level_id'])."' and a.seasonId='".addslashes($_POST['seasonId'])."' and a.market='".addslashes($_POST['market'])."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."' ");
	 ...
	 $rate_id	=	$rowRatePlanExisting->id;
			...
			if(num_rows($checkRateAssingExisting)>0){
				...
				$end_date	=	selectColumn(TBL_RATE_SEASON,'end_date'," WHERE `id` = '".$_REQUEST['seasonId']."'");	
							...
							echo '<p class="help-block">New Rate details has been added sucessfully.</p><script>window.setTimeout(function() { window.location.href = "editRateLetters.php?hotelId='.encryptor(encrypt,($_POST['hotelId'])).'&id='.encryptor(encrypt,($rate_id)).'&action=edit&page=1";}, 2000); </script>';
							unset($_POST);
			/*	}else{
					$err++;
...
if(num_rows($checkExistingRateDetailRoomWise1) == 0){
			...
			/*			}else{	
				...
				echo '<p class="help-block">'.selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".$_REQUEST['hotelId']."'").' rate has been updated sucessfully.</p><script>window.setTimeout(function() {window.location.href = "editRateLetters.php?id='.$_POST[id].'&hotelId='.addslashes(encryptor(encrypt,$_POST[hotelId])).'&action=edit&page=1";}, 2000);</script>';
