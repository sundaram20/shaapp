...
if($checkbox){ 
	$where = " inv_purch_details.id_inv_purch = inv_purch.id AND inv_purch.id_mst_party_supplier ='".$id_mst_party_supplier."' AND inv_purch.id_shop = '".addslashes($_SESSION['shop'])."' AND inv_purch_details.doc_type = '1' " ;
}else{
	$where = " inv_purch_details.id_inv_purch = inv_purch.id AND inv_purch.id_mst_party_supplier ='".$id_mst_party_supplier."' AND inv_purch.id_shop = '".addslashes($_SESSION['shop'])."' AND inv_purch_details.bal_qty>0  AND inv_purch_details.id_inv_purch = '".$id."' AND inv_purch_details.doc_type = '1'  " ;
}
 //$sql = "SELECT * FROM  `".TBL_INV_PURCH_DETAILS."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND `id_inv_purch` = '".$id."' AND `doc_type` = '1' ";
 $sql = "SELECT inv_purch.id as id_purch, inv_purch_details.* FROM  inv_purch, inv_purch_details WHERE $where ";
		                   	...
		                   	while($row22 = $db->fetch_object()){  
	                			...
	                			$po_date  =  selectColumn('inv_purch','po_date'," WHERE  `id` = '".addslashes($row22->id_inv_purch)."'");
								...
								if($type == 0){ 
		                   		$table .= ' <tr class="table-row" style="text-align: center;">
			                    		...
			                    		<td> 
											...
											<input type="text" class="form-control" id="wpop1" name="wpop1"  value="'.$row22->id_inv_purch.'" style="display: none;">
