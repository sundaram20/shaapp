...
if($_REQUEST['updateid']!=''){
	...
	$updateSql = mysqli_query($connNew,"SELECT * FROM pos_purch WHERE  id= '".encryptor(decrypt,$_REQUEST['updateid'])."'");
...
<!-- End Audit trail Modal -->
 ...
 <?php 
 	$array_s =  explode(",", $row1->id_inv_details_split);
...
<?php if($_REQUEST['updateid']!= ''){ ?>
	$('document').ready(function(){
		...
		var vals = '<?php echo $row1->id_inv_details_split; ?>';
		 setTimeout(() => {
...
function getItemRate(id,rowCount){
	...
	if(vat1 == null){ var vat =  data.Tax_vat; }else{ var vat =  $("#TotalTax_vat").val(); }
	if(surcharge1 == null){ var surcharge =  data.Tax_surcharge; }else{ var surcharge =  $("#TotalTax_surcharge").val(); }
	...
	var itemamount    = $("#item_amount"+rowCount).val();
	
	var itemsgst      = data.sgst;
...
function additionalDiscount(type,discountamount){
	...
	$.ajax({
		...
		success: function (result) {
			...
			var RoundOfAmount	=	 parseFloat(netamount_final)-parseFloat(netamount);
			///var RoundOfAmount	=	 parseFloat(netamount);
...
/*		
 function discount(discount){
	...
	$.ajax({
		type: "POST",
		url: 'ajax/ajaxGetLaundrySpaOrderItemList.php',
		data: 'discountnew='+discount+'&DbConnect=1',
		success: function (result) {
			//$( "#ViewOrderItemList" ).html(result);
			$( "#discountnew" ).val(discount);
			//alert(idjs);
	 	}
	});
}
