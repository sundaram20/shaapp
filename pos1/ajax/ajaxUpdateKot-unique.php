	...
	/*echo $UniqueCodeGen=$_REQUEST['UniqueCodeGen'];
	echo '<pre>';
	print_r($_REQUEST);
	...
	print_r($id_subsection);*/
	 ...
	 $sql = "SELECT * FROM `".TBL_DOC_TYPE_CONFIG."` WHERE `doc_type`='22' ";
		...
		while ($rowdoc = mysqli_fetch_object($resToPrint)){
			$idDocConfigDetail= $rowdoc->id;
		
		$doc_detail = "SELECT * FROM `".TBL_DOC_TYPE_CONFIG_DETAIL."` WHERE `id_mst_doc_type_config` IN (".$idDocConfigDetail.") ";
		$resToPrint1= mysqli_query($connNew,$doc_detail);
		$rowdoc1 =  mysqli_fetch_object($resToPrint1);
			 $id_subsection1 = $rowdoc1->id_subsection; 
	} 

	$doc_type_kot;

	foreach($_SESSION['POSKOT'][$UniqueCodeGen]['itemID'] as $uniqueCode =>$dataCode){
		 $id_subsection	=	$_SESSION['POSKOT'][$UniqueCodeGen]['id_outlet'][$uniqueCode];
	}
	// $id_subsection = 2;
	...
	foreach($_SESSION['POSKOT'][$UniqueCodeGen]['itemID'] as $uniqueCode =>$dataCode){
	 ...
	 /* $auditquery1 = "SELECT * From `".TBL_PURCH_DETAILS."` WHERE id_pos_purch = '".$pos_purch_id."'  ";
	  $auditresSQL1 = mysqli_query($connNew, $auditquery1);	
		while($auditrow1 = mysqli_fetch_object($auditresSQL1)){
		   $qty = $auditrow1 -> qty;
		if($qty != $_SESSION['POSKOT']['quantity'][$uniqueCode]){
			 $chh1 ="Quantity Details Changed from " .  $qty." - to - ".$_SESSION['POSKOT']['quantity'][$uniqueCode];
		}
	    ...
	    $insertPosKotDetail = "INSERT INTO `".TBL_PURCH_DETAILS."` SET 
							  ...
							  `id_mst_items`='".addslashes($_SESSION['POSKOT'][$UniqueCodeGen]['itemID'][$uniqueCode])."',
