...
if($posOuletResult->registered_office_address!='' && $RecordCount=='1'){
	...
	foreach ($details['item_id'] as $id_index => $id_item) {
			...
			$printer.=('<span style="">'.str_pad($printgroup[$index]['service_charge_display_alias_name'].":",25," ").'</span>');	
			...
			$printer.=("<br>");
				...
				//$printer.=(" <div style='  margin-top: 20px;float: right;width:265px;white-space:break-spaces;text-align: center;'>* THANK YOU, PLEASE VISIT AGAIN *</div>");
...
if($posOuletResult->registered_office_address!='' && $RecordCount=='1'){
	//$printer.=("<br>");
		...
		$printer.=("<div style=''><div style='text-align:left;font-size:13px;'>");
		...
		foreach($TotalArray as $totalData){
			...
			$printer.=(str_pad('',4," "));
			$printer.=("<span style='float:right;'>".trim(number_format((float)$totalData['total'], 2, '.', ''))."</span>");
			$printer.=("<br>");
		}
		$printer.=("-------------------------------------<br>");
		...
		$printer.=("</div></div>");
		//$printer.=("---------------------------------------");
		$printer.=("<div style='margin-left:69px;font-weight:bold;font-size:13px;'>");
			//$printer.=(str_pad("",5," "));
			//$printer.=(str_pad('Grand Total:',1," "));
			$printer.=("<br><span style='float:left;font-size:17px;'><b>Grand Total:</span>");
			//$printer.=(str_pad('',0," "));
		//$printer.=(trim(str_pad(round($GrandTotal,0),0," ")));
			$printer.=("<span style='float:right;font-size:17px;'>".str_pad(round($GrandTotal,0),5," ").'</b></span>');
			$printer.=("</div>");
