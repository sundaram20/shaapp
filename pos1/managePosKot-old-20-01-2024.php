...
<style>
	.discountvalue{border-radius:1px;width:10px!important;float: left;padding:0px!important;text-align:center;height: 17px!important;display:flex;align-items:center;
...
z-index: 1;
    ...
    <section class="content-header"> 
        ...
        <div class="col-md-5 col-xs-12 dd-f">
         ...
         <div class="btn-group"> <a type="button"  title="List KOT" class="btn n-btn pull-right" href="manageKot.php?submenu=178&session=22" > <i class="fas fa-list"></i> KOT</a> </div>
            ...
            <div class="btn-group"> <a type="button" onClick="AddPosGuest();" title="Add POS Guest" class="btn n-btn pull-right"> <i class="fas fa-plus"></i> POS Guest </a></div>
    ...
    <form name="FormPosKot" id="FormPosKot" action="kotbilling.php?submenu=177" method="post">
        ...
        <input type="hidden" id="enable_nationality_show" name="enable_nationality_show" value="<?php echo $enable_nationality;?>">
        <input type="hidden" value="<?php echo $enable_steward_passcode;?>" name="enable_steward_passcode" id="enable_steward_passcode">
...
<!-- Modal -->
<div class="modal" id="tableModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div id="myDIV">
            <div class="row">
                ...
                <div class="col-sm-6 col-md-3 box-header with-border paxbox mobpax" style="display:hnone;">
                <!--<h5 class="box-title">Main Group </h5>-->
              
              
                <label for="name"  title="Table"><i class="fas fa-chair"></i> : <span id="ViewSelectedTable2" style="color:#FF0000"></span></label>
              
                <label for="name" title="No of Pax">  <i class="fa fa-users"></i> : <span id="ViewSelectedPax2" style="color:#FF0000" ></span></label>
               
                <label for="name"  title="Steward Name"><i class="fa-solid fa-person"></i> : <span id="ViewSteward2" style="color:#FF0000"></span></label>
...
<?php       
		 if($enable_nationality=='1'){ ?>            
              <div class="col-sm-6 col-md-3 box-header with-border paxbox paxbox2" style="display:hnone;">
             <div class="" id="MyNationalitySelect" tyle="" >
                <div class="form-group mb-0">
              
                <div class="box-body table-responsive border-none" style="padding-left: 0px;padding-bottom: 0px;padding-top: 0px;padding-right: 4px;">
                    <div style="display:flex;">
                      <label for="name"  style="display:flex;" title="Nationality"><i class="fa-solid fa-flag"></i> : </label><table id="" class="table table-fixedTableGroup table-striped table-bordered dataTable no-footer" cellspacing="0" >
...
echo $categoryDropDownNat ='<tr><td><a href="#" class="mstcountrylang '.$ClassNameNat.'" name="id_mst_country_lang" id="'.$resultCatNat->id_country.'_'.$resultCatNat->name.'" type="button" value="'.ucfirst($resultCatNat->nationality).'"  onclick="SelectNationality(this.id);reset();" >'.ucfirst($resultCatNat->nationality).'</a></td></tr>';
         ...
         <?php } ?>
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 mt-10" >
                <div class="form-group mb-0">
                <label class="tablegrouplabel" for="name">Table<br> Group<font color="#FF0000">*</font> </label>
...
$resCat = selectSql(TBL_ATTRIBUTES,"where id_shop='".$_SESSION['shop']."'  and status = '1' AND table_name ='".'table_group'."' ",' ORDER BY `display_order` asc limit 0,1');
...

  {
	 ...
	 ?>
                            <td  class="btn tableviewBlockedbtn" onclick="TabeleSelect(this.id); PreviousOrderPax(this.id,<?php echo '\''.$UniqueCodeGen.'\''?>);PreviousOrder(this.id,<?php echo '\''.$UniqueCodeGen.'\''?>);ajaxRemoveAllItemList('removeAll',<?php echo '\''.$UniqueCodeGen.'\''?>);" id="TableGroup_<?php echo $rowContact->id ?>_<?php echo $rowContact->field_value;?>"><?php echo $rowContact->field_value;?></td>
  ...
  { ?>
                            <td style="hi2" class="btn tableviewbtn" onclick="TabeleSelect(this.id); PreviousOrderPax(this.id,<?php echo '\''.$UniqueCodeGen.'\''?>);PreviousOrder(this.id,<?php echo '\''.$UniqueCodeGen.'\''?>);ajaxRemoveAllItemList('removeAll',<?php echo '\''.$UniqueCodeGen.'\''?>);" id="TableGroup_<?php echo $rowContact->id ?>_<?php echo $rowContact->field_value;?>"><?php echo $rowContact->field_value;?></td>
                            <?php }
			...
			$categoryDropDown = '<select class="form-control select2" name="id_attribute_shift" data-parsley-required data-parsley-errors-container="#id_shiftError">
              ...
              <div class="col-md-11">
                <div class="form-group" style="margin-bottom: 0px !important;">
                    ...
                    <div class="box-body table-responsive" style="padding: 0px;">
                    <div >
                      <input type="password" class="form-group col-md-2 col-sm-2" style="
    padding: 10px 20px;
    ...
    font-size: 1.6rem!important;
}" value="<?php echo $enable_steward_passcode_value;?>" name="enable_steward_passcode_value" id="enable_steward_passcode_value" onkeyup="ValidatePasscode(this.value);">
						<span id="Msgpasscode_value" style="margin:25px;"></span>
                    </div>
                    </div>
                    </div></div>
                    <?php } ?>
              <!------------------------>
                  ...
                  <button type="button" style="padding:10px;" class="btn c-btn btn-default" data-dismiss="modal">Close</button>
   ...
   <!--Add Items Remarks Modal Popups Starts-->           
    <div class="modal" id="itemModalRemarks" class="itemModalRemarks" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
           <div class="modal-content">
              <div class="modal-header" style="background-color:#172635; color: #fff;text-align: center;">
                ...
                <div class="modal-body">
                    <div id="myDIV">
                        <div class="">
                            ...
                            <form method="post" role="form"  enctype="multipart/form-data">
                               <input type="hidden" name="specialRequestUniqueCode" id="specialRequestUniqueCode" value="" />
                               <input type="hidden" name="UniqueCodeGen" id="UniqueCodeGen" value="" />
                                <input type="text" class="form-control"  name="special_request_name" id="special_request_name" value="">
                              </form>
                            </div>
                        </div>   
                    </div>    
                  ...
                  <div class="modal-footer" style="background-color: #e4e4e4;color: #fff;text-align:center">
                   ...
                   <button type="button" class="btn c-btn" data-dismiss="modal"  onclick="ajaxAddSpecialRequest();"> <i class="far fa-save"></i> Add</button> 
...
<?php } ?>
           ...
           <div class="hr-box m-0">
              ...
              <div class="grouptitle maintext">Main
              </div>
            </div>  
            ...
            <input type="hidden" name="id_subgroup" id="id_subgroup" value="">
                ...
                <?php 
		         ...
		         echo '  <input name="selectmaingroup" id="selectmaingroup_'.$resultCat->id.'" type="button" class="mainmenu_btn activeset" value="All"  onclick="getsubgrouplist(this.id,\''.$UniqueCodeGen.'\');" style="margin-bottom:5px;">';
	                 ...
	                 $array_id_mst_attributes_group_main=array();
	                 while($row2 = $db->fetch_object2($resCatMain)){ 
					 $array_id_mst_attributes_group_main[] = $row2->id_mst_attributes_group_main;
					 
					 }
$array_id_mst_attributes_group_main = implode(',',$array_id_mst_attributes_group_main);
						  $resCat = selectSql(TBL_ATTRIBUTES,"where id_shop='".$_SESSION['shop']."'  and status = '1' and `table_name` = 'item_group_main' and field_value!='Laundry' and field_value!='Spa And Health Club' and id IN (".$array_id_mst_attributes_group_main.") ",' ORDER BY display_order ASC');
		    ...
		    echo '<input name="selectmaingroup" id="selectmaingroup_'.$resultCat->id.'" type="button" class="mainmenu_btn"  title="'.ucfirst($resultCat->field_value).'" value="'.ucfirst($resultCat->field_value).'"  onclick="getsubgrouplist(this.id,\''.$UniqueCodeGen.'\');" style="margin-bottom:5px;">';
        ...
        <div id="listsubgroup">
            ...
            <div class="form-group mb-0">
                <div class="box-body main2-boxbody table-responsive" style="padding-top: 10px;padding-left: 1px;padding-right: 5px;">
	  ...
	  $array_subGroup_id_mst_attributes_group_sub=array();
	  while($row = $db->fetch_object2($resCat)){ 
			$array_subGroup_id_mst_attributes_group_sub[]	=$row->id_mst_attributes_group_sub;
	  }
$array_subGroup_id_mst_attributes_group_sub = implode(',',$array_subGroup_id_mst_attributes_group_sub);


	  	

	  $SqlAttrbute = selectSql(TBL_ATTRIBUTES,"where id_shop='".$_SESSION['shop']."'  and status = '1' and `table_name` = 'item_group_sub' AND id IN(".$array_subGroup_id_mst_attributes_group_sub.") ORDER BY display_order ASC");

	  if($db->num_rows2($SqlAttrbute)){

	 while($resultAttrbute = $db->fetch_object2($SqlAttrbute)){
	  ...
	  echo $subGroup ='<tr><td style="padding:0px;" ><input name="selectitemlist" id="selectitemlist_'.$resultAttrbute->id.'" type="button" class="btn btn-success mainmenu_btn" value="'.ucfirst($resultAttrbute->field_value).'" title="'.ucfirst($resultAttrbute->field_value).'" onclick="getItemlist(this.id,\''.$UniqueCodeGen.'\');" style="margin-bottom:5px;padding: 5px 10px;" ></td></tr>';
            ...
            <div class="form-group">
                ...
                <table id="myTableSecond" class="table table-fixeditem table-striped table-bordered dataTable no-footer" cellspacing="0" >
                    ...
                    <tr>
                         <th class="input-box" title="Search Menu">
                             <input type="text" name="keywordsearch" id="keywordsearch"  placeholder="Search Menu"  onKeyUp="keysearch(this.value,'<?php echo $UniqueCodeGen; ?>')" >
                              <span class="icon">
				...
				?>
                        <td style=""><a name="addItemList" id="<?php echo $row->id;?>"  class="btn mainmenu_btn btn-success" value="<?php echo ucfirst($row->name);?>" title="<?php echo ucfirst($row->name);?>"  onclick="selectsubitem(this.id,'<?php echo $UniqueCodeGen; ?>');" ><?php echo ucfirst($row->name);?></a></td>
                        <?php	
				
				}else{
				?>
                        <td style=""><a name="addItemList" id="addItemList_<?php echo $row->id;?>"  class="btn mainmenu_btn btn-success" value="<?php echo ucfirst($row->name);?>" title="<?php echo ucfirst($row->name);?>"  onclick="AddgetItemlist(this.id,'<?php echo $UniqueCodeGen; ?>');" ><?php echo ucfirst($row->name);?></a></td>
                        <?php	
				}
				?>
                        <?php if($i==1){ $i=1;?>
      ...
      <div class="ordersdiv">   
            <div class="prebtn" style="    margin-left: 16px;">
                 ...
                 <div onclick="myFunction2()" class="btn btn-block  btn-sm" id="HideOpen2" style="display:none;"> Previous Order</div> 
...
<div class="modal fade" id="adminsOfferform" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	$CheckBlockedTable_Sql_1 = "SELECT id_attribute_table,doc_type FROM pos_purch WHERE pos_bill_type='1' AND id IN (SELECT id_pos_purch as posid FROM pos_purch_details WHERE qty-adj_qty>0  AND DATE(date_created)>'".$POSCurrentDate."') and doc_type='".$_SESSION['id_document']."'";
...
.tablegroupbtn, .tablenationalitybtn {
...
.activetablenaitonality, .tablenationalitybtn:hover, {
...
.activetablegroup, .tablegroupbtn:hover ,
.activetablenationality, .tablenationalitybtn:hover {
...
.table-fixeditem thead, .table-fixeditem tbody, .table-fixeditem tr, .table-fixeditem td, .table-fixeditem th {
...
.table-fixeditem tbody td a, .table-fixeditem tbody td input{
...
.table-fixedTableGroup tbody {
 ...
 #MyNumOfPax .table-fixed tbody,.table-fixedsteward tbody{
  height:132px;
 }
...
table.dataTable tbody th, table.dataTable tbody td ,.table>tbody>tr>td,.mainmenu_btn ,#SearchResult input {
...
.table-fixeditem tbody td a,#SearchResult td a,#SearchResult td input,.table-fixedsubmenu td input{
 padding:3px!important;
 font-size:12px;
 display:flex;
 justify-content: center;
...
function ajaxAddSpecialRequest(){
	...
	$("#itemRemarksLabelInput"+specialRequestUniqueCode).html("<input type='hidden' class='form-control' value='"+special_request_name+"'  name='item_special_request|'"+ specialRequestUniqueCode+"  id='item_special_request|'"+ specialRequestUniqueCode+">");
	$.ajax({
	   type: "POST",
	   url: 'ajax/ajaxUpdateSessionSpecialRequest.php',
	   data: 'specialRequestUniqueCode='+specialRequestUniqueCode+'&special_request_name='+special_request_name+'&UniqueCodeGen='+UniqueCodeGen, 
