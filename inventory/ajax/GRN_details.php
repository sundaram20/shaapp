...
if($eId !=''){  
	$where = " inv_purch_details.id_inv_purch = inv_purch.id AND inv_purch.id_mst_party_supplier ='".$id_mst_party_supplier."' AND inv_purch.id_shop = '".addslashes($_SESSION['shop'])."' AND inv_purch_details.id_inv_purch IN ($id)  AND inv_purch_details.doc_type = '4'  " ;
...
if($eId ==''){
 ...
 $sql = "SELECT inv_purch.id as id_purch, inv_purch_details.* FROM  inv_purch, inv_purch_details WHERE $where ";
	 ...
	 $sql = " SELECT * FROM  `".TBL_INV_PURCH_DETAILS."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND qty>0 AND doc_type = '5' AND id_inv_po IN($id) AND `id_inv_purch` = '".addslashes(encryptor(decrypt,$eId))."' ";
	//$sql = "SELECT inv_purch.id as id_purch, inv_purch_details.* FROM  inv_purch, inv_purch_details WHERE $where AND qty>0";
...
if($eId ==''){
	// $sqll = "SELECT * FROM inv_purch_details WHERE  bal_qty>0 AND id_inv_purch IN (".$id.") ";
}else{
	//$sqll = "SELECT * FROM inv_purch_details WHERE qty>0 AND id_inv_purch IN (".$id.") ";
}
...
$sql2 = " SELECT * FROM  `".TBL_INV_PURCH_DETAILS."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND qty>0  AND `id_inv_purch` = '".addslashes(encryptor(decrypt,$eId))."' ";		

$res12 = mysqli_query($connNew,$sql2);
								// $db->query($sql2); 
//while($rowsID = mysqli_fetch_object($res12)){ }
$rowsID = mysqli_fetch_object($res12);
 
 
  
if($eId !=''){ 
	//$qty_value = $rowsID->qty;
	$qty_value = $row->qty;
}else{
...
$doc_date  =  selectColumn('inv_purch','doc_date'," WHERE  `id` = '".addslashes($row->id_inv_purch)."'");
$date_po = date('d-m-Y' , strtotime(addslashes($doc_date)));
 ...
 //echo "SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `id_mst_party_supplier`='".$id_mst_party_supplier."' AND `doc_type`='4' ";
			
$returnData.='<tr id="edittrdelete'.$i.'">';
				
$returnData.= '<td style="width:10%"  class="form-group col-md-2"><select onchange="popupshow(this.id);" name="id_inv_po'.$i.'" id="id_inv_po'.$i.'" class="form-control select2" style="width:100%;">';
...
if($eId ==''){	
	...
	$sqlCharge = "SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `id_mst_party_supplier`='".$id_mst_party_supplier."' AND `doc_type`='4' AND `id` IN (".$row->id_inv_po.")";
...
$resCharge = mysqli_query($connNew,$sqlCharge);
	...
	$id_po = selectColumn(TBL_INV_PO,'id'," WHERE `id` = '".$rowCharge->id_inv_po."'");
	
	$returnData.='<option '.$selected.' value="'.$rowCharge->id.'">'.$rowCharge->doc_no.' | '.date('d-m-Y' , strtotime(addslashes($rowCharge->doc_date))). '</option>';
}

    $returnData.='</select>';
	
$returnData.= '<td style="width:7%"  id="hideshow_item_code"><input type="text"  autocomplete="off"  class="form-control" name="item_code'.$i.'" value='.$item_code.' id="item_code'.$i.'" readonly /></td><td class="form-group col-md-2"><input type="text"  autocomplete="off" class="form-control" name="item_description'.$i.'" value="'.$item_description.'" id="item_description'.$i.'" readonly /><input type="hidden"  autocomplete="off" class="form-control" name="counterby" value="'.$i.'" id="counterby" readonly /></td>';
	
	
$returnData.= '<td style="width:6%"  class="form-group col-md-2"><select class="form-control select2" name="id_mst_attributes_store'.$i.'" id="id_mst_attributes_store'.$i.'" style="width:100%;" >';
...
//	$returnData.= '<td style="width:10%"  class="form-group col-md-1"><input type="text"  autocomplete="off" class="form-control" name="item_remarks'.$i.'" id="item_remarks'.$i.'" placeholder="Remarks" value='.$row->item_remarks.' ></td>';
if($_REQUEST['ledger_id'] == 1){
$returnData.= '<td style="width:6%" class="form-group col-md-2"><select onchange="po_locals(this.id);" class="form-control select2" name="id_mst_charges_purchase_local'.$i.'" id="id_mst_charges_purchase_local'.$i.'" style="width:100%;" >';
...
$returnData.= '<td style="width:6%" class="form-group col-md-2"><select  onchange="po_interstate(this.id)" class="form-control select2" name="id_mst_charges_purchase_interstate'.$i.'" id="id_mst_charges_purchase_interstate'.$i.'" style="width:100%;" >';
...
$returnData.='<td style="width:11%;"><div id="localss'.$i.'" name="localss'.$i.'" >
<div style="color:red;font-size:11px;" id="s_amount'.$i.'">'.$sgst.'</div>
<div style="color:red;font-size:11px;" id="c_amount'.$i.'">'.$cgst.'</div>
</div>
 <div id="interstatess'.$i.'" name="interstatess'.$i.'">
	<span style="color:red;font-size:11px;" id="i_amount'.$i.'">'.$igst.'</span>
</div>
</td>';


	$returnData.= '<td style="width:21%"  class="form-group col-md-1"><input style="width:124px!important;" type="text"  autocomplete="off" class="form-control" name="item_remarks'.$i.'" id="item_remarks'.$i.'" placeholder="Remarks" value='.$row->item_remarks.' ></td>';

if($i>=1){
	$returnData.= '<td><a class="btn n-btn  abtn ibtnDel1" id="deletes'.$i.'" name="deletes'.$i.'" style="cursor:pointer;" title="Delete"><i class="fa fa-trash-o"></i></a></td>';
}
