...
if($eId !=''){  
	$where = " inv_purch_details.id_inv_purch = inv_purch.id AND inv_purch.id_mst_party_supplier ='".$id_mst_party_supplier."' AND inv_purch.id_shop = '".addslashes($_SESSION['shop'])."' AND inv_purch_details.qty>0  AND inv_purch_details.id_inv_purch IN ($id) AND inv_purch_details.doc_type IN (5,12)   " ;
	//$where = " inv_purch_details.id_inv_purch = inv_purch.id AND inv_purch.id_mst_party_supplier ='".$id_mst_party_supplier."' AND inv_purch.id_shop = '".addslashes($_SESSION['shop'])."' AND inv_purch_details.doc_type = '5' " ;
}else{
	$where = " inv_purch_details.id_inv_purch = inv_purch.id AND inv_purch.id_mst_party_supplier ='".$id_mst_party_supplier."' AND inv_purch.id_shop = '".addslashes($_SESSION['shop'])."'  AND inv_purch_details.qty>0 AND inv_purch_details.id_inv_purch IN ($id) AND inv_purch_details.doc_type IN (5,12)   " ;
}
...
if($eId ==''){	
	$sqlCharge = "SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `id_mst_party_supplier`='".$id_mst_party_supplier."' AND `doc_type` IN (5,12) ";
}else{	
	$sqlCharge = "SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `id_mst_party_supplier`='".$id_mst_party_supplier."' AND `doc_type` IN (5,12) AND `id` IN (".$row->id_inv_po.")";
}
