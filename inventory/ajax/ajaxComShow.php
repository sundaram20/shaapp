		   ...
		   //$data2 = mysqli_fetch_object($res2);
			...
			if($data->company_name==""){
				...
				//	echo  '<div class="col-form-label">Address : '.$data->address.' ,'.$data->city.'-'.$data->postcode.' ,'.$data->city.'<br>'.$data->id_mst_attributes_state.'<br> Email : '.$data->email.'<br>Mobile : '.$data->mobile.'<br>GST No : '.$data->gstin.'</div>';
					echo  '<div class="col-form-label">Address : '.$data->address.' ,'.$data->city.'-'.$data->postcode.' ,'.$data->city.'<br>State - '.$resultCat->name.' , '.$categoryDropDown.'<br> Email : '.$data->email.'<br>Mobile : '.$data->mobile.' ';
					 if($data->gstin!=""){echo '<br>GST No : '.$data->gstin; }'</div>';
			}
		}	  
	}
	else{
