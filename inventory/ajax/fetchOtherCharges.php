<?php
	...
	if($id_inv_poo != ''){ 
	 ...
	 $final = $_POST["inv_itemArray"];//rtrim($ids,',');
	 
	 $_REQUEST['id_inv_po']=$_POST["inv_itemArray"];
	 
	if($eId==''){
		 $sqll = "SELECT * FROM inv_po_details WHERE  bal_qty>0 AND id_inv_po IN (".$_REQUEST['id_inv_po'].") ";
	}else{
		 $sqll = "SELECT * FROM inv_po_details WHERE id_inv_po IN (".$_REQUEST['id_inv_po'].") ";
	}
	
	$ress = mysqli_query($connNew,$sqll);
	//echo $num = mysqli_num_rows($ress);
	while($roww = mysqli_fetch_object($ress)){
		$newid .= $roww->id_inv_po.',';
	}
 $finall = rtrim($newid,','); 
	 
	 
	 
		$valuecharges_others=array();
//echo "SELECT * FROM ".TBL_INV_OTHERS_CHARGES." WHERE id_inv_po IN (".$_REQUEST['id_inv_po'].") ";		
		
		if($id_inv_poo != '' && $id_inv_poo != 'undefined'){
			$sql = "SELECT * FROM ".TBL_INV_OTHERS_CHARGES." WHERE id_inv_po IN (".$final.") ";
		}else{
			$sql = "SELECT * FROM ".TBL_INV_OTHERS_CHARGES." WHERE id_inv_po IN (".$finall.") ";
		}
	...
	//echo $sql;	
		...
		while($row = mysqli_fetch_object($res)){
			//debugData($row);
//echo $row->id_mst_charges_discounts;
			...
			if($row->type=='1'){
        		...
        		$sqlCharge = "SELECT * FROM mst_charges where id_shop='".$_SESSION['shop']."' AND charges_account IN (4) ";
				...
				$returnData.= '<td><input onkeyup="subtotal_calc(this.id)" type="text"  autocomplete="off" placeholder="Percentage" class="form-control" name="others_charges_percent'.$i.'" value="'.$row->otherChargesPercent.'" id="others_charges_percent'.$i.'" /></td>'; 
        		...
        		$returnData.= '<td><input type="text"  autocomplete="off"  value="'.$row->others_charges_igst_amount.'" name="others_charges_igst_amount'.$i.'" id="others_charges_igst_amount'.$i.'"  placeholder="IGST Amount"  class="form-control" readonly/></td>';
        		...
        		$returnData.= '<td  style="display:none;"><div id="otherschargestaxconfig" id="otherschargestaxconfig"><!-- Discount --><input type="text"  autocomplete="off"  name="others_discount_percent'.$i.'" value="'.$row->others_discount_percent.'" id="others_discount_percent'.$i.'" placeholder="Discount"  class="form-control" /><input type="text"  autocomplete="off" value="'.$row->others_discount_amount.'"  name="others_discount_amount'.$i.'" id="others_discount_amount'.$i.'" placeholder="Amount"  class="form-control"  /></div></td>';        
	 		  	if($i>=1){
					$returnData.= '<td><a  class="btn n-btn abtn ibtnDel2" id="deletes'.$i.'" name="deletes'.$i.'" style="cursor:pointer;" title="Delete"><i class="fa fa-trash-o"></i></a></td>';
	 		  	}
				
				
$returnArr['others_total']+=$row->others_charges_amount;
$returnArr['others_charges_sgst_amount']+=$row->others_charges_sgst_amount;
$returnArr['others_charges_cgst_amount']+=$row->others_charges_cgst_amount;
$returnArr['others_charges_igst_amount']+=$row->others_charges_igst_amount;
			...
			$i++;
$valuecharges_others[]=$row->id_mst_charges_others;
		}
		$returnArr['count']=($i-1);
		$returnArr['id_mst_charges_others']=implode(',',$valuecharges_others);
