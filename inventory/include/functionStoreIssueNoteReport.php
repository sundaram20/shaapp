...
if($id_mst_party_supplier!=''){
	$SqlRepConn .=	" AND id_mst_attributes_department IN (".$id_mst_party_supplier.")";	
	}	
...
if($id_report_type!=''){
  ...
  $pos_purch_sql="select id as id_inv_indent, id_mst_attributes_department,doc_date, doc_no, mdoc_no, id_inv_items, item_code, name as item_description, sum(qty) as qty,sum(bal_qty) as bal_qty, id_mst_attributes_group_main, id_mst_attributes_group_sub,id_mst_attributes_department,    doc_type from ( select pp.id,pp.doc_date,pp.doc_no,pp.mdoc_no,ppp.id_inv_items,inv.name, ppp.id as id_purch_detail,inv.item_code, ppp.qty,ppp.bal_qty,  inv.id as id_item, inv.id_mst_attributes_group_main,inv.id_mst_attributes_group_sub,pp.doc_type ,pp.id_mst_attributes_department

FROM inv_indent pp 
LEFT JOIN  inv_indent_details ppp ON ppp.id_inv_indent=pp.id 
...
//echo '1 :'.$pos_purch_sql;
	...
	$maingroupName=$posPurchResult->mdoc_no;
	 $party_name	=strtoupper(selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE status='1' AND table_name ='".'department'."' AND `id` = '".$posPurchResult->id_mst_attributes_department."'"));
		 
		 
		 //strtoupper(selectColumn(TBL_PARTY,'company_name'," WHERE status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `id` = '".$posPurchResult->id_mst_party_supplier."'"));
		...
		$doc_name= selectField(APP_DOCTYPE,'name','WHERE id_doc_type="'.$posPurchResult->doc_type.'"',$appConnect); 
		 ...
		 $DatewiseArray['Report'][$doc_type][$maingroupName]['Heading-'.$maingroupName]['id_inv_purch']=$posPurchResult->id_inv_indent;
		...
		// $DatewiseArray['Report'][$doc_type][$maingroupName]['Heading-'.$maingroupName]['doc_name'] =$pageURL;;
		 ...
		 $DatewiseArray['Report'][$doc_type][$maingroupName][$posPurchResult->id_inv_items]['id_inv_items']=$posPurchResult->id_inv_indent;
 ...
 // echo $headerColor = 'hi';
	...
	<th  colspan="4" style="text-align:right;color:'.$headerTextColor.';background-color:'.$headerColor.'; font-size:13px !important"><b> Report Date: '.date('d-m-Y H:m:i').' </b></th></tr>';
		...
		$content .= '<tr  '.$listTagClass.' style="color:#ooo !important;font-size:14px !important; font-weight:600;" >';				
				...
				$content .= '<td style="background-color:'.$titleColor.';color:'.$titleTextColor.';text-align:left;width:40px;">Department </td>';
...
$PageURL="editIndent.php";
			...
			$content .= '<tr '.$listTagClass.' style="background-color:'.$subtitleColor.';color:#ooo !important;font-size:12px !important; font-weight:600">';				
				...
				$content .= '<td style="text-align:left;width:10px;"><a href="'.$PageURL.'?eId='.encryptor(encrypt, $data['id_inv_purch']).'&session='.$session.'&submenu='.$submenu.'&action=edit&page=&doc_type='.$doc_type.'&print=1"  target="_blank">'.$data['mdoc_no'].'</a></td>';	
				...
				$content .= '<td  colspan="1" style=text-align:right;width:34px;"></td>
				</tr>';
			}else{
...
$Filename='RequestionNoteReport_'.$date;
}else{
$Filename='RequestionNoteReport_'.$date;	
	}
...
function RequestionNoteReport($date,$id_main_group,$id_sub_group,$id_items,$id_report_type,$report_show,$id_order_by,$showItemReport,$id_mst_party_supplier){
...
//$content .= '</style>';
 ...
 // echo $headerColor = 'hi';
		...
		$subtitleTextColor = selectField(APP_COLOR_CONFIG,'report_subtitle_text_color','',$appConnect); 
			...
			$content .= '<table class="table table-striped text-center">';
	$content .= '<tr><th colspan="6" style="border-right:1px solid #0093c7 ; vertical-align:central;text-align:left;color:'.$headerTextColor.';background-color:'.$headerColor.';font-size:16px !important"><b>  Report Period '.$date.' Order By '.$OrderDiplay.''.$OrderDiplay22.' </b></th>
	<th  colspan="3" style="text-align:right;color:'.$headerTextColor.';background-color:'.$headerColor.'; font-size:13px !important"><b> Report Date: '.date('d-m-Y H:m:i').' </b></th></tr>';
		...
		$content .= '<tr  '.$listTagClass.' style="color:#ooo !important;font-size:14px !important; font-weight:600;" >';				
				...
				$content .= '<td style="background-color:'.$titleColor.';color:'.$titleTextColor.';text-align:right;width:15px;">Requested Qty</td>';
			$content .= '<td style="background-color:'.$titleColor.';color:'.$titleTextColor.';text-align:right;width:15px;">Balance Qty</td>';
				//$content .= '<td style="background-color:'.$titleColor.';color:'.$titleTextColor.';text-align:right;width:15px;">Item Amount</td>';
...
$PageURL="editIndent.php";
			...
			$content .= '<tr class="page_break" style="background-color:'.$subtitleColor.';color:#ooo !important;font-size:12px !important; font-weight:600;">';				
				$content .= '<td style="text-align:left;">'.$i++.'</td>';
				$content .= '<td style="text-align:left;">'.$data['doc_name'].'</td>';
				$content .= '<td  style=text-align:left;width:40px;"> '.$data['party_name'].'</td>';
				...
				$content .= '<tr   class="page_break" style="page-break-before: always;border:1px solid:font-size:11px !important;color: #000;   background-color:#fff; ">';				
...
//		die;
		$date=date('d-m-yy');
if($id_report_type==196){		
$Filename='RequestionNotePendingReport_'.$date;
}else{
$Filename='RequestionNotePendingReport_'.$date;	
	}
...
$canvas->page_text(720, 18, "Page: {PAGE_NUM} of {PAGE_COUNT}", $font, 8, [0,0,0]);
