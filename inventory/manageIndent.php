...
$resCat = selectSql(TBL_INV_INDENT," where id_shop= '".addslashes($_SESSION['shop'])."'  and doc_type = '1' ".$statuscase."order by last_modified desc  "); 
...
<div class="content-wrapper">
	 ...
	 <div class="form-group has-error mb-0" align="center">
        ...
        <div class="box-header "> 
		  ...
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn n-btn" href="editIndent.php?submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>" >Add Requisition Note</a>
          ...
          <div class="box">
            ...
            <div class="box-body table-responsive">
		        ...
		        <tbody>  
                  ...
                  <?php /*?><td><?php echo $row->status=='1'?'<span onclick="location.href=\'manageIndent.php?inactiveId='.encryptor(encrypt, $row->id).'&submenu='.$_GET['submenu'].'&session='.$_GET['session'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageIndent.php?activeId='.encryptor(encrypt,$row->id).'&submenu='.$_GET['submenu'].'&session='.$_GET['session'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>	<?php */?>	

				  <td class="d-flex"><img src="../images/edit.png" style="cursor:pointer;height:20px" title=" View / Edit " onClick="window.location.href='editIndent.php?eId=<?=encryptor(encrypt, $row->id)?>&session=<?php echo $_GET['session']; ?>&submenu=<?php echo $_GET['submenu'] ?>&action=edit&page=<?=$_REQUEST['page']?>&print=1';" />&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='manageIndent.php?delId='+sid+'&submenu=<?php echo $_GET["submenu"] ?>&session=<?php echo $_GET["session"] ?>&action=delete&page=<?=$_REQUEST["page"]?>';
	   } 
	   else
	   {
	      self.location='manageIndent.php?submenu=<?php echo $_GET["submenu"] ?>&session=<?php echo $_GET["session"] ?>';
	    }
	   });
	  }
  </script> 
