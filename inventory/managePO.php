...
if($_REQUEST['action'] == 'change'){
	...
	if($_REQUEST['activeId'] != ''){
		if(checkUserLevelPermission($_SESSION['userLevel'],TBL_INV_PO,'active')){

		$statusId = addslashes(encryptor(decrypt,$_REQUEST['activeId']));
		$statusSql = "	UPDATE `".TBL_INV_PO."`
						SET `status` = '1'
		...
		if(checkUserLevelPermission($_SESSION['userLevel'],TBL_INV_PO,'inactive')){
	...
	if(checkUserLevelPermission($_SESSION['userLevel'],TBL_INV_PO,'delete')){
		$delSql = "DELETE FROM `".TBL_INV_PO."` WHERE `id` = '".encryptor(decrypt,$_REQUEST['delId'])."'";
	
		if(executeSql($delSql)){
			$ids_indent_arr = array();
			$ids_indent ="SELECT id_inv_indent_details as id FROM ".TBL_INV_PO_DETAILS." WHERE id_inv_po='".encryptor(decrypt,$_REQUEST['delId'])."' " ;

			$ids_indent=mysqli_query($connNew,$ids_indent);
			while($rowIndent = mysqli_fetch_object($ids_indent)){
				array_push($ids_indent_arr,$rowIndent->id);
			}
			$delSql2 = "DELETE FROM `".TBL_INV_PO_DETAILS."` WHERE `id_inv_po` = '".encryptor(decrypt,$_REQUEST['delId'])."'";
			if(executeSql($delSql2)){	
				executeSql("UPDATE ".TBL_INV_INDENT_DETAILS." SET bal_qty=qty,ordered_qty=0 WHERE id IN (".implode(',',$ids_indent_arr).") ");
				$err = 0;
   ...
   <?php $session=$_GET['submenu']; ?>
    <section class="content-header">
       ...
       <div class="col-md-4 col-xs-12 dd-f">	
                 ...
                 <div class="icn-box">
                 	   <div class="btn-group"> <a type="button"  title="List Bill" class="btn n-btn pull-right" href="manageGRN.php?submenu=100&session=4" > <i class="fas fa-list "></i> Goods Receipt Note</a> </div>
	...
	<div class="form-group has-error  mb-0" align="center">
        ...
        <div class="box-header"> 
		  ...
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn n-btn" href="editPO.php?session=<?php echo $_GET['session']; ?>&submenu=<?php echo $_GET['submenu']; ?>" >Add <?php echo currentNavigation()['submenu'];  ?></a>
							  <!--<button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">
								<span class="caret"></span>
								<span class="sr-only">Toggle Dropdown</span>
							  </button>
							  <ul class="dropdown-menu" role="menu">
							<?php ?>	<li><a title="Export to excel file" href="exportTable.php?fileType=xls&tableName=<?php echo TBL_INV_PO;?>"><img src="../images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>
								<li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo TBL_INV_PO;?>"><img  src="../images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li><?php ?>
        ...
        <div class="col-xs-12">		     
          ...
          <div class="box">
            <div class="box-header table-h text-center">
              <h3 class="box-title">List Of <?php echo currentNavigation_id($session)['submenu']; ?></h3>
            </div>
            ...
            <div class="box-body table-responsive">
		        ...
		        <tbody>  
		        	...
		        	// $resCat = selectSql(TBL_INV_PO," where id_shop= '".addslashes($_SESSION['shop'])."'  and doc_type = '3'  order by last_modified DESC "); 
...
<td>
              ...
              <td><?php echo $sqlUserDetail = selectColumn(TBL_USERS,'name','where id="'.$row->id_mst_user_created_by.'" '); ?></td> 
                 <?php /*?> <td><?php echo $row->status=='1'?'<span onclick="location.href=\'managePO.php?inactiveId='.encryptor(encrypt, $row->id).'&submenu='.$_GET['submenu'].'&session='.$_GET['session'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'managePO.php?activeId='.encryptor(encrypt,$row->id).'&submenu='.$_GET['submenu'].'&session='.$_GET['session'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td><?php */?>		

				  <td><img src="../images/edit.png" style="cursor:pointer;height:20px;" title=" View / Edit " onClick="window.location.href='editPO.php?eId=<?=encryptor(encrypt, $row->id)?>&session=<?php echo $_GET['session']; ?>&submenu=<?php echo $_GET['submenu']; ?>&action=edit&page=<?=$_REQUEST['page']?>&print=1';" />&nbsp;&nbsp;&nbsp;&nbsp;
				  	<img src="../images/preview.png" style="height:20px;cursor:pointer;" title=" Page preview" onClick="window.location.href='<?php echo $print; ?>?eId=<?=encryptor(encrypt, $row->id)?>&session=<?php echo $_GET['session']; ?>&submenu=<?php echo $_GET['submenu']; ?>&action=edit&page=<?=$_REQUEST['page']?>'; " />&nbsp;&nbsp;&nbsp;&nbsp;
					<?php 
				  		$tableArray = array();
				  		$tableArray = array(TBL_INV_PURCH_DETAILS=>'doc_type=4 AND id_inv_po');
				  		if(!checkTransBeforeDelete($tableArray,$row->id)){

				  	?>
				  	<img src="../images/close.png" style="height:15px;cursor:pointer;" title="Delete" name="<?php echo $row->name; ?>" id="<?php echo encryptor(encrypt,$row->id);?>" onClick="deletes(this.id)"/>
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='managePO.php?delId='+sid+'&submenu=<?php echo $_GET["submenu"] ?>&session=<?php echo $_GET["session"] ?>&action=delete&page=<?=$_REQUEST["page"]?>';
	   } 
	   else
	   {
	      self.location='managePO.php?submenu=<?php echo $_GET["submenu"]; ?>&session=<?php echo $_GET["session"]; ?>';
	    }
	   });
	  }
  </script> 
