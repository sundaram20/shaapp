		   ...
		   // Pass Ajax Data  $( "#po_list" ).append(data);  $("#not_applicable").hide();$("#polist").html(data);
			var ledger_id = document.getElementById("ledger_id").value;
			 ...
			 $.ajax({
					...
					//url: "../ajax/"+phpfile,
					url: "ajax/"+phpfile,
					data:{id_inv_po:id_inv_po, array:myArray,counter1:counter1,match:match,id_mst_party_supplier:id_mst_party_supplier,doctype:doctype,eid:eid,ledger_id:ledger_id},
					//dataType: "html", 
					datatype:'JSON',					
					success: function(data){
						//var mydata = JSON.parse(data);
...
//alert(mydata['ledger']);
document.getElementById("ledger_id").value = mydata['ledger'];
...
if(counter11==0){
	...
	var counter1 =  counter11; 
      ...
      //  cols1 += '<td><select onchange="popupshow(this.id);" onclick="popupshow(this.id);"  name="id_inv_po' + counter1 + '" id="id_inv_po' + counter1 + '" class="form-control select2" style="width:100%"><option value="">Select <?php echo $table_field; ?></option><?php if($_GET['doc_type'] == '12'){?><option value="na" selected >NA</option><?php }else if($_GET['doc_type'] == '4' && $_GET['doc_type'] != '5'){ ?><option value="na" >NA</option><?php	} ?></select> </td>'; 
	   ...
	   cols1 += '<td><select onchange="popupshow(this.id);" onclick="popupshow(this.id);" name="id_inv_po' + counter1 + '" id="id_inv_po' + counter1 + '" class="form-control select3"  style="width:100%" data-parsley-required data-parsley-errors-container="#outletError4"><option>Select</option><?php if($_GET['doc_type'] == '12'){?><option value="na" selected >NA</option><?php } ?></select> <span id="outletError4"></span> </td>'; 
		...
		// var newRow11 =  $('<tr id="trdeletes' + counter1 + '">');
		 	 ...
		 	 cols1 += '<td><div id="local'+counter1+'" name="local'+counter1+'" style="display:none;"><select data-parsley-required data-parsley-errors-container="#outletError5" name="id_mst_charges_purchase_local' + counter1 + '" id="id_mst_charges_purchase_local' + counter1 + '" class="form-control select3" onchange="po_locals(this.id)" style="width:100%;"><option>Select Tax Register</option><?php 
	                ...
	                $sql = "SELECT mst_charges.*FROM mst_charges WHERE mst_charges.id_shop = '".addslashes($_SESSION['shop'])."' AND  charges_account = '2' and transaction_type = '2'";
	                   	 $db->query($sql); 
	                    while($row1 = $db->fetch_object()){ ?>
	                  		<option value="<?php echo $row1->id; ?>"><?php echo $row1->name ?></option> <?php }?></select><span id="outletError5"></span></div></td> '; 
	                ...
	                $sql = "SELECT mst_charges.*FROM mst_charges WHERE mst_charges.id_shop = '".addslashes($_SESSION['shop'])."' AND  charges_account = '2' and transaction_type = '2'";
	                    ...
	                    while($row1 = $db->fetch_object()){ ?>
	                  	...
	                  	cols1 += '<td><div id="localsss'+counter4+'" name="localsss'+counter4+'" style="display:none;"><span style="color:red;font-size:14px;" id="s_amount'+counter4+'"></span> <span style="color:red;font-size:14px;;font-size:14px;" id="c_amount'+counter4+'"></span></div><div id="interstatesss'+counter4+'" name="interstatesss'+counter4+'" style="display:none;"><span style="color:red;font-size:14px;" id="i_amount' + counter4 + '" name="i_amount' + counter4 + '"></span></div></td>';
	                  		 ...
	                  		 cols1 += '<td><input type="text"  autocomplete="off" placeholder="Remarks2" class="form-control" name="item_remarks' + counter4 + '" id="item_remarks' + counter4 + '"/></td>';
