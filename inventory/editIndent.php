...
if($_REQUEST['eId']==''){
	...
	//checkUserLevelPermission($_SESSION['userLevel'],TBL_INV_INDENT,'view');
	checkUserLevelPermission($_SESSION['userLevel'],TBL_INV_INDENT,'edit'); //echo 'Edit';die;
}
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Save') && empty($_POST['eId'])){//add 
			 ...
			 $doc_no = $_POST['doc_no'];
			  ...
			  $sql5 = " SELECT * FROM `".TBL_INV_INDENT."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_no`='".$doc_no."'  and `doc_type` = '1'  and `id_doc_type_configuration` = '".addslashes($_POST['id_doc_type_configuration'])."' ";
				$db->query($sql5);
              ...
              <div class="box-body">
              	...
              	<div class="card text-dark bg-light">
	              	...
	              	<div class="row">	
		                  ...
		                  <input data-parsley-required type="text" class="form-control pickerdate" placeholder="Enter Indent Date" id="doc_date" name="doc_date" value="<?php if($_POST) echo $_POST['doc_date'];elseif($row->doc_date!='') echo date('d-m-Y',strtotime($row->doc_date));else echo date('d-m-Y');?>" onchange="hideandshow();">
		                  </div> 
	              		</div>
		                  ...
		                  <?php if($row->id ==''){?>
	              			...
	              			<div id="ind" name="ind" style="display:none;">
		              			...
		              			<div class=" col-xs-6 col-md-4 col-sm-6" style="display:none;">
		              				...
		              				<input type="text" class="form-control" placeholder="Enter Requisition No" id="doc_no" name="doc_no" value="<?php if($_POST) echo $_POST['doc_no'];else echo stripslashes($row->doc_no);?>" readonly>
		              				</div> 
			                	</div>
 	...
 	<?php
		...
		$sqlResult1 = "SELECT * FROM ".TBL_ATTRIBUTES." WHERE table_name = 'items_type' AND field_category IN ('Ingredients Items','Both','Other Items','Store Items') AND id_shop = ".$_SESSION['shop'] ." ";
												...
												$list2=array();
													...
													$input = array_map("unserialize", array_unique(array_map("serialize",$list2)));
											$item_list2 = implode(',',$input);
										//echo  $sql = "SELECT inv_items.*, mst_attributes.field_value FROM inv_items, mst_attributes WHERE mst_attributes.id=inv_items.id_mst_attributes_group_main and inv_items.status=1 and inv_items.id_mst_attributes_item_type IN (".$item_list2.") and inv_items.id_shop = '".addslashes($_SESSION['shop'])."'  ";	 	 ?>
...
window.onload = function() { hideandshow();
};
...
<?php 
	$sql2 = " SELECT max(doc_date) as doc_date FROM `".TBL_INV_INDENT."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".'1'."' ";
		$db->query($sql2);   
			while($row2 = $db->fetch_object()){ 
