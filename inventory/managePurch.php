...
<div class="content-wrapper">
	 ...
	 <div class="form-group has-error mb-0" align="center">
        ...
        <div class="box-header with-border"> 
		  ...
		  <div class="btn-group  pull-right">
							<a type="button" class="btn n-btn" href="editPurch.php?doc_type=<?php echo $doc_id; ?>&submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>" >Add <?php echo $add; ?></a>
          ...
          <div class="box">
            ...
            <div class="box-body table-responsive">
		       ...
		       ///	echo "SELECT * FROM inv_purch  where id_shop= '".addslashes($_SESSION['shop'])."'  and doc_type = '".$doc_type."' order by last_modified DESC ";
                 ...
                 <?php /*?> <td><?php echo $row->status=='1'?'<span onclick="location.href=\'managePurch.php?inactiveId='.encryptor(encrypt, $row->id).'&submenu='.$_GET['submenu'].'&session='.$_GET['session'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'managePurch.php?activeId='.encryptor(encrypt,$row->id).'&submenu='.$_GET['submenu'].'&session='.$_GET['session'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td><?php */?>		

				  <td><img src="../images/edit.png" style="cursor:pointer;height:20px;" title=" View / Edit " onClick="window.location.href='editPurch.php?eId=<?=encryptor(encrypt, $row->id)?>&action=edit&page=<?=$_REQUEST['page']?>&submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>&doc_type=<?php echo $doc_id; ?>&print=0';" />&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<?php 
				  		...
				  		//$tableArray = array();
				  		//$tableArray = array(TBL_INV_PURCH_DETAILS=>'doc_type=8 AND id_inv_po');
					...
					//}else{echo '<img src="../images/chat.png" style="cursor:pointer; " title="In Use" />';} 
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='managePurch.php?delId='+sid+'&action=delete&page=<?=$_REQUEST["page"]?>&submenu=<?php echo $_GET["submenu"]; ?>&session=<?php echo $_GET["session"]; ?>&doc_type=<?=$_REQUEST["doc_type"]?>';
	   } 
	   else
	   {
	      self.location='managePurch.php?doc_type=<?=$_REQUEST["doc_type"]?>&submenu=<?php echo $_GET["submenu"]; ?>&session=<?php echo $_GET["session"]; ?>';
	    }
	   });
	  }
  </script> 
