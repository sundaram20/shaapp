...
if($_REQUEST['action'] == 'change'){
	...
	if(checkUserLevelPermission($_SESSION['userLevel'],TBL_INV_PURCH,'delete')){
		...
		if(executeSql($delSql)){	
			$ids_req_arr = array();
			$ids_req ="SELECT id_inv_indent_details as id FROM ".TBL_INV_PURCH_DETAILS." WHERE id_inv_purch='".encryptor(decrypt,$_REQUEST['delId'])."' " ;

			$ids_req=mysqli_query($connNew,$ids_req);
			while($rowReq = mysqli_fetch_object($ids_req)){
				array_push($ids_req_arr,$rowReq->id);
			}

			$delSql2 = "DELETE FROM `".TBL_INV_PURCH_DETAILS."` WHERE `id_inv_purch` = '".encryptor(decrypt,$_REQUEST['delId'])."'";
			if(executeSql($delSql2)){
				executeSql("UPDATE ".TBL_INV_INDENT_DETAILS." SET bal_qty=qty,ordered_qty=0 WHERE id IN (".implode(',',$ids_req_arr).") ");		
				$err = 0;
				$_SESSION['successMsg'] = 'One Unit  has been deleted sucessfully.';
			}
			else{
				$_SESSION['successMsg'] = 'Detail tables values not deleted.';
			}	
		}else{
			$err = 1;
...
$resCat = selectSql(TBL_INV_PURCH," where id_shop= '".addslashes($_SESSION['shop'])."'  and doc_type = '6' ".$statuscase."order by last_modified DESC "); 
   ...
   <?php $session=$_GET['submenu']; ?>
    <section class="content-header">
       ...
       <div class="col-md-4 col-xs-12 dd-f">	
                 ...
                 <div class="icn-box">
                 	   <div class="btn-group"> <a type="button"  title="List Bill" class="btn n-btn pull-right" href="manageIndent.php?submenu=96&session=1" > <i class="fas fa-list "></i> Requisition Note</a> </div>
	 ...
	 <div class="form-group has-error mb-0" align="center">
        ...
        <div class="box-header with-border"> 
		  ...
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn n-btn" href="editStoreIssueNote.php?submenu=<?php echo $_GET["submenu"]; ?>&session=<?php echo $_GET['session']; ?>" >Add Store Issue Note</a>
        ...
        <div class="col-xs-12">		     
          ...
          <div class="box">
            <div class="box-header table-h text-center">
              <h3 class="box-title  ">Store Issue Note List</h3>
            </div>
            ...
            <div class="box-body table-responsive">
		        ...
		        <tbody>  
		        	<?php  
		        	 ...
		        	 //$resCat = selectSql(TBL_INV_PURCH," where id_shop= '".addslashes($_SESSION['shop'])."'  and doc_type = '6' order by last_modified DESC "); 
                 ...
                 <?php /*?> <td><?php echo $row->status=='1'?'<span onclick="location.href=\'manageStoreIssueNote.php?inactiveId='.encryptor(encrypt, $row->id).'&submenu='.$_GET['submenu'].'&session='.$_GET['session'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageStoreIssueNote.php?activeId='.encryptor(encrypt,$row->id).'&submenu='.$_GET['submenu'].'&session='.$_GET['session'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>		
<?php */?>
				  <td><img src="../images/edit.png" style="height:20px;cursor:pointer;" title=" View / Edit " onClick="window.location.href='editStoreIssueNote.php?eId=<?=encryptor(encrypt, $row->id)?>&submenu=<?php echo $_GET["submenu"]; ?>&session=<?php echo $_GET['session']; ?>&action=edit&page=<?=$_REQUEST['page']?>&print=0';" />&nbsp;&nbsp;&nbsp;&nbsp;
				  	<img src="../images/preview.png" style="height:20px;cursor:pointer;" title="Page Preview " onClick="window.location.href='printStoreIssueNote.php?eId=<?=encryptor(encrypt, $row->id)?>&submenu=<?php echo $_GET["submenu"]; ?>&session=<?php echo $_GET['session']; ?>&action=edit&page=<?=$_REQUEST['page']?>'; " />&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='manageStoreIssueNote.php?delId='+sid+'&action=delete&page=<?=$_REQUEST["page"]?>&submenu=<?php echo $_GET["submenu"]; ?>';
	   } 
	   else
	   {
	      self.location='manageStoreIssueNote.php?submenu=<?php echo $_GET["submenu"]; ?>';
	    }
	   });
	  }
  </script> 
