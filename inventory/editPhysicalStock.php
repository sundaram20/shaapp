...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Save') && empty($_POST['eId'])){//add 
			 ...
			 $sql5 = " SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_no`='".$doc_no."'  and `doc_type` = '9'  and `id_doc_type_configuration` = '".addslashes($_POST['id_doc_type_configuration'])."'  ";
				$db->query($sql5);
				$numRows= $db->num_rows();
					if($numRows > 0){
			...
			$addSql = "   	INSERT INTO `".TBL_INV_PURCH."` SET
				...
				$addSql = "   	INSERT INTO `".TBL_INV_PURCH_DETAILS."` SET
							...
							`bal_qty` = '".addslashes($_POST['variance'])."',
							`alt_qty` = '".addslashes($_POST['variance'])."',   
							`id_mst_attributes_store` = '".addslashes($_POST['id_mst_attributes_store'])."',  
							
							`actual_stock` = '".addslashes($_POST['actual_stock'])."',	
							`stock_in_hand` = '".addslashes(round($_POST['stock_in_hand'],2))."',						
							`rate_per_alt_unit` = '".addslashes(round($_POST['rate'],2))."',
							`rate_per_main_unit` = '".addslashes(round($_POST['rate'],2))."',							  
							`item_amount_before_discount` = '".addslashes(round($_POST['actual_stock_value']))."',
							`item_amount` = '".addslashes(round($_POST['actual_stock_value']))."', 
				...
				//Purch Details Table Here For Loop Value Set
							...
							for($i = 1; $i <= $counter1; $i++){
								...
								if($_POST['id_inv_items'.''.$i] != ''){
									...
									`bal_qty` = '".addslashes($_POST['variance'.''.$i])."',  
									`alt_qty` = '".addslashes($_POST['variance'.''.$i])."',
									`id_mst_attributes_store` = '".addslashes($_POST['id_mst_attributes_store'.''.$i])."',   
									`actual_stock` = '".addslashes($_POST['actual_stock'.''.$i])."',
									
									`stock_in_hand` = '".addslashes(round($_POST['stock_in_hand'.''.$i],2))."',
									`rate_per_alt_unit` = '".addslashes(round($_POST['rate'.''.$i],2))."',
									`rate_per_main_unit` = '".addslashes(round($_POST['rate'.''.$i],2))."',							  
									`item_amount_before_discount` = '".addslashes(round($_POST['actual_stock_value'.''.$i]))."',
									`item_amount` = '".addslashes(round($_POST['actual_stock_value'.''.$i]))."', 
			...
			if(1){
				...
				header("location:printPhysicalStock.php?eId=".addslashes(encryptor(encrypt,$lastInsertId))."&submenu=".$_GET['submenu']."&session=".$_GET['session']."&action=edit&page=".$_REQUEST['page']."&doc_type=".$_REQUEST['doc_type']."&print=1");
				exit;
			}else{
				$err++;
		...
		//Update Section Here
			...
			if(1){  
				...
				header("location:printPhysicalStock.php?eId=".$_GET['eId']."&submenu=".$_GET['submenu']."&session=".$_GET['session']."&action=edit&page=".$_REQUEST['page']."&doc_type=".$_REQUEST['doc_type']."&print=1");
				 	exit;
			}else{
				$err++;
              ...
              <div class="box-body">
              	...
              	<div class="card text-dark bg-light">
	              	...
	              	<div class="row">	
		                  ...
		                  <input data-parsley-required type="text" class="form-control pickerdate" placeholder="Enter Physical Stock Date" id="doc_date" name="doc_date" value="<?php if($_POST) echo $_POST['doc_date'];elseif($row->doc_date!='') echo date('d-m-Y',strtotime($row->doc_date));else echo date('d-m-Y');?>" onchange="hideandshow()" onclick="hideandshow()">
		                  </div> 
	              		</div>
		                  ...
		                  <?php if($row->id ==''){?>
	              			...
	              			<?php } ?>
	              				<div class=" col-xs-12 col-md-12 col-sm-12">
		              				...
		              				<input type="text" class="form-control" placeholder="Enter Physical Stock No" id="mdoc_no2" name="mdoc_no2" value="<?php if($_POST) echo $_POST['mdoc_no2'];else echo stripslashes($row->mdoc_no);?>" readonly>
		              				</div> 
			                	</div>
	              			<div id="ind" name="ind" style="display:none;">
		              			...
		              			<div class=" col-xs-6 col-md-4 col-sm-6" style="display:none;">
		              				...
		              				<input type="text" class="form-control" placeholder="Enter Physical Stock No" id="doc_no" name="doc_no" value="<?php if($_POST) echo $_POST['doc_no'];else echo stripslashes($row->doc_no);?>" readonly>
		              				</div> 
			                	</div>
		            ...
		            <div class="row">
		            	 <hr class="br-line">
		            		...
		            		<div class="text-center ">
		              			<h6 class="tb-heads">Physical Stock Details</h6>
		              		</div>  
		            	<table id="myTable1" class=" table   table-striped  table-bordered dataTable no-footer  order-list1 max-h2">
				            ...
				            <tbody>
								...
								while($rowsID = $db->fetch_object()){
									 ...
									 //`rate_per_main_unit` = '".addslashes($_POST['actual_stock_value'.''.$i])."', 
							 		 $i++;
				                ...
				                <tr> 
				                	...
				                	<td class="form-group col-xs-12 col-md-3 col-sm-2"> 
                                     ...
                                     <?php $categoryDropDown = '<option value="">Select Item Code</option>';
										$sqlResult1 = "SELECT * FROM ".TBL_ATTRIBUTES." WHERE table_name = 'items_type' AND field_category NOT IN ('Menu') AND id_shop = ".$_SESSION['shop'] ." ";
										...
										/* $categoryDropDown = '<option value="">Select Item Code</option>';
				                    ...
				                    <td class="form-group col-md-2">  
					                	<select  class="form-control select2" name="id_mst_attributes_store<?php echo $k;?>" id="id_mst_attributes_store<?php echo $k;?>" style="width:100%" data-parsley-required data-parsley-errors-container="#id_mst_attributes_store<?php echo $k;?>Error">
										<?php $categoryDropDown = '<option value="">Select</option>';
											  ...
											  if($db->num_rows2($resCat)){
											 	...
											 	echo $categoryDropDown .= '</select><span id="id_mst_attributes_store"'.$k.'"Error"></span>'; 
										?> <?php // echo $id_mst_attributes_store'Error'; ?>
									
					                </td>
				                    <td class="form-group col-xs-12 col-sm-2"> 
				                        <input type="text"  autocomplete="off"  name="stock_in_hand<?php echo $k;?>" id="stock_in_hand<?php echo $k;?>" placeholder="Stock In Hand" class="form-control" value="<?php if($_POST) echo $_POST['stock_in_hand'];else echo stripslashes($array['stock_in_hand'.''.$j]); ?>" readonly/>
				                    </td> 
                                    <td class="form-group col-xs-12 col-sm-2"> 
				                        <input type="text"  autocomplete="off"  name="rate<?php echo $k;?>" id="rate<?php echo $k;?>" placeholder="rate" class="form-control" value="<?php if($_POST) echo $_POST['rate'];else echo stripslashes($array['rate'.''.$j]); ?>" onkeyup="actualcalc(this.id)"/>
				                    </td> 
				                    <td class="form-group col-xs-12 col-sm-2"> 
                                    ...
                                    <?php 
									if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){
									$actual_stock_value	=stripslashes($array['item_amount'.''.$j]);
										}else{
				                    ...
				                    <td class="form-group col-xs-12 col-sm-2"> 
				                        <input type="text"  autocomplete="off"  name="actual_stock_value<?php echo $k;?>" id="actual_stock_value<?php echo $k;?>"   class="form-control"
                                         value="<?php  echo stripslashes($actual_stock_value); ?>"  />
				                    </td>
				                    <?php if($k>=1){?>
				                    <td> 
				            ...
				            <tfoot>
				                <tr> 
			                        <td colspan="9" style="text-align: left;">
			                            <a  type="button" class="btn n-btn btn-block" id="addrow1" value="Add More"><span style="font-size:14px;font-weight:700"><i class="fa fa-plus"></i> Add Row</span> </a>
		        ...
		        ?>
		            ...
		            <br> 
		            <div class="row "> 	            	
						<div class="form-group mb-0 col-xs-12 col-md-6 col-sm-12 mb-0"> 
			   ...
			   <a type='button' value='Cancel' class="btn c-btn" onclick='location.replace("managePhysicalStock.php?submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>"); '><i class="far fa-window-close" aria-hidden="true"></i> Close</a>
...
<script type="text/javascript">
	...
	function itemget(clicked_id) {
		    ...
		    $.ajax({
					...
					data:{id_inv_items:id_inv_items, doc_date:doc_date},
					success: function(data){
						...
						document.getElementById("rate"+match).value = mydata['rate'];   
						var id_mst_attributes_store = mydata['id_mst_attributes_store'];
						var store = mydata['store']; 
						 
						//Store Section Here
						document.getElementById("id_mst_attributes_store"+match).innerHTML = "<option value='0' selected='selected'>Select Store</option>"+"<?php  $sql = "SELECT mst_attributes.*FROM mst_attributes WHERE mst_attributes.id_shop = '".addslashes($_SESSION['shop'])."' AND  status = '1' AND table_name ='store' ";$db->query($sql); while($row1 = $db->fetch_object()){ ?>  <option value='<?php echo $row1->id; ?>''><?php echo $row1->field_value ?></option> <?php } ?>"; 
 					}
				}); 
		}else{		 
			...
			if(id_inv_items != '') {
				...
				$.ajax({
					...
					success: function(data){ 
					  ...
					  document.getElementById("rate").value = mydata['rate']; 
						//Store Section Here
...
var others='po';
			$.ajax({
				...
				data:{doc_type:doc_type, clicked_id:clicked_id,others:others},
				success: function(data){
					console.log(data);
					var mydata = JSON.parse(data);  
					if(mydata['delete'] == 1){      
...
<?php 
	$sql2 = " SELECT max(doc_date) as doc_date FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".'9'."' ";
		$db->query($sql2);   
			while($row2 = $db->fetch_object()){ 
	      ...
	      <div class="modal-content"  style="margin-top: 50%; width: 72%;margin-left: 20%;"> 
	        ...
	        <div class="modal-body">
	        	<center>
		          ...
		          <a href="managePhysicalStock.php?submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>" type="button" class="btn btn-info"  id="buttons_radius"><i class="fa fa-info-circle" aria-hidden="true"> Close</i></a>  
 	...
 	$(document).ready(function() {
 		...
 		/*	if($row->id == ''){
			...
			while($row2 = $db->fetch_object()){ 
				$doc_date= $row2->doc_date; 
				if($doc_date == ''){
					$doc_date = selectColumn(TBL_DOC_TYPE_CONFIG,'effective_date'," WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='9' ");
				}
				$doc_date = date('d-m-Y' , strtotime(addslashes($doc_date)));  
			}   */
...
function actualcalc(clicked_id){
	...
	if(match >=1){
		...
		var rate = document.getElementById('rate'+match).value;
		
		//Calculations
		var total = Number(actual_stock) - Number(stock_in_hand);
		document.getElementById('variance'+match).value = total;
		document.getElementById('actual_stock_value'+match).value = rate*total;
		...
		document.getElementById('actual_stock_value').value = rate*total;
	}
...
//Select 2  Resolve Here
    ...
    $("#addrow1").on("click", function () { 	
       ...
       cols1 += '<td><select onchange="itemget(this.id)" name="id_inv_items' + counter1 + '" id="id_inv_items' + counter1 + '" class="form-control select3"  style="width:100%"><option>Select Item Code</option><?PHP $sqlResult1 = "SELECT * FROM ".TBL_ATTRIBUTES." WHERE table_name = 'items_type' AND field_category NOT IN ('Menu') AND id_shop = ".$_SESSION['shop'] ." ";
												$QuerySQL1	=	mysqli_query($connNew,$sqlResult1);
											...
											$item_list = rtrim($string,',');										
							                   	...
							                   	$sql = "SELECT inv_items.*, mst_attributes.field_value FROM inv_items, mst_attributes WHERE mst_attributes.id=inv_items.id_mst_attributes_group_main and inv_items.status=1 and inv_items.id_mst_attributes_item_type IN ($item_list) and inv_items.id_shop = '".addslashes($_SESSION['shop'])."'";
							                   	 ...
							                   	 $db->query($sql); 
							                    while($row1 = $db->fetch_object()){?><option  value="<?php echo $row1->id;?>"><?php echo ucfirst($row1->item_code.' | '.$row1->name)?></option><?php } ?></select> </td>';
		...
		cols1 += '<td><select  name="id_mst_attributes_store' + counter1 + '" id="id_mst_attributes_store' + counter1 + '" class="form-control select3"  style="width:100%"  data-parsley-required data-parsley-errors-container="#id_mst_attributes_store' + counter1 + 'Error"><option value="">Select Store</option><?php 
	                $sql = "SELECT mst_attributes.field_value, mst_attributes.id FROM  mst_attributes WHERE id_shop = '".addslashes($_SESSION['shop'])."' and table_name ='store' ";
	                   	 $db->query($sql); 
	                    while($row1 = $db->fetch_object()){ ?>
		...
		cols1 += '<td><input type="text"  autocomplete="off" placeholder="Rate" class="form-control" name="rate' + counter1 + '" id="rate' + counter1 + '" onkeyup="actualcalc(this.id)" /></td>'; 
        ...
        cols1 += '<td><input  type="text"  autocomplete="off" placeholder="Actual Stock Value" class="form-control" name="actual_stock_value' + counter1 + '" id="actual_stock_value' + counter1 + '" value="0" /></td>';
