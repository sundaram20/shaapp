...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Save') && empty($_POST['eId'])){//add 
			 ...
			 $sql5 = " SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_no`='".$doc_no."'  and `doc_type` = '1'   and `id_doc_type_configuration` = '".addslashes($_POST['id_doc_type_configuration'])."'  ";
				$db->query($sql5);
				$numRows= $db->num_rows();
					if($numRows > 0){
 ...
 for($i=0;$i<$itemDetailSizeOf;$i++){
  ...
  $id_inv_poo = rtrim($id_po,',');			
			...
			$addSql = "   	INSERT INTO `".TBL_INV_PURCH."` SET
							...
							`doc_date`='".date('Y-m-d',strtotime($_REQUEST['doc_date']))."',
							`doc_no`='".$doc_no."',
		...
		$no211 = rtrim($no_21,',');
				...
				if($doc_date != $_POST['doc_date']){
					...
					$indate = date('d-m-Y' , strtotime(addslashes($_POST['doc_date']))) ;
					 $ch2 ="Indent Date Details Changed from " .  $doc_date." - to - ".$indate;
				}
...
@keyframes spin {
  100% {
              ...
              <div class="box-body">
              	...
              	<div class="card text-dark bg-light">
	              	...
	              	<div class="row">	
		                  ...
		                  <input data-parsley-required type="text" class="form-control pickerdate" placeholder="Enter Store Issue Note Date" id="doc_date" name="doc_date" value="<?php if($_POST) echo $_POST['doc_date'];elseif($row->doc_date!='') echo date('d-m-Y',strtotime($row->doc_date));else echo date('d-m-Y');?>" onchange="hideandshow()" onclick="hideandshow()">
		                  ...
		                  <?php if($row->id ==''){?>
	              			...
	              			<div id="ind" name="ind"  style="display:none;">
		              			...
		              			<div class=" col-xs-6 col-md-4 col-sm-6" style="display:none;">
		              				...
		              				<input type="text" class="form-control" placeholder="Enter SIN No" id="doc_no" name="doc_no" value="<?php if($_POST) echo $_POST['doc_no'];else echo stripslashes($row->doc_no);?>" readonly>
		              				</div> 
			                	</div>
	...
	//echo "SELECT A.doc_no,A.doc_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent WHERE  A.id IN ($row->id_inv_po) GROUP BY B.id_inv_indent";
						   //data-parsley-required data-parsley-errors-container="#outletError2";
						   //req_details?>
							
							 <select class="form-control select2" autocomplete="off" name="id_req_no[]" multiple id="id_req_no" onchange="req_details(this,this.id);" style="width:100%" >
										<?php $categoryDropDown = '<option value="">Select Requestion No</option>';

										if($row->id != ''){
											//$sql="SELECT * FROM ".TBL_INV_INDENT." WHERE id IN ($row->id_inv_po)";
											$sql="SELECT A.doc_no,A.doc_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent WHERE  A.id IN ($row->id_inv_po)  GROUP BY B.id_inv_indent";
										}else{
											$sql="SELECT A.doc_no,A.doc_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent
						                   	WHERE A.doc_type=1 AND A.id_shop=".$_SESSION['shop']." AND B.bal_qty>'0' GROUP BY B.id_inv_indent";
												...
												$categoryDropDown .= '<option '.$selected.' value="'.$row1->id_inv_indent.'-'.$row1->id.'">'.ucfirst($row1->doc_no.' | '.date('d-m-Y',strtotime($row1->doc_date))).'</option>';
												}
...
//echo $sql="SELECT A.doc_no,A.doc_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent WHERE  B.id IN (".$array['id_inv_indent_details'.''.$j].") GROUP BY B.id_inv_indent"; ?>
					                 ...
					                 <select class="form-control select2" name="id_inv_indent<?php echo $k;?>" id="id_inv_indent<?php echo $k;?>" onchange="popupshow(this.id);" style="width:100%" data-parsley-required data-parsley-errors-container="#outletError3" >
										...
										//$sql="SELECT A.doc_no,A.doc_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent  	WHERE A.doc_type=1 AND A.id_shop=".$_SESSION['shop']." GROUP BY B.id_inv_indent";


										if($row->id != ''){
											$sql="SELECT A.doc_no,A.doc_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent WHERE  B.id IN (".$array['id_inv_indent_details'.''.$j].")   GROUP BY B.id_inv_indent";
											...
											//$sql="SELECT A.doc_no,A.doc_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent WHERE  A.id IN ($row->id_inv_po) GROUP BY B.id_inv_indent";
						                   	...
						                   	WHERE A.doc_type=1 AND A.id_shop=".$_SESSION['shop']."  AND B.bal_qty>'0' GROUP BY B.id_inv_indent";
												...
												$categoryDropDown .= '<option '.$selected.' value="'.$row1->id.'-'.$row1->id_inv_indent.'">'.ucfirst($row1->doc_no.' | '.date('d-m-Y',strtotime($row1->doc_date))).'</option>';
												}
												if($row->id !=''){
              ...
              <div class="overlay" style="background: #2222225c!important; z-index : 999!important;">
...
<?php 
	$sql2 = " SELECT max(doc_date) as doc_date FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".'6'."' ";
		$db->query($sql2);   
			while($row2 = $db->fetch_object()){ 
 	...
 	$( document ).ready(function() {
 		...
 		/*	if($row->id == ''){
			...
			while($row2 = $db->fetch_object()){ 
				$doc_date= $row2->doc_date; 
				if($doc_date == ''){
					$doc_date = selectColumn(TBL_DOC_TYPE_CONFIG,'effective_date'," WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='6' ");
				}
				$doc_date = date('d-m-Y' , strtotime(addslashes($doc_date)));  
			} */ 
...
*/	
       ...
       cols1 += '<td><select onchange="popupshow(this.id)"  name="id_inv_indent' + counter1 + '" id="id_inv_indent' + counter1 + '" class="form-control select3"  style="width:100%"><option>Select Requestion No</option><?php 
	                //$sql = "SELECT inv_indent.doc_date, inv_indent.mdoc_no,  inv_indent.doc_no,  inv_indent_details.qty,inv_indent_details.alt_qty, inv_indent_details.id, inv_indent_details.id_inv_indent, inv_indent_details.main_unit, inv_indent_details.alt_unit,inv_items.item_code, inv_items.name,  	mst_attributes.field_value 	FROM inv_items, mst_attributes, inv_indent_details, inv_indent WHERE mst_attributes.id=inv_indent.id_mst_attributes_department and inv_indent.id = inv_indent_details.id_inv_indent and inv_indent_details.id_inv_items = inv_items.id  and inv_indent.id_shop = '".addslashes($_SESSION['shop'])."' and inv_indent.doc_type = '2' group by inv_indent.doc_no  ";
