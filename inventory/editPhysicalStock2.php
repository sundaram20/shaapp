...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Save') && empty($_POST['eId'])){//add 
			 ...
			 $sql5 = " SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_no`='".$doc_no."'  and `doc_type` = '9'  ";
				$db->query($sql5);
				$numRows= $db->num_rows();
					if($numRows > 0){
...
<div class="content-wrapper">
   ...
   <?php  $session=$_GET['submenu']; ?>
	...
	<?php echo $sql2 ="SELECT inv_items.item_code, inv_items.name, 
						                   	mst_attributes.field_value 
						                   	FROM inv_items, mst_attributes WHERE mst_attributes.id=inv_items.id_mst_attributes_group_main and inv_items.status=1  and inv_items.id_shop = '".addslashes($_SESSION['shop'])."'  ";?>
    <!-- Main content -->
              	...
              	<div class="card text-dark bg-light">
	              	...
	              	<div class="row">	
		                  ...
		                  <input data-parsley-required type="text" class="form-control pickerdate" placeholder="Enter Physical Stock Date" id="doc_date" name="doc_date" value="<?php if($_POST) echo $_POST['doc_date'];elseif($row->doc_date!='') echo date('d-m-Y',strtotime($row->doc_date));else echo date('d-m-Y');?>" onchange="hideandshow()" onclick="hideandshow()">
                           ...
                           <input style="display: none;"  type="text" class="form-control pickerdate" placeholder="Enter Physical Stock Date" id="doc_date1" name="doc_date1" value="<?php if($_POST) echo $_POST['doc_date'];else echo stripslashes($row->doc_date);?>" >
		                  </div> 
	              		</div>
				            ...
				            <tbody>
				                ...
				                <tr> 
				                    ...
				                    <td class="form-group col-md-2">  
					                	<select data-parsley-required class="form-control select2" name="id_mst_attributes_store<?php echo $k;?>" id="id_mst_attributes_store<?php echo $k;?>" style="width:100%" data-parsley-errors-container="#id_mst_attributes_store<?php echo $k;?>Error">
...
//Select 2  Resolve Here
    ...
    $("#addrow1").on("click", function () { 	alert('Add Row');
       ...
       cols1 += '<td><select onchange="itemget(this.id)" name="id_inv_items' + counter1 + '" id="id_inv_items' + counter1 + '" class="form-control select3"  style="width:100%"><option>Select Item Code</option><?php 
				 	 $sql2 ="SELECT inv_items.id,inv_items.item_code, inv_items.name, mst_attributes.field_value FROM inv_items, mst_attributes WHERE mst_attributes.id=inv_items.id_mst_attributes_group_main and inv_items.status=1  and inv_items.id_shop = '".addslashes($_SESSION['shop'])."'";
	                   	 $db->query($sql2); 
	                    while($row1 = $db->fetch_object()){ ?><option value="<?php echo $row1->id; ?>"><?php echo $row1->item_code.' | '.$row1->name.' | '.$row1->field_value; ?></option> <?php } ?></select> </td>';
