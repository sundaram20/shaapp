...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Save') && empty($_POST['eId'])){//add 
			 ...
			 if($doc != '12'){
				 $sql5 = " SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_no`='".$doc_no."'  and `doc_type` = '4'  and `id_doc_type_configuration` = '".addslashes($_POST['id_doc_type_configuration'])."'  ";
			 }else{
				 $sql5 = " SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_no`='".$doc_no."'  and `doc_type` = '12'  and `id_doc_type_configuration` = '".addslashes($_POST['id_doc_type_configuration'])."'  ";
			 }
//echo $sql5;
...
if(others_charges_igst_percent=='undefined' || others_charges_igst_percent=='' ){
	var oig = 0;
}else{
		 	 ...
		 	 cols1 += '<td><div id="local'+counter1+'" name="local'+counter1+'" style="display:none;"><select  name="id_mst_charges_purchase_local' + counter1 + '" id="id_mst_charges_purchase_local' + counter1 + '" class="form-control select3" onchange="po_locals(this.id)" style="width:100%;"><option>Select Tax Register</option><?php 
	                ...
	                $sql = "SELECT mst_charges.*FROM mst_charges WHERE mst_charges.id_shop = '".addslashes($_SESSION['shop'])."' AND  charges_account = '2' and transaction_type = '2'";
	                    ...
	                    while($row1 = $db->fetch_object()){ ?>
	                  	...
	                  	cols1 += '<td><div id="localsss'+counter1+'" name="localsss'+counter1+'" style="display:none;"><div style="color:red;font-size:11px;" id="s_amount'+counter1+'"></div> <div style="color:red;font-size:11px;" id="c_amount'+counter1+'"></div></div><div id="interstatesss'+counter1+'" name="interstatesss'+counter1+'" style="display:none;"><div style="color:red;font-size:11px;" id="i_amount' + counter1 + '" name="i_amount' + counter1 + '"></div></div></td>';
...
//alert(match);
		...
		//$("#trdelete" + match).remove();
		
		//$("#rate_per_main_unit"+match).rules("remove", "required"); 
		//$("#id_inv_po"+match).rules("remove", "required"); 
		$("#rate_per_main_unit"+match).removeAttr("required");
		$("#id_inv_po"+match).removeAttr("required");
 			$("#trdelete"+match).hide();
		 ...
		 //var newRow11 =  $('<tr id="trdeletes' + counter4 + '">');
		 	 ...
		 	 cols1 += '<td><div id="local'+counter4+'" name="local'+counter4+'" style="display:none;"><select  name="id_mst_charges_purchase_local' + counter4 + '" id="id_mst_charges_purchase_local' + counter4 + '" class="form-control select3" onchange="po_locals(this.id)" style="width:100%;"><option>Select</option><?php 
	                ...
	                $sql = "SELECT mst_charges.*FROM mst_charges WHERE mst_charges.id_shop = '".addslashes($_SESSION['shop'])."' AND  charges_account = '2' and transaction_type = '2'";
	                    ...
	                    while($row1 = $db->fetch_object()){ ?>
	                  		<option value="<?php echo $row1->id; ?>"><?php echo $row1->name ?></option> <?php }?></select></div></td> '; 
	                  	cols1 += '<td><div id="localsss'+counter4+'" name="localsss'+counter4+'" style="display:none;"><div style="color:red;font-size:11px;" id="s_amount'+counter4+'"></div> <div style="color:red;font-size:11px;" id="c_amount'+counter4+'"></div></div><div id="interstatesss'+counter4+'" name="interstatesss'+counter4+'" style="display:none;"><div style="color:red;font-size:11px;" id="i_amount' + counter4 + '" name="i_amount' + counter4 + '"></div></div></td>';
