...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `field_value` = '".addslashes($_POST['field_value'])."' and `table_name` = 'party_category' ";
			$db->query($sql);
			$numRows= $db->num_rows();
			if($numRows == '0'){
			...
			$addSql = "   	INSERT INTO `".TBL_ATTRIBUTES."` SET
							...
							`table_name` = 'party_category',
							`field_name` = 'party_category_name',
			...
			if(executeSql($addSql)){
				...
				$_SESSION['successMsg'] = 'New party category details has been added sucessfully.';
				header("location:managePartyCategory.php?eId=".encryptor(encrypt,$lastInsertId)."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
				$_SESSION['errorMsg'] = 'party category details has not been saved. Please make corrections below.';
			}
			}else{
				$err++;
				$_SESSION['errorMsg'] = 'party category Master Name Already Exist.';
			}
		}
		...
		//Update Section Here
			...
			if(executeSql($editSql)){
				...
				header("location:managePartyCategory.php?eId=".$_GET['eId']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
		...
		$err++;
		$_SESSION['errorMsg'] = 'party category details has not been saved. Please make corrections.';
	}
}
// ----------cate---------
          ...
          <!-- general form elements -->
            ...
            <!-- form start -->  			        
			 <form name="form1"  method="post" enctype="multipart/form-data" data-parsley-validate autocomplete="off" >
              ...
              <div class="box-body">
				 ...
				 <div class="form-group">
                  <label for="name">Party Category Name<font color="#FF0000">*</font></label>
                  <div class="input-group"> 
              			<div class="input-group-addon">
							<i class="fa fa-crosshairs"></i> 
					   	</div>
                  <input type="text" class="form-control" placeholder="Enter Party Category Name" id="field_value" name="field_value" value="<?php if($_POST) echo $_POST['field_value'];else echo stripslashes($row->field_value);?>"  data-parsley-required>
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
			   <input type='button' value='Cancel' class="btn btn-danger" onclick='location.replace("managePartyCategory.php"); '>
			 </div>
            </form>			
          </div>
          <!-- /.box -->
