  ...
  $id_inv_poo = rtrim($id_po,',');			
			...
			if(1){
				...
				if($_POST['another']!=''){
					...
					header("location:editStoreIssueNote.php?eId=".addslashes(encryptor(encrypt,$lastInsertId))."&submenu=".$_GET['submenu']."&action=edit&page=".$_REQUEST['page']."&print=1");
				}
	...
	executeSql($auditeditSql);
			...
			if(1){  
				...
				if($_POST['another']!=''){
					...
					header("location:editStoreIssueNote.php?eId=".$_GET['eId']."&submenu=".$_GET['submenu']."&action=edit&page=".$_REQUEST['page']."&print=1"); 
				}
					...
					<?php if($row->id !=''){
	              		...
	              		<div class="form-group col-xs-12 col-md-4 col-sm-2" >
	              			...
	              			<div class="input-group-addon">
						   	...
						   	<select class="form-control select2" name="id_mst_attributes_department" id="id_mst_attributes_department" onchange="departments()" data-parsley-required <?php echo $readonly; ?> style="width:100%">
	...
	//echo "SELECT A.indent_no,A.indent_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent WHERE  A.id IN ($row->id_inv_po) GROUP BY B.id_inv_indent"; ?>
		        ...
		        <!-- The Modal -->
					    ...
					    <div class="modal-dialog">
					      <div class="modal-content"  style="width: 120%;">
					        ...
					        <div class="modal-header">
					          <h4 class="modal-title">Store Issue Note</h4>
		         ...
		         <div class="box-body">
              	...
              	<div class="card text-dark bg-light">
              		<div class="bg-primary text-center ">
              			<h5 style="padding: 5px;">Store Issue Note Details</h5>
              		</div>  
	              	<div class="row">
	              		<?php 
...
//echo $sql="SELECT A.indent_no,A.indent_date,B.* FROM ".TBL_INV_INDENT." A LEFT JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent WHERE  B.id IN (".$array['id_inv_indent_details'.''.$j].") GROUP BY B.id_inv_indent"; ?>
					                 ...
					                 <select class="form-control select2" name="id_inv_indent<?php echo $k;?>" id="id_inv_indent<?php echo $k;?>" onchange="popupshow(this.id);" style="width:100%" required >
										<?php $categoryDropDown = '<option value="">Select Requestion No</option>';
...
<!-- End Another Modal -->					
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp; 
			   <input type='button' value='Cancel' class="btn btn-success" onclick='location.replace("manageStoreIssueNote.php?submenu=<?php echo $_GET['submenu']; ?>"); '>
