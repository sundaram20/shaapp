	 ...
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border"> 
		  <div class="btn-group  pull-right">
							<a type="button" class="btn btn-success" href="editPurch.php?doc_type=<?php echo $doc_id; ?>&submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>" >Add <?php echo $add; ?></a>
            ...
            <div class="box-body table-responsive">
		       ...
		       ///	echo "SELECT * FROM inv_purch  where id_shop= '".addslashes($_SESSION['shop'])."'  and doc_type = '".$doc_type."' order by last_modified DESC ";
                 ...
                 <td><?php echo $sqlUserDetail = selectColumn(TBL_USERS,'name','where id="'.$row->id_mst_user_created_by.'" '); ?></td>  
                  ...
                  <td><?php echo $row->status=='1'?'<span onclick="location.href=\'managePurch.php?inactiveId='.encryptor(encrypt, $row->id).'&submenu='.$_GET['submenu'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'managePurch.php?activeId='.encryptor(encrypt,$row->id).'&submenu='.$_GET['submenu'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>		

				  <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editPurch.php?eId=<?=encryptor(encrypt, $row->id)?>&action=edit&page=<?=$_REQUEST['page']?>&submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>&doc_type=<?php echo $doc_id; ?>&print=0';" />&nbsp;&nbsp;&nbsp;&nbsp;
