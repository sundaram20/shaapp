...
if($_REQUEST['action'] == 'change'){
  if($_REQUEST['activeId'] != ''){
    //checkUserLevelPermission($_SESSION['userLevel'],TBL_OFFER_MASTER,'activate');
    $statusId = addslashes(encryptor(decrypt,$_REQUEST['activeId']));
    $statusSql = "  UPDATE `".TBL_OFFER_MASTER."`
            SET `status` = '1'
            ,`last_modified` = '".currenDateTime()."'
            ,`id_mst_user_modified_by` = '".$_SESSION['userId']."'
            WHERE `id` = '".addslashes($statusId)."'";
  }elseif($_REQUEST['inactiveId'] != ''){
    //checkUserLevelPermission($_SESSION['userLevel'],TBL_OFFER_MASTER,'deactivate');
  ...
  if(executeSql($statusSql)){
    $err = 0;
    $_SESSION['successMsg'] = 'Room Type '.selectColumn(TBL_OFFER_MASTER,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';
    
  }else{
    $err = 1;
    $_SESSION['errorMsg'] = 'Room Type '.selectColumn(TBL_OFFER_MASTER,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
  }
}
else if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){
  //checkUserLevelPermission($_SESSION['userLevel'],TBL_OFFER_MASTER,'delete');
  $delSql = "DELETE FROM `".TBL_OFFER_MASTER."` WHERE `id` = '".$_REQUEST['delId']."'";
  
  $sqlDelUserLevel = selectRow(TBL_OFFER_MASTER," WHERE `id` = '".$_REQUEST['delId']."'");
...
if($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])){
  ...
  $delSql = "DELETE FROM `".TBL_OFFER_MASTER."` WHERE `id` IN (".addslashes($deleteIds).")";
  if(executeSql($delSql)){    
    $err = 0;
...
// ----------cate---------
$sql = " SELECT * FROM `".TBL_OFFER_MASTER."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."'  ";
if($_REQUEST['id_hotel'] != ''){
  $sql .= " AND `id_hotel` = '".addslashes($_REQUEST['id_hotel'])."'";
}
if($_REQUEST['offer_type'] != ''){
  $sql .= " AND `offer_type` = '".addslashes($_REQUEST['offer_type'])."'";
}

if($_REQUEST['offer_name']!=''){
  $sql .= " AND `offer_name` LIKE '%".addslashes($_REQUEST['offer_name'])."%'";
}
   ...
   <div class="form-group has-error" align="center">
    ...
    <?php unset($_SESSION['successMsg']);}?>
      ...
      <div class="btn-group  pull-right">
                <a type="button" class="btn btn-success" href="editOfferMaster.php" >Add New Offer</a>
                &nbsp;&nbsp;
                ...
                <?php ?><li><a title="Export to excel file" href="exportTable.php?fileType=xls&tableName=<?php echo TBL_OFFER_MASTER;?>"><img src="../images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>
                <li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo TBL_OFFER_MASTER;?>"><img  src="../images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li><?php ?>
    ...
    <form name="searchForm" action="" method="get">
      ...
      <div class="col-md-6">
        ...
        <?php 
          ...
          if($_REQUEST['offer_type'] == '1'){
              $selected1 = 'selected="selected"';
          }elseif($_REQUEST['offer_type'] == '2'){
              $selected2 = 'selected="selected"';
          }
          elseif($_REQUEST['offer_type'] == '3'){
              $selected2 = 'selected="selected"';
              
          }
          elseif($_REQUEST['offer_type'] == '4'){
              $selected4 = 'selected="selected"';
          }
          echo $statusDropDown = '<select class="form-control select2" name="offer_type"> <option value="">---SELECT TYPE---</option>
          <option '.$selected1.' value="1">Special Offer</option>
          <option '.$selected2.' value="2">Package Deal</option>
          <option '.$selected3.' value="3">Advance Purchase</option>
            ...
            <div class="col-md-6">
              ...
              <input type="text" placeholder="Type offer name..." name="offer_name" class="form-control">
              </div>
            </div>
          </div>
          <!-- /.row -->
                ...
                <tr>
                  ...
                  <?php
                    ...
                    if($row->offer_type==1)
                      ...
                      $offer_type='Package Deal';
                    else if($row->offer_type==3)
                      $offer_type='Advance Purchase'; 
                    else if($row->offer_type==4)  
                      $offer_type='Promo Code Offer';    
                  ?>
                  <td><?=$offer_type;?></td>
                  <td><?=date('d-M-Y',strtotime($row->valid_from)).' to '.date('d-M-Y',strtotime($row->valid_till));?></td>
                  <td><?=$row->status=='1'?'<span  style="color:green;cursor:pointer;">Active</span>':'<span   style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>  

          <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editOfferMaster.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;</td>
                </tr>
               <?php }?> 
         <!--<tr>
