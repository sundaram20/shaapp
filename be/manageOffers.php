...
//checkUserLevelPermission($_SESSION['userLevel'],TBL_OFFER,'view');
if($_REQUEST['action'] == 'change'){
	if($_REQUEST['activeId'] != ''){
		//checkUserLevelPermission($_SESSION['userLevel'],TBL_OFFER,'activate');
		$statusId = addslashes(encryptor(decrypt,$_REQUEST['activeId']));
		$statusSql = "	UPDATE `".TBL_OFFER."`
						SET `status` = '1'
						,`last_modified` = '".currenDateTime()."'
						,`id_mst_user_modified_by` = '".$_SESSION['userId']."'
						WHERE `id` = '".addslashes($statusId)."'";
	}elseif($_REQUEST['inactiveId'] != ''){
		//checkUserLevelPermission($_SESSION['userLevel'],TBL_OFFER,'deactivate');
	...
	if(executeSql($statusSql)){
		$err = 0;
		$_SESSION['successMsg'] = 'Room Type '.selectColumn(TBL_OFFER,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';
		
	}else{
		$err = 1;
		$_SESSION['errorMsg'] = 'Room Type '.selectColumn(TBL_OFFER,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
	}
}
else if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){
	//checkUserLevelPermission($_SESSION['userLevel'],TBL_OFFER,'delete');
	$delSql = "DELETE FROM `".TBL_OFFER."` WHERE `id` = '".$_REQUEST['delId']."'";
	
	$sqlDelUserLevel = selectRow(TBL_OFFER," WHERE `id` = '".$_REQUEST['delId']."'");
...
if($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])){
	...
	$delSql = "DELETE FROM `".TBL_OFFER."` WHERE `id` IN (".addslashes($deleteIds).")";
	if(executeSql($delSql)){		
		$err = 0;
...
// ----------cate---------
$sql = " SELECT A.offer_name,A.offer_type,A.valid_from,A.valid_till,B.id_hotel,B.id FROM `".TBL_OFFER_MASTER."` A RIGHT JOIN ".TBL_OFFER." B ON A.id=B.id_offer_master WHERE A.`id_shop` = '".addslashes($_SESSION['shop'])."'  ";
if($_REQUEST['id_hotel'] != ''){
	$sql .= " AND `id_hotel` = '".addslashes($_REQUEST['id_hotel'])."'";
}
if($_REQUEST['offer_type'] != ''){
	$sql .= " AND A.`offer_type` = '".addslashes($_REQUEST['offer_type'])."'";
}

if($_REQUEST['offer_name']!=''){
  $sql .= " AND A.`offer_name` LIKE '%".addslashes($_REQUEST['offer_name'])."%'";
}

if($_REQUEST['order'] != ''){
	$sql .= " ORDER BY `last_modified` DESC";
}else{
	$sql .= "  ORDER BY B.`last_modified` DESC";
}
	 ...
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border">
		  ...
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn btn-success" href="editOffers.php" >Link Offer</a>
							  &nbsp;&nbsp;
							  ...
							  <ul class="dropdown-menu" role="menu">
								<?php ?><li><a title="Export to excel file" href="exportTable.php?fileType=xls&tableName=<?php echo TBL_OFFER;?>"><img src="../images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>
								<li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo TBL_OFFER;?>"><img  src="../images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li><?php ?>
			...
			<div class="col-md-4">
              <div class="form-group">
				...
				<?php 
				  ...
				  <option '.$selected4.' value="4">Promo Code Offer</option>
				  </select>';?>
              </div>
              <!-- /.form-group -->
            </div>
