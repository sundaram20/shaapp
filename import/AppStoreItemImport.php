...
//echo "<pre>";print_r($emapData);
   if ($count == 1) {
    	...
    	if($count>=1){   
				...
				$id_mst_attributes_group_main=trim($emapData[3]);  
				$id_mst_attributes_group_sub=trim($emapData[4]);  
				
				$id_mst_attributes_unit_main=trim($emapData[5]);	 
				$id_mst_attributes_unit_alt=trim($emapData[6]); 				
				
			
  $sqlgroup_main = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='item_group_main' AND  LOWER(field_value) LIKE '".strtolower(trim($emapData[3]))."' ";
	 ...
	 $sqlgroup_sub = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='item_group_sub' AND  LOWER(field_value) LIKE '".strtolower(trim($emapData[4]))."' ";
	 $resTogroup_sub = mysqli_query($connNew2,$sqlgroup_sub);
	  ...
	  $sqlunitMain = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='unit' AND  LOWER(field_value) LIKE '".strtolower(trim($emapData[5]))."' ";
	 $resTounitMain = mysqli_query($connNew2,$sqlunitMain);
	  ...
	  $sqlunitSub = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE  status='1' AND table_name='unit' AND  LOWER(field_value) LIKE '".strtolower(trim($emapData[6]))."' ";
	 $resTounitSub = mysqli_query($connNew2,$sqlunitSub);
		...
		$id_mst_attributes_unit_alt= $rowunitSub->id;
			//print_r($rowunitSub);
	            ...
	            $store='864';//$emapData[11]; 								
	            $itemMenuType='970'; // 17 is Ingredients
			$min_qty=$emapData[15]==''?'0.00':$emapData[15]; 	
			$max_qty=$emapData[16]==''?'0.00':$emapData[16]; 	
			
   $queryCompany = "SELECT item_code,name FROM inv_items WHERE name = '".$ItemName."' and item_code='".$itemCode."' and id_mst_attributes_item_type='".$itemMenuType."'" ;
...
$resultCompany = mysqli_query($connNew2,$queryCompany);
 $NumberCompany = mysqli_num_rows($resultCompany);
			...
			echo '<br><br><br><p style="color:red;font-weight:bold;">ItemCode Not Exist Error'.$itemCode.'================'.$ItemName.'</p><br>';
				if (empty($id_mst_attributes_group_main)) {
    echo "Error: id_mst_attributes_group_main is NULL<br>";
}
if (empty($id_mst_attributes_group_sub)) {
    echo "Error: id_mst_attributes_group_sub is NULL<br>";
}
if (empty($id_mst_attributes_unit_main)) {
    echo "Error: id_mst_attributes_unit_main is NULL<br>";
}
if (empty($id_mst_attributes_unit_alt)) {
    echo "Error: id_mst_attributes_unit_alt is NULL<br>";
}
	...
	//	echo  $NumberCompany.'==--->$id_mst_attributes_group_main:'.$id_mst_attributes_group_main.'===item_group_sub:'.$id_mst_attributes_group_sub.'===id_mst_attributes_unit_main:'.$id_mst_attributes_unit_main.'===id_mst_attributes_unit_alt:'.$id_mst_attributes_unit_alt;	
