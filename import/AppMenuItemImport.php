...
$DB_USERNAME='qds_hqce';
$DB_PASSWORD='&%HH90Oysb8vikfq';
echo 'Database Name: '.$DB_NAME='qds_hqce';
...
//echo "<pre>";print_r($emapData);
    	...
    	if($count>=1){   
				...
				$ids_mst_outlet='1';$emapData[26]; 
					$conversion_qty=$emapData[8];

	 $sqlMenu = "SELECT id FROM `mst_attributes` WHERE    table_name='items_type' AND field_value='Menu' ";
	//$sqlMenu = "SELECT id FROM `mst_attributes` WHERE    table_name='items_type' AND field_value='Laundry' ";		
	 $resToNat = mysqli_query($connNew,$sqlMenu);
 	 
	 $rowNat =  mysqli_fetch_object($resToNat);
		 $id_mst_attributes_item_type	=	$rowNat->id;
			
			
			
			$sqlgroup_main = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='item_group_main' AND  LOWER(field_value) = '".strtolower(trim($emapData[2]))."' ";
	 ...
	 $sqlgroup_sub = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='item_group_sub' AND  LOWER(field_value) = '".strtolower(trim($emapData[3]))."' ";
	 ...
	 $sqlunitMain = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='unit' AND  LOWER(field_value) = '".trim(strtolower($emapData[4]))."' ";
	 ...
	 $sqlunitSub = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE  status='1' AND table_name='unit' AND  LOWER(field_value) LIKE '%".trim(strtolower($emapData[5]))."%' ";
...
if($NumberCompany=='0' || $NumberCompany==''){		
			$addNewCompanyName ="INSERT INTO `inv_items` SET 
							 `item_code`='".$itemCode."',
							 `name`='".$ItemName."',
							 `id_mst_attributes_item_type`='".$id_mst_attributes_item_type."',
