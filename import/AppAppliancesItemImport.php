...
$DB_USERNAME='unit_bgr';
$DB_PASSWORD='5u0&a8q1S';
echo 'Database Name: '.$DB_NAME='unit_bgr';
...
//echo "<pre>";print_r($emapData);
    	...
    	if($count>=1){   
				...
				$itemCode=$emapData[0]; 
				$ItemName=$emapData[1]; 
				$id_mst_attributes_group_main=$emapData[2];  
				$id_mst_attributes_group_sub=$emapData[3];  
				
				$id_mst_attributes_unit_main=$emapData[4];	 
				$id_mst_attributes_unit_alt=$emapData[5]; 				
				//$rate=$emapData[6]; 
				
				//$id_mst_charges_sales_local=$emapData[7]; 
				//$id_mst_attributes_printer=$emapData[11]; 
				//$ids_mst_outlet=$emapData[26]; 
				//id_shop='".$_SESSION['shop']."' and status = '1' AND table_name ='".'item_group_main'."'
			
  $sqlgroup_main = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='item_group_main' AND  LOWER(field_value) = '".strtolower($emapData[2])."' ";
	 ...
	 $sqlgroup_sub = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='item_group_sub' AND  LOWER(field_value) = '".strtolower($emapData[3])."' ";
	 ...
	 $sqlunitMain = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE   status='1' AND table_name='unit' AND  LOWER(field_value) = '".strtolower($emapData[4])."' ";
	 ...
	 $sqlunitSub = "SELECT * FROM `".TBL_ATTRIBUTES."` WHERE  status='1' AND table_name='unit' AND  LOWER(field_value) LIKE '%".strtolower($emapData[5])."%' ";
	 $resTounitSub = mysqli_query($connNew2,$sqlunitSub);
		...
		$id_mst_attributes_unit_alt= $rowunitSub->id;
			//print_r($rowunitSub);
	            $open_qty=$emapData[18]; 		
	           // $store='860';//$emapData[11]; 								
	            $itemMenuType='906'; // 17 is Ingredients
			$min_qty=$emapData[15]==''?'0.00':$emapData[15]; 	
			$max_qty=$emapData[16]==''?'0.00':$emapData[16]; 	
			
  echo $queryCompany = "SELECT item_code,name FROM inv_items WHERE name = '".$ItemName."' and item_code='".$itemCode."' " ;
...
$NumberCompany = mysqli_num_rows($resultCompany);
			...
			echo '<br><br><br>'.$id_mst_attributes_group_main;
			echo '<br><br><br>'.$id_mst_attributes_group_sub;
			echo '<br><br><br>'.$id_mst_attributes_unit_main;
			echo '<br><br><br>'.$id_mst_attributes_unit_alt; 
			echo '<br><br>'.$NumberCompany;

if(($NumberCompany=='0') && $id_mst_attributes_group_main>0 && $id_mst_attributes_group_sub>0 && $id_mst_attributes_unit_main>0 && $id_mst_attributes_unit_alt>0 ){		
