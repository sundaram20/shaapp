...
if($appNumberOfRows=='1'){	
	...
	executeSql("Insert into api_request set channel_id = '1' , request='".$postData."',date_created='".date('Y-m-d H:i:s')."',company_name='".$CompanyName."',response_status='0',id_pms_response='0',failed_at='".date('Y-m-d H:i:s')."',count='1',booking_type='Commit' ,booking_referance_id='".$HotelReservations_ID."'");
...
if(@$xmlarray['Body']['OTA_HotelResNotifRQ']['@attributes']['ResStatus']=='Cancel'){
	...
	$sql = "SELECT id FROM fo_reservations WHERE other_reference='$otherRefrenceId'";
	$result = mysqli_query($connNew, $sql);
	...
	if($id_order > 0 && $id_order!=''){
		 ...
		 $updateSql = "UPDATE ".FO_RESERVATIONS." SET booking_status='4',last_modified='".date('Y-m-d H:i:s')."'  WHERE id='".$id_order."' AND other_reference='".$otherRefrenceId."' ";
		  ...
		  if (mysqli_query($connNew, $updateSql)) {
			...
			executeSql("Insert into api_request set channel_id = '1' , request='".$postData."',date_created='".date('Y-m-d H:i:s')."',company_name='0',booking_referance_id='0',response_status='0',id_pms_response='0',failed_at='".date('Y-m-d H:i:s')."',booking_type='cancel'");

			echo '<?xml version="1.0" encoding="UTF-8"?>
				<OTA_HotelResNotifRS xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
				xmlns:xsd="http://www.w3.org/2001/XMLSchema"
				xmlns="http://www.opentravel.org/OTA/2003/05" EchoToken="'.$reference.'" CrsResID_Value="'.$id_order.'"  PmsResID_Value="'.$otherRefrenceId.'" TimeStamp="'.date('Y-m-d H:i:s').'" Version="1.0">
...
//print_r($RoomStayCount);
	...
	echo '==============>'.$RoomType_NumberOfUnits=$RoomRateArray[$RoomRatecounts]['@attributes']['NumberOfUnits'];
 	$dated 			 = $checkin;
...
foreach($OtherChargesArrayList as $otherInc=>$OtherChargesArrayValue){
 ...
 $addons_details = " INSERT INTO `".FO_RESERVATION_ADDONS_DETAILS."` SET
		...
		`item` = '".$OtherChargesArrayValue['chargesDescription']."',
...
echo 'Invalid Username and Password'."select * from app_shops_api Where user_name='".$apiUserName."' and password='".$apiPassword."'";
