...
//print_r($RoomStayCount);
	...
	$SqlCheckOrderBY1 = mysqli_query($connNew,"SELECT * FROM fo_reservations_details WHERE id_fo_reservations='".$id_fo_reservations."' AND `id_mst_hotels`='".$id_mst_hotels."' AND `id_mst_guest`='".$id_mst_guest."'    order by id desc ");
		...
		if($NumRowOrderBY1	>0){ echo 'Selected	=';
...
while(strtotime($dated)!=strtotime($checkout)){ echo '<br>=============1';
...
$resRoomId = mysqli_fetch_object($queryRoomId);
	...
	$date_created = date('Y-m-d');
		...
		echo  $Insert_into_Order_Details= "INSERT INTO fo_reservations_details (id_fo_reservations,id_mst_hotels,id_mst_guest,id_mst_room_types,plan,id_rate,id_fo_rate_plan,dated,room_quantity,adults_per_room,tariff_price_per_day_per_room,tax_per_day_per_room,unique_code,checkin_status,id_shop,order_by_room) 
