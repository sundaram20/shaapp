...
WHERE pp.pos_bill_type='2' and pp.cancelled=0 and pp.doc_type=21 and pp.id_shop= '2'  GROUP BY  pp.id_mst_outlet,pp.id_mst_country_lang order by confimed_revenue asc"  ;
	...
	while($rowList = mysqli_fetch_object($resultList)){
	  ...
	  $outletName=  selectColumn(TBL_OUTLETS,'name'," WHERE `id` = '".$rowList->id_mst_outlet."'");
		...
		$reportChartArray['nationality'][strtolower($companyname==''?$emptytext7:$companyname)]['confimed_revenue']=$newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue);
		$reportChartArray['nationality'][strtolower($companyname==''?$emptytext7:$companyname)]['nationality']=$companyname;
		$reportChartArray['nationality'][strtolower($companyname==''?$emptytext7:$companyname)]['piechartName']=$companyname.'('.$newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue).')';
