...
function fetchdataprint($pos_purch_id_array,$grouparray=0){	
	...
	*/
	...
	$posPurchResult = mysqli_fetch_object($resultPosPurch);
		...
		// $pos_purch_detail_sql="SELECT * FROM ".TBL_INV_PURCH_DETAILS." AS A  WHERE `id` IN (98,105) ";
		...
		if($resResult->id_mst_items_details==0){
			...
			$printgroup['print_BillSplit'][$grouparray]['item_description'][]=$item_name;			
			$printgroup['print_BillSplit'][$grouparray]['item_qty'][]=$resResult->qty;	
			$printgroup['print_BillSplit'][$grouparray]['item_rate'][]=$resResult->rate_per_main_unit;	
			$printgroup['print_BillSplit'][$grouparray]['item_amount'][]=round(($resResult->rate_per_main_unit*$resResult->qty),2);	
			...
			/* calculations---------------------------------------------------------- */
			$printgroup['print_BillSplit'][$grouparray]['sub_total_items']  +=($resResult->rate_per_main_unit*$resResult->qty);			
			$printgroup['print_BillSplit'][$grouparray]['total_item_discount_amount']  += $resResult->discount_amount;			
			$printgroup['print_BillSplit'][$grouparray]['net_amount_items'] +=(($resResult->rate_per_main_unit*$resResult->qty)-$resResult->discount_amount);
			...
			$RoundOfAmount	+=	(($resResult->rate_per_main_unit*$resResult->qty)+$resResult->item_sgst_amount+$resResult->item_cgst_amount+$resResult->item_igst_amount+$resResult->item_cess_amount+$resResult->item_vat_amount+$resResult->item_surcharge_amount);
