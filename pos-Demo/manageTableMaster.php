...
// ----------cate---------
$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' and `table_name` = 'table'  ";
	 ...
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border"> 
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn btn-success" href="editTableMaster.php?submenu=<?php echo $_GET['submenu']; ?>" >Add Table Master</a>
        ...
        <div class="col-xs-12">		     
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Table Master List</h3>
            </div>
            ...
            <div class="box-body table-responsive">
		        ...
		        <tbody>  
                  ...
                  <td><?php echo $i++;?></td>
                   <td><?php echo 
$id_item_type=selectColumn(TBL_ATTRIBUTES,'field_value',"WHERE   id= '".$row->id_table_group."' and id_shop= '".addslashes($_SESSION['shop'])."' ");

				   ?></td>
                  <td><?php echo $row->field_value;?></td>
                  <td><?php echo $row->field_description;?></td>  

                  <td><?php echo $row->status=='1'?'<span onclick="location.href=\'manageTableMaster.php?inactiveId='.encryptor(encrypt,$row->id).'&submenu='.$_GET['submenu'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageTableMaster.php?activeId='.encryptor(encrypt,$row->id).'&submenu='.$_GET['submenu'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>		

                  <?php 
                  		$table_name = array(TBL_PURCH);						
				  		$ajaxCheckTransactions = CheckTransactionsTableMaster($row->id, $table_name); 

						if($ajaxCheckTransactions != '1'){
                 	 ?>
				  <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editTableMaster.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>&submenu=<?php echo $_GET['submenu']; ?>';" />&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='manageTableMaster.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>&submenu=<?=$_GET['submenu']?>';
	   } 
	   else
	   {
	       self.location='manageTableMaster.php?submenu=<?php echo $_GET['submenu']; ?>';
	    }
	   });
	  }
  </script> 
