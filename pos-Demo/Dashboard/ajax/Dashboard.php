		...
		// echo $subGroup;
            ...
            <div class="form-group col-md-10">
              <div class="box-bodyw">
                ...
                <button type="button" style="margin-right: 5px;" class="btn btn-default" id="dateColor2"   name="toggler" value="2" onclick="updateDateQuickSearch('<?php echo  date('d-m-Y', strtotime('-1 days')).' to '.date('d-m-Y', strtotime('-1 days'));?>','0');"> Yesterday </button>
                ...
                <button type="button" style="margin-right: 5px;" class="btn btn-foursquare  mobile-responseset" id="dateColorFinancialYear"   name="toggler" value="FinancialYear" data-target="#modal-warning" onclick="updateDateQuickSearch('<?php echo date("01-04-".$FinanceStarYear).' to '.date("31-03-".$FinanceEndYear);?>','1');"> This Year </button>
		 ...
		 $HeightWidthHotel='width="800" height="750"';
          ...
          <div id="WholeDivshowContent" class="toHideWholeDiv" >
			...
			if($enable_nationality=='1'){?>
                  <div class="col-md-6">
                <h5 class="text-center" style="background-color: #1c4c7c;margin: 5px;padding: 10px;color: #fff;"><strong><span class=""> </span>Nationality Report</strong></h5>   
                    
                        <p class="text-center"> <strong class="col-lg-offset-2">Revenue </strong> 
                          <!--<div class="row">
                              <div class="col-md-3  col-sm-3 col-xs-3 text-center"> &nbsp; </div>
                              <div class="col-md-3  col-sm-3 col-xs-3 text-center" style="background-color: #87CEFA;"> Last Year </div>
                              <div class="col-md-3 col-sm-3 col-xs-3 text-center" style="background-color: #3b8bba ;"> This Year </div>
                        ...
                        <div class="chart ">
                          <canvas id="NationalityPieRevenue-chart" <?php echo $HeightWidthHotel; ?>></canvas>
                        </div>
                      </div>
                  <?php } ?>
...
var enable_nationality;				  
		...
		//if(id_hotel){	
			$.ajax({
				...
				NationalityNameArr=data.NationalityNameArr;
				   var enable_nationality=data.enable_nationality;
                   alert(enable_nationality);
                    //alert(CustomeReportValuesNameData);
				    performanceChart(graphCount,viewMonthwise,enable_nationality);
    ...
    function performanceChart(graphCount,viewMonthwise,enable_nationality){
		...
		if(graphCount>0){
			...
			if(enable_nationality=='1'){
			NationalityRevenuePieChart.destroy();
			}
		...
		let ytdMtdChartHorizontalBarRevenue  = document.getElementById('horizontalBar-Ytd-Mtd-Revenue').getContext('2d');
	  	if(enable_nationality=='1'){
			let NationalityPiePerChart = document.getElementById('NationalityPieRevenue-chart').getContext('2d');
	...
	if(enable_nationality=='1'){
NationalityRevenuePieChart = new Chart(NationalityPiePerChart, {
 type: 'pie',
  	// The data for our dataset
            data: {
    labels: NationalityNameArr,
    datasets: [ { 
        data: NationalityRevenueArr,
        label: 'Grand Total : '+NationalityRevenueArr.reduce(sumofArray).toFixed(2)+'',
       backgroundColor: [
       "#2ecc71",
        "#3498db",
        "#95a5a6",
...
$("#ShowCompareReportData").html(data);
	...
	//=====================================================================
	 ...
	 function performanceTopHotelChart(){   
          // TopHotelRoomNightBarChart.destroy();
		  // TopHotelBarRevenueChart.destroy();		  
