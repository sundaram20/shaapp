	...
	$ItemList2	 ='<div class="box1 box-primary1">
		...
		<form name="listingForm" action="" method="post">
            <div class="box-body table-responsive" style="padding-top: 1px;padding-left: 1px;padding-right: 5px;">
				...
				if (!in_array($ids, $_SESSION['POSKOT']['itemID'])){
				...
				$resCat = selectSql(TBL_INV_ITEMS," where id_shop= '".addslashes($_SESSION['shop'])."' AND  id='".$_POST['selectSubgroup']."' ");
					...
					$resitemName1=mysqli_query($connNew,$itemNameSelectSQL1); 
						...
						$rowitemName1 = mysqli_fetch_object($resitemName1);
							...
							if($remove != 'removeOne'){
								$_SESSION['POSKOT']['name'][$AddsuniqueCode]	  = ucwords($row->name.' - '.$rowitemName1->name);
								$_SESSION['POSKOT']['itemID'][$AddsuniqueCode]	=$rowitemName1->id;
								$_SESSION['POSKOT']['price'][$AddsuniqueCode]	= $rowitemName1->rate;
					...
					foreach($reverseItemID as $uniqueCode =>$dataCode){
						...
						if($itemNameNumRows>0){
							$rowitemNamecode = mysqli_fetch_object($resitemNamecode);
							 if($_SESSION['POSKOT']['itemID'][$uniqueCode]==$_REQUEST['subid']){
