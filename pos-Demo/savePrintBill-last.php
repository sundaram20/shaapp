...
$auditquery1 = "SELECT * From `".TBL_PURCH."` WHERE id = '".$_REQUEST['id_posbilling']."'  ";
  $auditresSQL1 = mysqli_query($connNew, $auditquery1);	
	while($auditrow1 = mysqli_fetch_object($auditresSQL1)){ 
		...
		if($_REQUEST['id_attribute_steward']==''){
			$ste=$c1;
		}else{
			$ste=$_REQUEST['id_attribute_steward'];
		}
		...
		if($c4 != $pax){
		   $ch5 ="Pax Details Changed from " .   $c4 ." - to - " . $_REQUEST['noOfPax'];
		}
	}*/
				...
				`round_off_amount`= '".$RoundOfAmount."',
				`grant_total_amount`= '".$grant_total_amount."',
				`remarks` = '".$_REQUEST['remarks']."'  WHERE id='".$_REQUEST['id_posbilling']."' ";
  ...
  $auditresSQL = mysqli_query($connNew, $auditquery);	
	while($auditrow = mysqli_fetch_object($auditresSQL)){
		...
		if($c1 != $_REQUEST['id_pos_detail'][$porchdetailID]['item_discount_percentage']){
			if($_REQUEST['id_pos_detail'][$porchdetailID]['item_discount_percentage']!=''){
		    $ch1 =$c3 ." Disc% Details Changed from " .   $c1 ." - to - " . $_REQUEST['id_pos_detail'][$porchdetailID]['item_discount_percentage'] . " in Bill no ". $bill ;
			}
		}	
	}
