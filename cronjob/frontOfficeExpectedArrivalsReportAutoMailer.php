...
include_once($path."/frontoffice/functions/functionDateWiseExpectedArrivalsReport.php");
...
$DB_PERSISTENT_CONN             = false;
	...
	$sqlAutoReport = "SELECT * FROM app_auto_report_config_details where id_auto_report_config='10'  and status='1' ";	
	$resAutoReport = mysqli_query($appConnectReport,$sqlAutoReport);
$emailArray=array();
	...
	$resShopChkW = mysqli_query($appConnectReport,$sqlShopCodeChkW);
		...
		$id_shop_group	=	$dataAutoReport->id_shop.rand(1000, 9999).rand(1000, 9999);//$dataAutoReport->id_shop_group;	
...
$objPHPExcel = new PHPExcel();
  $pdfNameReport3=$dataShopChk->shop_code.'-ExpectedArrivals_'.date('d-m-Y');
...
echo $period= date('d-m-Y', strtotime('+1 days'))." to ".date('d-m-Y', strtotime('+7 days'));
...
echo ExpectedArrivalsDateWiseReport($period,$report_show,$shop,$pdfNameReport3,$cronSet);
			
	//ExpectedArrivalsDateWiseReport($date,$id_main_group,$id_sub_group,$id_items,$id_report_type,$report_show,$id_order_by,$showItemReport){	
		

$attach[$dataShopChk->shop_code]['pdf'][$dataShopChk->shop_code]=$pdfNameReport3;
...
$msg 	 = "Please find the attachment for the Expected Arrivals Report<br/><br/> RoomStatusHUB Team.";
$sub	 = $shop_code.'-  Expected Arrivals Report';
$cc='';		
$cc=array();
/*
...
//$mail->AddCC('sundaram@roomstatushub.com');
	...
	// $mail->addAddress("roomstatushublogs@gmail.com", "support");
