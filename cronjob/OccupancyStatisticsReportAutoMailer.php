...
include_once($path."/frontoffice/functions/functionsOccupancyStatisticsReport.php");
...
$DB_PERSISTENT_CONN             = false;
	...
	$GroupShopid=array();
	$sqlAutoReport = "SELECT * FROM app_auto_report_config_details where id_auto_report_config='11' and status='1' ";	
	$resAutoReport = mysqli_query($appConnectReport,$sqlAutoReport);
$emailArray=array();
...
$objPHPExcel = new PHPExcel();
  $pdfNameReport3=$dataShopChk->shop_code.'-OccupancyStatisticsReport-'.date('d-m-Y');
...
$period	=date('d-m-Y')." to ".date('d-m-Y', strtotime('+30 days'));

$id_report_type='270';

//echo $period;die;
echo OccupancyStatisticsReport($period,$id_report_type,$report_show,$_REQUEST['showItemReport'],$kot_nc,$appConnect,$connNew,$shop,$cronSet,$pdfNameReport3,$objPHPExcel);
...
$attach['SalesRegisterReport']['excel'][$dataShopChk->shop_code]=$pdfNameReport3;
...
echo $mailto = 'roomstatushublogs@gmail.com';
...
$msg 	 = "Please find the attachment for the Occupancy Statistics Summary Report<br/><br/> RoomStatusHUB Team.";
$sub	 = $shop_code .'- Occupancy Statistics Report';//.date('h:i:s A');
$cc='';		
$cc=array();
