...
echo $id_folio	     =  addslashes(encryptor(decrypt,$_REQUEST['id_folio']));

echo '==='.$id_fo_folio_to	 =  selectColumn(FO_BILL,'id_fo_folio_to'," WHERE `id` = '".$id_fo_bill."'");
echo '==='.$id_fo_folio		=  selectColumn(FO_BILL,'id_fo_folio_to'," WHERE `id` = '".$id_fo_bill."'");
echo '==='.$id_parent_folio		=  selectColumn('fo_folio','id_parent_folio'," WHERE `id` = '".$id_fo_folio."'");
...
$id_fo_reservations = ""; 
		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
			//$roomNumberArray=array();
				...
				echo '22222============'.	$id_fo_reservations	=$rowOrderDetail->id_fo_reservations;
					...
					//$RoomNoAndRoomName = date('d-m-Y',strtotime($rowOrderDetail->dated));


					//$RoomNoAndRoomName=$RoomName.' / '.$roomNo;	
					
					
					$RoomNoAndRoomNamety=$RoomName.' /Room no. '.$roomNo;	
					
					$tax	=	$rowOrderDetail->tax_per_day_per_room;
					$sgst	=	$tax/2;
					$cgst	=	$tax/2;
					$Days	=1;
					$DatedLine	=	date('d-m-Y',strtotime($rowOrderDetail->dated));
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['RoomType']=$RoomNoAndRoomName;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['Type']='Reservation / '.$RoomName.' /Room no. '.$roomNo;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['dated']= date('d-m-Y',strtotime($rowOrderDetail->dated));
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['checkoutdated']=  date('d-m-Y',strtotime($checkoutDated));
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['source']= 'Room Tariff '.$RoomName.' /Room no. '.$roomNo;//'Tariff '
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['sac_no']= '996311';
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['rate_plan']= $rate_plan;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['tariff'] +=$rowOrderDetail->tariff_price_per_day_per_room;
					//$folioArray[$RoomNoAndRoomName][$RoomNoAndRoomNamety]['tax'] +=$rowOrderDetail->tax_per_day_per_room;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['Days']+=$Days;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['sgst']+=$sgst;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['cgst']+=$cgst;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['Total'] +=$rowOrderDetail->tariff_price_per_day_per_room+$rowOrderDetail->tax_per_day_per_room;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['InvoiceNo']=$mdoc_no;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$RoomNoAndRoomNamety]['adults_per_room']=$rowOrderDetail->adults_per_room;
					...
					echo 'id_mst_guest=='.$id_mst_guest	=$rowOrderDetail->id_mst_guest;//selectColumn('fo_folio','id_mst_guest'," WHERE `id` = '".$rowFoBill23->id_fo_folio_to."'");/
				}
				
				
		} else { echo $sqlFoBill23	=	"SELECT * FROM ".FO_BILL." where id='".addslashes($id_fo_bill)."'".$id_parent_folio;
			if($id_parent_folio=='0'){
				  echo $sqlFoBill23	=	"SELECT * FROM ".FO_BILL." where id='".addslashes($id_fo_bill)."'";
...
//$id_parent_folio=$rowFoBill23->id_owner_room;
			...
			//$id_mst_guest
			echo	" WHERE `id` = '".$rowFoBill23->id_fo_folio_to."'";
		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
				...
				while($rowOrderDetail= mysqli_fetch_object($sqlOrderDetail)){
					...
					$outletName =selectColumn(TBL_OUTLETS,'name','WHERE id="'.$rowOrderDetail->id_mst_outlet.'" AND id_shop="'.$_SESSION['shop'].'" ');
									$RoomNoAndRoomName = date('d-m-Y',strtotime($rowOrderDetail->doc_date));
$sac_no=  selectColumn(TBL_OUTLETS,'sac_no','WHERE id="'.$rowOrderDetail->id_mst_outlet.'" AND id_shop="'.$_SESSION['shop'].'" ');
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['source']= $RoomName.' /Room no. '.$roomNo.' '.$outletName.' Bill No: '.$rowOrderDetail->mdoc_no;
...
//echo "<pre>"; print_r($folioArray);echo "</pre>";die;
$sqlOrderDetail = mysqli_query($connNew,"Select  * from `fo_reservations_addons_details` where id_fo_folio_to='".addslashes($id_folio)."'");
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
				...
				while($rowOrderDetail= mysqli_fetch_object($sqlOrderDetail)){
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['source']= $RoomName.' /Room no:'.$roomNo.' '.$chargesname;
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['sac_no']= $sac_no != '' ? $sac_no : '-';
...
$roomNumber= implode(',',$roomNumberArray);
 if($id_fo_folio_to>0 && $set==''){ 
	 ...
	 $id_owner_room = selectColumn('fo_bill', 'id_owner_room', "WHERE id_reservations = '$id_fo_reservations' AND id_fo_folio = '$id_fo_folio_to'");
	 
	 $id_mst_guest = selectColumn('fo_reservations_details', 'id_mst_guest', "WHERE id_fo_reservations = '$id_fo_reservations' AND id_mst_room_no_reserved='$id_owner_room' AND id_fo_folio_to = '$id_fo_folio_to'");
...
//Shared Guest============================
  ...
  //Shared Guest============================
	...
	//echo "WHERE id_fo_reservations = '$id_fo_reservations' AND id_mst_room_no_reserved='$id_owner_room' AND id_fo_folio = '$id_fo_folio_to'";
 //$id_owner_room = selectColumn('fo_bill', 'id_owner_room', "WHERE id_reservations = '$id_fo_reservations' AND id_fo_folio = '$id_fo_folio_to'");
	 
	//echo  $id_mst_guest = selectColumn('fo_reservations_details', 'id_mst_guest', "WHERE id_fo_reservations = '$id_fo_reservations' AND id_mst_room_no_reserved='$id_owner_room' AND id_fo_folio_to = '$id_fo_folio_to'");
	 
//$id_mst_guest	=selectColumn('fo_folio','id_mst_guest'," WHERE `id` = '".$rowFoBill23->id_fo_folio_to."'");

	echo $SQL = "select *  from ".TBL_GUEST." where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and  `id` = '".addslashes($id_mst_guest)."' ";
...
$id_bill_to_company	=  selectColumn('fo_folio','id_bill_to_company'," WHERE `id` = '".$id_fo_folio_to."'");
    ...
    $pincode = '-';$rownew->post_code;
		$is_bill_to_company =   $id_mst_company;
	}else{
                    ...
                    <td colspan="6">
                        <header class="invoice-header">
                            <div class="header-content">
							  ...
							  <?php } ?>
                                   ...
                                   <?php /*?> <img src="https://app.roomstatushub.in/uploaded_files/shop/LOGO%20-software84625.JPG"
                                        alt="Deogarh Resorts Logo" style="width: 80px;"><?php */?>
                                </div>
                                <div class="company-details">
                                    <div class="invoice-title">Tax Invoice</div>
                                    <h1><?php echo strtoupper($HotelName); ?></h1>
                                    <p> <?php echo $HotelAddress.'</br> '; ?>
                                  <?php echo $HotelCity .' - '.$HotelPincode,','.$HotelState; ?></p>
                                    <p>GST NO: <?php echo $HotelGST; ?></p>
                                     <?php if($HotelCIN!=''){?>
                                  <p> CIN:</b> <?php echo $HotelCIN; ?></p>
                                  <?php } ?>
                                    <p>PAN: <?php echo $Hotelpan; ?></p>
                                    <p>PH: +91 <?php echo $Hotelsecondary_landline.' '; ?> Email: <?php echo $HotelEmail; ?></p>
                                </div>
                            </div>
                        </header>
                        ...
                        <section class="booking-details">
                            <table>
                                ...
                                <tr>
                                    <th><b>Guest Name :</b> <span><?php echo $GuestName; ?> <?php  echo $GuestAddress.$GuestCity!=''?','.$GuestCity:'';
									$k=1;
...
<tr class="date-group-header">
                                            <td colspan="6"><?php echo $RoomName; ?></td> <!--Bill date-->
                                        </tr>
<?php 
...
?>
 <tr>
               ...
               <?php //echo $Array2['RoomType']; ?>
                ...
                if (!empty($Array2['adults_per_room'])) {
		...
		if($show_pos_bill_item=='1'){
                ...
                <?php } }?>
                                            ...
                                            <td class="align-right"> <?php echo $Array2['sac_no']; ?></td>
...
?>
                                ...
                                <tr class="date-subtotal-row">
                                            <td colspan="5" class="total-label" style="text-align:right;">Date Sub Total :</td>
                                            <td class="align-right"><?php echo number_format($SubCurrentTotal, 2); ?></td>
                                        </tr>
<?php 
    } // end inner loop
?>
...
// Outputs: One Million Two Hundred Thirty-Four Thousand Five Hundred Sixty-Seven Rupees and Eighty-Nine Paise
?>   

<!--<span id="rupees-in-words-text"></span>  need to check this  --> 
    <p style="text-align : right;">Rupees(In Words): <span id="rupees-in-text"><?php echo $convert_amount_to_words; ?></span></p>
</div>
                        </div>
                                    ...
                                    <thead>
                                        <tr>
                                         <th rowspan="2" style=" border-right: 1px solid #000;">Name</th>
                                            <th rowspan="2" style=" border-right: 1px solid #000;">Taxable Value</th>
                                            <th colspan="2">CGST</th>
                                            <th colspan="2">SGST</th>
                                            <th colspan="2">VAT</th>
                                            <th colspan="2">Surcharge</th>
                                            <th rowspan="2" style=" border-left: 1px solid #000;">Total Tax Amount</th>
                                        </tr>
                                        <tr>
