...
$formattedDate = date('d-m-Y', strtotime(str_replace('-', '/', $today)));?>
<script>
	
	$(function () {

    var defaultDate = "<?php echo date('d-m-Y',strtotime($today)); ?>";

    $('#datefilterNightAudit2').daterangepicker({
    singleDatePicker: true,
    showDropdowns: true,
		minDate: moment(defaultDate, "DD-MM-YYYY"), // 🚫 disable backdate
...
?>
			...
			<div>
    <button onclick="download_excel()" class="btn-export">⬇ Export Excel</button> 
</div>
...
function loadRoomFilterDate() {
    ...
    window.location.href = "ajax/ajaxRoomViewold.php?excel=1";
