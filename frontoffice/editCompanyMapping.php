...
if($_POST['Save']){
	...
	}else if($db->num_rows2(selectSql(TBL_COMPANY_MAPPING,"WHERE  `id` NOT IN('".addslashes(encryptor('decrypt',$_POST[eId]))."') and `id_shop` = '".addslashes($_SESSION['shop'])."' AND `company_id` = '".addslashes($_POST['company_id'])."' AND `channel_id` = '".addslashes($_POST['channel_id'])."' AND `booking_engine_name` = '".addslashes($_POST['booking_engine_name'])."'",''))){
		$err++;
