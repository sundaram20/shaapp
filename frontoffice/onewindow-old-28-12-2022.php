...
<div class="modal fade" id="ExtedDateModal" tabindex="-1" role="dialog" aria-labelledby="checkinModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="saveExtedndDateform" method="post"  class="saveExtedndDateform" data-parsley-validate autocomplete="off">
          ...
          <input type="hidden" name="ext_id" id="ext_id" value="<?php echo $ext_id;?>">
            ...
            <div class="box-header with-border">
              <h3 class="box-title">Extend Date</h3>
            ...
            <div class="form-group col-sm-12" style="background-color:#3C8DBC; color:#fff;"> 
              ...
              <div class="form-group col-sm-6">
                ...
                <input type="text" class="form-control datepickercheckin"  <?php echo $readonly; ?> placeholder="Enter checkin Date" id="checkinExtend_date" name="checkinExtend_date" value="<?php echo $checkin;?>"  >
              </div>
              <div class="form-group col-sm-6">
                <label for="checkin" style="float:left;">Check Out</label>
                <input type="text" class="form-control datepickercheckout"  <?php echo $readonly; ?> placeholder="Enter checkout Date" id="checkoutExtend_date" name="checkoutExtend_date" value="<?php echo $checkout;?>"  >
              </div>
             ...
             <button class="btn n-btn" onclick="saveIncentiveExistingLeadform();" type="button">Save</button> &nbsp;
        ...
        roomStats = (date = "") => {
            $.ajax({
                ...
                resp.forEach((value, key, arr) => {
                    ...
                    tableData += `<td><small class="label pull-right bg-red">Clean</small></td>`;
                    }
                   ...
                   tableData += `<td onclick="reservationDetails('${value.id_fo_reservations}')"><a href="#" style="color:black;">${value.res_id}</a></td>
                                    ...
                                    tableData += `<td>`;
									if (value.status == 3) {
                                        tableData += `<button class="btn btn-warning"  data-toggle="modal" data-target="#ExtedDateModal" onClick="updateExtendDate('${value.checkin}','${value.checkout}');">Extend</button>&nbsp;&nbsp;`;
                                        tableData += `<button class="btn btn-info" onClick="ChangeReservationData('${value.id_fo_reservations}');" >Edit</button>`;
...
function ChangeReservationData(id){
			
				$.ajax({
				type: "POST",
				url: 'ajax/ajaxChangeReservationForm.php',
				data: 'selectType=view&id='+id, 
				success: function (result) {
...
function updateExtendDate(checkin,checkout){
	  
	  $("#checkin_extend_date").val(checkin);
	  $("#checkout_extend_date").val(checkout);
...
function tariffCalculationIncEdit(id,value){
	...
	var payment_received= $("#payment_received").val();
		...
		$.ajax({
		   type: "POST",
		   url: 'ajax/ajaxTariffCalculation.php',
		   data: {"reservation_date" : reservation_date,"id_hotel" : id_hotel,"total_addon_price" : total_addon_price,"discount" : discount,"balance" : balance,"total_addon_price" : total_addon_price,"total_addon_price" : total_addon_price,"total_taxes" : total_taxes,"payment_received" : payment_received, "RoomArray" :RoomArray},
		   success: function (result) {				
...
function subtotalCalculation(){
	...
	var checkoutarray	=checkout_extend_date.split('-');
		...
		var  start = new Date(checkinarray[2] + "-" +checkinarray[1] + "-" +checkinarray[0]);
			...
			var tariffperroomsum=0;var taxessum=0;
		while(start <end){
			//alert(start);
...
<script type="text/javascript">
	$('.datepickercheckin').datepicker({ dateFormat: "dd-mm-yy"});
  $('.datepickercheckout').datepicker({ dateFormat: "dd-mm-yy"});
