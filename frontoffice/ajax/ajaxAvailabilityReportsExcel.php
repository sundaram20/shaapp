...
$id_mst_hotels = '1';
	...
	$reservation_date = explode(' to ',$_REQUEST['period']);
	...
	if($id_mst_room_types == 0){echo '111';
		echo "SELECT rt.name, ahr.id_mst_hotels,ahr.inventory, ahr.id_mst_room_types from `".TBL_ASSIGN_HOTEL_ROOM."` ahr left join `".TBL_ROOM_TYPE."` rt on ahr.id_mst_room_types = rt.id where ahr.status='1' and rt.status='1' and ahr.id_mst_hotels='".addslashes($id_mst_hotels)."'";
	$resRoom = executeSql("SELECT rt.name, ahr.id_mst_hotels,ahr.inventory, ahr.id_mst_room_types from `".TBL_ASSIGN_HOTEL_ROOM."` ahr left join `".TBL_ROOM_TYPE."` rt on ahr.id_mst_room_types = rt.id where ahr.status='1' and rt.status='1' and ahr.id_mst_hotels='".addslashes($id_mst_hotels)."'");
	}else{
	$resRoom = executeSql("SELECT rt.name, ahr.id_mst_hotels,ahr.inventory, ahr.id_mst_room_types from `".TBL_ASSIGN_HOTEL_ROOM."` ahr left join `".TBL_ROOM_TYPE."` rt on ahr.id_mst_room_types = rt.id where ahr.status='1' and rt.status='1' and ahr.id_mst_hotels='".addslashes($id_mst_hotels)."' and ahr.id_mst_room_types='".addslashes($id_mst_room_types)."'");
	}
echo '111';
	//echo $totalRoom = GetTotalRoom($id_mst_hotels);
...
$HotelName	=	selectColumn(TBL_HOTELS,'name'," WHERE `id` = '1'");
...
$Rowcount++;
	...
	if($id_mst_room_types == 0){ echo '11rrr';
$resRoom1 = executeSql("SELECT rt.name, ahr.id_mst_hotels,ahr.inventory, ahr.id_mst_room_types from `mst_assign_hotel_rooms` ahr left join `".TBL_ROOM_TYPE."` rt on ahr.id_mst_room_types = rt.id where ahr.status='1' and rt.status='1' and ahr.id_mst_hotels='".addslashes($id_mst_hotels)."'");
	}else{
	$resRoom1 = executeSql("SELECT rt.name, ahr.id_mst_hotels,ahr.inventory, ahr.id_mst_room_types from `mst_assign_hotel_rooms` ahr left join `".TBL_ROOM_TYPE."` rt on ahr.id_mst_room_types = rt.id where ahr.status='1' and rt.status='1' and ahr.id_mst_hotels='".addslashes($id_mst_hotels)."' and ahr.id_mst_room_types='".addslashes($id_mst_room_types)."'");
	}
		...
		while($rowRoom2 = $db->fetch_object2($resRoom1)){  
			...
			$totalRoom = GetAssignTotalRoom($id_mst_hotels,$rowRoom2->id_mst_room_types);		
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue("C".$Rowcount, $totalRoom);
			$totalRooms+=$totalRoom;
			$totalConfirmed = selectColumn('fo_reservations','count(room_quantity)','LEFT JOIN fo_reservations_details ON '.'fo_reservations'.'.id=fo_reservations_details.id_fo_reservations WHERE fo_reservations.id_mst_hotels="'.$id_mst_hotels.'" AND id_mst_room_types="'.$rowRoom2->id_mst_room_types.'" AND dated="'.date('Y-m-d',strtotime($checkinDate)).'" AND booking_status=1 ');

						$totalTentative = selectColumn('fo_reservations','count(room_quantity)','LEFT JOIN fo_reservations_details ON '.'fo_reservations'.'.id=fo_reservations_details.id_fo_reservations WHERE fo_reservations.id_mst_hotels="'.$id_mst_hotels.'" AND id_mst_room_types="'.$rowRoom2->id_mst_room_types.'" AND dated="'.date('Y-m-d',strtotime($checkinDate)).'" AND booking_status=2 ');

						$totalWaitListed = selectColumn('fo_reservations','count(room_quantity)','LEFT JOIN fo_reservations_details ON '.'fo_reservations'.'.id=fo_reservations_details.id_fo_reservations WHERE fo_reservations.id_mst_hotels="'.$id_mst_hotels.'" AND id_mst_room_types="'.$rowRoom2->id_mst_room_types.'" AND dated="'.date('Y-m-d',strtotime($checkinDate)).'" AND booking_status=3 ');

						 $netBooked=0;
						 $netBooked=$totalConfirmed+$totalTentative;
			//$blocked_hotel = GetTotalRoomBlocked_Hotel($startDate,$id_mst_hotels,$rowRoom2->id_mst_room_types);			
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue("D".$Rowcount, $netBooked);
			$totalBlock+=	$netBooked;		
			//$GetTotalRoomoffline_block_hotel = GetTotalRoomoffline_block_hotel($startDate,$id_mst_hotels,$rowRoom2->id_mst_room_types);			
			//$objPHPExcel->setActiveSheetIndex(0)->setCellValue("D".$Rowcount, $GetTotalRoomoffline_block_hotel);
						
			$roomAlloted = GetTotalRoomAlloted2($checkinDate,$id_mst_hotels,$rowRoom2->id_mst_room_types,$connNew);
			$roomAvailable = $roomAlloted;
			$totalAvil += $roomAvailable; 
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue("E".$Rowcount, $roomAvailable);
			
			//$roomAlloted = GetTotalRoomAllotedOld($startDate,$id_mst_hotels,$rowRoom2->id_mst_room_types);
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$Rowcount.':E'.$Rowcount++)->applyFromArray($styleThinBlackBorderOutline);		
	}//die;
	$totalRooms = 0;	
...
while (strtotime($checkinDate) < strtotime($checkoutDate_upadate)) {	
 ...
 if($id_mst_room_types == 0){ 
	...
	$resRoom1 = mysqli_query($connNew,"SELECT rt.name, ahr.id_mst_hotels,ahr.inventory, ahr.id_mst_room_types,ahr.over_booking_limit from `".TBL_ASSIGN_HOTEL_ROOM."` ahr left join `".TBL_ROOM_TYPE."` rt on ahr.id_mst_room_types = rt.id where ahr.status='1' and rt.status='1' and ahr.id_mst_hotels='".addslashes($id_mst_hotels)."' and ahr.id_mst_room_types='".addslashes($id_mst_room_types)."'");
	}
