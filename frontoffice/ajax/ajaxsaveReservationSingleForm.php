...
//1 for inclusive | 2 for exclusive	
    ...
    $rate_plan_id = $ReservationData2['rate_plan_id'][$roomInc] ?? 0;
			
	$tax_detail	=	selectColumn(TBL_RATE_PLAN,'tax_detail'," WHERE `id` = '".$rate_plan_id."'");
		
	if($tax_detail=='1'){//1 for inclusive
		
		
	 $price =$ReservationData2['tariff_per_room_inclusive_tax'][$roomInc];		
	...
	$price =$ReservationData2['tariff_per_room_per_night'][$roomInc];
	 $resNewTaxInclution = mysqli_query(
        $connNew,
        "SELECT * FROM `" . TBL_TAX_RULE . "` 
         WHERE id_shop='" . addslashes($_SESSION['shop']) . "' 
...
$roomdetails = " INSERT INTO `".FO_RESERVATIONS_DETAILS."` SET
						...
						`id_tax_configuration` ='".$tax_rule_id."',
						`tax_percent` ='".$tax_percent."',
  ...
  $roomdetails = " INSERT INTO `fo_reservations_addons_details` SET
	...
	$sqlMappingInventory = 'SELECT auto_sync_inv FROM '.TBL_CHANNEL_MANAGER.' AS A INNER JOIN '.TBL_HOTEL_MAPPING.' AS B ON A.id=B.channel_id
		...
		//if($autoInventoryUpdate==1){
			updateOTA($apiHotelID,$final_date,date('Y-m-d',strtotime($check_out)),$connNew);
		//}
	}
