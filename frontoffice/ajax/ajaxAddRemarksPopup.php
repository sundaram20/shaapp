  ...
  <div class="col-md-9 col-sm-9 col-xs-9" style="display : flex; flex-direction : column;  ">
      ...
      <table class="table order-list1 table-hover table-striped table-bordered" cellspacing="0">
        <thead>
         ...
         <tbody id="tableAddRemarkBodyData">
         
               
 <?php
			$start	=1;
			$sqlOrderDetail = mysqli_query($connNew,"Select  `fo_remarks_details`.* from `fo_remarks_details` where `id_fo_folio` = '".$_REQUEST['id_folio']."'  ");
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
		  ...
		  ?>
				...
				<tr style="border-top:3px solid #7FB3E0;" data-reservation-id="<?php echo $rowOrderDetail->id; ?>">
                <td><?php echo $type?></td>
                 <td><?php echo $rowOrderDetail->remark?></td>
                <td ><button type="button" value="Remove"
                                                onClick="RemoveDetailRecordRemarks('<?php echo $rowOrderDetail->id; ?>')" class="deleteBox">
  ...
  <div class="" style="padding : 15px; display : flex; justify-content: flex-end;">
    <button class="btn n-btn" onclick="saveChangeAddRemarks();" type="button" style="padding: 7px 20px!important; ">Save</button>
    &nbsp; &nbsp;
