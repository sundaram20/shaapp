...
if($ids!='0'){
	...
	// and `id_mst_room_no_allocation`='".addslashes($id_room)."'
		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
				...
				while($rowOrderDetail= mysqli_fetch_object($sqlOrderDetail)){
					$OrderDetailsId	=	$rowOrderDetail->id.'Res';
         ...
         // $room_select .= '<option value="'.$rowOrderDetail->id_mst_room_no_allocation.'">'.$roomNo.'</option>';
					...
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['RoomType']=$RoomNoAndRoomName;
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['Type']='Reservation';
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['dated']= date('d-m-Y',strtotime($rowOrderDetail->dated));
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['source']= 'Tariff';
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['tariff']=$rowOrderDetail->tariff_price_per_day_per_room;
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['tax']=$rowOrderDetail->tax_per_day_per_room;
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['Total']=$rowOrderDetail->tariff_price_per_day_per_room+$rowOrderDetail->tax_per_day_per_room;					
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['source_split_table']= FO_RESERVATIONS_DETAILS;
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['source_split_id']= $rowOrderDetail->id;
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['InvoiceNo']=$mdoc_no;
					...
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['Table']=FO_RESERVATIONS_DETAILS;
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['id_table']=$rowOrderDetail->id;
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['id_mst_room_no_allocation']=$rowOrderDetail->id_mst_room_no_allocation;
					$is_room_owner = ($rowOrderDetail->id_mst_room_no_allocation === $id_owner_room) ? '1' : 0;
					$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['is_room_owner']=$is_room_owner;
 ...
 $id_mst_guest_line_details = selectColumn('fo_reservations_details', 'id_mst_guest', "WHERE `id_fo_reservations` = '{$id_resevation}' AND id_mst_room_no_allocation = '{$rowOrderDetail->id_mst_room_no_allocation}'");
	...
	$guestName_line_details=$Title_line_details.' '.ucwords(strtolower($Firstname_line_details)).' '.ucwords(strtolower($Lastname_line_details));					
			$folioArray[$RoomNoAndRoomName][$OrderDetailsId]['guest_name']=$guestName_line_details;		
    ...
    // exit;
		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
				...
				$id_mst_room_no_allocation=selectColumn(TBL_ATTRIBUTES,'id_mst_room_no'," WHERE id_shop='".$_SESSION['shop']."'  and status = '1' AND id= '".$rowOrderDetail->id_attribute_table."'");	
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['Total']=($rowOrderDetail->sub_total_items-$rowOrderDetail->total_discount_items)+($rowOrderDetail->sgst_total_items+$rowOrderDetail->cgst_total_items+$rowOrderDetail->vat_total_items+$rowOrderDetail->surcharge_total_items);
					$rowOrderDetail->grant_total_amount;
					$CurrentTotal	+=($rowOrderDetail->sub_total_items-$rowOrderDetail->total_discount_items)+($rowOrderDetail->sgst_total_items+$rowOrderDetail->cgst_total_items+$rowOrderDetail->vat_total_items+$rowOrderDetail->surcharge_total_items);
