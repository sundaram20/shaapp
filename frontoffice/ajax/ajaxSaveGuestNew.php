...
if($_REQUEST['EditCustomerID']!=''){
 ...
 $addSql .= "	
	...
	//if($_POST['proof_type']!=''){			
				 $addSql .= ",			`proof_type` = '".addslashes($_POST['proof_type'])."',
							...
							`visa_expiry_date` = '".date('Y-m-d' , strtotime(addslashes($_POST['visa_expiry_date'])))."',
							`cform_expiry_date` = '".date('Y-m-d' , strtotime(addslashes($_POST['cform_expiry_date'])))."'";
	//}
				
				$addSql .= "	WHERE `id` = '".addslashes($_REQUEST['EditCustomerID'])."'"
				;
...
$addSql .= "	,`date_created` = '".currenDateTime()."'
	...
	executeSql($addSql); 
		...
		if($_REQUEST['guest_id_edit_reservation']!='' && $_REQUEST['guest_room_no']!='' &&   $_REQUEST['guest_id_mst_room_no_allocation']!='' &&  $_REQUEST['guest_type']=='2' && $_REQUEST['edit_order_by_room']!=''){
			//$id_shared_guest=array();
			$id_shared_guest	= selectColumn(FO_RESERVATIONS_DETAILS,'id_shared_guest','WHERE `id_fo_reservations` = "'.$_REQUEST['guest_id_edit_reservation'].'"  and order_by_room = "'.$_REQUEST['edit_order_by_room'].'" ');
			...
			echo $roomdetails = " UPDATE  `".FO_RESERVATIONS_DETAILS."` SET				 
						
						`id_shared_guest`='".$id_shared_guest."'					
						
						
						WHERE  
						`id_fo_reservations` = '".$_REQUEST['guest_id_edit_reservation']."'  and order_by_room = '".$_REQUEST['edit_order_by_room']."' 
		...
		?>
		 <select class="form-control select2" name="id_guest" id="id_guest" data-parsley-errors-container="#guestError" >
                      ...
                      <?php 
									...
									$resCat = "SELECT * FROM `".TBL_GUEST."` where status='1'  and id_shop='".addslashes($_SESSION['shop'])."' and id='".addslashes($lastInsertId)."'";
												 $db->query($resCat);
	$numRows= $db->num_rows();
