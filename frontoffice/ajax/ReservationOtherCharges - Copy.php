  ...
  $sql = "SELECT * FROM `".FO_RESERVATIONS."` where `id` = '".addslashes(encryptor('decrypt',$_REQUEST['id']))."'";
	...
	$BookingMdoc_no =$row->mdoc_no;
		...
		$ReservationTitle	='Post Charges';// <span style="font-weight:bold;">'.$BookingMdoc_no.'</span>';
		 ...
		 $sqlOrderDetailPOST = mysqli_query($connNew,"Select  `".FO_RESERVATIONS_DETAILS."`.* from `".FO_RESERVATIONS_DETAILS."` where `id_fo_reservations` = '".addslashes(encryptor(decrypt,$_REQUEST['id']))."' and id_fo_folio_to!=0  group by id_mst_room_no_allocation  ");
		if(mysqli_num_rows($sqlOrderDetailPOST) >0 ){
			...
			//$rowOrderDetail->id_mst_room_types;
				while($rowOrderDetailPost= mysqli_fetch_object($sqlOrderDetailPOST)){ 
				 ...
				 $id_fo_folio_to	= $rowOrderDetailPost->id_fo_folio_to;
    ...
    <div style="padding : 5px 10px;">
      ...
      <div class="form-group col-sm-2">
        ...
        <input type="text" class="form-control" id="bookingNo" name="bookingNo"
                    value="<?php echo $BookingMdoc_no; ?>" readonly>
      </div>
...
<td>
    <input type="text" class="form-control" name="res_no_of_Room"
                                id="res_no_of_Room" value="1"  onkeyup="PostChargesTaxCalculation('');">
  </td>
<td>
...
<td>
    <input type="text" class="form-control" name="res_tax"
                                id="res_tax" onkeyup="PostChargesTaxCalculation('');" value="0" >
  </td>
<td>
...
?>
                    ...
                    <?php
			$start	=1;
			$sqlOrderDetail = mysqli_query($connNew,"Select  `fo_reservations_addons_details`.* from `fo_reservations_addons_details` where `id_fo_folio_to` = '".$id_fo_folio_to."' ORDER BY  dated ");
                            ...
                            <table class="table table-hover" style="margin-bottom:none !important;">
                                <tbody>
                                    ...
                                    <td style="width: 100px;"><select
                                                ...
                                                id="room_number_id[]" data-parsley-required=""
                                                class="form-control room_number_id_<?php echo $rowOrderDetail->id;?>" readonly="readonly">
                                                <option value="0">Any</option><?php echo $ListRoomNo; ?>
                                            </select></td>    
  ...
  <div class="" style="padding : 15px; display : flex; justify-content: flex-end;">
    ...
    &nbsp; &nbsp;
    <button type="button" class="btn n-btn " onclick="newothrChrgsBtnClose();" style="padding: 7px 20px!important; ">Close</button>
...
res_room_type_new='POST';
    ...
    Utext +=
	 ...
	 Utext += '<td style="width: 100px;"><select name="PostChargesDataArray[' + res_room_type_new + '][' + uncodeRoomCode +
        '][room_number_id][]" id="room_number_id[]" data-parsley-required  class="form-control room_number_id_' + uncode +
