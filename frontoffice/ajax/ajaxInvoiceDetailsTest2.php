...
if(mysqli_num_rows($res_fo_receipt)>0){
	...
	$frontbillprint = selectColumn('mst_shops','fobill_url'," WHERE `id` = '".addslashes($_SESSION['shop'])."'");  
       ...
       // if($_SESSION['database']=='swanand' || $_SESSION['database']=='demo_swanand'){
        ...
        <div class="c-box2 mb-2">
          <a href="#" class="btn btn-block o-btn"
            onclick="updateFolioSplit(<?php echo $id_resevation?>,<?php echo $id_mst_guest?>,<?php echo $id_fo_bill;?>, <?php echo $id_owner_room;?>);">Split</a>
        </div>
        <?php
        }
        ?>
...
function confirmPreviewOption(idfobill, id_folio, id_room, submenu, session) {
  bootbox.dialog({
    ...
    message: `
      <p>Please select how you want to preview the bill:</p>
      <div class="form-group">
        ...
        <select id="previewType" class="form-control">
          ...
          <?php if (!empty($frontbillprint)) { ?>
			...
			<option value="daywise_standard">Day Wise - Standard</option>         
          <option value="roomwise">Room Wise</option>
          <?php }else{?>
<option value="daywise_standard" selected>Day Wise - Standard</option>         
          <option value="roomwise">Room Wise</option>
<?php } ?>
...
function openPreviewRoomWise(type, idfobill, id_folio, id_room, submenu, session, hideRoomType) {
