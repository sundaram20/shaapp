...
$objPHPExcel->getActiveSheet()->getStyle('A'.$con.':H'.$con)->applyFromArray($styleThinBlackBorderOutline);
			...
			$objPHPExcel->getActiveSheet()->getStyle('C'.$con.':H'.$con)->applyFromArray($styleThinBlackBorderOutline);
		$con=1;	
			...
			->setCellValue('B'.$con, 'Voucher')
			->setCellValue('C'.$con, 'Voucher No.')
			->setCellValue('D'.$con, 'Voucher Dt.')
...
while ($record = mysqli_fetch_object($querySalesReportPayment)) {
   ...
   $sqlReceipt = "
        SELECT 
            ...
            id_company
        FROM fo_receipt 
        WHERE id_fo_bill = '".addslashes($record->bill_id)."' and id_company='0'
...
$SalesRegisterArray =array_replace_recursive($SalesRegisterArray, $SalesRegisterArrayDr);
$count=0;
//die;
foreach($SalesRegisterArray as $SalesRegisterArraylist=> $SalesRegisterArray1){
	...
	foreach($SalesRegisterArray1 as $SalesRegisterArraylist1=> $SalesRegisterArray2){
		...
		foreach($SalesRegisterArray2 as $SalesRegisterArraylist2=> $SalesRegisterArray3){
			...
			$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue('D'.$con, date('d-m-Y',strtotime($SalesRegisterArray5['date_created']))); //trim(date('d-m-Y',strtotime($SalesRegisterArray5['date_created']))))
			...
			$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('G'.$con, $SalesRegisterArray5['dramount']>0?$SalesRegisterArray5['dramount']:0)

				->setCellValue('H'.$con, $SalesRegisterArray5['cramount']>0?$SalesRegisterArray5['cramount']:0);
