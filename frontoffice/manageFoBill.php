...
<div class="col-md-2 col-sm-6 col-xs-6">
 ...
 //$resCat = selectSql("pos_purch_details WHERE qty-adj_qty>0 AND id_pos_purch  IN  (SELECT id FROM pos_purch WHERE pos_bill_type= '1')");
					...
					// print_r($row);
					  $foliomdoc_no	= selectColumn('fo_folio','mdoc_no'," WHERE `id` = '".$row->id_fo_folio_to."'");
						   $folio_status	= selectColumn('fo_folio','folio_status'," WHERE `id` = '".$row->id_fo_folio_to."'");
                    ...
                    <tr>
					  ...
					  <td><?php echo $foliomdoc_no;//.'=='.$row->id;?></td>
					 <?php 
...
$id_folio=$row->id;
                      ...
                      if(mysqli_num_rows($sqlOrderDetail) >0 ){
                        ...
                        $sql = "SELECT amount FROM fo_receipt WHERE id_fo_bill = '".$row->id."'" ;
                      $fetch = mysqli_query($connNew, $sql);
                      $data = mysqli_fetch_assoc($fetch);
                      $netAmount = $data['amount'];


                      ?>
                      <!--<td><?php echo "₹".round($CurrentTotal,0)." /-"; ?></td>	-->
 ...
 $checkin_date=	date('d-m-Y',strtotime(selectColumn(FO_RESERVATIONS,'checkin','WHERE id="'.$row->id_reservations.'"')));
           ...
           $id_mst_company = selectColumn(FO_RESERVATIONS,'id_mst_company'," WHERE `id`= '".$row->id_reservations."'");
 
 $sql = "select * from mst_company where id = '".$id_mst_company."'";
 $fetch = mysqli_query($connNew,$sql);
 $data = mysqli_fetch_assoc($fetch);
 $source = $data['name'];			 
				//<span onclick="location.href=\'manageFoBillNew.php?activeId='.encryptor(encrypt,$row->id_fo_folio_to).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">	Open</span>	 
		?>
  ...
  <td><?php echo $folio_status=='1'?'<span onclick="location.href=\'manageFoBillNew.php?inactiveId='.encryptor(encrypt,$row->id_fo_folio_to).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Close</span>':'Open';?>&nbsp;<?php //echo $folio_status; ?></td>	
                  </td>
                      <td> 
                      ...
                      if($frontbillprint==''){
                       // $frontbillprint='fobillformat1.php';
                        ...
                        $frontbillprint = selectColumn('mst_shops','fobill_url'," WHERE `id` = '".addslashes($_SESSION['shop'])."'"); 
						 ...
						 if($_SESSION['database'] == 'charles_cotton_hotel'){
						 	$datewiseformat = 'ukfobillDatewise.php';
							$roomwiseformat = 'ukfobillRoomwise.php';
						 }else{
						 	$datewiseformat = 'fobillDatewise.php';
							$roomwiseformat = 'fobillRoomwise.php';
						 }  
						 
						$default_bill_format_name	= selectColumn('mst_doc_type_configuration_detail','default_bill_format'," WHERE `id_mst_doc_type_config` = '".$row->id_doc_type_configuration."'");
						 
                      ?>
                     <a href="javascript:void(0);" onclick="confirmPreviewOption('<?=encryptor(encrypt, $row->id);?>', '<?=encryptor(encrypt, $row->id_fo_folio_to);?>', '<?=encryptor(encrypt, $id_room);?>', '<?=$_GET['submenu'];?>', '<?=$_GET['session'];?>')">
    <img src="../images/preview.png" style="cursor:pointer;height:20px;" title="Page Preview" />
                   ...
                   <!--  <a target="_blank" href="<?=$frontbillprint;?>?idfobill=<?=encryptor(encrypt, $row->id);?>&id_folio=<?=encryptor(encrypt, $row->id_fo_folio_to);?>&id_mst_room_no_allocation=<?=encryptor(encrypt, $id_room);?>&submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>" ><img src="../images/preview.png" style="cursor:pointer;height:20px;" title="Page Preview "  /></a>&nbsp;&nbsp;-->
						  ...
						  <a  href="#" onclick="BillToCompany('<?=$row->id;?>','<?=$row->id_fo_folio_to;?>');"><i class="fa-solid fa-building"></i></a>&nbsp;&nbsp;
...
if ($db->num_rows2($resCat)) {
	while ($resultCat = $db->fetch_object2($resCat)) {
		$selected = ($id_bill_to_company == $resultCat->id) ? 'selected="selected"' : '';
		$CompanyList .= '<option '.$selected.' value="'.$resultCat->id.'">'.ucfirst($resultCat->name).'</option>';
	}
}
$CompanyList .= '</select>';
?>
