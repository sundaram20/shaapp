...
$SQL="SELECT *  from
".FO_RESERVATIONS." as pp WHERE pp.id!=0 and booking_status='2' and pp.doc_no>0".$statuscase11." ".$searchDocumentType." ORDER BY  pp.id desc
";
  ...
  $booking_status		=	selectColumn('fo_booking_status','name'," WHERE  id='".addslashes($row->booking_status)."'");
		...
		?>
                       ...
                       <td><?php echo $checkout_date= date('d-m-Y',strtotime($row->checkout));  ?></td> 
                        <td><?php echo $checkout_date= date('d-m-Y',strtotime($row->payment_date));  ?></td> 
