...
//Main Concept Here
	...
	if($doc_type == 4){
		$table_doc_type = "4";
		$redirect_page="manageGRN.php?submenu=100&session=".$_GET['session']."&doc_type=".$_GET['doc_type']." ";
	}elseif($doc_type == 5){
		$table_doc_type = "5";
...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add 
			 ...
			 if($doc != '12'){
				 $sql5 = " SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `po_no`='".$po_no."'  and `doc_type` = '4'  ";
			 }else{
				 $sql5 = " SELECT * FROM `".TBL_INV_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `po_no`='".$po_no."'  and `doc_type` = '12'  ";
			 }
//echo $sql5;
					 	...
					 	$auditquery = "SELECT * From `".TBL_INV_PURCH_DETAILS."` WHERE id = '".addslashes($_POST['update_id'])."'  ";
						  ...
						  $auditresSQL = mysqli_query($connNew, $auditquery);	
							...
							if($transaction_unit != $_POST["transaction_unit"]){ 
								$indent_transaction_unit_s = "Indent Transaction Unit Changed from ". $transaction_unit." - to - " .$_POST["transaction_unit"].' in Rowno 1 ';
							}
							...
							if($per_unit != $_POST["per_unit"]){ 
								$indent_per_unit_s = " Per Unit Changed from ". $per_unit." - to - " .$_POST["per_unit"].' in Rowno 1 ';
							}
							...
							if($discount_percent != $_POST["discount_percent"]){ 
								$indent_discount_percent_s = "Indent Discount Rate Changed from ". $discount_percent." - to - " .$_POST["discount_percent"].'  in  Rowno 1';
							}
							if($item_remarks != $_POST["item_remarks"]){ 
								$indent_item_remarks_s = "Indent Item Remarks Changed from ". $item_remarks." - to - " .$_POST["item_remarks"].' in Rowno 1 ';
							}
							if($id_mst_charges_purchase_local != $_POST["id_mst_charges_purchase_local"]){ 
								...
								$indent_charge_purchase_s = "Indent Charge Purchase Local Changed from ". $old_data." - to - " .$new_data.' in Rowno 1 ';
							}
						}
					 	...
					 	if($_GET['doc_type'] == '5'){
						  ...
						  $auditresSQL = mysqli_query($connNew, $auditquery);	
							...
							if($transaction_unit != $_POST["transaction_unit".''.$i]){ 
								$indent_transaction_unit_s .= "| Indent Transaction Unit Changed from ". $transaction_unit." - to - " .$_POST["transaction_unit".''.$i].' in Rowno '.$val;
							}
							...
							if($discount_percent != $_POST["discount_percent".''.$i]){ 
								$indent_discount_percent_s .= " | Indent Discount Rate Changed from ". $discount_percent." - to - " .$_POST["discount_percent".''.$i].' in Rowno '.$val;
							}
							if($item_remarks != $_POST["item_remarks".''.$i]){ 
								$indent_item_remarks_s .= " | Indent Item Remarks Changed from ". $item_remarks." - to - " .$_POST["item_remarks".''.$i].' in Rowno '.$val;
							}
							if($id_mst_charges_purchase_local != $_POST["id_mst_charges_purchase_local".''.$i]){ 
								...
								$indent_charge_purchase_s .= " | Indent Charge Purchase Local Changed from ". $old_data." - to - " .$new_data.' in Rowno '.$val;
							}
							}
					 	...
					 	$auditquery = "SELECT * From `".TBL_INV_OTHERS_CHARGES_PURCH."` WHERE id = '".addslashes($_POST['chargesupdate_id'])."'  ";
						  ...
						  $auditresSQL = mysqli_query($connNew, $auditquery);	
							...
							if($others_charges_amount != $_POST['others_charges_amount']){ 
								$others_charge_amount_s = "Amount Changed from ". $others_charges_amount." - to - " .$_POST['others_charges_amount']." in Rowno 1 ";
							}
						}
...
if($doc_type == '4'){
	...
	$form_name = "Direct Purchase";
							...
							`form_code` = '".$form_name."',
							`changes` =  '".addslashes($ch1).",".addslashes($ch2).",".addslashes($ch3).",".addslashes($ch4).",".addslashes($exchange_rate_s).",".addslashes($discount_s).",".addslashes($ch5).",".addslashes($indent_main_unit_s).",".addslashes($sto).",".addslashes($indent_transaction_unit_s).",".addslashes($indent_rate_per_main_unit_s).",".addslashes($indent_discount_percent_s).",".addslashes($indent_item_remarks_s).",".addslashes($indent_charge_purchase_s).",".addslashes($others_type_s).",".addslashes($others_charge_percent_s).",".addslashes($others_charge_amount_s).",".addslashes($others_charge_discount_s)."',
							`date_created` = '".currenDateTime()."',
							`last_modified` = '".currenDateTime()."',
							`id_mst_user_modified_by` = '".$_SESSION['userId']."',
							`id_mst_user_created_by` = '".$_SESSION['userId']."',
...
if($ch1=='' && $ch2=='' && $ch3=='' && $ch4=='' && $exchange_rate_s=='' && $discount_s=='' && $ch5=='' && $indent_main_unit_s=='' && $sto=='' && $indent_transaction_unit_s=='' && $indent_rate_per_main_unit_s=='' && $indent_discount_percent_s=='' && $indent_item_remarks_s=='' && $indent_charge_purchase_s=='' && $others_type_s=='' && $others_charge_percent_s=='' && $others_charge_amount_s=='' && $others_charge_discount_s==''  ){
...
?>		
					...
					<div class="form-group col-xs-12 col-md-6 col-sm-2 display" >  
...
//echo $row->id_inv_po;
								?>	
					                	...
					                	<select class="form-control select2" id="id_inv_po<?php echo $k;?>" name="id_inv_po<?php echo $k;?>" onchange="popupshow(this.id);" onclick="popupshow(this.id);" style="width:100%" required ><option value="">Select <?php echo $table_field; ?></option>
										
										<?php if($_GET['doc_type'] == '12' && $_GET['doc_type'] != '4'){ ?><option value="na" selected >NA</option> 
										<?php }else if($_GET['doc_type'] == '4' && $_GET['doc_type'] != '5'){ ?>
			 ...
			 <div class="box-footer">                                       
		       ...
		       <!--   <a href="editPurch.php?doc_type=<?=$_REQUEST['doc_type']?>&submenu=<?php echo $_GET['submenu']?>&session=<?php echo $_GET['session']?>" type="button" class="btn btn-info"><i class="fa fa-plus-circle" aria-hidden="true"> Another <?php echo $add; ?></i></a>  -->
		          <?php                   		 
                  	...
                  	?>
		        &nbsp;&nbsp;&nbsp;&nbsp;   <a href="<?php echo $print; ?>?eId='<?php echo $_GET['eId']; ?>'&doc_type=<?php echo $_GET['doc_type'] ?>&session=<?php echo $_GET['session']; ?>&submenu=<?php echo $_GET['submenu']; ?>&action=edit&page=<?php $_REQUEST['page']?>"  type="button" class="btn btn-success"><i class="fa fa-print" aria-hidden="true"> Print</i></a>  <input type='button' value='Audit Trail' class="btn btn-success"  onclick="audittrial(this.value);" style="float:right">   
