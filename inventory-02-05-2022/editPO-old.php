...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add 
			...
			$sql5 = " SELECT * FROM `".TBL_INV_PO."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `po_no`='".$po_no."'  and `doc_type` = '3'  ";
				$db->query($sql5);
				$numRows= $db->num_rows();
					if($numRows > 0)   {
			...
			if(1){
				...
				header("location:editPO.php?eId=".addslashes(encryptor(encrypt,$lastInsertId))."&submenu=".$_GET['submenu']."&session=".$_GET['session']."&action=edit&page=".$_REQUEST['page']."&print=1");
				exit;
			}else{
				$err++;
		...
		//Update Section Here
				...
				//Update  INV PO DETAILS Field More Fields Add Here
					 	...
					 	$auditquery = "SELECT * From `".TBL_INV_OTHERS_CHARGES."` WHERE id = '".addslashes($_POST['chargesupdate_id'])."'  ";
						  ...
						  $auditresSQL = mysqli_query($connNew, $auditquery);	
							...
							if($id_mst_charges_discounts != $_POST['id_mst_charges_discounts']){ 
							  ...
							  $bill_no = selectColumn('inv_po','mdoc_no'," WHERE `id` = '".$auditrow ->id_inv_po."' ");  
								 ...
								 //Change Data
							    	if($terms != $_POST['terms'.''.$i]){ 
									 	$terms_s .= " | Terms & Condition Changed from ". $terms." - to - " . $_POST['terms'.''.$i] ." in Rowno ". $val;
									}
						...
						//Update  Terms and Conditions Field More Fields Add Here
							...
							for($i = $counter3; $i > $termsupdate_count; $i--){
								...
								if($_POST['terms'.''.$i] != ''){
									...
									if($_POST['terms'.''.$i]){ 
									 	$terms_s .= " | Terms Condition Insert ". $_POST['terms'.''.$i];
									} 
			...
			if(1){  
				...
				header("location:editPO.php?eId=".$_GET['eId']."&submenu=".$_GET['submenu']."&session=".$_GET['session']."&action=edit&page=".$_REQUEST['page']."&print=1"); 	exit;
			}else{
				$err++;
              ...
              <div class="box-body">
              	...
              	<div class="card text-dark bg-light">
              		<div class="bg-primary text-center ">
              			<h5 style="padding: 5px;">Purchase Order Request</h5>
              		</div> 
              		<hr>
	              	...
	              	<div class="row">	
		               ...
		               <!--   <input data-parsley-required type="text" class="form-control pickerdate" placeholder="Enter PO Date" id="po_date" name="po_date" value="<?php if($_POST) echo $_POST['po_date'];else echo stripslashes($row->po_date);?>" onchange="hideandshow()" <?php echo $readonly; ?>> -->
		                   ...
		                   <input style="display: none;"  type="text" class="form-control pickerdate" placeholder="Enter PO Date" id="po_date1" name="po_date1" value="<?php if($_POST) echo $_POST['po_date'];else echo stripslashes($row->po_date);?>" >
		                  </div> 
	              		</div>
		                  ...
		                  <?php if($row->id ==''){?>
	              			...
	              			<div id="ind" name="ind">
		              			...
		              			<div class=" col-xs-12 col-md-4 col-sm-6">
		              				...
		              				<input type="text" class="form-control" placeholder="PO No" id="po_no" name="po_no" value="<?php if($_POST) echo $_POST['po_no'];else echo stripslashes($row->po_no);?>" readonly>
		              				</div> 
			                	</div>
			                	<div class=" col-xs-12 col-md-4 col-sm-6">
		            ...
		            <div class="row">	
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                  ...
		                  <div class="input-group"> 
			                  ...
			                  <select onchange="billtobe()" class="form-control select2" name="id_mst_party_billtobe" id="id_mst_party_billtobe" data-parsley-required  style="width:100%">
		        ...
		        <hr>
		         ...
		         <div class="box-body">
              	...
              	<div class="card text-dark bg-light">
              		<div class="bg-primary text-center ">
              			<h5 style="padding: 5px;">Purchase Order Details</h5>
              		</div>  
	              	<div class="row">
	              		<?php 
					    ...
					    <div class="modal-dialog">
					      <div class="modal-content"  style="width: 120%; ">
					        ...
					        <div class="modal-header">
					          <h4 class="modal-title">Pending Indent Purchase Order</h4>
				            ...
				            <tbody>
				                ...
				                <tr id="edittrdelete<?php echo $k;?>">
					                ...
					                <td class="form-group col-md-2"> 
					                 <select data-parsley-required class="form-control select2" name="id_inv_indent<?php echo $k;?>" id="id_inv_indent<?php echo $k;?>" onchange="popupshow(this.id);" style="width:100%">
										<?php $categoryDropDown = '<option value="">Select Indent No</option>'.'<option value="na">NA</option>';
											...
											$sql = "SELECT inv_indent.indent_date, inv_indent.mdoc_no,  inv_indent.indent_no, 
						                   	...
						                   	$sql="SELECT DISTINCT B.id_inv_indent,A.indent_no,A.indent_date FROM ".TBL_INV_INDENT." A CROSS JOIN ".TBL_INV_INDENT_DETAILS." B ON A.id=B.id_inv_indent WHERE  A.id_shop='".$_SESSION['shop']."' ".$condChk."  AND B.doc_type=2 ";*/	
							                    ...
							                    if($_REQUEST['id_inv_indent'] == $row1->id){
													$selected = 'selected="selected"';
												}elseif($array['id_inv_indent'.''.$j] == $row1->id_inv_indent){
													$selected = 'selected="selected"';
									 																										
												}else{
													$selected = '';
												}  
												$categoryDropDown .= '<option '.$selected.' value="'.$row1->id.'-'.$row1->id_inv_indent.'">'.ucfirst($row1->indent_no.' | '.date('d-m-Y' , strtotime(addslashes($row1->indent_date)))).'</option>';
												}
												if($row->id !=''){
													if($array['id_inv_indent'.''.$j] == 0)	 {
...
$item_list = rtrim($string,',');										
				                    ...
				                    <td class="form-group  col-md-1 "> 
				                        <input data-parsley-required type="text"  autocomplete="off"  name="qty<?php echo $k;?>" id="qty<?php echo $k;?>" placeholder="Qty" onkeyup="amount_calc(this.id);" onclick="amount_calc(this.id);"  class="form-control discountvalue" value="<?php if($_POST) echo $_POST['qty'];else echo $qty; ?>" />
				                    </td>
				                    <td class="form-group col-md-1">  
				                    ...
				                    <td class="form-group col-md-1"> 
					                 	 <input data-parsley-required type="text"  autocomplete="off"  name="rate_per_main_unit<?php echo $k;?>" id="rate_per_main_unit<?php echo $k;?>" placeholder="Rate"  class="form-control discountvalue" value="<?php if($_POST) echo $_POST['rate_per_main_unit'];else echo $rate_per_main_unit; ?>" onkeyup="amount_calc(this.id)" />
					                ...
					                <td class="form-group col-md-1">
				                         <input type="text"  autocomplete="off"  name="discount_percent<?php echo $k;?>" id="discount_percent<?php echo $k;?>" placeholder="%Discount"  class="form-control discountvalue" value="<?php if($_POST) echo $_POST['discount_percent'];else echo stripslashes($array['discount_percent'.''.$j]); ?>" onkeyup="amount_calc(this.id);"  onclick="amount_calc(this.id);" />
				                    </td>
					                <td class="form-group col-md-2"> 
					                 	 <input type="text"  autocomplete="off"  name="item_amount<?php echo $k;?>" id="item_amount<?php echo $k;?>" placeholder="Amount"  class="form-control discountvalue" value="<?php if($_POST) echo $_POST['item_amount'];else echo stripslashes($array['item_amount'.''.$j]); ?>" readonly/>
					                </td>					                
				                    ...
				                    <td class="form-group col-md-2"> 
				                  	...
				                  	<td>
				                  		<div id="localss<?php echo $k;?>" name="localss<?php echo $k;?>" >
											...
											<span style="color:red;font-size:14px;" id="vat_amount<?php echo $k;?>">
												<?php echo $vat;?>
											</span>
												
											<span style="color:red;font-size:14px;" id="cess_amount<?php echo $k;?>">
												<?php echo $cess;?>
											</span>
												
											
											<span style="color:red;font-size:14px;" id="surcharge_amount<?php echo $k;?>">
												<?php echo $surcharge;?>
											</span>
				            ...
				            <tfoot>
				                <tr> 
				                        <td colspan="12" style="text-align: left;">
											  ...
											  <input  type="button" class="btn btn-sm btn-block" style="font-size:14px;font-weight:700" id="addrow1" value="Add" />
				                        </td> 
				                </tr>
				                <tr>
				                </tr>
				            ...
				            <tfoot>
				                <tr> 
				                        <td colspan="12" style="text-align: left;">
											  ...
											  <input  type="button" class="btn btn-sm btn-block" style="font-size:14px;font-weight:700" id="addrow2" value="Add" />
				                        </td> 
				                </tr>
				                <tr>
				                </tr>
		        	...
		        	<!-- Terms And Conditions -->
		        	 <div class="card text-dark bg-light">
	              		<div class="bg-primary text-center ">
	              			<h5 style="padding: 5px;">Terms and Conditions</h5>
	              		</div>  
			            <div class="row">
			            	<table id="myTable3" class="table order-list3">
					            <thead>
					                <tr>
					              		...
					              		<tfoot>
						                	<tr> 
						                        <td colspan="12" style="text-align: left;">
													  ...
													  <input  type="button" class="btn btn-sm btn-block" style="font-size:14px;font-weight:700" id="addrow3" value="Add" />
						                        </td> 
							                </tr> 
							            </tfoot>
				                	</div>
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp; 
			   <input type='button' value='Cancel' class="btn btn-danger" onclick='location.replace("managePO.php?submenu=<?php echo $_GET['submenu']; ?>"); '>
			   <?php if($row->id !=''){?>
		          ...
		          <?php                   		 
                  	...
                  	?>
		          <a href="<?php echo $print; ?>?eId='<?php echo $_GET['eId']; ?>'&session=<?php echo $_GET['session']; ?>&submenu=<?php echo $_GET['submenu']; ?>&action=edit&page=<?php $_REQUEST['page']?>"  type="button" class="btn btn-primary"><i class="fa fa-print" aria-hidden="true"> Print</i></a>  <input type='button' value='Audit Trail' class="btn btn-success"  onclick="audittrial(this.value);" style="float:right">  
						...
						//Tax Config Method
						 ...
						 if(counter2 >=1){
							...
							if(condition == 1){
				 			...
				 			document.getElementById("sgst_net_amount").value =(Number(sgstnetamount)+Number(sgst2)).toFixed(2);  		 				
				 			...
				 			document.getElementById("cgst_net_amount").value =(Number(cgstnetamount)+Number(cgst2)).toFixed(2); 		 				
				 			...
				 			document.getElementById("igst_net_amount").value =(Number(igstnetamount)+Number(igst2)).toFixed(2); 
				 			}
				 		}			 				
		...
		$.ajax({
				...
				success: function(data){  
					...
					//alert('');	
						 ...
						 //SGST
						 var sgst = Number(mydata['sgst']);
						 var cgst = Number(mydata['cgst']);
						...
						var vat = mydata['vat'];
						 var amount = document.getElementById("item_amount"+match).value;
						 var vatnetamount = (amount * (vat / 100));
						 //document.getElementById("item_cgst_amount"+match).value =Number(cgstnetamount).toFixed(2);
						 if(vat>0){
							$("#vat_amount"+match).html(' VAT: ' + Number(vatnetamount).toFixed(2));
						}	
						
						//CESS
						var cess = mydata['cess'];
						 var amount = document.getElementById("item_amount"+match).value;
						 var cessnetamount = (amount * (cess / 100));
						 //document.getElementById("item_cgst_amount"+match).value =Number(cgstnetamount).toFixed(2);
						 if(cess>0){
							$("#cess_amount"+match).html(' CESS: ' + Number(cesstnetamount).toFixed(2));
						}	

						//SURCHARGE
						var surcharge = mydata['surcharge'];
						 var amount = document.getElementById("item_amount"+match).value;
						 var surchargenetamount = (amount * (surcharge / 100));
						 //document.getElementById("item_cgst_amount"+match).value =Number(cgstnetamount).toFixed(2);
						 if(surcharge>0){
							$("#surcharge_amount"+match).html(' SURCHARGE: ' + Number(surchargenetamount).toFixed(2));
						}	
						 ...
						 if(counter1 >=1){
			 				...
			 				document.getElementById("sgst_net_amount").value =(sgstamount).toFixed(2);
			 				document.getElementById("sgst2").value =sgstamount;
			 				//Discount Total Section 
			 				document.getElementById("cgst_net_amount").value =(cgstamount).toFixed(2);
						 ...
						 var sgstnetamount = (amount * (sgst / 100)).toFixed(2);
						 document.getElementById("item_sgst_amount").value = Number(sgstnetamount).toFixed(2);
						 if(sgst>0) {
						 ...
						 var cgst = mydata['cgst'];
						 var amount = document.getElementById("item_amount").value;
						 var cgstnetamount = (amount * (cgst / 100)).toFixed(2);
						 ...
						 if(counter1 >=1){
				 			...
				 			document.getElementById("sgst_net_amount").value =(sgstnetamount).toFixed(2);
				 			document.getElementById("cgst_net_amount").value =(cgstnetamount).toFixed(2);
				...
				success: function(data){  
					...
					if(match >=1){
						...
						//Id Get Here
						 ...
						 var igst = mydata['igst'];
						 var amount = document.getElementById("item_amount"+match).value;
						 var igstnetamount = (amount * (igst / 100)).toFixed(2);
	...
	function amount_calc(clicked_id){
		...
		if(match >=1){
			...
			var discount = document.getElementById("discount_percent"+match).value;
				...
				//var qty = parseFloat(regex1.exec(qty1));
				//var rate = parseInt(regex.exec(rate1));
				//var discount = parseInt(regex.exec(discount1));
...
//var regex1 = /[+-]?\d+(?:\.\d+)?/g;
			...
			//Unit Select Here 
				...
				//var qty = parseFloat(qty1);
				//var rate = parseInt(regex1.exec(rate1));
				//var discount = parseFloat(regex.exec(discount1));
...
<?php 
	$sql2 = " SELECT max(po_date) as po_date FROM `".TBL_INV_PO."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".'3'."' ";
		$db->query($sql2);   
			while($row2 = $db->fetch_object()){ 
...
<!-- <input data-parsley-required type="text"  autocomplete="off"  name="qty<?php echo $k;?>" id="qty<?php echo $k;?>" placeholder="Qty" onkeyup="amount_calc(this.id);" onclick="amount_calc(this.id);"  class="form-control discountvalue" value="<?php if($_POST) echo $_POST['qty'];else echo $qty; ?>" />  -->
		...
		cols1 += '<td><input  type="text" onkeyup="amount_calc(this.id)" onclick="amount_calc(this.id)"  autocomplete="off" placeholder="Qty" class="form-control discountvalue"  name="qty' + counter1 + '" id="qty' + counter1 + '"  data-parsley-required  /></td>'; 
		...
		cols1 += '<td><input  data-parsley-required onkeyup="amount_calc(this.id)" type="text"  autocomplete="off" placeholder="Rate" class="form-control discountvalue" name="rate_per_main_unit' + counter1 + '" id="rate_per_main_unit' + counter1 + '"/><input style="display:none;" type="text"  autocomplete="off"  class="form-control discountvalue"  name="item_amount_before_discount' + counter1 + '" id="item_amount_before_discount' + counter1 + '"/></td>'; 
        ...
        cols1 += '<td style="display:none;"><div id="taxconfig" id="taxconfig"><!-- SGST --><input type="text"  autocomplete="off"  name="id_mst_charges_sgst' + counter1 + '" id="id_mst_charges_sgst' + counter1 + '" placeholder="SGST"  class="form-control" /><input type="text"  autocomplete="off"  name="item_sgst_percent' + counter1 + '" id="item_sgst_percent' + counter1 + '" placeholder="SGST"  class="form-control" /><input type="text"  autocomplete="off"  name="item_sgst_amount' + counter1 + '" id="item_sgst_amount' + counter1 + '" placeholder="SGST Amount"  class="form-control"  /><!-- CGST --><input type="text"  autocomplete="off"  name="id_mst_charges_cgst' + counter1 + '" id="id_mst_charges_cgst' + counter1 + '" placeholder="CGST"  class="form-control" /><input type="text"  autocomplete="off"  name="item_cgst_percent' + counter1 + '" id="item_cgst_percent' + counter1 + '" placeholder="CGST"  class="form-control" /><input type="text"  autocomplete="off"  name="item_cgst_amount' + counter1 + '" id="item_cgst_amount' + counter1 + '" placeholder="CGST Amount"  class="form-control" /><!-- IGST --><input type="text"  autocomplete="off"  name="id_mst_charges_igst' + counter1 + '" id="id_mst_charges_igst' + counter1 + '" placeholder="IGST"  class="form-control" /><input type="text"  autocomplete="off"  name="item_igst_percent' + counter1 + '" id="item_igst_percent' + counter1 + '" placeholder="IGST"  class="form-control" /><input type="text"  autocomplete="off"  name="item_igst_amount' + counter1 + '" id="item_igst_amount' + counter1 + '" placeholder="IGST Amount"  class="form-control" /></div></td>';		 
		 ...
		 var newRow11 = $('<tr id="trdeletes' + counter1 + '">');
		 	 ...
		 	 cols2 += '<td><div id="local'+counter1+'" name="local'+counter1+'" style="display:none;"><select data-parsley-required  name="id_mst_charges_purchase_local' + counter1 + '" id="id_mst_charges_purchase_local' + counter1 + '" class="form-control select3" onchange="po_locals(this.id)" style="width:100%;"><option>Select Tax Register</option><?php 
	                ...
	                cols2 += '<td><div id="localsss'+counter1+'" name="localsss'+counter1+'" style="display:none;"><span style="color:red;font-size:14px;" id="s_amount'+counter1+'"></span> <span style="color:red;font-size:14px;;font-size:14px;" id="c_amount'+counter1+'"></span><span style="color:red;font-size:14px;;font-size:14px;" id="vat_amount'+counter1+'"></span><span style="color:red;font-size:14px;;font-size:14px;" id="cess_amount'+counter1+'"></span><span style="color:red;font-size:14px;;font-size:14px;" id="surcharge_amount'+counter1+'"></span></div><div id="interstatesss'+counter1+'" name="interstatesss'+counter1+'" style="display:none;"><span style="color:red;font-size:14px;" id="i_amount' + counter1 + '" name="i_amount' + counter1 + '"></span></div></td>';
	                  	cols2 += '<td></td>';
	                  	cols2 += '<td></td>';
	                  	cols2 += '<td></td>';
	                  	cols2 += '<td></td>';
    ...
    $("#addrow2").on("click", function () { 	
        ...
        cols2 += '<td><select class="form-control select3" id="type' + counter2 + '" name="type' + counter2 + '" onchange="type_funt(this.id)" style="width:100%"><option value="">select Charges</option><option value="1">OTHERS</option><option value="2">DISCOUNT</option></select></td>';
