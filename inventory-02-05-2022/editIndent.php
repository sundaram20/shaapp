...
if($_REQUEST['eId']==''){
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_INV_INDENT,'edit');
}
...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Save') && empty($_POST['eId'])){//add 
			 ...
			 $sql5 = " SELECT * FROM `".TBL_INV_INDENT."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `indent_no`='".$indent_no."'  and `doc_type` = '1'  ";
				$db->query($sql5);
				$numRows= $db->num_rows();
					if($numRows > 0)   {
						while($row5 = $db->fetch_object()){ 
							$indent_no= $row5->indent_no; 
			...
			if($_POST['prefix'] !='' OR $_POST['suffix'] !=''){
				$mdoc_no = $_POST['prefix'].''.$indent_no.''.$_POST['suffix'];
			}else{
		...
		if($_POST['qty']>'0'){	
			...
			$addSql = "   	INSERT INTO `".TBL_INV_INDENT."` SET
							...
							`indent_date` = '".date('Y-m-d' , strtotime($_POST['indent_date']))."',  
							`indent_no` = '".$indent_no."',  
				...
				$addSql = " INSERT INTO `".TBL_INV_INDENT_DETAILS."` SET
							...
							`remarks_indent_details` = '".$_POST['remarks_indent_details']."', 
							`id_shop` = '".$_SESSION['shop']."'";
					...
					executeSql($addSql);
				        ...
				        //Indent Details Table Here For Loop Value Set
							...
							for($i = 1; $i <= $counter1; $i++){
								...
								if($_POST['id_inv_items'.''.$i] != '' && $_POST['main_unit'.''.$i] !='' && $_POST['qty'.''.$i] > '0' ){
									...
									`alt_qty` = '".$_POST['alt_qty'.''.$i]."',  
									`qty` = '".$_POST['qty'.''.$i]."',  
									`bal_qty` = '".$_POST['qty'.''.$i]."',  
									`alt_unit` = '".$_POST['alt_unit'.''.$i]."', 
									`main_unit` = '".$_POST['main_unit'.''.$i]."', 
									`remarks_indent_details` = '".$_POST['remarks_indent_details'.''.$i]."', 
									`id_shop` = '".$_SESSION['shop']."'";
	...
	if($_POST['qty']>'0' && 1){	
				...
				$_SESSION['successMsg'] = 'New  Indent has been added sucessfully.';
				...
				$err++;
				$_SESSION['errorMsg'] = ' Indent has not been saved. Please make corrections below.';
			}
		}
		...
		//Update Section Here
			...
			if($_POST['prefix'] !='' OR $_POST['suffix'] !=''){
				$mdoc_no = $_POST['prefix'].''.$_POST['indent_no'].''.$_POST['suffix'];
			}else{
				$mdoc_no = $_POST['mdoc_no'];
			}
			  ...
			  $auditresSQL = mysqli_query($connNew, $auditquery);	
				...
				if($indent_date != $_POST['indent_date']){
					...
					$indate = date('d-m-Y' , strtotime(addslashes($_POST['indent_date']))) ;
					 $indent_date_s ="Indent Date Details Changed from " .  $indent_date." - to - ".$indate;
				}
			...
			//Multiple Data First Rows
					...
					$val = $val + 1;
						...
						if($remarks_indent_details != $_POST['remarks_indent_details'.''.$i]){
							// $remarks_indent_details_s .="Remarks Intent Details Details Changed from " .  $remarks_indent_details." - to - ".$_POST['remarks_indent_details'.''.$i] . " in Row ". $val ." ";
							 $remarks_indent_details_s .=" Remarks Details in ". $old_data ." |". $old_data1 ." |". $old_data3 ." Changed from " .  $remarks_indent_details." - to - ".$_POST['remarks_indent_details'.''.$i] ." <br/> ";
						}
				...
				if($_POST['qty']>'0'){						
						...
						$editSql = "  UPDATE `".TBL_INV_INDENT."` SET  
							...
							`indent_date` = '".date('Y-m-d' , strtotime(addslashes($_POST['indent_date'])))."',  
							`indent_no` = '".addslashes($_POST['indent_no'])."',  
						...
						//Update Id For Loops Section
							...
							$auditeditSql = " INSERT audit_trail SET 
								...
								`form_code` = 'Requestion Note',
								`changes` =  '".addslashes($department_s).",".addslashes($indent_date_s).",".addslashes($doc_type_s).",".addslashes($remarks_s).",".addslashes($id_inv_items_s).",".addslashes($qty_s).",".addslashes($alt_qty_s).",".addslashes($alt_unitt).",".addslashes($remarks_indent_details_s)."',
		...
		$err++;
		$_SESSION['errorMsg'] = ' Indent has not been saved. Please make corrections.';
	}
}
// ----------cate---------
...
<?php //echo encryptor(decrypt,$_REQUEST['eId']);?>
			 ...
			 <form name="indent_form" action=""  method="post" enctype="multipart/form-data" data-parsley-validate autocomplete="off" id="indent_form">
              ...
              <div class="box-body">
              	...
              	<div class="card text-dark bg-light">
	              	...
	              	<div class="row">	
	              		...
	              		<div class="form-group col-xs-6 col-md-2 col-sm-6 " >
	              			<label for="name">Document Type</label>
		              			...
		              			<select class="form-control select2" id="doc_type" name="doc_type" onchange="hideandshow()" >	                	  		                  	  
			                  	 	<option selected="selected" value="1">Requestion</option>  
			                  	</select>	 
	              			<?php 
		                  ...
		                  <input data-parsley-required type="text" class="form-control pickerdate" placeholder="Enter Indent Date" id="indent_date" name="indent_date" value="<?php if($_POST) echo $_POST['indent_date'];elseif($row->indent_date!='') echo date('d-m-Y',strtotime($row->indent_date));else echo date('d-m-Y');?>" onchange="hideandshow()">
		                  </div> 
	              		</div>
		                ...
		                <div class="form-group col-xs-12 col-md-2 col-sm-12 p-0">
		                  <?php if($row->id ==''){?>
	              			...
	              			<?php } ?>
	              				<div class=" col-xs-12 col-md-12 col-sm-12">
								   ...
								   <!--<?php/*	//$sql2 = " SELECT * FROM `".TBL_DOC_TYPE_CONFIG."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND `id`='".$row->indent_no."' ";
								$db->query($sql2);   
									while($row2 = $db->fetch_object())

										*/?>-->
		              			<input type="text" class="form-control" placeholder="Enter Requisition No" id="mdoc_no2" name="mdoc_no2" value="<?php if($_POST) echo $_POST['mdoc_no2'];else echo stripslashes($row->mdoc_no);?>" readonly>
		              				</div> 
			                	</div>
	              			<div id="ind" name="ind" style="display:none;">
		              			...
		              			<div class=" col-xs-6 col-md-4 col-sm-6" style="display:none;">
		              				...
		              				<input type="text" class="form-control" placeholder="Enter Requisition No" id="indent_no" name="indent_no" value="<?php if($_POST) echo $_POST['indent_no'];else echo stripslashes($row->indent_no);?>" readonly>
		              				</div> 
			                	</div>
			                ...
			                <?php if($row->id ==''  || $prefix != ''){ ?>
		                  	...
		                  	<div id="hideandshow" name="hideandshow">
				                <div class="form-group col-xs-12 col-md-12 col-sm-6">
				                  <label for="name">Manual Requisition No</label>
				                  <div class="input-group"> 
			              			<div class="input-group-addon">
										<i class="fa fa-list-ol"></i> 
								   	</div>
				                  <input type="text" class="form-control" placeholder="Enter Manual Indent No" id="mdoc_no" name="mdoc_no" value="<?php if($_POST) echo $_POST['mdoc_no'];else echo stripslashes($row->mdoc_no); ?>">
				                  </div> 
				                </div> 			                 
				            </div> 
		                </div> 			                	                
	              		...
	              		<div class="form-group col-xs-12 col-md-2 col-sm-6" >
	              			...
	              			<?php $categoryDropDown = '<select class="form-control select2" name="id_mst_attributes_department" id="id_mst_attributes_department"  data-parsley-required data-parsley-errors-container="#outletError" style="width:100%">
		         ...
		         <div class="box-body table-responsive2">
              	...
              	<div class="card text-dark bg-light">
	              	...
	              	<div class="row">
	              		<?php 
              				$sql2 = " SELECT * FROM `".TBL_INV_INDENT."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".'1'."' ";
							$db->query($sql2);  
							$numRows= $db->num_rows();
		            ...
		            <div class="row">
              		...
              		<div class="text-center ">
              			<h6 class="tb-heads">Requisition Details</h6>
              		</div>  
				            ...
				            <tbody id="tableBody">
				                ...
				                <tr>
				                	<td class="form-group col-xs-12 col-md-3 col-sm-2"> 
					                 ...
					                 <select class="form-control select2" name="id_inv_items<?php echo $k;?>" id="id_inv_items<?php echo $k;?>" onchange="itemget(this.id)" style="width:100%" data-parsley-required data-parsley-errors-container="#outletError4">
				                    ...
				                    <td class="form-group col-xs-12 col-sm-1"> 
				                    	<?php if($k>='1'){ ?>
				                        <input type="text"  autocomplete="off"  name="qty<?php echo $k;?>" id="qty<?php echo $k;?>" placeholder="Qty" onkeyup="qtycalc_rows(this.id)" class="form-control discountvalue" value="<?php if($_POST) echo $_POST['qty'];else echo stripslashes($array['qty'.''.$j]); ?>" required />
				                    <?php } else{?>
				                    	<input type="text"  autocomplete="off"  name="qty<?php echo $k;?>" id="qty<?php echo $k;?>" placeholder="Qty" onkeyup="qtycalc(this.id)" class="form-control discountvalue" value="<?php if($_POST) echo $_POST['qty'];else echo stripslashes($array['qty'.''.$j]); ?>" required />
				                    <?php }?>
				                    </td>
				                    <td class="form-group col-xs-12 col-sm-1"> 
				                    ...
				                    <td class="form-group col-xs-12 col-sm-1">
				                    	<?php if($k>='1'){ ?>
				                        <input type="text"  autocomplete="off"  name="alt_qty<?php echo $k;?>" id="alt_qty<?php echo $k;?>" placeholder="Alt Qty" onkeyup="altqtycalc_rows(this.id)" class="form-control discountvalue" value="<?php if($_POST) echo $_POST['alt_qty'];else echo stripslashes($array['alt_qty'.''.$j] ); ?>" />
				                         <?php } else{?>
			   ...
			   <a type='button' value='Cancel' class="btn c-btn" onclick='location.replace("manageIndent.php?submenu=<?php echo $_GET['submenu']?>&session=<?php echo $_GET['session']; ?>"); '>
			   	<i class="far fa-window-close"></i>
			   	Close
			   </a>
			   

		      <!--    &nbsp;&nbsp;&nbsp;&nbsp; <a href="editIndent.php?submenu=<?php echo $_GET['submenu'] ?>&session=<?php echo $_GET['session']; ?>"  class="btn btn-success" id="buttons_radius"><i class="fa fa-plus-circle" aria-hidden="true">Another</i></a> -->
...
<script type="text/javascript">
	...
	function hideandshow() {
	  ...
	  var submenu = document.getElementById("submenu").value;
		...
		if(doc_type != '' && indent_date !='') {
			$('#ind').show(); 
			
			$.ajax({
				type: "POST",
				url: "../ajax/IndentManage.php",
				data:{doc_type:doc_type, indent_date:indent_date},
				success: function(data){
					var mydata = JSON.parse(data);  
					if(mydata['method'] == 1){
						$('#hideandshow').hide();   
						$('#ind').show();   
						<?php if($row->id == ''){?>
						$("#mdoc_no2").val( mydata['prefix']+mydata['indent_no']+ mydata['suffix']);
							document.getElementById("indent_no").value = mydata['indent_no'];
...
<?php 
	$sql2 = " SELECT max(indent_date) as indent_date FROM `".TBL_INV_INDENT."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".'1'."' ";
		$db->query($sql2);   
			while($row2 = $db->fetch_object()){ 
				$indent_date= $row2->indent_date;
				$indent_date = date('d-m-Y' , strtotime(addslashes($indent_date)));  
			}  
	...
	<div class="modal" id="myModal">
	    <div class="modal-dialog">
	      <div class="modal-content"  style="margin-top: 50%; width: 95%;margin-left: 20%;"> 
	        ...
	        <div class="modal-body">
	        	<center>
	          <a href="editIndent.php?submenu=<?php echo $_GET['submenu'] ?>&session=<?php echo $_GET['session']; ?>" type="button" class="btn btn-success"  id="buttons_radius"><i class="fa fa-plus-circle" aria-hidden="true"> Another Indent</i></a> 
	          <a href="print.php?eId=<?php echo $_GET['eId']; ?>&session=<?php echo $_GET['session']; ?>&submenu=<?php echo $_GET['submenu'] ?>&action=edit&page=<?php $_REQUEST['page']?>"  type="button" class="btn btn-primary"  id="buttons_radius"><i class="fa fa-print" aria-hidden="true"> Print</i></a> 
	          ...
	          <a href="manageIndent.php?submenu=<?php echo $_GET['submenu'] ?>&session=<?php echo $_GET['session']; ?>" type="button" class="btn btn-info"  id="buttons_radius"><i class="fa fa-info-circle" aria-hidden="true"> Close</i></a>
 ...
 <script type="text/javascript">
 	function saveornot(){
		...
		if(id_mst_attributes_department == '' && id_inv_items==''){
			window.location.href="editIndent.php?submenu="+submenu+"&session="+session;
		}else{
			//alert();
	...
	function nosave(){
 	...
 	$( document ).ready(function() {
 		var dates = '<?php echo date('d-m-Y',strtotime($indent_date)); ?>'; 
 		hideandshow();
...
//Select 2  Resolve Here
    ...
    $("#addrow1").on("click", function () { 
        ...
        var newRow1 = $("<tr >");
        var cols1 = ""; 
        cols1 += '<td ><select onchange="itemget(this.id)" name="id_inv_items' + counter1 + '" id="id_inv_items' + counter1 + '" class="form-control select3" style="width:100%" data-parsley-required data-parsley-errors-container="#outletError7"><option>Select Item Code</option><?php 
...
$item_list = rtrim($string,',');		
	                ...
	                $sql = "SELECT inv_items.*, mst_attributes.field_value FROM inv_items, mst_attributes WHERE  mst_attributes.id=inv_items.id_mst_attributes_group_main and inv_items.status=1 and inv_items.id_mst_attributes_item_type='".$item_list."' and inv_items.id_shop = '".addslashes($_SESSION['shop'])."'";
	                   	 $db->query($sql); 
	                    while($row1 = $db->fetch_object()){ ?>
