...
// ----------cate---------
$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' and `table_name` = 'party_category'  ";
...
<div class="content-wrapper">
	 ...
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border"> 
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn btn-success" href="editPartyCategory.php" >Add Party Category</a>
        ...
        <div class="col-xs-12">		     
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Party Category List</h3>
            </div>
            ...
            <div class="box-body table-responsive">
		        ...
		        <thead>
				  ...
				  <th>Party Category Description</th>
                  <th>Status</th>
				  <th>Action</th>
                </tr>
		          </thead>
		        <tbody>  
                  ...
                  <td><?php echo $row->status=='1'?'<span onclick="location.href=\'managePartyCategory.php?inactiveId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'managePartyCategory.php?activeId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>		

                  <?php 
				  		$table_name = array(TBL_PARTY);
				  		$ajaxCheckTransactions = CheckTransactionsPartyCategoery($row->id, $table_name); 

						if($ajaxCheckTransactions != '1'){
                 	 ?>
				  <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editPartyCategory.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>&submenu=<?php echo $_SESSION['submenu']; ?>';" />&nbsp;&nbsp;&nbsp;&nbsp;
				  	...
				  	<?php } else{ ?> 
				  		<td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editPartyCategory.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='managePartyCategory.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	   } 
	   else
	   {
	       self.location='managePartyCategory.php';
	    }
	   });
	  }
  </script> 
