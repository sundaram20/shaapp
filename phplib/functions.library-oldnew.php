...
//echo "SELECT * FROM `".TBL_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".$doc_type."' and `id_doc_type_configuration` = '".$id."' and `id_mst_outlet`='".$id_subsection."' AND date(`doc_date`) <= date('".$date."')";


	 $sql2 =  mysqli_query($connNew, "SELECT * FROM `".TBL_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".$doc_type."' and `id_doc_type_configuration` = '".$id."' and `id_mst_outlet`='".$id_subsection."' AND date(`doc_date`) <= date('".$date."')");    
	...
	if($start_no == '0'){
	 ...
	 $sql3 = mysqli_query($connNew," SELECT * FROM `".TBL_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".$doc_type."' and `id_doc_type_configuration` = '".$id."' and `id_mst_outlet`='".$id_subsection."' ");
...
$sql2 =  mysqli_query($connNew, "SELECT * FROM `".TBL_PURCH."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' and `doc_type`='".$doc_type."' and `id_doc_type_configuration` = '".$id."' and `id_mst_outlet`='".$id_subsection."' ORDER BY ID DESC LIMIT 1");    
			while($row2 = mysqli_fetch_object($sql2)){
				$po_no= $row2->doc_no; 
				$res['po_no'] = $po_no + 1;
			} 
...
function checkUserLevelPermission($userLevel,$related_table='',$actionName){
	...
	return true;
		...
		if($pageChk > 0  ){
			...
			$chkPerSql = "SELECT id FROM ".TBL_USER_PERMISSIONS." WHERE id_sub_menu='".$id_sub_menu."' AND FIND_IN_SET(".$resActionId.",ids_user_actions) ";
