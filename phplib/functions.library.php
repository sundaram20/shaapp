...
function checkKotIsBilledOrNot($pos_purch_id){
	global $connNew;
	$returnArray=array();
	$CheckBlockedTable_Sql = mysqli_query($connNew,"SELECT *  from pos_purch pp 
 								where pp.id_shop= '".addslashes($_SESSION['shop'])."' AND pp.pos_bill_type=2 AND pp.doc_type=21 and pp.doc_type!=24 AND  cancelled!=1 
								AND pp.id!=0 and FIND_IN_SET('".$pos_purch_id."',pp.kot_doc_no)");
	$RowCount	=	mysqli_num_rows($CheckBlockedTable_Sql);
	
	if($RowCount>0){		
		$returnArray['msg']=' This KOT Already Billed. Can`t Remove';
	    echo $Data= json_encode($returnArray);	
		exit;
	}else{
		return true;
		}
...
function checkUserLevelPermission($userLevel,$related_table='',$actionName){
	...
	return true;
		...
		$selfPage = strtoupper(basename($_SERVER['PHP_SELF']));
		 ...
		 $pageChkSql = "SELECT id FROM ".APP_SUB_MENU." WHERE UPPER(file_name)='".$referPage ."'";//UPPER(file_name)='".$referPage ."' ";
		...
		if($pageChk=='0' ||( $id_sub_menu !=$_REQUEST['submenu']) ){			 
  		$pageChkSql2 = "SELECT id FROM ".APP_SUB_MENU." WHERE id='".$_REQUEST['submenu'] ."' ";//die;
		$pageChk2 = mysqli_num_rows(mysqli_query($appConnect,$pageChkSql2));			
		$pageChk=$pageChk2;		
		$id_sub_menu=mysqli_fetch_object(mysqli_query($appConnect,$pageChkSql2))->id;
		}
...
function checkUserLevelPermissionButton($userLevel,$related_table='',$actionName,$referPage2){
	global $connNew;
	global $appConnect;
	if($userLevel == 1){
		//super admin all access
	return true;
		...
		$id_sub_menu=mysqli_fetch_object(mysqli_query($appConnect,$pageChkSql))->id;
		if($pageChk=='0' ||( $id_sub_menu !=$_REQUEST['submenu']) ){			 
  		 $pageChkSql2 = "SELECT id FROM ".APP_SUB_MENU." WHERE id='".$id_sub_menu ."' ";//die;
		$pageChk2 = mysqli_num_rows(mysqli_query($appConnect,$pageChkSql2));			
		$pageChk=$pageChk2;		
		$id_sub_menu=mysqli_fetch_object(mysqli_query($appConnect,$pageChkSql2))->id;
		}
...
//die;
			if($chkPer>0 ){
				...
				//echo $_SESSION['errorMsg'] = 'You Don\'t have '.$statusTxt.$actionName.' permission.' ;
				return false;
				if($referPage!=$selfPage){
...
/***** MYSQLI DB FUNCTIONS WRITTEN BY   *****/
...
function docConfigReciptValidator($doc_type,$date,$id_subsection){
	global $connNew;
	 $doc_type = $doc_type;	 
	 $po_date = date('Y-m-d' , strtotime($date)); 
	 $date = $date;//date('Y-m-d');
...
//echo "SELECT * FROM `".TBL_DOC_TYPE_CONFIG."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".$doc_type."' and `id` = '".$idss."' limit 1 ";
	...
	if($numRows == 0){
	 ...
	 $result = getDocConfigReciptIncValue($doc_type,$date,$id,$method,$id_subsection,$start_no,$prefix,$suffix);	
	
	return $result;
	
	}
	
function getDocConfigReciptIncValue($doc_type,$date,$id,$method,$id_subsection,$start_no,$prefix,$suffix){
	
	global $connNew;
	if($method == '1'){

	$sql2 =  mysqli_query($connNew, "SELECT * FROM `pos_receipt` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".$doc_type."' and `id_doc_type_configuration` = '".$id."' and `id_mst_outlet`='".$id_subsection."' AND date(`doc_date`) <= date('".$date."') order by doc_no desc limit 0,1");    
	...
	$sql3 = mysqli_query($connNew," SELECT * FROM `pos_receipt` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `doc_type`='".$doc_type."' and `id_doc_type_configuration` = '".$id."' and `id_mst_outlet`='".$id_subsection."' order by doc_no desc limit 0,1");
