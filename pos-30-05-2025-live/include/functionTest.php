...
function fetchdataprint($pos_purch_id_array,$grouparray=0){	
	...
	//================================================
	$pos_purch_sqlDoc="SELECT max(id) as ids FROM ".TBL_DOC_TYPE_CONFIG." AS A  WHERE id='".$posPurchResult->id_doc_type_configuration."'";
	...
	echo $pos_AccountDoc="SELECT enable_split_bill_by_sales_account_group FROM ".TBL_DOC_TYPE_CONFIG." AS A  WHERE id='".$posDocResult->ids."'";
...
if($posOuletResult->registered_office_address!=''){
	...
	$printer.=('<div style="text-align:center;font=size:10px;font-weight:bold;">Reg off </div>');
			...
			$printer.=('Discount : '.$printgroup[0]['total_item_discount_amount']);
			$printer.=("<br>");
			}
...
while($Records	   =	mysqli_fetch_object($query)){
    ...
    //$checkshift =$shiftSummary;
		...
		if($count2>1){
			...
			$con=$con+1;
			cellColorExcel('C'.$con.':Z'.$con,'e0e0e0', $objPHPExcel);
			$objPHPExcel->getActiveSheet()->getStyle('C'.$con.':AA'.$con)->applyFromArray($styleThinBlackBorderOutline);
			$objPHPExcel->setActiveSheetIndex(0)
		...
		cellColorExcel('A'.$con.':Z'.$con,'ecf0f5', $objPHPExcel);
		$objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'.$con.':AA'.$con);
	...
	$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('AA'.$con, 'REMARK');									

			$objPHPExcel->getActiveSheet()->getStyle('A'.$con.':AA'.$con)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('A'.$con.':AA'.$con)->applyFromArray($styleThinBlackBorderOutline);
			$con++;
		}
...
while($RecordsSalesReportPayment	   =	mysqli_fetch_object($querySalesReportPayment)){
	...
	if($RecordsSalesReportPayment->payment_type=='CARD' || $RecordsSalesReportPayment->payment_type=='ONLINETRANSFER'){
		...
		if($RecordsSalesReportPayment->payment_type=='ROOMTO'){
			...
			$id_reservations = selectColumn(FO_BILL,'id_reservations'," where  id_mst_shops='".addslashes($_SESSION['shop'])."' and id= '".addslashes($RecordsSalesReportPayment->id_fo_bill)."' ",' ');
...
while($RecordsSalesReport	   =	mysqli_fetch_object($querySalesReport)){
	...
	$pos_AccountDoc="SELECT enable_split_bill_by_sales_account_group FROM ".TBL_DOC_TYPE_CONFIG." AS A  WHERE id='".$posDocResult->ids."'";
	$resultpos_AccountDoc = mysqli_query($connNew, $pos_AccountDoc); 
	$numRowspos_AccountDoc = mysqli_num_rows($resultpos_AccountDoc);
	$posDocResultpos_AccountDoc = mysqli_fetch_object($resultpos_AccountDoc);
...
if($posOuletResult->registered_office_address!='' && $RecordCount=='1'){
	//$printer.=("<br>");
			...
			$sc_charges_net_amount	= $printgroup[$index]['sc_charges_net_amount'];
				...
				$details['net_amount']=$details['net_amount']-($printgroup[$index]['sc_charges_net_amount']+$printgroup[$index]['sc_sgst']+$printgroup[$index]['sc_cgst']);	$sc_charges_net_amount	= 0; $printgroup[$index]['sc_charges_net_amount'];
				}
			...
			$printer.=("<div style='font-weight:bold;font-size:13px;'>Grand Total:".str_pad(round($details['net_amount'],0),0," ").'</div>');
			$printer.="</div>";
			$printer.="</div>";$printer.="</div>";
			$printer.=("<br>");
			//$printer.=("<br>");
			//$printer.=("<br>");
			//$printer.=("<br>");
...
select
	  ...
	  case  when (payment_mode='CASH' and amount>0) then 'CASH' 
			...
			when (payment_mode='GIFTVOUCHER' and amount>0) then 'GIFTVOUCHER'
