...
function settlementSummaryReportAPI($Date,$id_outlet,$id_shift,$connNewQ){
	...
	if($Date != ''){
		...
		$SqlConn .= " AND `doc_date` BETWEEN '".date('Y-m-d',strtotime($startDate))."' And '".date('Y-m-d',strtotime($endDate))."'";
	}
	if($id_outlet != ''){
...
while($Records	   =	mysqli_fetch_object($query)){
    ...
    $resultAttrbuteTable = mysqli_fetch_object($SqlAttrbuteTable);
				...
				$transaction['service_charges']= $Records->sc_charges_net_amount;
				$transaction['sgst']= $Records->sgst+$Records->sc_sgst;
				$transaction['cgst']= $Records->cgst+$Records->sc_cgst;
				$transaction['igst']= $Records->igst+$Records->sc_igst;
