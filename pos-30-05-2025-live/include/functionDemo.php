...
$city = selectColumn(TBL_OUTLETS,'city','WHERE id="'.$id_outlet.'" AND id_shop="'.$_SESSION['shop'].'" ');
	...
	//tes
			...
			if($posOuletResult->fssai_no!=''){
			$printer.=('<div style="text-align:center;margin-left:55px;">FSSAI No : '.$posOuletResult->fssai_no.'</div>');
...
if($posOuletResult->registered_office_address!='' && $RecordCount=='1'){
	//$printer.=("<br>");
			...
			$printer.=('Sub Total : '.(($printgroup[$index]['sub_total_items']+$printgroup[$index]['sc_charges_net_amount'])-($details['discount_amount_additional']+$printgroup[$index]['total_item_discount_amount'])));
			$printer.=("<br>");
