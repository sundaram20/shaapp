...
if(($_REQUEST['pdf']==1) || ($_REQUEST['summaryReportType'] == '4') || ($_REQUEST['summaryReportType'] == '63')){ //Agentwise Summary
 
 
 if($_REQUEST['reportType']==1){     
   $sql = "select pp.doc_date,pp.kot_doc_no,ppp.id_mst_items,ppp.item_description, ppp.id as id_purch_detail,inv.item_code, ppp.qty,  ppp.item_discount_amount,
...
WHERE pp.pos_bill_type='2' and pp.cancelled=0 and pp.doc_type=21 and pp.id_shop= '2'  GROUP BY  ppp.id_mst_items order by confimed_revenue asc"  ;
 ...
 if($_REQUEST['reportType']==2){} 
	...
	while($rowList = mysqli_fetch_object($resultList)){
	  $companyname= selectColumn('inv_items','name'," WHERE `id` = '".$rowList->id_mst_items."'");
	    ...
	    $reportArray['Top Item'][strtolower($companyname==''?$emptytext7:$companyname)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	    $reportArray['Top Item'][strtolower($companyname==''?$emptytext7:$companyname)]['confimed_revenue']=$newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue);
	    $reportArray['Top Item'][strtolower($companyname==''?$emptytext7:$companyname)]['name_executive']=$rowList->name_executive;
...
//LASTYEAR SQL START >>================================================================================================================	
 ...
 if(($_REQUEST['pdf']==1) || ($_REQUEST['summaryReportType'] == '4') || ($_REQUEST['summaryReportType'] == '63')){ 
...
if($_REQUEST['reportType']==1){
$lastYearPickUpsql ="select pp.doc_date,pp.kot_doc_no,ppp.id_mst_items,ppp.item_description, ppp.id as id_purch_detail,inv.item_code, ppp.qty,  ppp.item_discount_amount,
...
if(($_REQUEST['pdf']==1 && $_REQUEST['summaryReportType'] == '3') || ($_REQUEST['summaryReportType'] == '63' )){   //Hotel Wise  Summary PDF PICKUP Report
...
foreach($reportArray as $maintitle=>$mainDatalist){
	...
	$content .='<tr><th colspan="10" style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important"><b>Compare '.$maintitle.'  Breakup For Period '.$reportPeriod.'</b></th></tr>';
    $content .='<tr>
    <th   colspan="1" style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;">Item Name</th>
    <th colspan="3" style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;">Qty</th>
...
$jsonData['pie_top_hotel_lable']=$reportSingleArray['pie_top_hotel_lable'];
$jsonData['pie_top_hotel_data']=$reportSingleArray['pie_top_hotel_data'];
