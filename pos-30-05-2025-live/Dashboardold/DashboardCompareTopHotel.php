...
if(($_REQUEST['pdf']==1) || ($_REQUEST['summaryReportType'] == '4') || ($_REQUEST['summaryReportType'] == '63')){ //Agentwise Summary
 ...
 if($_REQUEST['reportType']==1){     
   $sql = " SELECT `".TBL_ORDERS."`.id_hotel,
			...
			FROM `".TBL_ORDERS."`  
      LEFT join `fs_hotels` on `fs_hotels`.id=fs_orders.id_hotel and   `fs_hotels`.status=1
        
      
       ".$cond." ".$allUser134.$$allUser.$HotelFilterConn." and   `fs_hotels`.status=1  GROUP BY  `".TBL_ORDERS."`.id_hotel Order BY newConfirmed desc" ;
       //$BusinessSourceName=  selectColumn(TBL_GROUP,'name'," WHERE `id_group` = '".$rowList->id_default_group."'");
 }   
 if($_REQUEST['reportType']==2){       
$sql = "SELECT `fs_orders`.* 
...
INNER join `fs_order_detail` on fs_orders.id_order=fs_order_detail.id_order 
 ...
 ".$cond."
  AND `fs_hotels`.status=1  AND ( ( `fs_order_detail` .dated BETWEEN '".date('Y-m-d',strtotime($from_book))."' And '".date('Y-m-d',strtotime($to_book))."')) ".$condhotelAccess.$$allUser.$HotelFilterConn." GROUP BY `".TBL_ORDERS."`.id_hotel Order BY newConfirmed desc ";
	...
	while($rowList = mysqli_fetch_object($resultList)){
	  ...
	  $companypincode = $companypincode!=''?'-'.$companypincode:'';       
	    ...
	    $reportArray['Top Hotels'][strtolower($companyname==''?$emptytext7:$companyname)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	    $reportArray['Top Hotels'][strtolower($companyname==''?$emptytext7:$companyname)]['confimed_revenue']=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
	    $reportArray['Top Hotels'][strtolower($companyname==''?$emptytext7:$companyname)]['name_executive']=$rowList->name_executive;
	    $reportArray['Top Hotels'][strtolower($companyname==''?$emptytext7:$companyname)]['CityPinCode']=$companyCity.$companypincode;
...
if($_REQUEST['reportType']==1){
$lastYearPickUpsql =" SELECT `".TBL_ORDERS."`.id_hotel,
       ...
       ".$cond." ".$allUser1234.$allUser123.$HotelFilterConn."  and   `fs_hotels`.status=1  GROUP BY  `".TBL_ORDERS."`.id_hotel order by confimed_revenue asc" ;
...
INNER join `fs_order_detail` on fs_orders.id_order=fs_order_detail.id_order 
  ...
  and   `fs_hotels`.status=1 AND ( ( `fs_order_detail` .dated BETWEEN '".$FromLastYearDate."' And '".$ToLastYearDate."')) ".$allUser34.$condhotelAccess.$HotelFilterConn." GROUP BY `".TBL_ORDERS."`.id_hotel ";
...
if(($_REQUEST['pdf']==1 && $_REQUEST['summaryReportType'] == '3') || ($_REQUEST['summaryReportType'] == '63' )){   //Hotel Wise  Summary PDF PICKUP Report
    ...
    $TaleName='Hotel Wise Source';
	  ...
	  $companyname=   strtolower(selectColumn(TBL_HOTELS,'name','WHERE id='.$rowlastYearList->id_hotel.' '));//selectColumn(TBL_COMPANY,'name'," WHERE `id_company` = '".$rowlastYearList->id_company."'");
	    $exeNameArr[]=ucwords(strtolower($companyname));
    ...
    $reportArray['Top Hotels'][strtolower($companyname==''?$emptytext7:$companyname)]['lastYearroomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
    $reportArray['Top Hotels'][strtolower($companyname==''?$emptytext7:$companyname)]['lastYearconfimed_revenue']=$newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue);
    $reportArray['Top Hotels'][strtolower($companyname==''?$emptytext7:$companyname)]['name_executive']=$rowlastYearList->name_executive;
    $reportArray['Top Hotels'][strtolower($companyname==''?$emptytext7:$companyname)]['CityPinCode']=$companyCity.$companypincode;
...
array_multisort($confimed_revenue, SORT_DESC, $lastYearroomnights, SORT_ASC, $mainDatalist);
...
$Filename='TopHotels'.$ReportTypeMainTitle.'_Report_'.date("Y-m-d H:i:s");
	...
	$dompdf->stream($Filename.'.pdf', array("Attachment" => true));
            ...
            if($_REQUEST['reportType']==1){
                       // $ReportTypeMainTitle ='PICKUP ';
                        $Filename='TopHotelsCompareView-PickupRepor_'.date("Y-m-d H:i:s").'.xls';
                    }
                    if($_REQUEST['reportType']==2){
                        $ReportTypeMainTitle ='BOB ';
                         $Filename='TopHotelsCompareView-BobReport_'.date("Y-m-d H:i:s").'.xls';
                    }
        $test=$content;
