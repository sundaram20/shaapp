...
function fetchdataprint($pos_purch_id_array,$grouparray=0){	
	...
	$posPurchResult = mysqli_fetch_object($resultPosPurch);
		...
		while($resResult = mysqli_fetch_object($resultPosPurchDerails)){
			...
			$printgroup['print_BillSplit'][$grouparray]['kot_doc_no']=$posPurchResult->kot_doc_no; 
...
function consolidatedItemWiseReport($date,$id_main_group,$id_sub_group,$id_items){	
	global $connNew;
if($date!=''){
...
$content .= '<table  class="table" style=" text-align:center;margin-bottom: 0px;border: 0px;  ">
		...
		<th width="100px;"><b>Item Code</b></th>
...
order by id_mst_attributes_group_main,id_mst_attributes_group_sub";
	...
	foreach($DatewiseArray['Report']['name'] as $id_main=>$subindexvalue){
		...
		$content .= '<tr style="'.$BackgroundColorMain.'color:#ooo !important;font-size:16px !important;">
			<th colspan="5" ><b>'.$maingroupName.'</b></th>
			</tr>';
		...
		foreach($subindexvalue as $id_subindex=>$data){
			...
			<td width="100px;">'.$DatewiseArray['Report']['qty'][$id_main][$id_subindex][$k].'</td>
			<td width="100px;">'.$DatewiseArray['Report']['grandtotal'][$id_main][$id_subindex][$k].'</td>
			</tr>';
			$subgroupTotalQTY+=$DatewiseArray['Report']['qty'][$id_main][$id_subindex][$k];
			$SubGroupTotalAmount+=$DatewiseArray['Report']['grandtotal'][$id_main][$id_subindex][$k];
			
			$MainGroupTotalQTY+=$DatewiseArray['Report']['qty'][$id_main][$id_subindex][$k];
			$MainGroupTotalAmount+=$DatewiseArray['Report']['grandtotal'][$id_main][$id_subindex][$k];
			
			$i++;
			$k++;
		}
		$content .= '<tr style="font-size:14px !important;color:#800000;">
			<td colspan="3" style="text-align:right;margin-left:50px !important;"><b>Sub Groups Total '.ucfirst(strtolower($subgroupName)).'</b></td>
						<td  ><b>'.$subgroupTotalQTY.'</b></td>
						<td  ><b>'.$SubGroupTotalAmount.'</b></td>
			</tr>';
			
		}
		
		$content .= '<tr style="font-size:14px !important;color:#800000;">
			<td colspan="3" style="text-align:right;margin-left:50px !important;"><b>Main Groups Total '.ucfirst(strtolower($maingroupName)).'</b></td>
						<td  ><b>'.round($MainGroupTotalQTY,3).'</b></td>
						<td  ><b>'.round($MainGroupTotalQTY,3).'</b></td>
			</tr>';
		...
		//echo $content;
		//die;
$Filename='consolidatedItemWiseReport';
