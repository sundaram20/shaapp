...
if(($_REQUEST['pdf']==1) || ($_REQUEST['summaryReportType'] == '4') || ($_REQUEST['summaryReportType'] == '63')){ //Agentwise Summary
 ...
 if($_REQUEST['reportType']==1){     
   $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_COMPANY."`.id_default_group,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_USERS."`.ids_team,
       ...
       LEFT JOIN `".TBL_GROUP."` ON `".TBL_GROUP."`.id_group = `".TBL_COMPANY."`.id_default_group
       ...
       ".$cond." ".$allUser.$HotelFilterConn."  AND `".TBL_GROUP."`.name='Travel Agent'  GROUP BY  `".TBL_ORDERS."`.id_company Order BY newConfirmed desc" ;
...
where `fs_orders`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND `".TBL_GROUP."`.name='Travel Agent' AND ( ( `fs_order_detail` .dated BETWEEN '".date('Y-m-d',strtotime($from_book))."' And '".date('Y-m-d',strtotime($to_book))."')) ".$condhotelAccess.$HotelFilterConn." GROUP BY `".TBL_ORDERS."`.id_company Order BY newConfirmed desc ";
...
//LASTYEAR SQL START >>================================================================================================================	
 ...
 if(($_REQUEST['pdf']==1) || ($_REQUEST['summaryReportType'] == '4') || ($_REQUEST['summaryReportType'] == '4')){ 
...
$lastYearPickUpsql =" SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_COMPANY."`.id_default_group,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_USERS."`.ids_team,
       ...
       ".$cond." ".$allUser.$HotelFilterConn."  AND `".TBL_GROUP."`.name='Travel Agent'  GROUP BY  `".TBL_ORDERS."`.id_company " ;
 ...
 if($_REQUEST['reportType']==2){           
   $lastYearPickUpsql =
    "SELECT `fs_orders`.*,`fs_users`.name as name_executive,`fs_company_group`.name as name_company_group, `fs_order_detail`.room_id ,`mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_USERS."`.ids_team,`".TBL_COMPANY."`.id_default_group,
      sum(`fs_order_detail`.room_quantity * `fs_order_detail`.adults) as adults,
      sum(`fs_order_detail`.room_quantity * `fs_order_detail`.infants) as infants,
      sum(`fs_order_detail`.room_quantity * `fs_order_detail`.child) as child ,
      sum(`fs_order_detail`.room_quantity) as room_quantity 
...
where `fs_orders`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND `".TBL_GROUP."`.name='Travel Agent' AND ( ( `fs_order_detail` .dated BETWEEN '".$FromLastYearDate."' And '".$ToLastYearDate."')) ".$condhotelAccess.$HotelFilterConn." GROUP BY `".TBL_ORDERS."`.id_company ";
