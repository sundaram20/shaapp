...
$id_fo_reservations = "";
      ...
      while($rowOrderDetail	= mysqli_fetch_object($sqlOrderDetail)){
        // $Days += 1;
          ...
          $folioArray[$RoomNoAndRoomName][$rowOrderDetail->id_mst_room_types]['sac_no']='996311';
       	...
       	if($id_parent_folio!='' && $id_parent_folio>0){ 
  ...
  $id_resevation	=  selectColumn(FO_BILL,'id_reservations'," WHERE `id` = '".$id_fo_bill."'");
			 ...
			 $id_fo_folio_to	=  selectColumn(FO_BILL,'id_fo_folio_to'," WHERE `id` = '".$id_fo_bill."'");
			 $sqlOrderDetail = mysqli_query($connNew,"Select  `".FO_RESERVATIONS_DETAILS."`.* from `".FO_RESERVATIONS_DETAILS."` where id_fo_reservations='".addslashes($id_resevation)."'");
        if(mysqli_num_rows($sqlOrderDetail) >0 ){
          $roomNumberArray=array();
            while($rowOrderDetail	= mysqli_fetch_object($sqlOrderDetail)){
              $roomNumberArray[$rowOrderDetail->id_mst_room_no_allocation]=selectColumn(TBL_ROOMNO,'room_no'," WHERE `id` = '".$rowOrderDetail->id_mst_room_no_allocation."'");
    ...
    if ($checkoutDated == '') {
        $checkoutDated	=	selectColumn(FO_RESERVATIONS_DETAILS,'dated'," WHERE id_fo_folio_to='".addslashes($id_parent_folio)."' order by dated DESC");
    }
		//pos_purch_details
		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
				...
				while($rowOrderDetail= mysqli_fetch_object($sqlOrderDetail)){
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['sac_no']='996331';
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['tariff']=$rowOrderDetail->sub_total_items;//$rowOrderDetail->grant_total_amount-($rowOrderDetail->sgst_total_items+$rowOrderDetail->cgst_total_items);
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['Total']=($rowOrderDetail->sub_total_items+$rowOrderDetail->sgst_total_items+$rowOrderDetail->cgst_total_items);//$rowOrderDetail->grant_total_amount+$rowOrderDetail->tax_per_day_per_room;
					
					$CurrentTotal	+=($rowOrderDetail->sub_total_items+$rowOrderDetail->sgst_total_items+$rowOrderDetail->cgst_total_items);//$rowOrderDetail->grant_total_amount;
				}
...
$sqlOrderDetail = mysqli_query($connNew,"Select  * from `fo_reservations_addons_details` where id_fo_folio_to='".addslashes($id_folio)."' ");
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
          ...
          $days = 1;
          if ($checkoutDated != '') {
            $date1 = new DateTime(date('d-m-Y',strtotime($checkin)));
            $date2 = new DateTime(date('d-m-Y',strtotime($checkoutDated)));
            ...
            $days = $date1->diff($date2)->days + 1;
					...
					$tax	=	$rowOrderDetail->tax_value;//$rowOrderDetail->qty;//*$rowOrderDetail->qty
					$sgst	=	($tax/2);
					$cgst	=	($tax/2);
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['RoomType']='';
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['Type']='POS';
          $folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['sac_no']='996311';
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['dated']= date('d-m-Y',strtotime($rowOrderDetail->dated));
          $folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['checkoutdated'] = $checkoutDated != ''? date('d-m-Y',strtotime($checkoutDated)) : '';
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['source']= $chargesname;
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['tariff']=$rowOrderDetail->rate*$rowOrderDetail->qty;
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['tariff_text']=" x ".$rowOrderDetail->days." days";
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['sgst']=($sgst*$rowOrderDetail->days)*$rowOrderDetail->qty;
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['cgst']=($cgst*$rowOrderDetail->days)*$rowOrderDetail->qty;
          // $folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['Days']=$days;
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['Total']=round($rowOrderDetail->total);
...
$checkout_date_temp=	date('d-m-Y',strtotime(selectColumn(FO_RESERVATIONS,'checkout','WHERE id="'.$id_fo_reservations.'"')));
 
 		 
		  $checkout_date1 = $rowFoBill->status=='2'?date('d-m-Y',strtotime($rowFoBill->checkout_date)):'-';
		  $checkout_date = $checkout_date1=='-'?date('d-m-Y',strtotime($checkout_date_temp)):$checkout_date1;


		  $checkin_date= date('d-m-Y',strtotime($checkin));            
//FO BILL STATUS==========================================
$id_fo_folio_to	=  selectColumn(FO_BILL,'id_fo_folio_to'," WHERE `id` = '".$id_fo_bill."'");
$receipt_amount	=	selectColumn('fo_receipt','sum(amount)','WHERE id_fo_folio="'.$id_folio.'" and payment_mode!="COMPANY"');
...
?>
          ...
          <div id="">
            ...
            <?php } ?>   
                            <td class="pm" style="width:80%;font-family: sans-serif;font-size:11px;">
                              <center>
                                <p style="font-size:20px;font-weight:500;padding-bottom:0;"><b>Tax Invoice</b></p>
                              </center>
                              <center>
                      ...
                      <table class="table dataTable table-striped no-footer " width="100%" border="0" cellspacing="0"
                        ...
                        <tbody>
                          ...
                          <td class="pm"
                              ...
                              <b>
                                <center>
                                  <p style="padding:5px;margin:0;">Company Name </p>
                                </center>
                              </b>
                            </td>
                                <td class="pm" style="text-align:center;width:20%;border-right:.4px solid #000;border-bottom:.4px solid #000;font-family: sans-serif;">
                                  <p style="font-size:11px;"><?php echo $checkin_date; ?>&nbsp;<?php echo $checkin_time != '' ? $checkin_time : ""; ?>
                                </p>
                                </td>

                                <td class="pm" style="text-align:center;width:20%;border-right:.4px solid #000;border-bottom:.4px solid #000;font-family: sans-serif;">
                                  <p style="font-size:11px;"><?php  echo $checkout_date; ?>&nbsp;<?php echo $checkout_time != '' ? $checkout_time : ""; ?>
                                </p>
                                </td>
                                <td class="pm" style="text-align:center;width:20%;border-bottom:.4px solid #000;font-family: sans-serif;">
                         ...
                         <tr>
                          ...
                          <tr>
                              ...
                              <td class="pm" colspan="1" style="text-align:left;font-family: sans-serif;margin:0;padding:5px !important;border-right: 0.4px solid #000;">
                                <p style="font-size:11px;"><span> <b>Authorised by :</b></span>
                                <?php echo $res_internal_remarks;?></span>
                              </p>
                              </td>

                              <td class="pm" colspan="3" style="text-align:left;font-family: sans-serif;margin:0;padding:5px !important;">
                                <p style="font-size:11px;"><span> <b>Booking By :</b></span>
                                <?php echo $company_contacts;?></span>
                              </p>
                              </td>
                        ...
                        <thead>
                          <tr>
                           <?php /*?> <td class="pm"
                            ...
                            <td class="pm"
                              style="font-size:10px;font-family: sans-serif;padding:5px 4px!important;margin:0;border-right: 0.4px solid #000;border-bottom: 0.4px solid #000;">
                              <p style="margin:3px;"><b>Date</b></p>
                            </td>
                        
                            <td class="pm"
                              style="width:45%;font-size:10px;font-family: sans-serif;width:26%;padding:5px 4px!important;margin:0;border-right: 0.4px solid #000;border-bottom: 0.4px solid #000;">
                            ...
                            <td class="pm"
                              style="font-size:10px;font-family: sans-serif;width: 19%;padding:5px 4px!important;margin:0;border-right: 0.4px solid #000;border-bottom: 0.4px solid #000;">
                              <p style="margin:3px;"><b>Charges</b></p>
                            </td>
                            <td class="pm"
                              style="font-size:10px;font-family: sans-serif;width: 11%;padding:5px 4px!important;margin:0;border-right: 0.4px solid #000;border-bottom: 0.4px solid #000;">
                              <p style="margin:3px;"><b>SGST</b></p>
                            </td>
                            <td class="pm"
                              style="font-size:10px;font-family: sans-serif;width: 11%;padding:5px 4px!important;margin:0;border-right: 0.4px solid #000;border-bottom: 0.4px solid #000;">
                              <p style="margin:3px;"><b>CGST</b></p>
                            </td>
		...
		foreach($folioArray as $RoomName=>$Array1){
			...
			foreach($Array1 as $rowid=>$Array2){?>
                          ...
                          <tr>
                            ...
                            <td class="pm"
                              style="width: 20%;margin:0;border-right:.4px solid #000;border-bottom:.4px solid #000;padding:10px 4px!important">
                              <p
                            ...
                            <td class="pm"
                              style="border-bottom:.4px solid #000;padding:10px 4px!important;margin:0;border-right:.4px solid #000;">
                            ...
                            <td class="pm"
                              ...
                              <p style="font-size:10px;font-family: sans-serif;padding:0!important;margin:0!important;">
                              <?php
                                  if (!isset($Array2['Days'])) {
                                    echo round($Array2['tariff']);
                                    if (isset($Array2['tariff_text'])) {
                                      echo $Array2['tariff_text'];
                                    }
                                  } else {
                                    echo round($Array2['tariff']/$Array2['Days']);
                                    if ($Array2['Days']!='' && $Array2['Days'] > 1) {
                                      echo ' X '.$Array2['Days'].' Days = '.round($Array2['tariff']).'';
                                    }
                                  }
                              ?>
                                </p>
                            ...
                            <td class="pm"
                              style="border-bottom:.4px solid #000;padding:10px 4px!important;margin:0;border-right:.4px solid #000;">
                              <p style="font-size:10px;font-family: sans-serif;padding:0!important;margin:0!important;">
                               <?php echo $Array2['sgst'];?> </p>
                            </td>
                            <td class="pm"
                              style="border-bottom:.4px solid #000;padding:10px 4px!important;margin:0;border-right:.4px solid #000;">
                              <p style="font-size:10px;font-family: sans-serif;padding:0!important;margin:0!important;">
                               <?php echo $Array2['cgst'];?></p>
                            </td>
                            </td>
                           
                            <td class="pm"
                              style="border-bottom:.4px solid #000;width: 10%;padding:10px 4px!important;margin:0;border-right:.4px solid #000;">
                              <p style="font-size:10px;font-family: sans-serif;padding:0!important;margin:0!important;">
                            </p>
                            </td>

                            <td class="pm"
                              style="border-bottom:.4px solid #000;width: 20%;padding:10px 4px!important;margin:0;">
                              <p style="font-size:10px;font-family: sans-serif;padding:0!important;margin:0!important;">
 ...
 $id_fo_bill=addslashes(encryptor(decrypt,$_REQUEST['idfobill']));
	...
	$id_fo_folio_to	=  selectColumn(FO_BILL,'id_fo_folio_to'," WHERE `id` = '".$id_fo_bill."'");								
$sql_fo_receipt	=	"SELECT * FROM `fo_receipt` where  id_fo_folio='".$id_folio."' and payment_mode!='COMPANY'";									
...
<?php } ?>                          
                          <tr>
                            ...
                            <td class="pm"
                              style="padding-top:4px!important;width: 15%;padding:12px!important;padding-left:5px!important;margin:0;border-right:.4px solid #000;">
                              <p style="font-size:11px;font-family: sans-serif;padding:0!important;margin:0!important;">
                              <b>Sub Total  : </b>
                              </p>
                            </td>
                            ...
                            <td class="pm"
                              style="padding-top:4px!important;width: 10%;padding:12px!important;padding-left:5px!important;margin:0;border-right:.4px solid #000;">
                              <p style="font-size:11px;font-family: sans-serif;padding:0!important;margin:0!important;">
                              <?php echo round($receipt_amount,2); ?>
                            </p>
                            </td>

                            <td class="pm" style="padding-top:4px!important;width: 20%;padding:12px!important;padding-left:5px!important;margin:0;">
                              <p style="font-size:11px;font-family: sans-serif;padding:0!important;margin:0!important;">
...
?>
                      <table id="myTable1" class="table table-striped  dataTable" border="0" width="100%">
                        ...
                        <tbody>
                          <tr>
                           ...
                           <td class="pm" colspan="5" style="font-family: sans-serif;font-size:12px;padding: 5px 12px !important;">
 ...
 <tr>
                      <td style="border-top:.4px solid #000;">
                          <table id="myTable1" class="table table-striped  dataTable" border="0" width="100%">
                              <tbody>
                                ...
                                <tr >
                                      <td style="margin:10px !important; padding:10px !important">  <b>Taxes :</b>   GST 5% on Rooms and GST 5% on Food Items</td>
                                </tr>
                      ...
                      <td style="border-top:.4px solid #000;">
                          <table id="myTable1" class="table table-striped  dataTable" border="0" width="100%">
                              <tbody>
                                ...
                                ?>
                                <!--<tr >
                                      <td style="margin:10px !important; padding:10px !important">  <b>Remarks :</b>  <?php echo  $rowOrderDetail->remark."<br />"; ?></td>
                                </tr>-->
                                <?php
                                            }
                                        }
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td class="pm" style="padding: 5px 14px 16px 10px!important;font-family: sans-serif;font-size:11px;">
                              <p style="font-weight:100;font-size:13px;"> Yours Faithfully <br> For <b class="text-uppercase"> <?php echo $HotelName; ?></b></p>
                             <p style="text-transform:uppercase;padding-top: 6.5rem; padding-bottom : 0.2rem;font-size:13px;"><b>Authorised Signatory</b></p>
                             </td>
                          
                            <td class="pm" style="padding: 100px 14px 16px 10px!important;font-family: sans-serif;font-size:13px;text-align: center;">
                              <p style="text-transform:uppercase;"><b>Guest Signature</b></p>
                            </td>
                  ...
                  <tr>
                    <td>
                      <p style="text-transform:uppercase; text-align : center; margin-bottom : 0!important; padding : 5px 0px!important;"><b>Please
