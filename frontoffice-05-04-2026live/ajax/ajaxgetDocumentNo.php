...
if (mysqli_num_rows($Vali) > 0) {
  ...
  $selectnew = "SELECT mst_doc_type_configuration.doc_type, mst_doc_type_configuration_detail.* FROM mst_doc_type_configuration
	...
	$doc_table_name = 'fo_bill';
	$id_subsection = '1';
	$id_shop = $_SESSION['shop'];
	$docConfig = docTypeConfig($doc_type,$NightAuditDated,$id_subsection,$doc_table_name,$connNew,$id_shop);
	...
	if (mysqli_query($connNew,$sql)) {
		$data = array();
		$data['status'] = '1';
		$data['message'] = 'FO Bill Generated Sucuessfully. Invoice no: '.$mdoc_no;
