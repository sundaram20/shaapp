...
$res 	= 	mysqli_query($connNew,$sql);
	...
	//echo "Select  `".FO_RESERVATIONS_DETAILS."`.* from `".FO_RESERVATIONS_DETAILS."` where DATE(dated) >='".date('Y-m-d',strtotime($startDate))."' group by id_mst_room_types,id_fo_rate_plan,adults_per_room,id_mst_room_no_reserved ";
		//$sqlOrderDetail = mysqli_query($connNew,"Select  `".FO_RESERVATIONS_DETAILS."`.* from `".FO_RESERVATIONS_DETAILS."` where DATE(dated) >='".date('Y-m-d',strtotime($startDate))."' group by id_mst_room_types,id_fo_rate_plan,adults_per_room,id_mst_room_no_reserved ");
		
		
	$sqlOrderDetail = mysqli_query($connNew,"Select  `".FO_RESERVATIONS_DETAILS."`.* from `".FO_RESERVATIONS_DETAILS."` where DATE(dated) >='".date('Y-m-d',strtotime($startDate))."' and  id_mst_room_no_reserved >0  and no_showoff ='0' group by id_fo_reservations,id_mst_room_no_reserved ");
	 ...
	 $dated	   =	selectColumn(FO_RESERVATIONS_DETAILS,'dated'," WHERE `id_fo_reservations` = '".$rowOrderDetail->id_fo_reservations."' and id_mst_room_no_reserved = '".$rowOrderDetail->id_mst_room_no_reserved."' and  `no_showoff`='1'");
...
$sqla = "SELECT checkin_status FROM ".FO_RESERVATIONS_DETAILS." WHERE id_fo_reservations='".$rowOrderDetail->id_fo_reservations."' and order_by_room='".$rowOrderDetail->order_by_room."' and id_mst_hotels = '".$rowOrderDetail->id_mst_hotels."' and id_mst_room_types = '".$rowOrderDetail->id_mst_room_types."' order by  order_by_room desc";
		...
		$data[] = array(
		  ...
		  'resourceId'   => $rowOrderDetail->id_mst_room_no_reserved.'- s',
		  ...
		  'id_room'	=> $rowOrderDetail->id_mst_room_no_reserved,
...
$totalInventory = (int) selectColumn(
    TBL_ASSIGN_HOTEL_ROOM,
    'SUM(inventory)',
    "WHERE id_mst_hotels = '$id_hotel'
     AND id_mst_room_types IN ($id_mst_room_types)"
);
...
SELECT 
    ...
    allocation_date
FROM ".FO_INVENTORY."
WHERE DATE(allocation_date) BETWEEN '$startDate' AND '$startEnd'
...
$resnew = mysqli_query($connNew,$sqlnew);
    ...
    . ' AVL'
		 // 'color'=>'#08ce4e'
			...
			//`".TBL_ASSIGN_HOTEL_ROOM."` WHERE `id_mst_hotels` = '".$objHot->id."' 
			$booking_no	   =	selectColumn(FO_RESERVATIONS,'booking_no'," WHERE `id` = '".$rowOrderDetail->id_fo_reservations."'");
			 $inv = selectColumn(TBL_ASSIGN_HOTEL_ROOM,'inventory',"WHERE `id_mst_hotels` = '".$objHot->id."' AND `id_mst_room_types`='".$rownew->id_mst_room_types."'" );
    ...
    . ' AVL',
		...
		$data5[$rownew->allocation_date]['Blocked']+=$rownew->blocked_hotel;
		...
		$data[] = array(
		  'resourceId'   => ($rownew->id_mst_room_types/10.28),
		  'parentId'   => $row->id_mst_hotels,
		  'start'   => $rownew->allocation_date,
		  'title' =>  $rownew->blocked_hotel,
		  'backgroundColor'=>'transparent'
		);
