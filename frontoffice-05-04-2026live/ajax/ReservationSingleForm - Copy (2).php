...
.form-group label{display: block;         /* Ensure the label behaves like a block element */
...
<p class="error id_mst_hotels_new-error"></p>
    ...
    <div style="display: flex; align-items: center;">
        ...
        <div class="col-sm-2" style="padding: 0px!important;">
            <button type="button" class="btn btn-primary othrChrgsBtn" style="height: 100%; width: 100%;">+</button>
        </div>
    </div>
...
<!-- Drawer Structure -->
<div id="othrChrgsOverlay" class="othrChrgsOverlay"></div>
<div id="othrChrgsDrawer" class="othrChrgsDrawer">
    <div class="othrChrgsHeader">
        <h4>Drawer Header</h4>
        <button type="button" class="close othrChrgsCloseBtn">&times;</button>
    </div>
    <div class="othrChrgsContent">
        <p>Your content goes here. Add as much content as you need; this area will scroll if it overflows.</p>
    </div>
    <div class="othrChrgsFooter">
        <button type="button" class="btn btn-default othrChrgsCloseBtn">Close</button>
...
.othrChrgsHeader, .othrChrgsFooter {
    padding: 15px;
...
<script>
    ...
    // Open Drawer
    $('.othrChrgsBtn').on('click', function() {
        $('#othrChrgsOverlay').fadeIn();
        $('#othrChrgsDrawer').css('transform', 'translateX(0)');
        $('#othrChrgsDrawer').fadeIn();
    });

    // Close Drawer
    $('.othrChrgsCloseBtn, #othrChrgsOverlay').on('click', function() {
        $('#othrChrgsDrawer').css('transform', 'translateX(100%)');
        $('#othrChrgsDrawer').fadeOut();
        $('#othrChrgsOverlay').fadeOut();
    });
});
...
<div class="form-group col-md-2">
        ...
        <div class="form-group col-sm-2">
         <label for="checkin" style="float:left;" readonly="readonly">Complimentry Booking</label>
         <select class="form-control select2" style="width: 100%;" id="res_bookingStatus_new" name="res_bookingStatus_new" ata-parsley-required-message="Please insert your name" >
<option>No</option>
<option>Yes</option>
                        </select>
...
<option value="">Select RoomType</option>
                    ...
                    ?>
                            ...
                            </select><p class="error res_room_type_new-error"></p></td>
            <td id="folio_guestname"><select class="form-control select2" name="res_rate_plan_new" id="res_rate_plan_new"
              ...
              $selectnewp="SELECT ".TBL_RATE_PLAN.".id,".TBL_RATE_PLAN.".name FROM  ".TBL_RATE_PLAN."
               ...
               ?>
                            ...
                            </select><p class="error res_rate_plan_new-error"></p></td>
...
?>
            ...
            <td id="foliostatus" class="text-center"><input type="text" class="form-control" name="res_tariff_per_room_inclusive_tax" value="0"
                                id="res_tariff_per_room_inclusive_tax"  onkeyup="tariffCalculationInclusiveNew('');">
                       </td>
                       <td><div class="input-group-addon"  style="width: auto;
    border: 1px solid #fefefe;" title="add"> 

                            <a href="#" onClick="AddTextBox();">
				   ...
				   ?>
                    ...
                    <option  <?php echo $selectedNo; ?> value="0">No</option>
                                        ...
                                        <td style="width: 83px;"><button type="button" value="Remove"
                                                onClick="RemoveTextBoxEdit('<?php echo $Random; ?>')" class="deleteBox">
...
function checkTableBody() {  
        if ($('#tableBody').children().length > 0) {
            ...
            $('#checkin_extend_date').prop('disabled', true);
        } else {
            // Table body is empty
            $('#checkin_extend_date').prop('disabled', false);
        }
    }	
