...
$guestName=$Title.' '.ucwords(strtolower($Firstname)).' '.ucwords(strtolower($Lastname));
	...
	?>
                                 ...
                                 <tr>
                                    ...
                                    <td><button class="btn btn-success btn-xs" onclick="getRoomDetailsSingleForm(<?php echo $reservation_id;?>,2,'<?php echo $id; ?>','<?php echo $row->id_mst_room_types;?>','<?php  echo date('d-m-Y',strtotime($row->checkin)); ?>');" data-toggle="tooltip" title="view rooms"><i class="fa fa-eye"></i></button></td>
...
?>
								  ...
								  <?php if($_SESSION['shop_code']=='TIG'){ ?>
								  <a href="generateOrderPdfTig.php?id=<?=encryptor(encrypt, $id_fo_reservations);?>" class="btn n-btn" target="_blank"><img src="../images/pdf-icon.png" style="cursor:pointer;height:20px;" title="Download "  /> Generate PDF</a>&nbsp;&nbsp;
                      
        &nbsp; &nbsp;
								  <?php }else{ ?>

       <a href="generateOrderPdf.php?id=<?=encryptor(encrypt, $id_fo_reservations);?>" class="btn n-btn" target="_blank"><img src="../images/pdf-icon.png" style="cursor:pointer;height:20px;" title="Download "  /> Generate PDF</a>&nbsp;&nbsp;
