...
function printDefaultPreview($printgroup){	
	...
	//tes
		...
		if($_SESSION['database']=='jungle_home'){
	
		
			$printer .='<div><img src='.$SITE_URL.'/uploaded_files/outlets/'.$logoImage.'  alt="" style="width:100%;"></div>';
...
function settlementSummaryReportItemWise($Date,$id_outlet,$id_shift,$objPHPExcel,$appConnect,$connNew,$id_shop,$cronSet,$pdfName){
...
$RoomAndTable	=	$RoomNO; 
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		$con++;
		$objPHPExcel->setActiveSheetIndex(0)			
			->setCellValue('D'.$con, 'Description')
			->setCellValue('E'.$con, 'Qty')
			->setCellValue('F'.$con, 'Amount')				
			;									

			$objPHPExcel->getActiveSheet()->getStyle('D'.$con.':F'.$con)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('D'.$con.':F'.$con)->applyFromArray($styleThinBlackBorderOutline);
			$con++;
		 $pos_Details_sql="SELECT * FROM pos_purch_details  WHERE id_pos_purch='".$Records->id_purch_payment."' ";	
	$pos_Details_query = mysqli_query($connNew, $pos_Details_sql); 
	while($pos_Details_records = mysqli_fetch_object($pos_Details_query)){
	
	//===============footer_remarks=================================	
		$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('D'.$con, $pos_Details_records->item_description)
				->setCellValue('E'.$con, $pos_Details_records->qty)
				->setCellValue('F'.$con, $pos_Details_records->item_amount*$pos_Details_records->qty);
				$con++;
	}
