...
$CheckBlockedTable_Sql = "SELECT * FROM pos_purch_details WHERE qty-adj_qty>0  $Outlet AND id_pos_purch  IN  (SELECT id FROM pos_purch WHERE pos_bill_type= '1' and cancelled=0 AND id IN(".$id_kot.") )";	
  ...
  $CheckBlockedTable_Sql = "SELECT * FROM pos_purch_details WHERE  id_pos_purch  IN  (".$kot_doc_no.") $Outlet";
}
...
*/
	...
	$RoundOfAmount	=	round((round($NetAmount,0)-$totalBeforeRound),2);
				...
				$GetPrevious	.='<td><input type="text" class="form-control"  readonly name="quantity|'.$uniqueCode.'" id="quantity|'.$uniqueCode.'" value="'.$DiscountAmount.'"   style="width: 60px;float: left;padding: 1px 12px;height: 24px;" onKeyup="calculateQuantityPrice(\''.$uniqueCode.'\',\'2\',this.value)"></td>
  ...
  <div class="row">
    ...
    <div class="form-group col-xs-12 col-md-3 col-sm-2">
      <div class="input-group">
        ...
        <input type="text" class="form-control" placeholder="Discount Amount" id="additional_discount_amount" name="additional_discount_amount" value="<?php if($_SESSION['discountamount']) echo $_SESSION['discountamount'];else echo '';?>" onKeyup="additionalDiscount(2,this.value);" style="text-align:right;">
      </div>
    </div>
  </div>
