				...
				if($itemNameNumRows>0){
					while($rowitemName = mysqli_fetch_object($resitemName)){
						...
						if($remove != 'removeOne'){
							...
							$_SESSION['POSKOT']['name'][$AddsuniqueCode]	  = ucwords($rowitemName->name.' - '.$row->name);

							$_SESSION['POSKOT']['itemID'][$AddsuniqueCode]	=$rowitemName->id;

							$_SESSION['POSKOT']['price'][$AddsuniqueCode]	= $rowitemName->rate;
