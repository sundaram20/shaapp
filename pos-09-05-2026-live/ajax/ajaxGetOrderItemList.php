...
$edit_id = rtrim($string1,','); 
 ...
 $CheckBlockedTable_Sql = "SELECT * FROM ".TBL_PURCH_DETAILS."  WHERE  id  IN (".$edit_id.")";
...
*/
	...
	$RoundOfAmount	=	round((round($NetAmount,0)-$totalBeforeRound),2);
				...
				if($_SESSION['LINELEVEL']['purchdetailitemID'][$ResultBlockedtable1->id] == ''){
					...
					$GetPrevious	.='
					<input type="text" '.$readonlyLineWise.' class="form-control first-input discountvalue"  name="discount|'.$ResultBlockedtable1->id.'" id="discount|'.$ResultBlockedtable1->id.'" value="'.round($val,2).'"   style="width: 60px;float: left;padding: 1px 12px;height: 24px;" onchange="calculateDiscountSingleItem('.$ResultBlockedtable1->id.',2,this.value)" pattern="^\d*(\.\d{0,2})?$" >
					<br>';				
			...
			/*	if($_REQUEST['id_posbilling']==''){
					$GetPrevious	.='
					<input type="text" class="form-control first-input"  name="discount|'.$ResultBlockedtable1->id.'" id="discount|'.$ResultBlockedtable1->id.'" value="0.00"   style="width: 60px;float: left;padding: 1px 12px;height: 24px;" onkeyup="calculateDiscountSingleItem('.$ResultBlockedtable1->id.',2,this.value)"  >
					<br>';
				}else{				
					$GetPrevious	.='
					<input type="text" class="form-control first-input"  name="discount|'.$ResultBlockedtable1->id.'" id="discount|'.$ResultBlockedtable1->id.'" value="'.$_SESSION['LINELEVEL']['purchdetailitemID'][$ResultBlockedtable1->id].'"   style="width: 60px;float: left;padding: 1px 12px;height: 24px;" onchange="calculateDiscountSingleItem('.$ResultBlockedtable1->id.',2,this.value)"  >
					<br>';
				} */
				...
				<td><input type="text" class="form-control first-input discountvalue" name="id_pos_detail['.$ResultBlockedtable1->id.'][item_BillSplit]" id="id_pos_detail" value="1" style="width: 50px;float: left;padding: 1px 12px;height: 24px;" '.$SplitStatusDisable.'/></td>
