...
$Printer_ip_port=selectColumn(TBL_ATTRIBUTES,'printer_port','WHERE table_name="printer"  and id="'.$id_printer.'"');
	...
	if($Printer_ip!=''){
			...
			$connectorNew = new NetworkPrintConnector($ip_address,$Printer_ip_port);
			$printer = new Printer($connectorNew);	
			/* Start the printer */
	...
	$printer ->text(str_pad("Printer:".$printer_name,19," "));
	$printer ->text("\n");
	$printer -> setJustification(Printer::JUSTIFY_LEFT);
	$printer ->text(str_pad("KOT NO:".$listprintHeaderArray['printHeaderview']['kot_no'],19," "));
	$printer -> setJustification(Printer::JUSTIFY_RIGHT);	
	$printer ->text('Date:'.$listprintHeaderArray['printHeaderview']['doc_date']."\n");
	$printer -> setJustification(Printer::JUSTIFY_LEFT);
	$printer-> text(str_pad('Table No:'.$listprintHeaderArray['printHeaderview']['table_no'],19," "));
	$printer -> setJustification(Printer::JUSTIFY_RIGHT);
	$printer-> text('Time: '.$listprintHeaderArray['printHeaderview']['doc_time']."\n");
	$printer-> text("\n");
	
	$printer -> setJustification(Printer::JUSTIFY_LEFT);
	$printer-> text(str_pad('Steward:'.$listprintHeaderArray['printHeaderview']['steward'],30," "));
	$printer -> setJustification(Printer::JUSTIFY_RIGHT);
	$printer-> text('Pax:'.$listprintHeaderArray['printHeaderview']['pax']);
	$printer-> text("\n");
	$printer-> text("--------------------------------------\n");
	$printer-> text(str_pad("S.no.  Description ",32," "));
	//$printer.=("<br>");
	$printer-> text("Qty");
	$printer-> text("\n-----------------------------------\n");
	$sno=1;
	...
	foreach($GroupNameArray  as $k1=>  $itemlist){
		...
		//$printer->text(columnify($sno++,trim(strtoupper($itemlist['item_description'])),1,27,4));
		//$printer->text(round($itemlist['qty'],0)."\n");
		$printer -> setJustification(Printer::JUSTIFY_LEFT);
		$printer->text(str_pad($sno++.".       ".strtoupper($itemlist['item_description']),38," "));
		$printer -> setJustification(Printer::JUSTIFY_RIGHT);
		$printer->text(round($itemlist['qty'],0)."\n");
