...
/*Daily  sales  Summary starts css*/
#invoice-POS {
...
/*Daily  sales  Summary starts ends*/
</style>
    <?php 
...
<a type="button" class="btn c-btn"  href="javascript:void(0)" onClick="printData('div_print');"><i class="fa fa-fw fa-print"></i> Print</a>
</div>
  ...
  <div class="form-group">
    <label>Method</label>
      ...
      <select class="form-control select2 parsley-error"  onChange="getReportOrderBy(this.value);" name="id_report_type" data-parsley-required id="id_report_type" style="width:100%">
        ...
        <?php
            $sqlSubMenu="SELECT * FROM ".APP_SUB_MENU." WHERE 1=1 and status='1' and type='2'  AND `id_document` = '266'  ".$subMenuCond."   order by display_order";
...
<div class="col-md-2 col-sm-12">
  ...
  <input data-parsley-required type="text" class="form-control pickerdate" placeholder="Enter Date" id="per_report_date" name="per_report_date" value="<?php if($_POST) echo $_POST['per_report_date'];elseif($row->per_report_date!='') echo date('d-m-Y',strtotime($row->per_report_date));else echo date('d-m-Y');?>">
  </div>
  <!-- /.form-group --> 
</div>
...
//Form Data Clear
	...
	var showItemReport = $("#showItemReport").val();
        ...
        let url2 = 'ajax/ajaxPosSalesReport.php?period='+period+'&id_report_type='+id_report_type+'&id_main_group='+id_main_group+'&id_sub_group='+id_sub_group+'&id_item='+id_item+'&ReportShowType='+ReportShowType+'&showItemReport='+showItemReport;
         window.open(url2);
   ...
   function loadSalesReport(ReportShowType){
     ...
     $("#loading").show();
		...
		var id_item = $("#id_item").val();
		 ...
		 reportTypeFile  ='ajaxPosFlashReport.php';
		 $.ajax({
				url:'ajax/'+reportTypeFile,
				type:'POST',
				data:'period='+period+'&id_report_type='+id_report_type+'&id_data_main_group='+id_data_main_group+'&ReportShowType='+ReportShowType+'&showItemReport='+showItemReport,
