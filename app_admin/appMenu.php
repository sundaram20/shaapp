...
if(isset($_REQUEST['update'])){
	...
	if(updateData(APP_MENU,$updateData,'id='.$_REQUEST['id'].' ',$appConnect)){
...
if(isset($_POST['name'])){
	...
	$_POST['ids_module']=implode(',',$_POST['ids_module']);

	insertData(APP_MENU,$_POST,$appConnect);
}
?>
			...
			<div class="row">
				<div class="col-md-3">
					<div class="input-group ">
						...
						?>
						<select multiple="multiple" name="ids_module[]" id="ids_module" class="form-control select2" placeholder=""  required="required">
							<?php
				...
				<div class="col-md-2">
					<div class="input-group ">
						<label>&nbsp;</label>
						<input type="submit"  value="Add Menu" class="form-control btn btn-info">
					</div>
				</div>
			</div>
		</form>
				...
				<?php
					...
					while($row = mysqli_fetch_object($res)){
						echo'<tr><td>'.$sno++.'</td>
							...
							<td><input onchange="updateMe(\'ids_module\','.$row->id.',this.value);" class="form-control" name="name" value="'.$row->ids_module.'" ></td>
...
<script type="text/javascript">
	...
	function updateMe(column,id,value){
		window.location.href='appMenu.php?update=1&column='+column+'&id='+id+'&value='+value;
	}
</script>
<?php
