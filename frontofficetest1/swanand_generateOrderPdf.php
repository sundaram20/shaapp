...
if($resultHotelDetail->image!=''){
  	$hotelLogo='<img src="../../uploaded_files/hotel_gallery/'.$resultHotelDetail->image.'"  style="width: auto; height: 70px; margin-top : 5px;" />';
 }
 else{
	  	$hotelLogo='';
 }	  
						...
						<tr>
						  <td width="30%" style="padding:0!important;">';
						   ...
						   <td width="70%" style="padding:0!important; border:0px solid red; float:right;text-align:right;" >
							...
							<p style="font-family:Tahoma; font-size:14px; font-style: normal; font-variant: normal; font-weight: bold; line-height: 18.4px;text-align:right;text-transform:uppercase; margin-bottom:0px"><b>  '.$ShopShortCode.'  '.$resultHotelDetail->name.'<br> (Swanand)  </b>
							</p>';
							
							$content .= '<span style="font-size:12px;line-height:18px;"><b ></b>'.$resultHotelDetail->address.'<br>'.
								$resultHotelDetail->city.' - '.$resultHotelDetail->pincode.' '.
								selectColumn(TBL_STATE,'name'," WHERE `id_state` = '".addslashes($resultHotelDetail->state)."'").', India <br></span>';
...
$content .='<table class="table" border="0" style="width:400px!important;">';
		...
		$content .='<td  style="width:350px!important;padding-top:0!important;font-size:9px;">';
			...
			$content .='<div style="margin-top:2px;"><span style="font-weight:bold">STATUS : </span><span style="color:#000;font-size:10px;;">'.$BookingStatus.' </span></div>';
		...
		if($rowOrder->other_reference ==''){
			$content .='<div style="margin-top:2px;"><span style="font-weight:bold">GUEST NAME : </span><span style="color:#000;font-size:10px;">'.$guestName.' </span></div>';
			//$content .='<div style="margin-top:2px;"><span style="font-weight:bold">NATIONALITY : </span><span style="color:#000;font-size:10px;">'.selectColumn('mst_nationalities','name'," WHERE `id` = '".$id_mst_nationality."'").'</span></div>';
				...
				$content .='<div style="margin-top:2px;"><span style="font-weight:bold">GUEST PHONE NUMBER : </span><span style="color:#000;font-size:10px;">'.$rowGuestDetail->mobile.'</span></div>';
...
<?php 
				...
				$CounRoomSql = executeSQl("SELECT * from `fo_reservations_details` WHERE `id_fo_reservations`='".addslashes(encryptor('decrypt',$_REQUEST['id']))."' and `id_mst_room_types`='".$rowOrderDetail->id_mst_room_types."' group by order_by_room ");
					
				$RoomCount 	=	mysqli_num_rows($CounRoomSql);
				$TotalAdult='';
				$Totalchild='';
				while($resultAdult= mysqli_fetch_object($CounRoomSql)){
					$TotalAdult +=$resultAdult->adults_per_room;
					$Totalchild +=$resultAdult->child_below_5_year+$resultAdult->child_above_5_year;
				}
			 $ys= $Totalchild=='0'?'-':$Totalchild;
			$priceValue = 0;
					$content .='
...
<td style="font-size:9px;padding-top:0!important;"><span style="font-weight:bold">NUMBER OF ROOMS: </span><span style="color:#000;font-size:10px;"> '.$RoomCount.'</span></td>
</tr>


<tr>
<td style="width:350px;font-size:9px;padding-top:0!important;"><span style="font-weight:bold">NUMBER OF ADULTS: </span><span style="color:#000;font-size:10px;"> '.$TotalAdult.'</span></td>
</tr>
      <tr>
...
<td style="width:350px;font-size:9px;padding-top:0!important;"><span style="font-weight:bold">NUMBER OF KIDS : </span><span style="color:#000;font-size:10px;"> '.$ys.'</span></td>
...
/*$content .= '<table class="table" border="0" width="100%" ><tr>
     <td colspan="2" style="padding-top:0!important;font-size:9px;border:0px solid red;"><span style="text-align:left;font-weight:bold;">INCLUSIONS : </span></b><span style="font-size:10px;color:#000;">'.ucwords(strtolower($remarks)).'</span></td></tr>';
...
<div style="margin-top: 3px!important; font-size : 11px!important;">Check-in and Check-out Timing 12:00 noon to 12:00 noon <br><br> Thanks and Regards </div>
</p>';
$content .='';
	...
	$content .='</div></p>
	<a href="tel:+91 '.$rowShop->phone.'" style="font-size : 12px!important; margin-top: -10px;"><img src="icons/phone.png" title="Call us" style="margin-top:4px;" height="12px ">&nbsp; '.$rowShop->phone.'</a><br> 	
