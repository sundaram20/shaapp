...
function frontofficeSalesRegisterReportTally($Date,$id_outlet,$id_shift,$objPHPExcel,$connNew,$id_shop,$cronSet,$pdfName){
...
while ($record = mysqli_fetch_object($querySalesReportPayment)) {
   ...
   $sqlReceipt = "
        ...
        AND IFNULL(amount, 0) > 0 order by id_company desc
    ";
    ...
    while ($rowPay = mysqli_fetch_object($queryReceipt)) {
        ...
        if (!empty($rowPay->id_company)) {
            $payment_remarks = (selectColumn(MST_COMPANY, 'name', "WHERE id='{$rowPay->id_company}' AND id_shop='{$id_shop}'"));//ucwords(strtolower(selectColumn(MST_COMPANY, 'name', "WHERE id='{$rowPay->id_company}' AND id_shop='{$id_shop}'")));
...
while ($reservation = mysqli_fetch_object($reservation_query)) {
    ...
    $reservation_percentage[] = $percentage;
				...
				$Account_Name = 'ROOM TARIFF INCOME 5%';//'Room Tariff Sales';
...
sum(ppd.item_discount_amount) as item_discount_amount_new,
...
$keys = array_keys($array, $searchObject);
	...
	$SalesRegisterArray['Sales Register'][$id_fo_folio_to][$matchedMdocNo][$RecordsSalesReport->id_mst_charges_sales_local][$taxMethod_sales_local]['cramount']=($RecordsSalesReport->sub_item_amount-$RecordsSalesReport->item_discount_amount_new);
		
	$SalesRegisterArray['Sales Register'][$id_fo_folio_to][$matchedMdocNo][$RecordsSalesReport->id_mst_charges_sales_local][$taxMethod_sales_local]['assessable_value']=($RecordsSalesReport->sub_item_amount-$RecordsSalesReport->item_discount_amount_new);//$RecordsSalesReport->sub_item_amount;
	...
	-($RecordsSalesReport->total_discount_items+$RecordsSalesReport->item_discount_amount_new));
