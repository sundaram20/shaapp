...
function OccupancyStatisticsReport($Date,$id_report_type,$report_show,$showItemReport,$kot_nc,$appConnect,$connNew,$shop,$cronSet,$pdfNameReport3,$objPHPExcel){
	...
	while (strtotime($checkinDate) <= strtotime($checkoutDate)) {
		...
		if ($isFirstDayOfRange || $isFirstDayOfMonth) {

			// Spacer at month boundary (not at the very start of the range)
			if ($isFirstDayOfMonth && !$isFirstDayOfRange) {
			...
			foreach ($dateRows as $r) {
				...
				$reportOccupancyArray[$hotelId][$lableDate]['rns_'.$r->id_mst_room_types][] = $roomTypeName.' Sold';
			}
		...
		$reportOccupancyArraySum[$hotelId]['Total']['totalRoominventory_'.$last_day][] = $totalRoominventory;
	...
	// =========================================================
	// LOOP 2: Revenue (Total Revenue + ARR) — unchanged logic
	// =========================================================
	$reservation_date = explode(' to ', $Date);
	$checkinDate      = date("Y-m-d", strtotime($reservation_date['0']));
	$checkoutDate     = date("Y-m-d", strtotime($reservation_date['1']));
	...
	while (strtotime($checkinDate) <= strtotime($checkoutDate)) {
		...
		while ($rowRoom_update = mysqli_fetch_object($sql)) {
			...
			if (strtotime($rowallocation_date) == strtotime($checkinDateOrg)
				|| date("Y-m-d", strtotime($rowallocation_date)) == date("Y-m-01", strtotime($rowallocation_date))) {
				...
				$reportOccupancyArray[$hotelId][$lableDate]['plan'][] = $rowRoom_update->name;
			}

			$allocation_date_month = date("D", strtotime($rowallocation_date));
			$reportOccupancyArray[$hotelId][$rowallocation_date]['revenue'][] = round($rowRoom_update->confimed_revenue);

			$last_day = date('Y-m-t', strtotime($rowallocation_date));
			$reportOccupancyArraySum[$hotelId]['Total'][$rowRoom_update->name.'_'.$last_day][] = round($rowRoom_update->confimed_revenue);

			if ($last_day == $rowallocation_date || $checkoutDateOrg == $rowallocation_date) {
				$reportOccupancyArray[$hotelId]['Total_'.$hotelId.'_'.$allocation_date_month][$rowRoom_update->name][]
					= array_sum($reportOccupancyArraySum[$hotelId]['Total'][$rowRoom_update->name.'_'.$last_day]);
				$j++;
			}
			$Numk++;
		}
			...
			foreach ($data3 as $lab => $subarray3) {
				...
				foreach ($subarray3 as $FinalDataList) {
					if (!$isEmptyCol) {
						...
						if ($lab == 'Occupancy' || $lab == 'revenue'|| ($lab == 'plan' && ($FinalDataList == 'Total Revenue' || $FinalDataList == 'ARR'))
							|| $lab == 'Total Revenue' || $lab == 'ARR') {
