		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
			//$roomNumberArray=array();
				while($rowOrderDetail	= mysqli_fetch_object($sqlOrderDetail)){
					...
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['RoomType']=$RoomNoAndRoomName;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['Type']='Reservation';
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['dated']= date('d-m-Y',strtotime($checkin));
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['checkoutdated']=  date('d-m-Y',strtotime($checkoutDated));
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['DayWiseDated']=  date('d-m-Y',strtotime($rowOrderDetail->dated));
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['source']= 'Tariff';
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['sac_no']= '996311';
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['rate_plan']= $rate_plan;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['tariff'] +=$rowOrderDetail->tariff_price_per_day_per_room;
					//$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id_mst_room_types]['tax'] +=$rowOrderDetail->tax_per_day_per_room;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['Days']+=$Days;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['sgst']+=$sgst;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['cgst']+=$cgst;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['Total'] +=$rowOrderDetail->tariff_price_per_day_per_room+$rowOrderDetail->tax_per_day_per_room;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['InvoiceNo']=$mdoc_no;
					$folioArray[$RoomNoAndRoomName.$DatedLine][$rowOrderDetail->id_mst_room_types]['adults_per_room']=$rowOrderDetail->adults_per_room;
	...
	$set='1';
		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
				...
				while($rowOrderDetail= mysqli_fetch_object($sqlOrderDetail)){
					//$roomNo= selectColumn(TBL_ROOMNO,'room_no'," WHERE `id` = '".$rowOrderDetail->id_mst_room_no_allocation."'");
					//$RoomName	=	selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = '".$rowOrderDetail->id_mst_room_types."'");
					//echo '<pre>';print_r($rowOrderDetail);				
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['DayWiseDated']= date('d-m-Y',strtotime($rowOrderDetail->doc_date));
...
$sqlOrderDetail = mysqli_query($connNew,"Select  * from `fo_reservations_addons_details` where id_fo_folio_to='".addslashes($id_folio)."'");
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
				...
				while($rowOrderDetail= mysqli_fetch_object($sqlOrderDetail)){
					...
					$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['DayWiseDated']= date('d-m-Y',strtotime($rowOrderDetail->dated));
...
//Shared Guest============================
  $guests = array();
if($id_parent_folio=='0'){
$id_parent_folioShar	=	$id_fo_folio_to;
}else{
$id_parent_folioShar	=	$id_parent_folio;
}
  $id_shared_guest=selectColumn(FO_RESERVATIONS_DETAILS,'id_shared_guest','WHERE id_fo_folio_to="'.$id_parent_folioShar.'"');
  if ($id_shared_guest != '') {
                          ...
                          <tr>
                            ...
                            <td class="pm"
                              ...
                              <p style="font-size:10px;font-family: sans-serif;margin:0!important;padding-left:1px;">
                                <?php echo date('d-m-Y',strtotime($Array2['DayWiseDated'])); ?> <?php 
                    ...
                    <thead>
                      <tr>
                        <td class="pm"
                          ...
                          colspan="5"> Tax Summary </td>
                      </tr>
                      <tr>
                        <td class="pm"
                          style="width:40%;font-size:11px;font-family: sans-serif; padding: 0px!important;margin:0;border-right: 0.4px solid #000;border-bottom: 0.4px solid #000; text-align : right; vertical-align : middle; padding-right : 10px!important;"
