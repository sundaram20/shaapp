...
define(TBL_ASSIGN_HOTEL_ROOM,'mst_assign_hotel_rooms');
              ...
              <div class="box-body">			  
			   <div class="form-group">
                 ...
                 <?php $categoryDropDown = '<select class="form-control select2" name="room_id" id="room_id">
											  ...
											  $resCat = mysqli_query($connNew,"Select rt.name,rt.id as roomtypeid, ahr.* from  `".TBL_ASSIGN_HOTEL_ROOM."` ahr left join  `".TBL_ROOM_TYPE."` rt on  ahr.id_mst_room_types = rt.id where rt.status='1' and ahr.status='1' and ahr.id_mst_hotels='".$sqlHotelMappingDetail->hotel_id."'");
											  if(mysqli_num_rows($resCat)){
											  	while($resultCat = mysqli_fetch_object($resCat)){
													if($row->room_id == $resultCat->roomtypeid){
														$selected = 'selected="selected"';
													}else{
														$selected = '';
													}
													$categoryDropDown .= '<option '.$selected.' value="'.$resultCat->roomtypeid.'">'.ucfirst($resultCat->name).'</option>';
												}
											  }
