...
if($_REQUEST['action'] == 'change'){
	...
	if($_REQUEST['activeId'] != ''){
		...
		checkUserLevelPermission($_SESSION['userLevel'],TBL_HOTEL_MAPPING,'activate');

		$statusId = addslashes(encryptor('decrypt',$_REQUEST['activeId']));

		$statusSql = "	UPDATE `".TBL_HOTEL_MAPPING."`
		...
		checkUserLevelPermission($_SESSION['userLevel'],TBL_HOTEL_MAPPING,'deactivate');
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_HOTEL_MAPPING,'delete');

	$delSql = "DELETE FROM `".TBL_HOTEL_MAPPING."` WHERE `id` = '".addslashes(encryptor('decrypt',$_REQUEST['delId']))."'";

	$sqlDelUserLevel = selectRow(TBL_HOTEL_MAPPING," WHERE `id` = '".addslashes(encryptor('decrypt',$_REQUEST['delId']))."'");
...
if($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])){
	...
	$delSql = "DELETE FROM `".TBL_HOTEL_MAPPING."` WHERE `id` IN (".addslashes($deleteIds).")";
...
$sql = " SELECT * FROM `".TBL_HOTEL_MAPPING."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."'";

if($_REQUEST['hotel_id'] != ''){

	$sql .= " AND `hotel_id` = '".addslashes($_REQUEST['hotel_id'])."%'";
    ...
    <section class="content">
        <div class="box box-default">
            ...
            <div class="box-header with-border">
                ...
                <a type="button" class="btn btn-success" href="editHotelMapping.php"> Add Hotel Mapping</a>
  ...
  <div class="form-group">
    ...
    <label>Hotel</label>				
     ...
     <?php $categoryDropDown = '<select class="form-control select2" name="hotel_id">
                                  ...
                                  $resCat = selectSql(TBL_HOTELS," where status='1' AND id_shop='".$_SESSION['shop']."' ",' ORDER BY `id`');
...
<input name="Search" type="submit" class="btn btn-primary" value="Apply" />
...
<tr>
  ...
  <th>Mapping Id</th>
     ...
     <th>Auto Update <br>Channel Inventory</th>

<th>Auto Update <br>Channel rate</th>
...
$channelCRM	=	selectSql(TBL_CHANNEL_MANAGER,"WHERE  `channel_type` = '3'  AND `id` = '".addslashes($row->channel_id)."'",'');

if($db->num_rows2($channelCRM)){
//$HotelName= 'All Hotel' ;
}else{
$HotelName	=	 selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".$row->hotel_id."'");
$HotelCity	=	 selectColumn(TBL_HOTELS,'city'," WHERE `id` = '".$row->hotel_id."'");
...
<tr>
  ...
  <td><?php echo  $HotelName.', '.$HotelCity;  ?></td>

  <td><?=$row->booking_engine_id;?></td>	

  <td> <?php  if($row->auto_sync_inv==1){
       echo "Yes";
  }  else{
      echo "No" ;}?></td>
   <td> <?php  if($row->auto_sync_rate==1){
    echo "Yes"; 
 } else{
      echo "No" ;}?></td>
  <td><?=$row->status=='1'?'<span onclick="location.href=\'manageHotelMapping.php?inactiveId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageHotelMapping.php?activeId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>			 

  <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editHotelMapping.php?eId=<?=encryptor('encrypt',$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="../images/delete.gif" style="cursor:pointer;" title="Delete" onClick="if(confirm('Are you sure that you want to delete this record <?=$row->name;?>?')){window.location.href='manageHotelMapping.php?delId=<?=encryptor('encrypt',$row->id)?>&action=delete&page=<?=$_REQUEST['page']?>';}"/></td>
