...
$folioArray=array();
		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
          ...
          $folioArray[$RoomNoAndRoomName][$mdoc_no]['source']= 'Front Office';
		...
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
			echo '====================='.$id_fo_folio_to;
				while($rowOrderDetail= mysqli_fetch_object($sqlOrderDetail)){
					...
					echo '==='.$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['dated']= date('d-m-Y',strtotime($rowOrderDetail->doc_date));
					echo '==='.$folioArray[$RoomNoAndRoomName][$rowOrderDetail->id]['source']= $outletName;
