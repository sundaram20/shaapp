	...
	$sql1 = executeSql("SELECT * FROM `".TBL_INV_ITEMS."` ORDER BY id DESC LIMIT 1");
	 ...
	 if($idd == '0'){
		...
		/*	$auditaddSql = "INSERT INTO audit_trail SET
							`voucher_id` = '".$last_id."',
							`tables_name` = 'inv_items',
							`form_code` = 'items_manager_form',
							`changes` = 'No Change',
							`date_created` = '".currenDateTime()."',
							`last_modified` = '".currenDateTime()."',
							`id_mst_user_modified_by` = '".$_SESSION['userId']."',
		...
		//if($_POST['open_qty'] > 0){
				...
				$sql12 = " SELECT books_start_from FROM `".TBL_SHOP."` WHERE `id`='".addslashes($_SESSION['shop'])."' ";
					...
					$addSql = " INSERT INTO `".TBL_PURCH."` SET
						...
						`status` = '".addslashes($_POST['status'])."'";
					executeSql($addSql);
					$lastInsertId_purch23= $db->insert_id();	
					...
					//Purch Details Table
						...
						$addSql12 = " INSERT INTO `".TBL_PURCH_DETAILS."` SET
							`id_pos_purch` = '".addslashes($lastInsertId_purch23)."', 
                            `id_mst_outlet` = '".implode(',',$_REQUEST['ids_mst_outlet'])."',	
							`item_amount` = '".$_REQUEST['detail_rate'][$i]."',   
							`id_mst_items` = '".addslashes($lastInsertId_items)."',
							`id_mst_items_details` = '".addslashes($lastInsertId_purch1)."', 
							`qty` = '".addslashes($qty_total)."', 
				...
				executeSql($addSql22);
					...
					//Purch Table
					 $addSql = " INSERT INTO `".TBL_PURCH."` SET
						...
						`status` = '".addslashes($_POST['status'])."'";
					executeSql($addSql);
						$lastInsertId_purch12= $db->insert_id();
					//Purch Details Table
						if($_POST["id_mst_attributes_unit_main"] == $_POST["id_mst_attributes_unit_alt"]){
							$qty_total = $_POST['open_qty'];
					 		$alt_qty = $qty_total * $_POST['conversion_qty'];
					 ...
					 $addSql = " INSERT INTO `".TBL_INV_PURCH_DETAILS."` SET
						...
						executeSql($addSql);
							...
							//`alt_qty` = '".addslashes($alt_qty)."',  
							//`conver_rate_per_unit` = '".addslashes($_POST['conversion_qty'])."', 
							//`id_mst_items_details` = '0',
							//  
						 $addSql12 = " INSERT INTO `".TBL_PURCH_DETAILS."` SET
							`id_pos_purch` = '".addslashes($lastInsertId_purch12)."', 
                            `id_mst_outlet` = '".implode(',',$_REQUEST['ids_mst_outlet'])."',	
							`item_amount` = '".addslashes($_POST['sale_rate'])."',   
							`id_mst_items` = '".addslashes($lastInsertId_items)."', 
							`id_mst_items_details` = '0', 
...
$auditquery = "SELECT * From `".TBL_INV_ITEMS."` WHERE id = '".addslashes(encryptor(decrypt,$_POST[eId]))."' ";
   $auditresSQL = mysqli_query($connNew, $auditquery);	
	...
	if($c6 != $_POST['item_class']){
		 $ch6 ="Item Class Details Changed from " .   $c6 ." - to - " . $_POST['item_class'];
	}
	...
	if($c9 != $_POST['id_mst_attributes_store']){
	...
	if($c10 != $_POST['id_mst_attributes_printer']){
	...
	if($c11 != $_POST['id_mst_charges_sales_local']){
	...
	if($c12 != $_POST['id_mst_charges_sales_interstate']){
	...
	if($c13 != $_POST['id_mst_charges_purchase_local']){
		 $old_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$c13."'");
		 $new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_purchase_local']."'  ");
		 $ch13 = "Purchase Account Local Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
		 if($c13 == '0'){
		 $ch13 = "";
	    }
	}	
	if($c14 != $_POST['id_mst_charges_purchase_interstate']){
		 $old_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$c14."'");
		 $new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_purchase_interstate']."'  ");
		 $ch14 = "Purchase Account Interstate Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
		 if($c14 == '0'){
		 $ch14 = "";
	    }
	}
	if($c15 != $_POST['sale_rate']){
		$ch15 ="Standard Sale Rate Details Changed from " .   $c15 ." - to - " . $_POST['sale_rate']." in Item Name ".$c3;
	}
	if($c16 != $_POST['purchase_rate']){
		 $ch16 ="Standard Purchase Rate Details Changed from " .   $c16 ." - to - " . $_POST['purchase_rate']." in Item Name ".$c3;
	}
	if($c17 != $_POST['min_qty']){
		$ch17 ="Min Stock Level Details Changed from " .   $c17 ." - to - " . $_POST['min_qty']." in Item Name ".$c3;
	}
	if($c18 != $_POST['max_qty']){
		$ch18 ="Max Stock Level Details Changed from " .   $c18 ." - to - " . $_POST['max_qty']." in Item Name ".$c3;
	}
	if($c19 != $_POST['rol']){
		$ch19 ="Rol Details Changed from " .   $c19 ." - to - " . $_POST['rol']." in Item Name ".$c3;
	}
	if($c20 != $_POST['roq']){
		$ch20 ="Re Order Qty Details Changed from " .   $c20 ." - to - " . $_POST['roq']." in Item Name ".$c3;
	}
	if($c21 != $_POST['open_qty']){
		$ch21 ="Opening Qty Details Changed from " .   $c21 ." - to - " . $_POST['open_qty']." in Item Name ".$c3;
	}
	if($c22 != $_POST['open_amount']){
	...
	if($c25 != $_POST['item_production_item']){
	...
	if($c26 != $_POST['item_allow_additional']){
	...
	if($c27 != $_POST['stockable_enable_disable']){
	...
	if($c30 != $_POST['display_order']){
		$ch30 ="Display Order Details Changed from " .   $c30 ." - to - " . $_POST['display_order']." in Item Name ".$c3;
			...
			$editSql .= "	,`id_shop` = '".addslashes($_SESSION['shop'])."'
				...
				//	executeSql("DELETE FROM `".TBL_PURCH_DETAILS."` WHERE `id_mst_items` = '".addslashes(encryptor(decrypt,$_POST[eId]))."'");
			...
			executeSql("DELETE FROM `".TBL_PURCH_DETAILS."` WHERE `id_mst_items` = '".addslashes(encryptor(decrypt,$_POST[eId]))."'");
				 ...
				 $addSqle2 = "   	INSERT INTO `".TBL_PURCH."` SET
						`doc_type` = '".addslashes($doc_type)."',
						`id_mst_outlet` = '".implode(',',$_REQUEST['ids_mst_outlet'])."',
						`doc_date` = '".date('Y-m-d' , strtotime($books_start_from))."', 		       
						`id_shop` = '".addslashes($_SESSION['shop'])."'";
						$addSqle2 .= ",`date_created` = '".currenDateTime()."',
						`last_modified` = '".currenDateTime()."',
						`id_mst_user_modified_by` = '".$_SESSION['userId']."',
						`id_mst_user_created_by` = '".$_SESSION['userId']."',
						`status` = '".addslashes($_POST['status'])."'";
					executeSql($addSqle2);
						$lastInsertId_purcedh= $db->insert_id();
				...
				$addSqlee = "INSERT INTO `".TBL_PURCH_DETAILS."` SET
							`id_pos_purch` = '".addslashes($lastInsertId_purcedh)."',
						    `id_mst_outlet` = '".implode(',',$_REQUEST['ids_mst_outlet'])."',
							`id_mst_items` = '".addslashes(encryptor(decrypt,$_REQUEST[eId]))."', 
							`id_mst_items_details` = '".addslashes($lastInsertId_purch2)."', 
							`qty` = '".addslashes($qty_total)."', 
							`main_unit` = '".$_REQUEST['detail_unit'][$i]."', 
							`item_amount` = '".$_REQUEST['detail_rate'][$i]."',  
							`date_created` = '".currenDateTime()."',
					...
					//Purch Details Table
					 ...
					 $addSql = " UPDATE  `".TBL_INV_PURCH_DETAILS."` SET	
							`doc_type` = '".addslashes($doc_type)."', 
							`item_amount` = '".addslashes($_POST['sale_rate'])."',   
							`id_inv_items` = '".addslashes(addslashes(encryptor(decrypt,$_REQUEST[eId])))."', 
							`id_inv_items_details` = '0', 
							`qty` = '".addslashes($qty_total)."', 
							`main_unit` = '".addslashes($main_unit)."',  
							`alt_unit` = '".addslashes($alt_unit)."', 
							...
							WHERE  id_inv_items = '".addslashes(encryptor(decrypt,$_REQUEST[eId]))."'";
						executeSql($addSql); 
					...
					/*	$addSql = " INSERT INTO `".TBL_INV_PURCH_DETAILS."` SET
					 ...
					 $addSql12 = " UPDATE `".TBL_PURCH_DETAILS."` SET
                            `id_mst_outlet` = '".implode(',',$_REQUEST['ids_mst_outlet'])."',	
							`item_amount` = '".addslashes($_POST['sale_rate'])."',   
							`id_mst_items` = '".addslashes(addslashes(encryptor(decrypt,$_REQUEST[eId])))."', 
							`id_mst_items_details` = '0', 
							`qty` = '".addslashes($qty_total)."', 
							`main_unit` = '".addslashes($main_unit)."',
							`date_created` = '".currenDateTime()."',
							...
							`id_mst_user_modified_by` = '".$_SESSION['userId']."',
							`id_mst_user_created_by` = '".$_SESSION['userId']."'
WHERE  id_mst_items = '".addslashes(encryptor(decrypt,$_REQUEST[eId]))."' ";
executeSql($addSql12);	
					...
					/*	$addSql12 = " INSERT INTO `".TBL_PURCH_DETAILS."` SET
                            ...
                            `id_mst_outlet` = '".implode(',',$_REQUEST['ids_mst_outlet'])."',	
							`item_amount` = '".addslashes($_POST['sale_rate'])."',   
							`id_mst_items` = '".addslashes(addslashes(encryptor(decrypt,$_POST[eId])))."', 
							`id_mst_items_details` = '0', 
							`qty` = '".addslashes($qty_total)."', 
							`main_unit` = '".addslashes($main_unit)."',
							`date_created` = '".currenDateTime()."',
...
if($submenu=='88'){
	$sqlResult1 = "SELECT * FROM ".TBL_ATTRIBUTES." WHERE table_name = 'items_type' AND field_category='Menu Items' AND id_shop = ".$_SESSION['shop'] ." ";
	...
	$sqlResult1 = "SELECT * FROM ".TBL_ATTRIBUTES." WHERE table_name = 'items_type' AND field_category='Store Items' AND id_shop = ".$_SESSION['shop'] ." ";
	$QuerySQL1	=	mysqli_query($connNew,$sqlResult1);
				...
				echo $categoryDropDown .= '</select>';
		            ...
		            <div class="row">
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                 ...
		                 <?php $categoryDropDown = '<select class="form-control select2" name="id_mst_attributes_unit_main" id="id_mst_attributes_unit_main" data-parsley-required onchange="changeFunc()" style="width:100%">
		            ...
		            <div class="row">
		            	<div class="form-group col-xs-12 col-md-6 col-sm-2">
		                 ...
		                 <?php $categoryDropDown = '<select class="form-control select2" name="id_mst_attributes_unit_alt" id="id_mst_attributes_unit_alt" data-parsley-required onchange="changeFunc()" style="width:100%">
		                ...
		                <style type="text/css">
							#main{display: none; color: green;} 
							#alt{display: none; color: green;} 
						</style>
		                ...
		                <div class="col-md-6 col-sm-2">
			                  ...
			                  <input type="text" class="form-control" placeholder="Enter Conversion Quantity" id="conversion_qty" name="conversion_qty" value="<?php if($_POST) echo $_POST['conversion_qty'];else echo stripslashes($row->conversion_qty);?>" ></div>
			              </div>
			              <div class="col-md-3 col-sm-2">
...
//echo "select * from ".mst_outlets."  where status='0' AND id IN (".$row->ids_mst_outlet.") ";
									...
									if($db->num_rows2($resCat1)){
										while($resultCat1 = $db->fetch_object2($resCat1)){
											...
											$categoryDropDown .= '<option selected="selected" value="'.$resultCat1->id.'">'.$resultCat1->name.'</option>';
											  ...
											  $resCat = selectSql(mst_outlets," where id_shop='".$_SESSION['shop']."' AND  status = '1' and id NOT IN (4,6) ",'');
...
function removeGrid(id){
    ...
    $('#grid'+id).remove();
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp; 
			   <input type='button' value='Close' class="btn btn-danger" onclick='location.replace("manageItems.php?submenu=<?php echo $_GET['submenu'] ?>"); '>
