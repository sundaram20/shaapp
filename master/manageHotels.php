...
if($_REQUEST['action'] == 'change'){
	...
	$sqlDelUserLevel = selectRow(TBL_HOTELS," WHERE `id` = '".$deleteIds."'");
...
$countSql = "SELECT * FROM `".TBL_HOTELS."` WHERE id_shop='".addslashes($_SESSION['shop'])."'";
...
if($_REQUEST['hotel_category'] != ''){
	$sql .= " AND `id_mst_hotel_category` = '".addslashes($_REQUEST['hotel_category'])."'";
}
...
$countsql = mysqli_query($connNew, $countSql);
$countRecord = mysqli_num_rows($countsql);*/

if($_REQUEST['searchFormSubmit']==''){
			$sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_HOTELS."' AND id_shop = ".$_SESSION['shop'] ." ");
			...
			$sqlResultOrder = mysqli_query($appConnect, "SELECT field_name FROM ".TBL_REPORT." WHERE table_name = '".TBL_HOTELS."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY ID DESC LIMIT 0,1" );
		...
		$sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_HOTELS."' AND id_shop = ".$_SESSION['shop'] ." AND field_name IN ('".$fieldname."') ");
...
$sql = local_SelectQuery_Mst_Hotels($_REQUEST['tableName'],$field_name,$field_label,$EnableOrderBy);
$QuerySQL	=	mysqli_query($connNew,$sql);
$numRows= @mysqli_num_rows($QuerySQL);

$TotalCountRecord = selectColumn(TBL_HOTELS,'count(id)',' WHERE id_shop="'.$_SESSION['shop'].'" ');
	...
	<div class="box box-default">
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border">
		  ...
		  <a type="button" class="btn btn-success" href="editHotels.php" > Add Hotel</a>
			<button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">
			<span class="caret"></span>
			<span class="sr-only">Toggle Dropdown</span>
		  </button>
		  <ul class="dropdown-menu" role="menu">
			<?php ?><li><a title="Export to excel file" href="masterExportTable.php?fileType=xls&tableName=<?php echo TBL_HOTELS;?>"><img src="../images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>
	            <li><a title="Export to csv file" href="masterExportTable.php?fileType=csv&tableName=<?php echo TBL_HOTELS;?>"><img  src="../images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li>
			<?php ?>
		  </ul> 
		</div>
       		...
       		<div class="box-body">
          		<div class="row">
		            ...
		            <div class="col-md-6 col-sm-6">
		            	<div class="form-group">
		            		<label>Hotel Name - City</label>				 
							<select class="form-control select2 hotelName" name="search_name" id="search_name"  style="width:100%;">
					...
					<div class="col-md-6">
              			<div class="form-group">
                			<label>Hotel Category</label>				
				 			<?php $categoryDropDown = '<select class="form-control select2" name="id_mst_hotel_category" style="width:100%">
          			...
          			<div class="col-md-6">
          				<div class="form-group">
          					<label>Hotel City</label>
          						<?php
          							$cityDropDown = '<select class="form-control select2 " name="city" id="city" style="width:100%">
          							<option value="">Select City</option>';
          							$resCity = selectSql(TBL_HOTELS," where id_shop='".$_SESSION['shop']."' ",'group by city');
          							if($db->num_rows2($resCity)){
          								while($resultCity = $db->fetch_object2($resCity)){
          									if($_REQUEST['city'] == $resultCity->city){
          										$selected = 'selected="selected"';
											}else{
												$selected = '';
											}
											$cityDropDown .= '<option '.$selected.' value="'.$resultCity->city.'">'.ucfirst($resultCity->city).'</option>';
          								}
          							}
          							echo $cityDropDown .= '</select>';
          						?>
          				</div>
          			</div>
		        				...
		        				<div class="box-body">
	        						<div class="row">
	        							...
	        							<div class="col-md-3 col-sm-3">
	        								<select class="form-control select2" style="width:100%;" name="EnableOrderBy" id="EnableOrderBy" >
	        									...
	        									<?php
	    											$sqlResult = mysqli_query($appConnect, "SELECT field_name,field_label FROM ".TBL_REPORT." WHERE table_name = '".TBL_HOTELS."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc" );
	        						...
	        						<div class="row">
											...
											is_array($_REQUEST['field_name']) ? '' : $_REQUEST['field_name']=array();
		    									$sqlResult = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_HOTELS."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
	    								...
	    								<?php } }?>		
	    								<input type="hidden" name="tableName" value="<?php echo TBL_HOTELS;?>" />	
		    						</div>
...
//print_r($row) ;
								...
								if($cell=='Active'){						
									...
									$cell='<span onclick="location.href=\'manageHotels.php?inactiveId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>';
								}else if($cell=='Inactive'){
									$cell='<span onclick="location.href=\'manageHotels.php?activeId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';
								}
								if($value == 1){
									$Id = $cell;
								}else{
							...
							$strLocation = 'editHotels.php?eId='.encryptor(encrypt,$Id).'&action=edit&page='.$_REQUEST['page'];

							$table_name = array(TBL_USERS);
							$table_name1 = array(TBL_ROOMNO);
                			$ajaxCheckTransactions = CheckTransactionsHotels($Id, $table_name, $table_name1); 
  ...
  <script type="text/javascript">
    ...
    swal({
     ...
     function(isConfirm)
     {
	     ...
	     if (isConfirm)
	     {  
	          self.location='manageHotels.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	     } 
	     else
	     {
	        self.location='manageHotels.php';
	      }
	     });
    }
    ...
    $('.hotelName').select2({
        ...
        ajax: {
          url: "ajax/ajaxSearchHotelName.php",
          dataType: 'json',
          delay: 50,
