<?php 
	...
	if($_REQUEST['searchFormSubmit']==''){
	 ...
	 $sql = local_SelectQuery_Mst_Items1($_REQUEST['tableName'],$field_name,$field_label,$EnableOrderBy);
	//echo "hello"; exit;
        	...
        	<div class="box-header with-border">
        		<h3 class="box-title">Search <small>Total Records: (<?=$total;?>) &nbsp;</small> </h3>
		  		<!-- <div class="btn-group  pull-right">
		  			<a type="button" class="btn btn-success" href="editLaundry.php">Add Items</a>
				...
				<div class="btn-group  pull-right">
		          	<a type="button" class="btn btn-success" href="editLaundry.php?submenu=<?php echo $_GET['submenu']; ?>">Add Laundry Items</a>
			         <button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">
			            <span class="caret"></span>
			            <span class="sr-only">Toggle Dropdown</span>
			        </button>
							...
							while($row = @mysqli_fetch_row($QuerySQL)){
								...
								//print_r($row) ;
									...
									if($cell=='Active'){						
										...
										$cell='<span onclick="location.href=\'manageLaundry.php?inactiveId='.encryptor(encrypt,$Id).'&submenu='.$_GET['submenu'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>';
									}else if($cell=='Inactive'){
										$cell='<span onclick="location.href=\'manageLaundry.php?activeId='.encryptor(encrypt,$Id).'&submenu='.$_GET['submenu'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';
									}
									if($value == 1){
										$Id = $cell;
									}else{
								...
								$strLocation = 'editLaundry.php?eId='.encryptor(encrypt,$Id).'&submenu='.$_GET['submenu'].'&action=edit&page='.$_REQUEST['page'];
  ...
  <script type="text/javascript">
    ...
    swal({
     ...
     function(isConfirm)
     {
	     ...
	     if (isConfirm)
	     {  
	          self.location='manageLaundry.php?delId='+sid+'&submenu=<?=$_GET["submenu"]?>&action=delete&page=<?=$_REQUEST["page"]?>';
	     } 
	     else
	     {
	        self.location='manageLaundry.php?submenu=<?php echo $_GET["submenu"] ?>';
	      }
	     });
    }
