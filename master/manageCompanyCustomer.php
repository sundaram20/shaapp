<?php 
	...
	if($_REQUEST['action'] == 'change'){
		...
		if(executeSql($statusSql)){
			$err = 0;
			$_SESSION['successMsg'] = 'Room Type '.selectColumn(TBL_COMPANY_CONTACTS,'status'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';
		}else{
			$err = 1;
			$_SESSION['errorMsg'] = 'Room Type '.selectColumn(TBL_COMPANY_CONTACTS,'status'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
		}

	}else if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){

		checkUserLevelPermission($_SESSION['userLevel'],TBL_COMPANY_CONTACTS,'delete');
		$deleteIds = encryptor(decrypt,$_REQUEST['delId']);
		$delSql = "DELETE FROM `".TBL_COMPANY_CONTACTS."` WHERE `id` = '".addslashes($deleteIds)."'";
		$sqlDelUsers = selectRow(TBL_COMPANY_CONTACTS," WHERE `id` = '".addslashes($deleteIds)."'");
		if(executeSql($delSql)){
			$err = 0;
			
			$_SESSION['successMsg'] = 'One Room Type '.$sqlDelUsers["name"].' has been deleted sucessfully.';
			}else{
				$err = 1;
				$_SESSION['errorMsg'] = 'Unable to delete Room Type '.$sqlDelUsers["name"];
		}
	}
	...
	if($_REQUEST['searchFormSubmit']==''){
			$sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_COMPANY_CONTACTS."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
			...
			$sqlResultOrder = mysqli_query($appConnect, "SELECT field_name FROM ".TBL_REPORT." WHERE table_name = '".TBL_COMPANY_CONTACTS."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc LIMIT 0,1" );
		...
		$sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_COMPANY_CONTACTS."' AND id_shop = ".$_SESSION['shop'] ." AND field_name IN ('".$fieldname."')  ORDER BY display_order");
	...
	$sql = local_SelectQuery_Company_Customer($_REQUEST['tableName'],$field_name,$field_label,$EnableOrderBy);
	//echo "hello"; exit;
	$QuerySQL	=	mysqli_query($connNew,$sql);
	$numRows = @mysqli_num_rows($QuerySQL);

	$TotalCountRecord = selectColumn(TBL_COMPANY_CONTACTS,'count(id)',' WHERE id_shop= "'.addslashes($_SESSION['shop']).'" ');
		...
		<div class="box box-default">
        	...
        	<div class="box-header with-border">
        		<h3 class="box-title">Search <small>Total Records: (<?=$TotalCountRecord;?>) &nbsp;</small> </h3>
		  			...
		  			<a type="button" class="btn btn-success" href="editCustomer.php">Add Customer</a>
			            	...
			            	<div class="form-group">
			            		<label>Customer Name</label>
			                      ...
			                      <?php $categoryDropDown = '<select class="form-control select2" name="search_name" style="width:100%">
			                        ...
			                        $resUserLevel = selectSql(TBL_COMPANY_CONTACTS," WHERE `status` = '1' AND  `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `first_name`');
			                        if($db->num_rows2($resUserLevel)){
			                          while($resultUserLevel = $db->fetch_object2($resUserLevel)){
			                            ...
			                            $categoryDropDown .= '<option '.$selected.' value="'.$resultUserLevel->id.'">'.ucfirst($resultUserLevel->first_name).'</option>';
			                              }
			                          }
			            ...
			            <div class="col-md-6 col-sm-6">
			            	<div class="form-group">
			            		<label>Company Name</label>
			                      ...
			                      <?php $categoryDropDown = '<select class="form-control select2" name="company_name" style="width:100%">
			                        <option value="">Select Company Name</option>';
			                        $resUserLevel = selectSql(TBL_COMPANY," WHERE `status` = '1' AND  `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `name`');
			                        if($db->num_rows2($resUserLevel)){
			                          while($resultUserLevel = $db->fetch_object2($resUserLevel)){
			                            if($_REQUEST['search_name'] == $resultUserLevel->id){
			                              $selected = 'selected="selected"';
			        				...
			        				<div class="box-body">
		        						<div class="row">
		        							...
		        							<div class="col-md-3 col-sm-3">
		        								<select class="form-control select2" style="width:100%;" name="EnableOrderBy" id="EnableOrderBy" >
		        									...
		        									<?php
		    											$sqlResult = mysqli_query($appConnect, "SELECT field_name, field_label FROM ".TBL_REPORT." WHERE table_name = '".TBL_COMPANY_CONTACTS."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc" );
		        						...
		        						<div class="row">
											...
											is_array($_REQUEST['field_name']) ? '' : $_REQUEST['field_name']=array();
			    									$sqlResult = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_COMPANY_CONTACTS."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
	    								...
	    								<?php } }?>		
	    								<input type="hidden" name="tableName" value="<?php echo TBL_COMPANY_CONTACTS;?>" />	
		    						</div>
						...
						<tr>
							...
							<?php 
					  			for($i=0; $i<$fields_num; $i++){
									...
									if($field->name != 'id' && $field->name != 'id_mst_company'){
							...
							while($row = @mysqli_fetch_row($QuerySQL)){
								...
								foreach($row as $cell){
									...
									if($cell=='Active'){						
										...
										$cell='<span onclick="location.href=\'manageCompanyCustomer.php?inactiveId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>';
									}else if($cell=='Inactive'){
										$cell='<span onclick="location.href=\'manageCompanyCustomer.php?activeId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';
									}
									if($value == 1){
										$Id = $cell;
									}else if($value == 2){
								...
								$strLocation = 'editCustomer.php?eId='.encryptor(encrypt,$eId).'&id='.encryptor(encrypt,$Id).'&action=edit&page='.$_REQUEST['page'];
  ...
  <script type="text/javascript">
    ...
    swal({
     ...
     function(isConfirm)
     {
	     ...
	     if (isConfirm)
	     {  
	          self.location='manageCompanyCustomer.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	     } 
	     else
	     {
	        self.location='manageCompanyCustomer.php';
	      }
	     });
    }
