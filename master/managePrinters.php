...
// ----------cate---------
$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' and `table_name` = 'printer'  ";
...
?>
        ...
        <div class="box-header with-border"> 
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn btn-success" href="editPrinters.php" >Add Printer</a>
        ...
        <div class="col-xs-12">		     
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Printer List</h3>
            </div>
            ...
            <div class="box-body table-responsive">
		        ...
		        <tbody>  
                  ...
                  <td><?php echo $row->printer_port;?></td>
	      			 <td>   <ul>	<?php
	        		$outletSql = "SELECT * FROM ".TBL_USERS." WHERE id_shop='".$_SESSION['shop']."'  AND status = '1' ";
	        		$resOutlet = mysqli_query($connNew,$outletSql);
	        		$rowOutlet = mysqli_fetch_object($resOutlet);
	       					if($row->ids_mst_user !=""){	
	       				$hotels = explode(',',$row->ids_mst_user);

	       				
	       						 for($i=0 ;$i<count($hotels);$i++ ){
				  		echo '<li>'.selectColumn(TBL_USERS,'name'," WHERE `id` = '".$hotels[$i]."'").'</li>';
				  	  }	
				  	}else{ echo "<li>All Users</li>";}?>	
                  ...
                  <td><?php echo $row->status=='1'?'<span onclick="location.href=\'managePrinters.php?inactiveId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'managePrinters.php?activeId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>		

                  	<?php 
				  		$table_name = array(TBL_INV_ITEMS);
				  		$ajaxCheckTransactions = CheckTransactionsPrinter($row->id, $table_name); 

						if($ajaxCheckTransactions != '1'){
                 	 ?>
				  	<td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editPrinters.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='managePrinters.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	   } 
	   else
	   {
	      self.location='managePrinters.php';
	    }
	   });
	  }
  </script> 
