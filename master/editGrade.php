...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `field_value` = '".addslashes(trim($_POST['field_value']))."' and `table_name` = 'grade' ";
			$db->query($sql);
			$numRows= $db->num_rows();
			if($numRows == '0'){
			...
			if(executeSql($addSql)){
				...
				$_SESSION['successMsg'] = 'New grade details has been added sucessfully.';
				header("location:manageGrade.php?eId=".encryptor(encrypt,$lastInsertId)."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
				$_SESSION['errorMsg'] = 'Grade details has not been saved. Please make corrections below.';
			}
			}else{
				$err++;
				$_SESSION['errorMsg'] = 'Grade Master Name Already Exist.';
			}
		}
		...
		//Update Section Here
			...
			if(executeSql($editSql)){
				...
				header("location:manageGrade.php?eId=".$_GET['eId']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
		...
		$err++;
		$_SESSION['errorMsg'] = 'Grade details has not been saved. Please make corrections.';
	}
}
// ----------cate---------
...
<div class="content-wrapper">
  ...
  <!--  <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="manageGrade.php"><i class="fa fa-dashboard"></i> Home</a></li>
        ...
        <div class="col-md-12">
          <!-- general form elements -->
			...
			<ul class="nav nav-tabs">
			   <li class="active" ><a href="#tab_1" data-toggle="tab">Grade</a></li>  
            </ul>
            ...
            <!-- form start -->  			        
			 <form name="form1"  method="post" enctype="multipart/form-data" data-parsley-validate autocomplete="off" >
              ...
              <div class="box-body">
				 ...
				 <div class="form-group">
                  <label for="name">Grade Name<font color="#FF0000">*</font></label>
                  <div class="input-group"> 
              			<div class="input-group-addon">
							<i class="fa fa-graduation-cap"></i> 
					   	</div>
                  <input type="text" class="form-control" placeholder="Enter Grade Name" id="field_value" name="field_value" value="<?php if($_POST) echo $_POST['field_value'];else echo stripslashes($row->field_value);?>"  data-parsley-required>
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
			   <input type='button' value='Close' class="btn btn-danger" onclick='location.replace("manageGrade.php"); '>
			 </div>
            </form>			
          </div>
          <!-- /.box -->
