<?php 
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_ROOM_AMENITIES,'view');

	if($_REQUEST['action'] == 'change'){
		if($_REQUEST['activeId'] != ''){
			checkUserLevelPermission($_SESSION['userLevel'],TBL_ROOM_AMENITIES,'activate');
			//$statusId = addslashes($_REQUEST['activeId']);
			$statusId = addslashes(encryptor(decrypt,$_REQUEST['activeId']));
			$statusSql = "	UPDATE `".TBL_ROOM_AMENITIES."` 
						SET `status` = '1' 
						,`last_modified` = '".currenDateTime()."'
						,`id_mst_user_modified_by` = '".$_SESSION['userId']."'
						WHERE `id` = '".$statusId."'";
		}elseif($_REQUEST['inactiveId'] != ''){
			checkUserLevelPermission($_SESSION['userLevel'],TBL_ROOM_AMENITIES,'deactivate');
		...
		if(executeSql($statusSql)){
			$err = 0;
			$_SESSION['successMsg'] = 'Room Amenities '.selectColumn(TBL_ROOM_AMENITIES,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';
		}else{
			$err = 1;
			$_SESSION['errorMsg'] = 'Room Amenities '.selectColumn(TBL_ROOM_AMENITIES,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
		}

	}else if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){

		checkUserLevelPermission($_SESSION['userLevel'],TBL_ROOM_AMENITIES,'delete');
		$deleteIds = encryptor(decrypt,$_REQUEST['delId']);
		$delSql = "DELETE FROM `".TBL_ROOM_AMENITIES."` WHERE `id` = '".addslashes($deleteIds)."'";
		$sqlDelUsers = selectRow(TBL_ROOM_AMENITIES," WHERE `id` = '".addslashes($deleteIds)."'");
		if(executeSql($delSql)){
			$err = 0;
			
			$_SESSION['successMsg'] = 'One Room Amenities '.$sqlDelUsers["name"].' has been deleted sucessfully.';
			}else{
				$err = 1;
				$_SESSION['errorMsg'] = 'Unable to delete Room Amenities '.$sqlDelUsers["name"];
		}
	}
	...
	if($_REQUEST['searchFormSubmit']==''){
			$sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_ROOM_AMENITIES."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
			...
			$sqlResultOrder = mysqli_query($appConnect, "SELECT field_name FROM ".TBL_REPORT." WHERE table_name = '".TBL_ROOM_AMENITIES."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc LIMIT 0,1" );
		...
		$sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_ROOM_AMENITIES."' AND id_shop = ".$_SESSION['shop'] ." AND field_name IN ('".$fieldname."')  ORDER BY display_order");
	...
	$sql = local_SelectQuery_Mst_Room_Amenities($_REQUEST['tableName'],$field_name,$field_label,$EnableOrderBy);
	//echo "hello"; exit;
	$QuerySQL	=	mysqli_query($connNew,$sql);
	$numRows = @mysqli_num_rows($QuerySQL);

	$TotalCountRecord = selectColumn(TBL_ROOM_AMENITIES,'count(id)',' WHERE id_shop= "'.addslashes($_SESSION['shop']).'" ');
        	...
        	<div class="box-header with-border">
        		<h3 class="box-title">Search <small>Total Records: (<?=$TotalCountRecord;?>) &nbsp;</small> </h3>
		  		<div class="btn-group  pull-right">
		  			<a type="button" class="btn btn-success" href="editRoomAmenities.php">Add Room Amenities</a>
			            	...
			            	<div class="form-group">
			            		<label>Room Amenities</label>
			                      ...
			                      <?php $categoryDropDown = '<select class="form-control select2" name="search_name" style="width:100%">
			                        ...
			                        $resUserLevel = selectSql(TBL_ROOM_AMENITIES," WHERE `status` = '1' AND  `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `name`');
			        				...
			        				<div class="box-body">
		        						<div class="row">
		        							...
		        							<div class="col-md-3 col-sm-3">
		        								<select class="form-control select2" style="width:100%;" name="EnableOrderBy" id="EnableOrderBy" >
		        									...
		        									<?php
		    											$sqlResult = mysqli_query($appConnect, "SELECT field_name, field_label FROM ".TBL_REPORT." WHERE table_name = '".TBL_ROOM_AMENITIES."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc" );
		        						...
		        						<div class="row">
											...
											is_array($_REQUEST['field_name']) ? '' : $_REQUEST['field_name']=array();
			    									$sqlResult = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_ROOM_AMENITIES."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
							...
							while($row = @mysqli_fetch_row($QuerySQL)){
								...
								//print_r($row) ;
									...
									if($cell=='Active'){						
										...
										$cell='<span onclick="location.href=\'manageRoomAmenities.php?inactiveId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>';
									}else if($cell=='Inactive'){
										$cell='<span onclick="location.href=\'manageRoomAmenities.php?activeId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';
									}
									if($value == 1){
										$Id = $cell;
									}else{
								...
								$strLocation = 'editRoomAmenities.php?eId='.encryptor(encrypt,$Id).'&action=edit&page='.$_REQUEST['page'];

								$table_name = array(TBL_ASSIGN_HOTEL_ROOM);
	                			$ajaxCheckTransactions = CheckTransactionsRoomAmenities($Id, $table_name); 
  ...
  <script type="text/javascript">
    ...
    swal({
     ...
     function(isConfirm)
     {
	     ...
	     if (isConfirm)
	     {  
	          self.location='manageRoomAmenities.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	     } 
	     else
	     {
	        self.location='manageRoomAmenities.php';
	      }
	     });
    }
