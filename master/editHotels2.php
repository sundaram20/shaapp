...
if($_POST['Save']){
	...
	}else if($db->num_rows2(selectSql(TBL_HOTELS,"WHERE `id` NOT IN('".addslashes(encryptor(decrypt,$_POST[eId]))."') and `id_shop` = '".addslashes($_SESSION['shop'])."' AND `name` = '".addslashes($_POST['name'])."'",''))){
		$err++;
		$err_name = '<font style="color:red;font-weight:normal;" ><br>Hotel name all-ready exists in our database.</font>';
	}
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			$addSql = "   	INSERT INTO `".TBL_HOTELS."` SET 
							...
							`id_hotel_category` = '".addslashes($_POST['hotel_category'])."',
							`hotel_code` = '".addslashes($_POST['hotel_code'])."',
							`name` = '".addslashes($_POST['name'])."',
							`address` = '".addslashes($_POST['address'])."',
							`city` = '".addslashes($_POST['city'])."',
							`id_zonal` = '".addslashes($_POST['zonal'])."',
							...
							`ids_unique_experiences` = '".addslashes(implode(',',$_POST['unique_experiences']))."',
        ...
        <div class="col-md-12">
          <!-- general form elements -->
			...
			<div class="box-header with-border">
              <h3 class="box-title"><?php echo $_REQUEST['eId']==''?'Add':'Edit'?> Hotel : <a><?php echo selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".addslashes(encryptor(decrypt,$_REQUEST['eId']))."'"); ?></a></h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->  			        
			 <form name="form1"  method="post" enctype="multipart/form-data" data-parsley-validate autocomplete="off" >
              ...
              <div class="box-body">			  
			   <div class="form-group">
                  <label for="name">Hotel Category <font color="#FF0000">*</font></label>
                 <?php $categoryDropDown = '<select class="form-control select2" name="hotel_category" data-parsley-required>
											  ...
											  if($db->num_rows2($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
													...
													}elseif($row->id_hotel_category == $resultCat->id){
														$selected = 'selected="selected"';
													}else{
														$selected = '';
													}
				...
				<?php echo $err_hotel_code;?>
				 ...
				 <div class="form-group">
                  ...
                  <input type="text" class="form-control" placeholder="Enter hotel name" id="name" name="name" value="<?php if($_POST) echo $_POST['name'];else echo stripslashes($row->name);?>" data-parsley-required>
				<?php echo $err_name;?>
                </div>
				...
				<div class="form-group">
                  ...
                  <?php $stateDropDown = '<select class="form-control select2" name="id_mst_state" data-parsley-required>
											  								<option value="">Select state</option>';
											  $resCat = selectSql(TBL_STATE," where id_mst_country_lang='110'",' ORDER BY `name`');
											  if($db->num_rows2($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
													if($_REQUEST['id_mst_state'] == $resultCat->id_state){
														$selected = 'selected="selected"';
													}elseif($row->id_state == $resultCat->id_state){
														$selected = 'selected="selected"';
													}else{
														$selected = '';
													}
				...
				<div class="form-group">
                  ...
                  <input type="text" class="form-control" placeholder="Enter alternate phone number" id="phone2" name="phone2" value="<?php if($_POST) echo $_POST['phone2'];else echo stripslashes($row->phone2);?>">
				<?php echo $err_phone2;?>
                </div>
						...
						<div class="form-group">				
						 <label for="image">Menu Image &nbsp;&nbsp;</label>
							<div class="btn btn-default btn-file">
							 ...
							 <input type="file" class="form-control" placeholder="Menu Image" id="image" name="image" value="">	
               ...
               <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <?php 
					...
					while($resUserActions = $db->fetch_object2($sqlUserActions)){
						$chkSql = "SELECT * FROM `".TBL_HOTELS."` WHERE FIND_IN_SET('".$resUserActions->id."',ids_mst_hotel_general_services ) and id='".addslashes(encryptor(decrypt,$_REQUEST['eId']))."' ";
                ...
                <div class="form-group">
                  ...
                  <?php 
					...
					$iCounterActions = 0;
					while($resUserActions = $db->fetch_object2($sqlUserActions)){
						$chkSql = "SELECT * FROM `".TBL_HOTELS."` WHERE FIND_IN_SET('".$resUserActions->id."',ids_mst_hotel_outdoor_services ) and id='".addslashes(encryptor(decrypt,$_REQUEST['eId']))."' ";
						if($db->num_rows2(executeSql($chkSql)) > 0){
							$selected = 'selected="selected"';
						}else if($_POST[$selected]){
						$selected = 'selected="selected"';
				...
				<div class="form-group">
                  ...
                  <?php 
					...
					$iCounterActions = 0;
					while($resUserActions = $db->fetch_object2($sqlUserActions)){
						$chkSql = "SELECT * FROM `".TBL_HOTELS."` WHERE FIND_IN_SET('".$resUserActions->id."',ids_mst_hotel_dining_services ) and id='".addslashes(encryptor(decrypt,$_REQUEST['eId']))."' ";
						if($db->num_rows2(executeSql($chkSql)) > 0){
							$selected = 'selected="selected"';
						}else if($_POST[$selected]){
						$selected = 'selected="selected"';
                ...
                <div class="form-group">
                  ...
                  <?php 
					...
					$iCounterActions = 0;
					while($resUserActions = $db->fetch_object2($sqlUserActions)){
						$chkSql = "SELECT * FROM `".TBL_HOTELS."` WHERE FIND_IN_SET('".$resUserActions->id."',ids_mst_hotel_conference_services ) and id='".addslashes(encryptor(decrypt,$_REQUEST['eId']))."' ";
						if($db->num_rows2(executeSql($chkSql)) > 0){
							$selected = 'selected="selected"';
						}else if($_POST[$selected]){
						$selected = 'selected="selected"';
                ...
                <div class="form-group">
                  ...
                  <?php 
					...
					$iCounterActions = 0;
					while($resUserActions = $db->fetch_object2($sqlUserActions)){
						$chkSql = "SELECT * FROM `".TBL_HOTELS."` WHERE FIND_IN_SET('".$resUserActions->id."',ids_mst_hotel_kids_related_services ) and id='".addslashes(encryptor(decrypt,$_REQUEST['eId']))."' ";
						if($db->num_rows2(executeSql($chkSql)) > 0){
							$selected = 'selected="selected"';
						}else if($_POST[$selected]){
						$selected = 'selected="selected"';
				...
				<!--Unique experience-->
                <div class="form-group">
                  <label for="userlevelId">Unique Experiences</label>
				  <select class="form-control select2" name="unique_experiences[]" multiple="multiple">				  
                  <?php 
					$sqlUserActions = selectSql(TBL_UNIQUE_EXPERIENCES," where id_shop='".$_SESSION['shop']."' ",'');
					$iCounterActions = 0;
					while($resUserActions = $db->fetch_object2($sqlUserActions)){
						$chkSql = "SELECT * FROM `".TBL_HOTELS."` WHERE FIND_IN_SET('".$resUserActions->id."',ids_unique_experiences ) and id='".addslashes(encryptor(decrypt,$_REQUEST['eId']))."' ";
						if($db->num_rows2(executeSql($chkSql)) > 0){
							$selected = 'selected="selected"';
						}else if($_POST[$selected]){
						$selected = 'selected="selected"';
				...
				<div class="form-group">
                  ...
                  <input type="text" class="form-control" placeholder="Enter Google Map url" id="gmap_url" name="gmap_url" value="<?php if($_POST) echo $_POST['gmap_url'];else echo stripslashes($row->google_gmap_url);?>">
				...
				<div class="form-group">
                  ...
                  <input type="text" class="form-control" placeholder="Enter page url" id="page_url" name="page_url" value="<?php if($_POST) echo $_POST['page_url'];else echo stripslashes($row->page_url);?>" data-parsley-required>
                ...
                <!--<div class="form-group">
                 ...
                 <input class="flat-red" type="radio"  <?php if($_POST['excel_display_weekday'] == '1'){echo "checked";}else{if($row->excel_display_weekday == 1)echo "checked";}?> value="1" name="excel_display_weekday"/> Weekday & Weekend 
