...
?>
    ...
    <div class="page">
        <div class="page-content">
			...
			if ($row_Guest) {
				...
				$guest_gst_no = ($row_Guest && isset($row_Guest->guest_gst_no)) ? $row_Guest->guest_gst_no : 'N/A';
				...
				$aadhar = ($row_Guest && isset($row_Guest->adhar_no)) ? $row_Guest->adhar_no : 'N/A';
				
				$passport_no = ($row_Guest && isset($row_Guest->passport_no)) ? $row_Guest->passport_no : 'N/A';
				$passport_raw = ($row_Guest && !empty($row_Guest->passport_expiry_date)) ? $row_Guest->passport_expiry_date : '';
				$passportExp = (!empty($passport_raw) && $passport_raw != '0000-00-00') ? date("d/m/Y", strtotime($passport_raw)) : 'N/A';
				...
				$visaExp = (!empty($visa_raw) && $visa_raw != '0000-00-00') ? date("d/m/Y", strtotime($visa_raw)) : 'N/A';
			...
			$nationality = ($row_Country && isset($row_Country->nationality)) ? $row_Country->nationality : 'N/A';
			$country_name = ($row_Country && isset($row_Country->name)) ? $row_Country->name : 'N/A';
			
			//==============fo_reservations_details===============
			
			$SQL_other_details = "SELECT * FROM fo_reservations_details WHERE id_mst_guest = ? ORDER BY id DESC LIMIT 1";
			$otherDetail = $connNew->prepare($SQL_other_details);
			$otherDetail->bind_param("i", $id_guest);
			$otherDetail->execute();
			$result = $otherDetail->get_result();
			$latestdetail = $result->fetch_assoc();
			
			$res_id = ($latestdetail && !empty($latestdetail['id_fo_reservations'])) ? $latestdetail['id_fo_reservations'] : 0;
			
			$c_form_no = ($latestdetail && !empty($latestdetail['c_form_no'])) ? $latestdetail['c_form_no'] : 'N/A';
			
			$arrivalDate_raw = ($latestdetail && !empty($latestdetail['arrival_in_india'])) ? $latestdetail['arrival_in_india'] : '';
			$arrival_date = (!empty($arrivalDate_raw) && $arrivalDate_raw != '0000-00-00') ? date("d/m/Y", strtotime($arrivalDate_raw)) : 'N/A';
			
			$purpose_of_visit = ($latestdetail && !empty($latestdetail['purpose_of_visit'])) ? $latestdetail['purpose_of_visit'] : 'N/A';
			
			$arrival_from = ($latestdetail && !empty($latestdetail['arrival_from'])) ? $latestdetail['arrival_from'] : 'N/A';
			
			$departure_to = ($latestdetail && !empty($latestdetail['departure_to'])) ? $latestdetail['departure_to'] : 'N/A';
			...
			$payment_instruction = ($reservationRow && !empty($reservationRow['res_payment_instruction'])) ? $reservationRow['res_payment_instruction'] : 'N/A';
			
			$special_instruction = ($reservationRow && !empty($reservationRow['res_special_notes'])) ? $reservationRow['res_special_notes'] : 'N/A';
			...
			$room_type = ($roomRow && !empty($roomRow['name'])) ? $roomRow['name'] : 'N/A';
			...
			$room_no = ($roomNoRow && !empty($roomNoRow['room_no'])) ? $roomNoRow['room_no'] : 'N/A';
			...
			$rate_plan = ($rateplanRow && !empty($rateplanRow['name'])) ? $rateplanRow['name'] : 'N/A';
			...
			$company_name = ($companyRow && !empty($companyRow['name'])) ? $companyRow['name'] : 'N/A';
			$company_city = ($companyRow && !empty($companyRow['city'])) ? $companyRow['city'] : '';
			
			//===============fo_receipt================
			
			// Query to get the FIRST res_detail record
			$SQL_first_detail = "SELECT id_fo_bill, id_fo_folio FROM fo_reservations_details WHERE id_mst_guest = ? AND id_fo_bill > 0 ORDER BY id ASC LIMIT 1";
			$stmt_first = $connNew->prepare($SQL_first_detail);
			$stmt_first->bind_param("i", $id_guest); 
			$stmt_first->execute();
			...
			$id_fo_folio = (isset($firstDetail['id_fo_folio'])) ? $firstDetail['id_fo_folio'] : '';
			
			$SQL_receipt = "SELECT * FROM fo_receipt WHERE id_fo_bill = ? AND id_fo_folio = ? ORDER BY id DESC LIMIT 1";
			...
			$payment_mode = ($receiptRow && !empty($receiptRow['payment_mode'])) ? $receiptRow['payment_mode'] : 'N/A';
			
			$id_pay_company = ($receiptRow && !empty($receiptRow['id_company'])) ? $receiptRow['id_company'] : 0;
			
			if (!empty($id_pay_company) && $id_pay_company > 0) {
    			$billed_company_name = selectColumn('mst_company', 'name', "WHERE id = '$id_pay_company'") ?? 'N/A';
			} else {
    			$billed_company_name = 'N/A';
			}
    ...
    <tbody>
        <tr>
            <td class="value" style="border-left: none;"><?=$room_type;?></td>
            <td class="value"><?=$room_no;?></td>
            <td class="value"><?=$adults_per_room;?></td>
            <td class="value"><?=$rate_plan;?></td>
...
<table style="border-top: none;">
    <thead>
        <tr>
            ...
            <!--<th style="border-top: none;">Travel Agent</th>
    ...
    <tbody>
        <tr>
            <td class="value" style=" border-left: none; border-bottom: none;"><?=$company_name.' '.$company_city;?></td>
    ...
    <div class="field inline"><label>Company Name:</label><div class="value"><?=$billed_company_name;?></div></div>
    <!--<div class="field inline"><label>Designation:</label><div class="value">Regional Manager</div></div>-->
    <div class="field inline"><label>Purpose of Visit:</label><div class="value"><?=$purpose_of_visit;?></div></div>
    <div class="field inline"><label>Arriving From:</label><div class="value"><?=$arrival_from;?></div></div>
    <div class="field inline"><label>Proceeding To:</label><div class="value"><?=$departure_to;?></div></div>
    <div class="field inline"><label>Billing Instruct:</label><div class="value"><?=$payment_instruction;?></div></div>
    <div class="field inline"><label>Mode of Payment:</label><div class="value"><?=$payment_mode;?></div></div>
    <div class="field inline" style="margin-top: 12px;"><label>Guest Status:</label><div class="value">___</div></div>
    <div class="field inline"><label>Guest GST No:</label><div class="value"><?=$guest_gst_no?></div></div>
</div>
                </div>
    ...
    <div class="field"><label>Date of Birth</label><div class="value"><?=$birth_day;?></div></div>
				  <?php if (!empty($nationality) && $nationality !== 'N/A'): ?>
    <div class="field"><label>Nationality</label><div class="value"><?= $nationality; ?></div></div>
<?php endif; ?>
		
				  <?php if (!empty($aadhar) && $aadhar !== 'N/A'): ?>
    <div class="field"><label>Aadhar No.</label><div class="value"><?=$aadhar;?></div></div>
				 <?php endif; ?>
				  <?php if (!empty($passport_no) && $passport_no !== 'N/A'): ?>
    <div class="field"><label>Passport No.</label><div class="value"><?=$passport_no;?></div></div>
	<div class="field"><label>Date of  Passport Expiry</label><div class="value"><?=$passportExp;?></div></div>
				 <?php endif; ?> 
    ...
    <div class="field"><label>Arrival in India</label><div class="value"><?=$arrival_date;?></div></div>
