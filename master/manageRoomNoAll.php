...
/////////////////////////////////////////////////////////////////////////////////////
//if($_REQUEST['eId'] == ''){ header("location:manageRoomNoAll.php?submenu=291&session=0"); }
/////////////////////////////////////////////////////////////////////////////////////
...
// ----------cate---------
$sql = executeSql(" SELECT * FROM `".TBL_ROOMNO."` WHERE status='1'  order by id_mst_room_types");
...
<script type="text/javascript">
	function updateOrder(id,value){
		window.location.href='manageRoomNoAll.php?display_order=set&eId=<?php echo $_REQUEST['eId'];?>&table=<?php echo TBL_ROOMNO;?>&id='+id+'&value='+value;
			}
...
<div class="content-wrapper">
        ...
        <div class="col-xs-12">	
		 <div class="nav-tabs-custom">
			<ul class="nav nav-tabs">
			   <li class="active"><a href="manageRoomNoAll.php?eId=<?php echo $_GET['eId']; ?>&action=edit&page=<?php echo $_GET['page']; ?>" data-toggle="tab">Room No</a></li>
            ...
            <div class="box-body table-responsive">
              <table id="example2" class="table table-bordered table-striped">
				...
				if($db->num_rows2($sql) > 0){$counter = 1;
				  ...
				  $_GET['eId']=encryptor(encrypt,$row->id_mst_hotels);
    ...
    ?></td>
				...
				<td><?=$row->status=='1'?'<span onclick="location.href=\'manageAssignHotelRoom.php?inactiveId='.encryptor(encrypt,$row->id).'&eId='.$_GET['eId'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageAssignHotelRoom.php?activeId='.encryptor(encrypt,$row->id).'&eId='.$_GET['eId'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>
				  ...
				  <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editmanageRoomNoAll.php?eId=<?=$_GET['eId']?>&id=<?=encryptor(encrypt,$row->id)?>&id_mst_room_types=<?php echo encryptor(encrypt,$row->id_mst_room_types);?>&action=edit&page=<?=$_REQUEST['page']?>';" />
					  
					  &nbsp;&nbsp;&nbsp;&nbsp;<?php /*?><img src="../images/delete.gif" style="cursor:pointer;" title="Delete" onClick="if(confirm('Are you sure that you want to delete this record <?=$row->name;?>?')){window.location.href='manageRoomNoAll.php?delId=<?=encryptor(encrypt,$row->id)?>&eId=<?=$_GET['eId']?>&id=<?=encryptor(encrypt,$row->id)?>&id_mst_room_types=<?php echo $_GET['id_mst_room_types']?>&action=delete&page=<?=$_REQUEST['page']?>';}"/><?php */?>
