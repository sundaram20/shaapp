...
if($_REQUEST['search_name'] != ''){
	$sql .= " AND `name` LIKE '%".$_REQUEST['search_name']."%'";
}


if($_REQUEST['id_area'] != ''){
	$sql .= " AND `id_area` = '".addslashes($_REQUEST['id_area'])."%'";
}
if($_REQUEST['status'] != ''){
	$sql .= " AND `status` = '".addslashes($_REQUEST['status'])."%'";
}
if($_REQUEST['id_default_group'] != ''){
	$sql .= " AND `id_company_group` = '".addslashes($_REQUEST['id_default_group'])."'";
}
if($_REQUEST['id_email'] != ''){
	$sql .= " AND `email` LIKE '%".addslashes($_REQUEST['id_email'])."%' ";
}
if($_REQUEST['id_phone'] != ''){
	$sql .= " AND `primary_mobile` LIKE '%".addslashes($_REQUEST['id_phone'])."%' ";
	$sql .= " AND `primary_landline` LIKE '%".addslashes($_REQUEST['id_phone'])."%' ";
}
        ...
        <div class="box-header with-border">
		  ...
		  <div class="btn-group  pull-right">
                  ...
                  <ul class="dropdown-menu" role="menu">
                    ...
                    <li><a title="Export to excel file" href="exportTable.php?fileType=xls&tableName=<?php echo TBL_COMPANY;?>"><img src="../images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>
                    <li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo TBL_COMPANY;?>"><img  src="../images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li>
              ...
              <div class="form-group">
                <label>Company Name - City</label>				
				 <?php $categoryDropDown = '<select class="form-control select2" name="search_name" id="search_name" style="width: 100%">
											  ...
											  $resCat = selectSql(TBL_COMPANY," where  id_shop='".addslashes($_SESSION['shop'])."' and name !=' ' AND FIND_IN_SET(id_area,'".$_SESSION['teamMyAreas']."')  ",' ORDER BY `name` ');
			...
			<div class="col-md-6">
              <div class="form-group">
                <label>Company Group</label>				
				 <?php $categoryDropDown = '<select class="form-control select2" name="id_default_group" style="width: 100%">
											    <option value="">Select Company Group</option>';
											  $resCat = selectSql(TBL_GROUP," where status='1' and id_shop='".addslashes($_SESSION['shop'])."'  ",' ORDER BY `id`');
											  if($db->num_rows2($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
		            ...
		            <div class="col-md-3">
		                <div class="form-group">
		                  <label>Area </label>				
		  				 <?php $categoryDropDown = '<select class="form-control select2" name="id_area" style="width: 100%">
		            ...
		            <!--Search by Company Email-->
		     		  <div class="col-md-3">
		                   <div class="form-group">
		     				...
		     				<?php 
		     					$categoryDropDown = '<select class="form-control select2" name="id_email" style="width: 100%">
		                   ...
		                   <!--Search by Company Mobile-->
		            		  <div class="col-md-3">
		                          <div class="form-group">
		            				...
		            				<?php 
		     					$categoryDropDown = '<select class="form-control select2" name="id_phone" style="width: 100%">
		  								  ...
		  								  $resCat = selectSql(TBL_COMPANY," where status='1' and id_shop='".addslashes($_SESSION['shop'])."'",' ORDER BY `name`');
		  									  if($db->num_rows2($resCat)){
		  									  	while($resultCat = $db->fetch_object2($resCat)){
		  											...
		  											if($resultCat->primary_contact == 1){
            ...
            <div class="box-body table-responsive">
              <table id="example2" class="table table-bordered table-striped">
                ...
                <tr>
				  ...
				  <td><?php echo selectColumn(TBL_AREAS,'name'," WHERE `id` = '".$row->id_area."'");  ?></td>
				  <td><?php echo selectColumn(TBL_AREAS,'description'," WHERE `id` = '".$row->id_area."'");  ?></td>
				  <?php if($_SESSION['userLevel']==1){ ?>
