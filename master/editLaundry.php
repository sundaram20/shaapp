...
if($_REQUEST['eId']=='') 
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_INV_ITEMS,'edit');
...
if($_POST['Save']){
	...
	$sql1 = executeSql("SELECT * FROM `".TBL_INV_ITEMS."` where id='".$lastInsertId_items."'");
	while($row = $db->fetch_object2($sql1)){
		$idd = $row -> id;
		$name = $row -> name;
		$save = $name ." Details Added";
	}

				$auditaddSql = "INSERT INTO audit_trail SET
							`voucher_id` = '".$lastInsertId_items."',
							`tables_name` = 'inv_items , inv_items_details',
							`form_code` = 'Laundry Item',
							`changes` = '".$save."',
							`date_created` = '".currenDateTime()."',
							`last_modified` = '".currenDateTime()."',
							`id_mst_user_modified_by` = '".$_SESSION['userId']."',
		...
		//if($_POST['open_qty'] > 0){
				//Shop Table
				$sql12 = " SELECT books_start_from FROM `".TBL_SHOP."` WHERE `id`='".addslashes($_SESSION['shop'])."' ";
					$db->query($sql12);   
					while($row12 = $db->fetch_object()){ 
						$books_start_from = $row12->books_start_from;  
					} 
					
					//Purch Table
				    $addSql22 = " INSERT INTO `".TBL_INV_PURCH."` SET

						`doc_type` = '".addslashes($doc_type)."', 					 
						`po_date` = '".date('Y-m-d' , strtotime($books_start_from))."', 		       
						`id_shop` = '".addslashes($_SESSION['shop'])."'";
						$addSql22 .= ",`date_created` = '".currenDateTime()."',
						`last_modified` = '".currenDateTime()."',
						`id_mst_user_modified_by` = '".$_SESSION['userId']."',
						`id_mst_user_created_by` = '".$_SESSION['userId']."',
						`status` = '".addslashes($_POST['status'])."'";
					...
					//Purch Details Table
						...
						$addSql = " INSERT INTO `".TBL_INV_PURCH_DETAILS."` SET
							...
							`id_inv_items` = '".addslashes($lastInsertId_items)."', 
							`id_inv_items_details` = '".addslashes($lastInsertId_purch1)."', 
			...
			if(1){
				...
				header("location:manageLaundry.php?eId=".encryptor(encrypt,$lastInsertId_items)."&submenu=".$_GET['submenu']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
...
$auditquery = "SELECT * From `".TBL_INV_ITEMS."` WHERE id = '".addslashes(encryptor(decrypt,$_POST[eId]))."' ";
   $auditresSQL = mysqli_query($connNew, $auditquery);	
	while($auditrow = mysqli_fetch_object($auditresSQL)){ 
	
	  $c1 = $auditrow -> id_mst_attributes_item_type;
	  $c2 = $auditrow -> item_code;
	  $c3 =$auditrow -> name;
	  $c4= $auditrow -> id_mst_attributes_unit_main;
	  $c5= $auditrow -> id_mst_attributes_unit_alt;
	  $c6= $auditrow -> item_class;
	  $c7= $auditrow -> id_mst_attributes_group_main;
	  $c8= $auditrow -> id_mst_attributes_group_sub;
	  $c9= $auditrow -> id_mst_attributes_store;
	  $c10 = $auditrow -> id_mst_attributes_printer;
	  $c11 = $auditrow -> id_mst_charges_sales_local;
	  $c12 = $auditrow -> id_mst_charges_sales_interstate;
	  $c13 = $auditrow -> id_mst_charges_purchase_local;
	  $c14 = $auditrow -> id_mst_charges_purchase_interstate;
	  $c15= $auditrow -> sale_rate;
	  $c16= $auditrow -> purchase_rate;
	  $c17= $auditrow -> min_qty;
	  $c18= $auditrow -> max_qty;
	  $c19= $auditrow -> rol;
	  $c20= $auditrow -> roq;
	  $c21= $auditrow -> open_qty;
	  $c22= $auditrow -> open_amount;
	  
	  $c23= $auditrow -> item_enable_desc_billing;
	  $c24= $auditrow -> batch_details;
	  $c25= $auditrow -> item_production_item; 
	  $c26= $auditrow -> item_allow_additional;
	  $c27= $auditrow -> stockable_enable_disable;
	  $c28= $auditrow -> status;
	  $c29= $auditrow -> deactivate_date;
	  $c30= $auditrow -> display_order;
 
	
    if($c1 != $_POST['id_mst_attributes_item_type']){
		$old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c1."'");
		 $new_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$_POST['id_mst_attributes_item_type']."'  ");
		 $ch1 = "Item Type Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
	}
	
	if($c2 != $_POST['item_code']){
		 $ch2 ="Item_code Details Changed from " .   $c2 ." - to - " . $_POST['item_code']." in Item Name ".$c3;
	}
	if($c3 != $_POST['name']){
		$ch3 ="Item Name Details Changed from " .   $c3 ." - to - " . $_POST['name']." in Item Name ".$c3;
	}
	 
	if($c4 != $_POST['id_mst_attributes_unit_main']){
		$old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c4."'");
		 $new_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$_POST['id_mst_attributes_unit_main']."'  ");
		$ch4 ="Item Main Unit  Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
	}
	if($c5 != $_POST['id_mst_attributes_unit_alt']){
		$old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c5."'");
		$new_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$_POST['id_mst_attributes_unit_alt']."'  ");
		$ch5 ="Item Alt Unit Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
	} 
	if($c6 != $_POST['item_class']){
		 $ch6 ="Item Class Details Changed from " .   $c6 ." - to - " . $_POST['item_class']." in Item Name ".$c3;
	}
	
	if($c7 != $_POST['id_mst_attributes_group_main']){
		$old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c7."'");
		 $new_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$_POST['id_mst_attributes_group_main']."'  ");
		$ch7 ="Main Group Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
	}
	if($c8 != $_POST['id_mst_attributes_group_sub']){
		 $old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c8."'");
		 $new_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$_POST['id_mst_attributes_group_sub']."'  ");
		 $ch8 = "Sub Group Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
	}
	
	if($_POST['id_mst_attributes_store'] ==''){
		$sto = '0';
	}else{
		$sto = $_POST['id_mst_attributes_store'];
	}
	
	if($c9 != $sto){
		 $old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c9."'");
		 $new_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$_POST['id_mst_attributes_store']."'  ");
		 $ch9 = "Store Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
	}
	if($_POST['id_mst_attributes_printer'] ==''){
		$emp = '0';
	}else{
		$emp = $_POST['id_mst_attributes_printer'];
	}
	
	if($c10 != $emp){
		 $old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c10."'");
		 $new_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$_POST['id_mst_attributes_printer']."'  ");
		 $ch10 = "Printer Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
	}
	if($_POST['id_mst_charges_sales_local'] ==''){
		$sal = '0';
	}else{
		$sal = $_POST['id_mst_charges_sales_local'];
	}
	if($c11 != $sal){
		 $old_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$c11."'");
		 $new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_sales_local']."'  ");
		 $ch11 = "Sales Account Local Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
	}
		
	if($_POST['id_mst_charges_sales_interstate'] ==''){
		$sai = '0';
	}else{
		$sai = $_POST['id_mst_charges_sales_interstate'];
	}
		
	if($c12 != $sai){
		 $old_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$c12."'");
		 $new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_sales_interstate']."'  ");
		 $ch12 = "Sales Account Interstate Details Changed from " .  $old_data ." - to - " . $new_data." in Item Name ".$c3;
		 if($c12 == '0'){
		 $ch12 = "";
	   }
	}	
	...
	if($c15 != $sr){
		$ch15 ="Standard Sale Rate Details Changed from " .   $c15 ." - to - " . $_POST['sale_rate']." in Item Name ".$c3;
	}
	if($_POST['min_qty'] ==''){
	$min = '0';
}else{
...
if($_POST['max_qty'] ==''){
	...
	if($c20 != $roq){
		$ch20 ="Re Order Qty Details Changed from " .   $c20 ." - to - " . $_POST['roq']." in Item Name ".$c3;
	}
if($_POST['open_qty'] ==''){
	$oq = '0';
}else{
	...
	if($c21 != $oq){
		$ch21 ="Opening Qty Details Changed from " .   $c21 ." - to - " . $_POST['open_qty']." in Item Name ".$c3;
	}
if($_POST['open_amount'] ==''){
	$oa = '0';
}else{
	$oa = $_POST['open_amount'];
}	
	if($c22 != $oa){
		$ch22 ="Opening Value Details Changed from " .   $c22 ." - to - " . $_POST['open_amount']." in Item Name ".$c3;
	}
	if($c23 != $_POST['item_enable_desc_billing']){
		if($c23 == 1){$old='Active';}else{$old='Inactive';}
		if( $_POST['item_enable_desc_billing'] == 1){$new='Active';}else{$new='Inactive';}
		$ch23 ="Enable Description Details Changed from " .   $old ." - to - " . $new." in Item Name ".$c3;
	}

if($_POST['batch_details'] ==''){
	$bd = '0';
}else{
	$bd = $_POST['batch_details'];
}	
	
	if($c24 != $bd){
		if($c24 == 1){$old='Active';}else{$old='Inactive';}
		if( $_POST['batch_details'] == 1){$new='Active';}else{$new='Inactive';}
		$ch24 ="Batch Details Changed from " .   $old ." - to - " . $new." in Item Name ".$c3;
	}
	
if($_POST['item_production_item'] ==''){
	$pi = '0';
}else{
	$pi = $_POST['item_production_item'];
}	
	if($c25 != $pi){
		if($c25 == 1){$old='Active';}else{$old='Inactive';}
		if( $_POST['item_production_item'] == 1){$new='Active';}else{$new='Inactive';}
		$ch25 ="Production Item Details Changed from " .   $old ." - to - " . $new." in Item Name ".$c3;
	}
	
	
if($_POST['item_allow_additional'] ==''){
	$ia = '0';
}else{
	$ia = $_POST['item_allow_additional'];
}	
	if($c26 != $ia){
		if($c26){$old='Active';}else{$old='Inactive';}
		if( $_POST['item_allow_additional'] == 1){$new='Active';}else{$new='Inactive';}
		$ch26 ="Allow Additional Details Changed from " .   $old ." - to - " . $new." in Item Name ".$c3;
	}
	
if($_POST['stockable_enable_disable'] ==''){
	$sn = '0';
}else{
	$sn = $_POST['stockable_enable_disable'];
}	
	
	if($c27 != $sn){
		if($c27 == 1){$old='Active';}else{$old='Inactive';}
		if( $_POST['stockable_enable_disable'] == 1){$new='Active';}else{$new='Inactive';}
		$ch27 ="Non Stockable Details Changed from " .   $old ." - to - " . $new." in Item Name ".$c3;
	}
...
if($_POST['status'] ==''){
	...
	if($c28 != $st){
		if($c28 == 1){$old='Active';}else{$old='Inactive';}
		if( $_POST['status'] == 1){$new='Active';}else{$new='Inactive';}
		$ch28 ="Status Details Changed from " .   $old ." - to - " . $new." in Item Name ".$c3;
	}
	

	if($c29 != $_POST['deactivate_date']){
		$ch29 ="Disable Date Details Changed from " .   $c29 ." - to - " . $_POST['deactivate_date']." in Item Name ".$c3;
	}
	
if($_POST['display_order'] ==''){
	$do = '0';
}else{
	$do = $_POST['display_order'];
}	
	
	if($c30 != $do){
		$ch30 ="Display Order Details Changed from " .   $c30 ." - to - " . $_POST['display_order']." in Item Name ".$c3;
	}	
...
if($i < count($unit)){
 ...
 $auditquery1 = "SELECT * From `".TBL_INV_ITEMS_DETAILS."` WHERE id = '".$lastInsertId_purch2."'  ";
$auditresSQL1 = mysqli_query($connNew, $auditquery1);	
while($auditrow1 = mysqli_fetch_object($auditresSQL1)){
	
$item = $auditrow1 -> name;
			
				if($namee[$i] != $_REQUEST['detail_name'][$i]){
					 $ch01 .= $name .'<br>';
				}
				
				if($sub_code[$i] != $_REQUEST['detail_subcode'][$i]){
					 $ch02 .= $item .$qtyy .'<br>' ;
				}
				
				if($rate[$i] != $_REQUEST['detail_rate'][$i]){
					 $ch03 .= $item .$rate1 .'<br>'  ;
				}
				
				if($unit[$i] != $_REQUEST['detail_unit'][$i]){
					 $ch04 .= $item .$unit1  .'<br>' ;
				}
				
				if($enabled[$i] != $_REQUEST['enabled'][$i]){
					 $ch05 .= $item .$enabledd  .'<br>' ;
				}
...
//$ch02='';
					...
					$addSqled = " INSERT INTO `".TBL_INV_PURCH_DETAILS."` SET
							`id_inv_purch` = '".addslashes($lastInsertId_purch)."',
							`doc_type` = '".addslashes($doc_type)."',  
							`id_inv_items` = '".addslashes(addslashes(encryptor(decrypt,$_POST[eId])))."', 
 ...
 $auditeditSql = " INSERT audit_trail SET 
	...
	`tables_name` = 'inv_items , inv_items_details',
	`form_code` = 'Laundry Item',
	`changes` =  '".addslashes($ch1).",".addslashes($ch2).",".addslashes($ch3).",".addslashes($ch4).",".addslashes($ch5).",".addslashes($ch6).",".addslashes($ch7).",".addslashes($ch8).",".addslashes($ch9).",".addslashes($ch10).",".addslashes($ch11).",".addslashes($ch12).",".addslashes($ch15).",".addslashes($ch17).",".addslashes($ch18).",".addslashes($ch19).",".addslashes($ch20).",".addslashes($ch21).",".addslashes($ch22).",".addslashes($ch23).",".addslashes($ch24).",".addslashes($ch25).",".addslashes($ch26).",".addslashes($ch27).",".addslashes($ch28).",".addslashes($ch29).",".addslashes($ch30).",".addslashes($ch01).",".addslashes($ch02).",".addslashes($ch03).",".addslashes($ch04).",".addslashes($ch05)." ',
	`date_created` = '".currenDateTime()."',
	`last_modified` = '".currenDateTime()."',
	`id_mst_user_modified_by` = '".$_SESSION['userId']."',
	`id_mst_user_created_by` = '".$_SESSION['userId']."',
 ...
 executeSql($auditeditSql);						  
			...
			if(1){
				...
				header("location:manageLaundry.php?eId=".$_GET['eId']."&submenu=".$_GET['submenu']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
...
<!--   <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="manageItems.php"><i class="fa fa-dashboard"></i> Home</a></li>
        ...
        <div class="col-md-12">
          <!-- general form elements -->
			...
			<div class="box-header with-border">
              <h3 class="box-title"><?php echo $_REQUEST['eId']==''?'Add':'Edit'?> <?php echo currentNavigation_id($session)['submenu']; ?> : <span style="color:#3c8dbc"> <?php echo $row->name ?> </span>
              ...
              <div class="box-body" style="padding-top : 0px;">
              	...
              	<div class="card text-dark bg-light">
	              	...
	              	<div class="row">
		                  ...
		                  <label for="name">Service Type <font color="#FF0000">*</font></label>
		                  <div class="input-group"> 
	              			<div class="input-group-addon">
								<i class="fa fa-book"></i> 
						   	</div>
		                 <?php $categoryDropDown = '<select class="form-control select2" name="id_mst_attributes_item_type" data-parsley-required style="width:100%"> ';
						 ...
						 $resCat1 = selectSql(TBL_ATTRIBUTES," where  table_name ='items_type' AND status='0' AND id='".$row->id_mst_attributes_item_type."' ");
								 ...
								 $resCat = selectSql(TBL_ATTRIBUTES,"where id_shop='".$_SESSION['shop']."' and status = '1' and table_name ='items_type' and id='177' ",' ORDER BY `field_value`');
								  if($db->num_rows2($resCat)){
								  	while($resultCat = $db->fetch_object2($resCat)){
										if($_REQUEST['id_mst_attributes_item_type'] == $resultCat->id){
											$selected = 'selected="selected"';
										}else if($row->id_mst_attributes_item_type == $resultCat->id){
											$selected = 'selected="selected"';
										}else{
											$selected = '';
										}
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                  <label for="name">Service Code<font color="#FF0000">*</font></label>
		                  <div class="input-group"> 
	              			<div class="input-group-addon">
		            ...
		            <div class="row">
		            	<div class="form-group col-xs-12 col-md-6 col-sm-2">
		                  <label for="name">Service Name<font color="#FF0000">*</font></label>
		                  <div class="input-group"> 
	              			<div class="input-group-addon">
...
<?php
	...
	$rows = mysqli_num_rows(mysqli_query($connNew,"SELECT * FROM ".TBL_PURCH_DETAILS." WHERE id_mst_items='".$Id."' "));
...
<?php }  ?>	
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                  <label for="name"> Main Unit <font color="#FF0000">*</font></label>
		                  <div class="input-group"> 
	              			<div class="input-group-addon">
		            ...
		            <div class="row">
		            	<div class="form-group col-xs-12 col-md-6 col-sm-2">
		                  <label for="name"> Alt Unit <font color="#FF0000">*</font></label>
		                  <div class="input-group"> 
	              			<div class="input-group-addon">
								<i class="fa fa-underline"></i> 
						   	</div>
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
			                  <label for="name">Service Class <font color="#FF0000">*</font></label>
...
<?php }	 ?>						
		                ...
		                <div class="col-md-6 col-sm-2">
			                  ...
			                  <input type="text" class="form-control" placeholder="Enter Conversion Quantity" id="conversion_qty" name="conversion_qty" value="<?php if($_POST) echo $_POST['conversion_qty'];else echo stripslashes($row->conversion_qty);?>" onchange="conversion()"  ></div>
			              </div>
			              <div class="col-md-3 col-sm-2">
...
<?php }	 ?>	
 ...
 <?php  $attributes_outlet = explode(',', $row->ids_mst_outlet);

 $categoryDropDown = '<select multiple="multiple" class="form-control select2" id="ids_mst_outlet" name="ids_mst_outlet[]" data-parsley-required data-parsley-errors-container="#outlet" style="width:100%">

											    <option value="">Select Outlet</option>';
												
								if($row->ids_mst_outlet!=''){		
									$resCat1 = selectSql(mst_outlets," where status='0' AND id IN ($row->ids_mst_outlet) ");
									if($db->num_rows2($resCat1)){
										while($resultCat1 = $db->fetch_object2($resCat1)){
											if($row->ids_mst_outlet == $resultCat1->id){
												$selected = 'selected="selected"';
											}else{
												$selected = '';
											}
											$categoryDropDown .= '<option selected="selected" value="'.$resultCat1->id.'">'.$resultCat1->name.'</option>';
										}
									}		
								}			

											  $resCat = selectSql(mst_outlets," where id_shop='".$_SESSION['shop']."' AND  status = '1' and id IN (4,6) ",'');

											  if($db->num_rows2($resCat)){

											  	while($resultCat = $db->fetch_object2($resCat)){
												if(in_array($resultCat->id,$attributes_outlet)){
																		$selected = 'selected="selected"';
		        ...
		        <div class="card text-dark bg-light">
		          ...
		          <!--  <div class="row">
		               ...
		               <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                 ...
		                 <?php 
						 $categoryDropDown = '<select class="form-control select2" name="id_mst_charges_purchase_interstate" id="id_mst_charges_purchase_interstate" onchange="purchaseaccountinterstate()" style="width:100%">
		       ...
		       <!-- <div class="card text-dark bg-light">
	              	...
	              	<div class="row">
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                  ...
		                  <input type="hidden" class="form-control" placeholder="Enter Standard Sale Rate" id="sale_rate" name="sale_rate" value="0" >
		     ...
		     <!--   <div class="card text-dark bg-light">
		        ...
		        <input type="hidden" class="form-control" placeholder="Enter Opening Qty" id="open_qty" name="open_qty" value="1" >
					<!--	<?php echo $err_item_srno;?></div>
		                </div>
                ...
                <?php 
				        	if($row->item_details == '' || $row->item_details=='2'){
				        		 $item_details = 2;
    			...
    			<div class="card text-dark bg-light">
              		<div class="bg-primary text-center ">
              			<h5 style="padding: 5px;">Sub Service Details</h5>
              		</div> 
              		<hr>
	              	<div class="row">
                      ...
                      <div id="grid<?php echo $i;?>" style="float:left; width:100%;">  
							...
							<div class="form-group col-xs-12 col-md-2 col-sm-2">
			                  ...
			                  <label for="name">Enabled</label>
			                 	...
			                 	<select class="form-control select2" name="enabled[]"  style="width: 100%">
									<option value="1">Yes</option>
									<option value="2">No</option>
								</select>
...
<?php $rows = mysqli_num_rows(mysqli_query($connNew,"SELECT * FROM pos_purch_details WHERE id_mst_items_details='".$rowitemDetail->id."' ")); 
if($rows <= 0){
?>
...
<!--    <a class="btn btn-danger btn-sm" style="margin-top: 29px;" href="javascript:void(0);"  onclick="removeGrid(<?php echo $i++; ?>);"><i class="fa fa-trash-o fa-lg"></i> </a> -->
                    ...
                    <?php }
			               ...
			               <!--   <input type="text" class="form-control" placeholder="Enter Unit" id="detail_unit" name="detail_unit[]" value="<?php if($_POST) echo $_POST['prefix'];else echo stripslashes($rowDocCofig->prefix);?>">  -->
                            ...
                            <a class="btn btn-danger btn-sm" style="margin-top: 29px;margin-left:-23px" href="javascript:void(0);"  onclick="removeGrid(<?php echo $i++; ?>);"><i class="fa fa-trash-o fa-lg"></i> </a>
		        ...
		        <div class="card text-dark bg-light">
					...
					<!--	<div class="form-group col-md-4 col-sm-3">	                
                        ...
                        <div class="form-group col-md-4 col-sm-3" id="containers">
                        ...
                        <label for="item_details">Sub Service :</label>
                        
                        <input type="radio" name="ItemDetails" id="ItemDetails" onClick="SelectItemDetails(2);"  <?php if($_POST['ItemDetails'] == '2'){echo "checked";}else{if($item_details == 2)echo "checked";}?> value="2"/>Active
 ...
 <div class="red box">You have selected <strong>red radio button</strong> so i am here</div>
		            ...
		            <div class="row" style="display: none;">
		           	...
		           	<div class="row">
		           		...
		           		<div class="form-group col-md-4 col-sm-3" >
		                	<label for="deactivate_date">Disable Date</label>
		                	<div class="input-group"> 
		              			<div class="input-group-addon">
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp; 
			   <input type='button' value='Close' class="btn btn-danger" onclick='location.replace("manageLaundry.php?submenu=<?php echo $_GET['submenu'] ?>"); '>
...
<?php if($row->item_details == '' || $row->item_details=='2' || $row->item_details=='0'){ ?>
$('document').ready(function(){
		$('#ItemDetails').click();
});
<?php } ?>
