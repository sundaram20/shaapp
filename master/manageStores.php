...
// ----------cate---------
$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' and `table_name` = 'store'  ";
...
<div class="content-wrapper">
	 ...
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border"> 
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn btn-success" href="editStores.php" >Add Store</a>
          ...
          <div class="box">
            ...
            <div class="box-body table-responsive">
		        ...
		        <tbody>  
                  ...
                  <td><?php echo $row->status=='1'?'<span onclick="location.href=\'manageStores.php?inactiveId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageStores.php?activeId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>		

                  <?php 
				  		$table_name = array(TBL_INV_ITEMS);
				  		$ajaxCheckTransactions = CheckTransactionsStore($row->id, $table_name); 

						if($ajaxCheckTransactions != '1'){
                 	?>
				  <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editStores.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='manageStores.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	   } 
	   else
	   {
	       self.location='manageStores.php';
	    }
	   });
	  }
  </script> 
