<?php 
  ...
  checkUserLevelPermission($_SESSION['userLevel'],TBL_PORTFOLIO_ACCOUNT,'view');

  if($_REQUEST['action'] == 'change'){
    if($_REQUEST['activeId'] != ''){
      checkUserLevelPermission($_SESSION['userLevel'],TBL_PORTFOLIO_ACCOUNT,'activate');
      //$statusId = addslashes($_REQUEST['activeId']);
      $statusId = addslashes(encryptor(decrypt,$_REQUEST['activeId']));
      $statusSql = "  UPDATE `".TBL_PORTFOLIO_ACCOUNT."` 
            SET `status` = '1' 
            ,`last_modified` = '".currenDateTime()."'
            ,`id_mst_user_modified_by` = '".$_SESSION['userId']."'
            WHERE `id` = '".$statusId."'";
    }elseif($_REQUEST['inactiveId'] != ''){
      checkUserLevelPermission($_SESSION['userLevel'],TBL_PORTFOLIO_ACCOUNT,'deactivate');
    ...
    if(executeSql($statusSql)){
      $err = 0;
      $_SESSION['successMsg'] = 'Portfolio '.selectColumn(TBL_PORTFOLIO_ACCOUNT,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';
    }else{
      $err = 1;
      $_SESSION['errorMsg'] = 'Portfolio '.selectColumn(TBL_PORTFOLIO_ACCOUNT,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
    }

  }else if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){

    checkUserLevelPermission($_SESSION['userLevel'],TBL_PORTFOLIO_ACCOUNT,'delete');
    $deleteIds = encryptor(decrypt,$_REQUEST['delId']);
    $delSql = "DELETE FROM `".TBL_PORTFOLIO_ACCOUNT."` WHERE `id` = '".addslashes($deleteIds)."'";
    $sqlDelUsers = selectRow(TBL_PORTFOLIO_ACCOUNT," WHERE `id` = '".addslashes($deleteIds)."'");
    if(executeSql($delSql)){
      $err = 0;
      
      $_SESSION['successMsg'] = 'One Portfolio '.$sqlDelUsers["name"].' has been deleted sucessfully.';
      }else{
        $err = 1;
        $_SESSION['errorMsg'] = 'Unable to delete Portfolio '.$sqlDelUsers["name"];
    }
  }
  ...
  if($_REQUEST['searchFormSubmit']==''){
      $sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_PORTFOLIO_ACCOUNT."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
      ...
      $sqlResultOrder = mysqli_query($appConnect, "SELECT field_name FROM ".TBL_REPORT." WHERE table_name = '".TBL_PORTFOLIO_ACCOUNT."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc LIMIT 0,1" );
    ...
    $sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_PORTFOLIO_ACCOUNT."' AND id_shop = ".$_SESSION['shop'] ." AND field_name IN ('".$fieldname."')  ORDER BY display_order");
  ...
  $sql = local_SelectQuery_Mst_Portfolio_Account($_REQUEST['tableName'],$field_name,$field_label,$EnableOrderBy);
  //echo "hello"; exit;
  $QuerySQL = mysqli_query($connNew,$sql);
  $numRows = @mysqli_num_rows($QuerySQL);

  $TotalCountRecord = selectColumn(TBL_PORTFOLIO_ACCOUNT,'count(id)',' WHERE id_shop= "'.addslashes($_SESSION['shop']).'" ');
      ...
      <div class="form-group has-error" align="center">
        ...
        <?php unset($_SESSION['successMsg']);}?>
          ...
          <div class="btn-group  pull-right">
            <a type="button" class="btn btn-success" href="editPortfolio.php">Add Portfolio</a>
          ...
          <div class="row">
                    ...
                    <div class="form-group">
                      <label>Area</label>
                            ...
                            <?php $categoryDropDown = '<select class="form-control select2" name="search_name" style="width:100%">
                              ...
                              $resUserLevel = selectSql(TBL_PORTFOLIO_ACCOUNT," WHERE `status` = '1' AND  `id_shop` = '".addslashes($_SESSION['shop'])."' ",' GROUP BY `name`');
                              if($db->num_rows2($resUserLevel)){
                                while($resultUserLevel = $db->fetch_object2($resUserLevel)){
                                  ...
                                  $categoryDropDown .= '<option '.$selected.' value="'.$resultUserLevel->name.'">'.ucfirst($resultUserLevel->name).'</option>';
                                    }
                                }
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <label>Corporate Executive / Primary User</label>       
                        <?php $categoryDropDown = '<select class="form-control select2" name="id_mst_users_primary" style="width:100%">
                          <option value="">Select Corporate Executive</option>';
                    ...
                    <div class="col-md-6">
                      <div class="form-group">
                        <label>Unit Executive / Secondary User</label>       
                          <?php $categoryDropDown = '<select class="form-control select2" name="ids_mst_users_secondary" style="width:100%">
                            <option value="">Select Unit Executive</option>';
                             $resCat = selectSql(TBL_USERS," where status='1' and id_shop='".addslashes($_SESSION['shop'])."'  ",' ORDER BY `id`');
                                if($db->num_rows2($resCat)){
                                  while($resultCat = $db->fetch_object2($resCat)){
                                    if($_REQUEST['user_id'] == $resultCat->id){
                  ...
                  <div  id="fieldset" class="<?php echo $viewClass; ?>">
                    ...
                    <div  id="moreReportDiv" style="padding-left:0px;<?php echo $DispalyClass; ?>">
                        ...
                        <div class="row">
                          ...
                          <div class="col-md-3 col-sm-3">
                            <select class="form-control select2" style="width:100%;" name="EnableOrderBy" id="EnableOrderBy" >
                              ...
                              <?php
                              $sqlResult = mysqli_query($appConnect, "SELECT field_name, field_label FROM ".TBL_REPORT." WHERE table_name = '".TBL_PORTFOLIO_ACCOUNT."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc" );
                      ...
                      is_array($_REQUEST['field_name']) ? '' : $_REQUEST['field_name']=array();
                            $sqlResult = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_PORTFOLIO_ACCOUNT."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
                      ...
                      <?php } }?>   
                      <input type="hidden" name="tableName" value="<?php echo TBL_PORTFOLIO_ACCOUNT;?>" />  
                    </div>
              ...
              while($row = @mysqli_fetch_row($QuerySQL)){
                ...
                //print_r($row) ;
                  ...
                  if($cell=='Active'){            
                    ...
                    $cell='<span onclick="location.href=\'managePortfolio.php?inactiveId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>';
                  }else if($cell=='Inactive'){
                    $cell='<span onclick="location.href=\'managePortfolio.php?activeId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';
                  }
                  if($value == 1){
                    $Id = $cell;
                  }else{
                ...
                $strLocation = 'editPortfolio.php?eId='.encryptor(encrypt,$Id).'&action=edit&page='.$_REQUEST['page'];

                $strLocation1 = 'editPortfolio.php?eId='.encryptor(encrypt,$Id).'&date=adddate&action=edit&page='.$_REQUEST['page'];

                $table_name = array(TBL_COMPANY);
                $ajaxCheckTransactions = CheckTransactionsPortfolio($Id, $table_name); 

                if($ajaxCheckTransactions != '1'){

                  $dataWrite .='<td><img src="../images/add.png" style="cursor:pointer;" title=" add new record " onclick="window.location.href=\''.$strLocation1.'\'" />&nbsp;&nbsp;<img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onclick="window.location.href=\''.$strLocation.'\'" />&nbsp;&nbsp;<img src="../images/delete.gif" style="cursor:pointer;" title="Delete" name="'.$row->name.'" id="'.encryptor(encrypt,$Id).'" onClick="deletes(this.id);"/></td>';
                }else{

                  $dataWrite .='<td><img src="../images/add.png" style="cursor:pointer;" title=" add new record " onclick="window.location.href=\''.$strLocation1.'\'" />&nbsp;&nbsp;<img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onclick="window.location.href=\''.$strLocation.'\'" />&nbsp;&nbsp;<img src="../images/chat.png" style="cursor:pointer; " title="In Use" /></td>';
                }
  ...
  $(document).ready(function(){
    ...
    swal({
     ...
     function(isConfirm)
     {
       ...
       if (isConfirm)
       {  
            self.location='managePortfolio.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
       } 
       else
       {
          self.location='managePortfolio.php';
        }
       });
    }
