...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			if($db->num_rows2(selectSql(TBL_DINING_SERVICES,"WHERE `id` NOT IN('".addslashes(encryptor(decrypt,$_POST[eId]))."') and `id_shop` = '".addslashes($_SESSION['shop'])."'AND `name` = '".addslashes(trim($_POST['name']))."'",''))){
				$_SESSION['errorMsg'] = 'Dining Services name already exists in our database.';
			}else{
          ...
          <div class="box box-primary">
            ...
            <!-- form start -->  			        
              	...
              	<div class="box-body">
                	<div class="row">
                		<div class="form-group col-md-12">
		                  	...
		                  	<input type="text" class="form-control" placeholder="Enter Dining Services Name" id="name" name="name" value="<?php if($_POST) echo $_POST['name'];else echo stripslashes($row->name);?>">
							<?php echo $err_name;?>
		                </div>
		                <div class="form-group col-md-12">
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
			   <input type='button' value='Close' class="btn btn-danger" onclick='location.replace("manageDiningServices.php"); '>
			 </div>
            </form>			
          </div>
          <!-- /.box -->
