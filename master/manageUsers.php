<?php 
	...
	if($_REQUEST['action'] == 'change'){
		...
		$delSql = "DELETE FROM `".TBL_USERS."` WHERE `id` = '".addslashes($deleteIds)."'";
		$sqlDelUsers = selectRow(TBL_USERS," WHERE `id` = '".addslashes($deleteIds)."'");
	...
	if($_REQUEST['searchFormSubmit']==''){
			$sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_USERS."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY ID DESC");
			...
			$sqlResultOrder = mysqli_query($appConnect, "SELECT field_name FROM ".TBL_REPORT." WHERE table_name = '".TBL_USERS."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY ID DESC LIMIT 0,1" );
		...
		$sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_USERS."' AND id_shop = ".$_SESSION['shop'] ." AND field_name IN ('".$fieldname."')  ORDER BY ID DESC");
	...
	$sql = local_SelectQuery_Mst_Users($_REQUEST['tableName'],$field_name,$field_label,$EnableOrderBy);
	//echo "hello"; exit;
	$QuerySQL	=	mysqli_query($connNew,$sql);
	$numRows = @mysqli_num_rows($QuerySQL);

	$TotalCountRecord = selectColumn(TBL_USERS,'count(id)',' WHERE id_shop="'.$_SESSION['shop'].'" ');
			       ...
			       <div class="modal-dialog">
			         ...
			         <div class="modal-content" style="width: 300px; margin: 0px auto;">
			           <div class="modal-header">
			             ...
			             <h4 class="modal-title">Import Users</h4><br>
			           ...
			           <div class="modal-body">
			             <form name="companyimport" method="post" enctype="multipart/form-data" id="companyimport">
			               ...
			               <div >
			               	 <input type="hidden" name="table_name" value="<?php echo TBL_USERS;?>" />
        	...
        	<div class="box-header with-border">
        		<h3 class="box-title">Search <small>Total Records: (<?=$TotalCountRecord;?>) &nbsp;</small> </h3>
		  		<div class="btn-group  pull-right">
		  			<a type="button" class="btn btn-success" href="editUsers.php">Add Users</a>
					<button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">
		  			...
		  			<ul class="dropdown-menu" role="menu">
		            	...
		            	<li><a title="Export to excel file" href="masterExportTable.php?fileType=xls&tableName=<?php echo TBL_USERS;?>"><img src="../images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>
		            	<li><a title="Export to csv file" href="masterExportTable.php?fileType=csv&tableName=<?php echo TBL_USERS;?>"><img  src="../images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li>
					...
					<div class="row">
              			...
              			<div class="form-group col-md-6">
                			...
                			<!--<input type="text" name="search_name" id="search_name" value="<?php echo trim($_REQUEST['search_name']);?>" class="form-control" />-->
               				<?php $categoryDropDown = '<select class="form-control select2" name="name" style="width:100%">
                       		...
                       		<div class="form-group col-md-6">
	                			<label> User Level</label>				
					 			<?php $categoryDropDown = '<select class="form-control select2" name="id_mst_user_levels" style="width:100%">
				        				...
				        				<div class="box-body">
			        						<div class="row">
			        							...
			        							<div class="col-md-3 col-sm-3">
			        								<select class="form-control select2" style="width:100%;" name="EnableOrderBy" id="EnableOrderBy" >
			        									...
			        									<?php
			    											$sqlResult = mysqli_query($appConnect, "SELECT field_name,field_label FROM ".TBL_REPORT." WHERE table_name = '".TBL_USERS."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc" );
			        						...
			        						<div class="row">
												...
												is_array($_REQUEST['field_name']) ? '' : $_REQUEST['field_name']=array();
				    									$sqlResult = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_USERS."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
		    								...
		    								<?php } }?>		
		    								<input type="hidden" name="tableName" value="<?php echo TBL_USERS;?>" />	
			    						</div>
							...
							while($row = @mysqli_fetch_row($QuerySQL)){
								...
								//print_r($row) ;
									...
									if($cell=='Active'){						
										...
										$cell='<span onclick="location.href=\'manageUsers.php?inactiveId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>';
									}else if($cell=='Inactive'){
										$cell='<span onclick="location.href=\'manageUsers.php?activeId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';
									}
									if($value == 1){
										$Id = $cell;
									}else{
								...
								$strLocation = 'editUsers.php?eId='.encryptor(encrypt,$Id).'&action=edit&page='.$_REQUEST['page'];

								//echo $Id.' ';
								$rows = mysqli_num_rows(mysqli_query($connNew,"SELECT * FROM mst_hotels WHERE id_mst_user_created_by='".$Id."' "));
					                   ...
					                   $dataWrite .='<td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onclick="window.location.href=\''.$strLocation.'\'" />&nbsp;&nbsp;&nbsp;&nbsp;';
					                    ...
					                    $dataWrite .='<img src="../images/delete.gif" style="cursor:pointer;" title="Delete" name="'.$row->name.'" id="'.encryptor(encrypt,$Id).'" onClick="deletes(this.id);"/>';
					                 	 $dataWrite .= '</td>';
					                   }
  ...
  <script type="text/javascript">
    ...
    swal({
     ...
     function(isConfirm)
     {
	     ...
	     if (isConfirm)
	     {  
	          self.location='manageUsers.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	     } 
	     else
	     {
	        self.location='manageUsers.php';
	      }
	     });
    }
