...
if($_REQUEST['action'] == 'change'){
	if($_REQUEST['activeId'] != ''){
		...
		$assignHotelId = selectColumn(TBL_ASSIGN_HOTEL_ROOM,'id_mst_hotels'," WHERE `id` = '".$statusId."'");
		$assignRoomId = selectColumn(TBL_ASSIGN_HOTEL_ROOM,'id_mst_room_types'," WHERE `id` = '".$statusId."'");
...
// ----------cate---------
$sql = executeSql(" SELECT * FROM `".TBL_ASSIGN_HOTEL_ROOM."` WHERE `id_mst_hotels` = '".encryptor(decrypt,$_REQUEST['eId'])."' ORDER BY status_active_date DESC ");
...
<script type="text/javascript">
	function updateOrder(id,value){
		window.location.href='manageAssignHotelRoom.php?display_order=set&eId=<?php echo $_REQUEST['eId'];?>&table=<?php echo TBL_ASSIGN_HOTEL_ROOM;?>&id='+id+'&value='+value;
			}
            ...
            <div class="box-body table-responsive">
              <table id="example2" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><?php echo date('d-m-Y',strtotime($row->status_active_date));  ?></td>
				  <td><?php echo selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = '".$row->id_mst_room_types."'");   ?></td>
				  <td><?php echo $row->inventory;   ?></td>
					<td><input style="width:80px;" id="<?php echo $row->id ; ?>"  class="" onchange="updateOrder(this.id,this.value);" value="<?php echo $row->display_order;?>" type="text" name="display_order" /></td>
				  ...
				  <td><img src="../images/add.png" style="cursor:pointer;" title=" add new date " onClick="window.location.href='editAssignHotelRoom.php?eId=<?=$_GET['eId']?>&id=<?=encryptor(encrypt,$row->id)?>&date=adddate&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editAssignHotelRoom.php?eId=<?=$_GET['eId']?>&id=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="../images/view_room.png" style="cursor:pointer;width:24px;height:24px;" title=" ManageRoomNo" onClick="window.location.href='manageRoomNo.php?eId=<?=$_GET['eId']?>&id_mst_room_types=<?=encryptor(encrypt,$row->id_mst_room_types)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="../images/delete.gif" style="cursor:pointer;" title="Delete" onClick="if(confirm('Are you sure that you want to delete this record <?=$row->name;?>?')){window.location.href='manageAssignHotelRoom.php?delId=<?=encryptor(encrypt,$row->id)?>&eId=<?=$_GET['eId']?>&action=delete&page=<?=$_REQUEST['page']?>';}"/></td>
                </tr>
