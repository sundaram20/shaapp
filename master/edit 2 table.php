	 ...
	 if($idd == '0'){
		...
		if($_POST['open_qty'] > 0){
			...
			if(1){
				...
				header("location:manageItems.php?eId=".encryptor(encrypt,$lastInsertId_items)."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
   ...
   $auditresSQL = mysqli_query($connNew, $auditquery);	
    ...
    if($c1 != $_POST['id_mst_attributes_item_type']){
		$old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c1."'");
		 ...
		 $ch1 = "Item Type Changed from " .  $old_data ." - to - " . $new_data;
	}
	
	if($c2 != $_POST['item_code']){
		 $ch2 ="Item_code Details Changed from " .   $c2 ." - to - " . $_POST['item_code'];
	}
	if($c3 != $_POST['name']){
		$ch3 ="Item Name Details Changed from " .   $c3 ." - to - " . $_POST['name'];
	}
	...
	if($c4 != $_POST['id_mst_attributes_unit_main']){
		...
		$ch4 ="Item Main Unit  Details Changed from " .  $old_data ." - to - " . $new_data;
	}
	if($c5 != $_POST['id_mst_attributes_unit_alt']){
		...
		$ch5 ="Item Alt Unit Details Changed from " .  $old_data ." - to - " . $new_data;
	} 
	...
	if($c7 != $_POST['id_mst_attributes_group_main']){
		...
		$ch7 ="Main Group Details Changed from " .  $old_data ." - to - " . $new_data;
	}
	if($c8 != $_POST['id_mst_attributes_group_sub']){
		 ...
		 $ch8 = "Sub Group Details Changed from " .  $old_data ." - to - " . $new_data;
	}
	if($c9 != $_POST['id_mst_attributes_store']){
		 ...
		 $ch9 = "Store Details Changed from " .  $old_data ." - to - " . $new_data;
	}
	if($c10 != $_POST['id_mst_attributes_printer']){
		 ...
		 $ch10 = "Printer Details Changed from " .  $old_data ." - to - " . $new_data;
	}
	if($c11 != $_POST['id_mst_charges_sales_local']){
		 ...
		 $ch11 = "Sales Account Local Details Changed from " .  $old_data ." - to - " . $new_data;
	}
	...
	if($c12 != $_POST['id_mst_charges_sales_interstate']){
		 ...
		 $ch12 = "Sales Account Interstate Details Changed from " .  $old_data ." - to - " . $new_data;
		 if($c12 == '0'){
		 $ch12 = "";
	   }
	}	
	...
	if($c13 != $_POST['id_mst_charges_purchase_local']){
		 ...
		 $ch13 = "Purchase Account Local Details Changed from " .  $old_data ." - to - " . $new_data;
		 if($c13 == '0'){
		 $ch13 = "";
	    }
	}	
	if($c14 != $_POST['id_mst_charges_purchase_interstate']){
		 ...
		 $ch14 = "Purchase Account Interstate Details Changed from " .  $old_data ." - to - " . $new_data;
		 if($c14 == '0'){
		 $ch14 = "";
	    }
	}
	if($c15 != $_POST['sale_rate']){
		$ch15 ="Standard Sale Rate Details Changed from " .   $c15 ." - to - " . $_POST['sale_rate'];
	}
	if($c16 != $_POST['purchase_rate']){
		 $ch16 ="Standard Purchase Rate Details Changed from " .   $c16 ." - to - " . $_POST['purchase_rate'];
	}
	if($c17 != $_POST['min_qty']){
		$ch17 ="Min Stock Level Details Changed from " .   $c17 ." - to - " . $_POST['min_qty'];
	}
	if($c18 != $_POST['max_qty']){
		$ch18 ="Max Stock Level Details Changed from " .   $c18 ." - to - " . $_POST['max_qty'];
	}
	if($c19 != $_POST['rol']){
		$ch19 ="Rol Details Changed from " .   $c19 ." - to - " . $_POST['rol'];
	}
	if($c20 != $_POST['roq']){
		$ch20 ="Re Order Qty Details Changed from " .   $c20 ." - to - " . $_POST['roq'];
	}
	if($c21 != $_POST['open_qty']){
		$ch21 ="Opening Qty Details Changed from " .   $c21 ." - to - " . $_POST['open_qty'];
	}
	if($c22 != $_POST['open_amount']){
		$ch22 ="Opening Value Details Changed from " .   $c22 ." - to - " . $_POST['open_amount'];
	}
	...
	if($c25 != $_POST['item_production_item']){
		...
		$ch25 ="Production Item Details Changed from " .   $old ." - to - " . $new;
	}
	if($c26 != $_POST['item_allow_additional']){
		...
		$ch26 ="Allow Additional Details Changed from " .   $old ." - to - " . $new;
	}
	if($c27 != $_POST['stockable_enable_disable']){
		...
		$ch27 ="Non Stockable Details Changed from " .   $old ." - to - " . $new;
	}
	if($c28 != $_POST['status']){
		if($c28 == 1){$old='Active';}else{$old='Inactive';}
		if( $_POST['status'] == 1){$new='Active';}else{$new='Inactive';}
		$ch28 ="Status Details Changed from " .   $old ." - to - " . $new;
	}
	if($c29 != $_POST['deactivate_date']){
		$ch29 ="Disable Date Details Changed from " .   $c29 ." - to - " . $_POST['deactivate_date'];
	}
	if($c30 != $_POST['display_order']){
		$ch30 ="Display Order Details Changed from " .   $c30 ." - to - " . $_POST['display_order'];
	}	
			...
			if(1){
				...
				header("location:manageItems.php?eId=".$_GET['eId']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
        ...
        <div class="col-md-12">
          <!-- general form elements -->
			...
			<div class="box-header with-border">
              <h3 class="box-title"><?php echo $_REQUEST['eId']==''?'Add':'Edit'?> Items : <a><?php echo selectColumn(TBL_INV_ITEMS,'name'," WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND 'id' = '".addslashes(encryptor(decrypt,$_REQUEST['eId']))."'"); ?></a></h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->  			        
              	...
              	<div class="card text-dark bg-light">
	              	...
	              	<div class="row">
		                 ...
		                 <?php $categoryDropDown = '<select class="form-control select2" name="id_mst_attributes_item_type" data-parsley-required style="width:100%">
								 ...
								 $resCat = selectSql(TBL_ATTRIBUTES,"where id_shop='".$_SESSION['shop']."' and status = '1' and table_name ='items_type' ",' ORDER BY `field_value`');
		        ...
		        <div class="card text-dark bg-light">
	              	...
	              	<div class="row"> 
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                 ...
		                 <?php $categoryDropDown = '<select class="form-control select2" name="id_mst_attributes_group_main" data-parsley-required style="width:100%">
								  ...
								  if($db->num_rows2($resCat)){
								  	while($resultCat = $db->fetch_object2($resCat)){
										if($_REQUEST['id_mst_attributes_group_main'] == $resultCat->id){
											$selected = 'selected="selected"';
										}elseif($row->id_mst_attributes_group_main == $resultCat->id){
											$selected = 'selected="selected"';
										}else{
											$selected = '';
										}
 ...
 $categoryDropDown = '<select multiple="multiple" class="form-control select2" id="ids_mst_outlet" name="ids_mst_outlet[]" data-parsley-required data-parsley-errors-container="#outlet" style="width:100%">
											  ...
											  if($db->num_rows2($resCat)){
												...
												if(in_array($resultCat->id,$attributes_outlet)){
													...
													/*if($row->id_mst_attributes_outlet == $resultCat->id){
...
function removeGrid(id){
    ...
    $('#grid'+id).remove();
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp; 
			   <input type='button' value='Close' class="btn btn-danger" onclick='location.replace("manageItems.php"); '>
