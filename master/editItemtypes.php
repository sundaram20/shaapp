...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `field_value` = '".addslashes(trim($_POST['field_value']))."' and `table_name` = 'items_type' ";
			$db->query($sql);
			$numRows= $db->num_rows();
			if($numRows == '0'){
			...
			$addSql = "   	INSERT INTO `".TBL_ATTRIBUTES."` SET
							...
							`field_name` = 'items_name',
							`field_value` = '".addslashes(trim($_POST['field_value']))."',
							`field_category` = '".addslashes($_POST['field_category'])."',
			...
			if(executeSql($addSql)){
				...
				$_SESSION['successMsg'] = 'New Items Type details has been added sucessfully.';
				header("location:manageItemTypes.php?eId=".encryptor(encrypt,$lastInsertId)."&submenu=".$_GET['submenu']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
				$_SESSION['errorMsg'] = 'Items Type details has not been saved. Please make corrections below.';
			}
			}else{
				$err++;
				$_SESSION['errorMsg'] = 'Items Type Master Name Already Exist.';
			}
		}
		...
		//Update Section Here
			...
			if(executeSql($editSql)){
				...
				header("location:manageItemTypes.php?eId=".$_GET['eId']."&submenu=".$_GET['submenu']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
		...
		$err++;
		$_SESSION['errorMsg'] = 'Items Type details has not been saved. Please make corrections.';
	}
}
// ----------cate---------
        ...
        <div class="col-md-12">
          <!-- general form elements -->
			...
			<ul class="nav nav-tabs">
			   <li class="active" ><a href="#tab_1" data-toggle="tab">Item Types</a></li>  
            </ul>
            ...
            <!-- form start -->  			        
			 <form name="form1"  method="post" enctype="multipart/form-data" data-parsley-validate autocomplete="off" >
              ...
              <div class="box-body">
                  ...
                  <label for="name">Items Type<font color="#FF0000">*</font></label>
                  <div class="input-group"> 
              			<div class="input-group-addon">
							<i class="fa fa-ioxhost"></i> 
					   	</div>
                  <input type="text" class="form-control" placeholder="Enter Items Type" id="field_value" name="field_value" value="<?php if($_POST) echo $_POST['field_value'];else echo stripslashes($row->field_value);?>"  data-parsley-required>
				<?php echo $err_unit_name;?></div>
                </div>
				
				
				<div class="form-group">
			                  <label for="name">Item Category  <font color="#FF0000">*</font></label>
			                 	...
			                 	<?php if($row->id == !''){  ?>
			                 		...
			                 		<?php 
			                 		if($row->field_category == 'Menu Items'){
			                 			$categoryDropDown = '<select class="form-control select2" name="field_category" data-parsley-required style="width: 100%">
										<option value="Menu Items">Menu Items</option>';
			                 		}elseif($row->field_category == "Ingredients Items"){
			                 			$categoryDropDown = '<select class="form-control select2" name="field_category" data-parsley-required style="width: 100%">
										<option value="Ingredients Items">Ingredients Items</option>';
			                 		}elseif($row->field_category == "Store Items"){
			                 			$categoryDropDown = '<select class="form-control select2" name="field_category" data-parsley-required style="width: 100%">
										<option value="Store Items">Store Items</option>';
			                 		}elseif($row->field_category == "Both"){
			                 			$categoryDropDown = '<select class="form-control select2" name="field_category" data-parsley-required style="width: 100%">
			                 	...
			                 	<select class="form-control select2" name="field_category" data-parsley-required style="width: 100%">
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
			   <input type='button' value='Close' class="btn btn-danger" onclick='location.replace("manageItemTypes.php"); '>
			 </div>
            </form>			
          </div>
          <!-- /.box -->
