...
// ----------cate---------
$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' and `table_name` = 'currency'  ";
...
<div class="content-wrapper">
	 ...
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border">
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn btn-success" href="editCurrency.php" >Add Currency</a>
          ...
          <div class="box">
            ...
            <div class="box-body table-responsive">
		        ...
		        <tbody>  
                  ...
                  <td><?php echo $row->status=='1'?'<span onclick="location.href=\'manageCurrency.php?inactiveId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageCurrency.php?activeId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>		

                  <?php 
						$table_name = array(TBL_PARTY,TBL_INV_PO);
				  		$ajaxCheckTransactions = CheckTransactionsCurrency($row->id, $table_name); 

						if($ajaxCheckTransactions != '1'){
                 	 ?>
				  <td><img src="../images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editCurrency.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <script type="text/javascript">
	  ...
	  swal({
	   ...
	   if (isConfirm)
	   {  
	        self.location='manageCurrency.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	   } 
	   else
	   {
	       self.location='manageCurrency.php';
	    }
	   });
	  }
  </script> 
