...
if($_POST['Save']){
	...
	/*if(empty($_POST['name'])){
		...
		$err++;
		$err_name = '<font style="color:red;font-weight:normal;" ><br>Call Center Name all-ready exists in our database.</font>';
	}*/
	...
	}else if($db->num_rows2(selectSql(TBL_COMPANY,"WHERE `id` NOT IN('".$_REQUEST[eId]."') AND `email` = '".addslashes(trim($_POST['email']))."'",'')) && $_POST['Save'] == 'Add'){
		$err++;
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			$addSql = "   	INSERT INTO `".TBL_COMPANY."` SET 
							...
							`id_mst_portfolio_account` = '".addslashes($_POST['id_mst_portfolio_account'])."',
...
$sql1 = executeSql("SELECT * FROM `".TBL_COMPANY."` ORDER BY id DESC LIMIT 1");
	 ...
	 if($idd == '0'){
		 ...
		 $last_id =  $idd + 1;
			      ...
			      $auditaddSql = "INSERT INTO audit_trail SET
							`voucher_id` = '".$last_id."',
							`tables_name` = 'mst_company',
							`form_code` = 'mst_company_form',
							`changes` = 'No Change',
			...
			if(executeSql($addSql)){
				...
				header("location:editCallcenter.php?eId=".addslashes(encryptor(encrypt,$lastInsertId))."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
...
$auditquery = "SELECT * From `".TBL_COMPANY."` WHERE id = '".addslashes(encryptor(decrypt,$_POST[eId]))."'  ";

  $auditresSQL = mysqli_query($connNew, $auditquery);	
	while($auditrow = mysqli_fetch_object($auditresSQL)){ 
	
	 $idd = $auditrow -> id;
	  $c1 = $auditrow -> id_mst_attributes_company_group;
	  $c2 = $auditrow -> name;
	  $c3 = $auditrow -> email;
	  $c4 = $auditrow -> secondary_email;
	  $c6 = $auditrow -> primary_contact_type;
	  $c5 = $auditrow -> primary_mobile;
	  $c7 = $auditrow -> primary_landline;
	  $c8= $auditrow -> secondary_contact_type;
	  $c9 = $auditrow -> secondary_landline;
	  $c10 = $auditrow -> secondary_mobile;
	  $c11= $auditrow -> fax;
	  $c12= $auditrow -> address;
	  $c13= $auditrow -> city;
	  $c14= $auditrow -> id_mst_country_lang;
	  $c15= $auditrow -> id_mst_state;
	  $c16= $auditrow -> postcode;
	  $c17= $auditrow -> details;
	  $c18= $auditrow -> id_mst_portfolio_account;
	  $c19= $auditrow -> company_credibility;
	  $c20= $auditrow -> id_nac;
	  $c21= $auditrow -> deals_in;
	  $c22= $auditrow -> status;
	  
	
     if($c1 != $_POST['id_mst_attributes_company_group']){ 
		$old_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$c1."' AND table_name='company_group'");
		$new_data = selectColumn(TBL_ATTRIBUTES,'field_value'," WHERE `id` = '".$_POST['id_mst_attributes_company_group']."'  AND table_name='company_group'");
		$ch1 = "Company Group Changed from ".  $old_data ." - to - " . $new_data;
	}
	if($c2 != $_POST['name']){
		 $ch2 ="Call Center Name Changed from " .  $c2." - to - ".$_POST['name'];
	}
	if($c3 != $_POST['email']){
		 $ch3 ="Email Changed from " .   $c3 ." - to - " . $_POST['email'];
	} 
	if($c4 != $_POST['secondary_email']){
		$ch4 ="Secondary Email Details Details Changed from " .   $c4 ." - to - " . $_POST['secondary_email'];
	} 
	if($c5 != $_POST['primary_mobile']){
		 $ch5 = "Primary Mobile Changed from " .  $c5 ." - to - " . $_POST['primary_mobile'];
	}
	if($c7 != $_POST['primary_landline']){
		$ch6 ="Primary Landline Details Changed from " .   $c7 ." - to - " . $_POST['primary_landline'];
	}
	if($c9 != $_POST['secondary_landline']){
		$ch7 ="Secondary Landline Details Changed from " .   $c9 ." - to - " . $_POST['secondary_landline'];
	}
	if($c10 != $_POST['secondary_mobile']){
		$ch8 ="Secondary Mobile Details Changed from " .   $c10 ." - to - " . $_POST['secondary_mobile'];
	}
	if($c11 != $_POST['fax']){
		$ch9 ="Fax Details Changed from " .   $c11 ." - to - " . $_POST['fax'];
	}
	if($c12 != $_POST['address']){
		$ch34 ="Address Details Changed from " .   $c12 ." - to - " . $_POST['address'];
	}
	if($c13 != $_POST['city']){
		$ch10 ="City Details Changed from " .   $c13 ." - to - " . $_POST['city'];
	}
	if($c14 != $_POST['id_mst_country_lang']){
		$old_data = selectColumn(TBL_COUNTRY_LANG,'name'," WHERE `id_country` = '".$c14."'");
		$new_data = selectColumn(TBL_COUNTRY_LANG,'name'," WHERE `id_country` = '".$_POST['id_mst_country_lang']."'  ");
		$ch11 ="Country Details Changed from " .$old_data ." - to - " . $new_data;
	}
	if($c15 != $_POST['id_mst_state']){
		$old_data = selectColumn(TBL_STATE,'name'," WHERE `id_state` = '".$c15."'");
		$new_data = selectColumn(TBL_STATE,'name'," WHERE `id_state` = '".$_POST['id_mst_state']."'  ");
		$ch12 ="State Details Changed from " .$old_data ." - to - ".$new_data;
	}
	if($c16 != $_POST['postcode']){
		$ch13 ="Pincode Details Changed from " .   $c16 ." - to - " . $_POST['postcode'];
	}
	if($c17 != $_POST['details']){
		$ch14 =" Details Changed from " .   $c17 ." - to - " . $_POST['details'];
	}
	if($c18 != $_POST['id_mst_portfolio_account']){
		$old_data = selectColumn(TBL_PORTFOLIO_ACCOUNT,'name'," WHERE `id` = '".$c18."'");
		$new_data = selectColumn(TBL_PORTFOLIO_ACCOUNT,'name'," WHERE `id` = '".$_POST['id_mst_portfolio_account']."'  ");
		$ch15 ="Executive Name Details Changed from " .$old_data." - to - " .$new_data;
	}
	if($c19 != $_POST['company_credibility']){
		if($c19 == 1){$c19='Credit Allowed';}else{$c19='Credit Not Allowed';}
		if( $_POST['company_credibility'] == 1){$old_data='Credit Allowed';}else{$old_data='Credit Not Allowed';}
		$ch16 ="Company Credibility Changed from " .   $c19 ." - to - " . $old_data;
	}
	if($c20 != $_POST['id_nac']){		
		if($c20 == 1){$c20='A';}else{$c20='B';}
		if( $_POST['id_nac'] == 1){$old_data='A';}else{$old_data='B';}
		$ch17 ="National Account Code Changed from " .   $c20 ." - to - " . $old_data;
	}
	if($c21 != $_POST['deals_in']){
		$old_data = selectColumn(TBL_COMPANY_AREA,'name'," WHERE `id` = '".$c21."'");
		$new_data = selectColumn(TBL_COMPANY_AREA,'name'," WHERE `id` = '".$_POST['deals_in']."'  ");		
		$ch18 ="Deals In Details Changed from " . $old_data ." - to - " .$new_data;
	}
	if($c22 != $_POST['status']){
		if($c22 == 1){$c22='Active';}else{$c22='Inactive';}
		if( $_POST['status'] == 1){$old_data='Active';}else{$old_data='Inactive';}
		$ch19 ="Hide/Show Details Changed from " .   $c22 ." - to - " . $old_data;
	}
           ...
           executeSql($auditeditSql);
			...
			if(executeSql($editSql)){
				...
				header("location:editCallcenter.php?eId=".$_GET['eId']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>   <span style="color: #f25e74;">Call Center Manager  </span><small>Manage Company</small> </h1>
    <ol class="breadcrumb">
      ...
      <div class="col-md-12">
        ...
        <div class="nav-tabs-custom">
          <ul class="nav nav-tabs">
            <li class="active" ><a href="#tab_1" data-toggle="tab">Comapany</a></li>
          ...
          <div class="box-header with-border" style="padding-bottom:10px">
            <h3 class="box-title"><?php echo $_REQUEST['eId']==''?'Add Company ':'Edit Company : '?>  <a><?php echo selectColumn(TBL_COMPANY,'name'," WHERE `id` = '".addslashes(encryptor(decrypt,$_REQUEST['eId']))."'"); ?></a></h3>
          </div>
          <!-- /.box-header -->
          <!-- form start -->
            ...
            <div class="box-body" style="padding-top: 0px">
            	<div class="card text-dark bg-light">
              		<div class="bg-primary text-center">
                    	<h5 style="padding: 5px;">Company General Details</h5>
                	</div>
                	<hr/>
                </div>
            	<div class="row">
	                <div class="form-group col-sm-4">
	                  <label for="id_mst_attributes_company_group">Business Source<font color="#FF0000">*</font></label>
	                  	 <?php $categoryDropDown = '<select class="form-control select2"  style="width:100%" name="id_mst_attributes_company_group" id="id_mst_attributes_company_group" data-parsley-required data-parsley-errors-container="#err_default_group">
									<option value="">Select Company  Group</option>';
												  ...
												  if($db->num_rows2($resCat)){
												  	while($resultCat = $db->fetch_object2($resCat)){
														...
														}else if($row->id_mst_attributes_company_group == $resultCat->id ){
															$selected = 'selected="selected"';
														}else if($resultCat->id == '265' ){
															$selected = 'selected="selected"';
														}else{
															$selected = '';
														}
	                ...
	                <div class="form-group col-sm-4">
	                  <label for="name">Call Center Name<font color="#FF0000">*</font></label>
	                  <input autocomplete="off" type="text" class="form-control awesomplete" data-list="#mylist" placeholder="Enter Callcenter name" id="name" name="name" value="<?php if($_POST) echo $_POST['name'];else echo stripslashes($row->name);?>" data-parsley-required >
          ...
          <div class="row">
	        ...
	        <div class="card text-dark bg-light" >
	            <div class="bg-primary text-center">
	                <h5 style="padding: 5px;">Company Accounts Details</h5>
	            </div> 
	            <hr>
	        </div>  
              <div class="row">
                ...
                <div class="form-group col-sm-3">
                  <label for="area">Executive Name<font color="#FF0000">*</font></label>
                  <?php 
                  // AND FIND_IN_SET(id,'".$_SESSION['teamMemberAreas']."')
                  $areaDropDown = '<select class="form-control select2" name="id_mst_portfolio_account" id="area" style="width:100%" data-parsley-required onChange="areaOnChg(this.value);">
								<option value="">Select Executive Name</option>';
											  $resCat = selectSql(TBL_PORTFOLIO_ACCOUNT," where status='1' and id_shop='".addslashes($_SESSION['shop'])."' ",' ORDER BY `name`');
											  if($db->num_rows2($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
													if($_REQUEST['id_mst_portfolio_account'] == $resultCat->id){
														$selected = 'selected="selected"';
													}elseif($row->id_mst_portfolio_account == $resultCat->id){
														$selected = 'selected="selected"';
													}else{
														$selected = '';
													}
                  ...
                  <span id="areaError"><?php echo $err_area;?></span> 
              		<span id="areaExe" style="color: red"></span>	
              	</div>
                ...
                <div class="form-group col-sm-3">
					  ...
					  <select class="form-control select2" onChange="openCreditLimit(this.value);" name="company_credibility" id="company_credibility" style="width:100%" data-parsley-errors-container="#company_credibilityError" data-parsley-required>
						<option value="0" <?php if($_REQUEST['company_credibility']=='0'){echo 'selected="selected"';}
						?>>Credit Not Allowed</option>
					...
					<!--	<option value="1" <?php if($_REQUEST['company_credibility']=='1'){echo 'selected="selected"';}elseif($row->company_credibility=='1'){echo 'selected="selected"';} ?>>Credit Allowed</option>
					  ...
					  <span id="company_credibilityError"><?php echo $err_company_credibility;?></span> 
				</div>
              	<?php 
              ...
              <div class="row">
              	<div class="form-group col-sm-4">
              		<label for="id_nac">National account code </label>
					
              		<select name="id_nac" id="id_nac" class="form-control select2" style="width: 100%" >
              ...
              <div class="form-group col-sm-4 col-md-4">
                      	 <label for="creditform">Credit Form</label><br>
                           	...
                           	if($row->credit_form != ''){
                           		...
                           		echo '<a style="color:#333333;" href="'.$link.'"><i  class="fa fa-cloud-download fa-3x" value="" ></i></a> &nbsp <span>"'.$row->credit_form.'"</span> Download';
                           	}
                           	else
                           		$link = "#";
                           	?>
                   ...
                   <div class="row">
          				...
          				<?php if(!empty($_REQUEST['eId'])){ 
                  			...
                  			checkUserLevelPermission($_SESSION['userLevel'],TBL_COMPANY_CONTACTS,'view');
                  			//if($_REQUEST['eId'] == ''){ header("location:editCallcenter.php"); }

							if($_REQUEST['action'] == 'change' && ($_REQUEST['activeId'] || $_REQUEST['inactiveId'])){

								if($_REQUEST['activeId'] != ''){

									checkUserLevelPermission($_SESSION['userLevel'],TBL_COMPANY_CONTACTS,'activate');
									...
									$statusSql = "	UPDATE `".TBL_COMPANY_CONTACTS."`
										...
										checkUserLevelPermission($_SESSION['userLevel'],TBL_COMPANY_CONTACTS,'deactivate');
									...
									if(executeSql($statusSql)){
										...
										$_SESSION['successMsg'] = selectColumn(TBL_COMPANY_CONTACTS,'first_name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';

									}else{

										$err = 1;

										$_SESSION['errorMsg'] = selectColumn(TBL_COMPANY_CONTACTS,'first_name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
								...
								if($_REQUEST['eId'] != ''){
									...
									$sql .= " WHERE `id_mst_company` = '".addslashes(encryptor(decrypt,$_REQUEST['eId']))."' ";
													...
													<?php 				 				
														if($total > 0){ $counter = 1;
														  while($rowcontact = $db->fetch_object()){?>
											                  ...
											                  <td><?=$rowcontact->status=='1'?'<span onclick="location.href=\'editCallcenter.php?inactiveId='.encryptor(encrypt,$rowcontact->id).'&eId='.$_GET['eId'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:darkgreen;cursor:pointer;">Active</span>':'<span onclick="location.href=\'editCallcenter.php?activeId='.encryptor(encrypt,$rowcontact->id).'&eId='.$_GET['eId'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>			 
        ...
        $("#creditimport").submit(function(e){
          ...
          if(fileName == ""){
            ...
            success     : function(data){
              ...
              $("#downloadDiv").html('<a style="color:#333333;" href="'+data['target_url']+ '" download><i  class="fa fa-cloud-download fa-3x" value="" ></i></a> &nbsp "'+data['filename']+'" Download');
            } 
           })
          }
        });
