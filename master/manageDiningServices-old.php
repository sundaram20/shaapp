...
if($_REQUEST['action'] == 'change'){
	...
	$delSql = "DELETE FROM `".TBL_DINING_SERVICES."` WHERE `id` = '".addslashes(encryptor(decrypt,$_REQUEST['delId']))."'";
	$sqlDelUserLevel = selectRow(TBL_DINING_SERVICES," WHERE `id` = '".addslashes(encryptor(decrypt,$_REQUEST['delId']))."'");
...
if($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])){
	...
	$delSql = "DELETE FROM `".TBL_DINING_SERVICES."` WHERE `id` IN (".addslashes($deleteIds).")";
	if(executeSql($delSql)){		
		$err = 0;
...
// ----------cate---------
$sql = " SELECT * FROM `".TBL_DINING_SERVICES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' ";
        ...
        <div class="box-header with-border">
		  ...
		  <div class="btn-group  pull-right">
							  ...
							  <ul class="dropdown-menu" role="menu">
								<?php ?><li><a title="Export to excel file" href="exportTable.php?fileType=xls&tableName=<?php echo TBL_DINING_SERVICES;?>"><img src="images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>
								<li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo TBL_DINING_SERVICES;?>"><img  src="images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li><?php ?>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label>Dining Services Title</label>				
        ...
        <div class="col-xs-12">		     
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Dining Services List</h3>
            </div>
            ...
            <div class="box-body table-responsive">
              <table id="example2" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><?=$row->name;?></td>
                  <td><?=$row->status=='1'?'<span onclick="location.href=\'manageDiningServices.php?inactiveId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageDiningServices.php?activeId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>			 
				  <td><img src="images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editDiningServices.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/delete.gif" style="cursor:pointer;" title="Delete" onClick="if(confirm('Are you sure that you want to delete this record <?=$row->name;?>?')){window.location.href='manageDiningServices.php?delId=<?=encryptor(encrypt,$row->id)?>&action=delete&page=<?=$_REQUEST['page']?>';}"/></td>
                </tr>
               <?php }?> 
			   <!--<tr>
