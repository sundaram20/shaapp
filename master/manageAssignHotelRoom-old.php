...
if($_REQUEST['action'] == 'change'){
	if($_REQUEST['activeId'] != ''){
		...
		$assignHotelId = selectColumn(TBL_ASSIGN_HOTEL_ROOM,'id_hotel'," WHERE `id` = '".$statusId."'");
		$assignRoomId = selectColumn(TBL_ASSIGN_HOTEL_ROOM,'id_room'," WHERE `id` = '".$statusId."'");
	...
	if(executeSql($delSql)){		
		...
		$_SESSION['successMsg'] = 'One Hotel Room assigned '.selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".$sqlDelUsers['id_hotel']."'").'-'.selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = '".$sqlDelUsers['id_room']."'").' has been deleted sucessfully.';
	}else{
		$err = 1;
		$_SESSION['errorMsg'] = 'Unable to delete hotel Room assign '.selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".$sqlDelUsers['id_hotel']."'").'-'.selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = '".$sqlDelUsers['id_room']."'");
	}
}
...
// ----------cate---------
$sql = executeSql(" SELECT * FROM `".TBL_ASSIGN_HOTEL_ROOM."` WHERE `id_hotel` = '".encryptor(decrypt,$_REQUEST['eId'])."' ");
