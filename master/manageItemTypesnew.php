<?php 
	...
	if($_REQUEST['action'] == 'change'){
		...
		if(executeSql($statusSql)){
			$err = 0;
			$_SESSION['successMsg'] = 'Items '.selectColumn(TBL_ATTRIBUTES,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';
		}else{
			$err = 1;
			$_SESSION['errorMsg'] = 'Items '.selectColumn(TBL_ATTRIBUTES,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
		}
	...
	$sql = local_SelectQuery_Mst_Items($_REQUEST['tableName'],$field_name,$field_label,$EnableOrderBy);
	//echo "hello"; exit;
	$QuerySQL	=	mysqli_query($connNew,$sql);
	$numRows = @mysqli_num_rows($QuerySQL);

	$TotalCountRecord = selectColumn(TBL_ATTRIBUTES,'count(id)',' WHERE id_shop= "'.addslashes($_SESSION['shop']).'" ');
			            	...
			            	<div class="form-group">
			            		<label>Items Type </label>
			                      ...
			                      <?php $categoryDropDown = '<select class="form-control select2" name="search_name" style="width:100%">
			                        ...
			                        $resUserLevel = selectSql(TBL_ATTRIBUTES," WHERE `table_name` = 'items_type' AND  `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `field_value`');
		        						...
		        						<div class="row">
		        							...
		        							<div class="col-md-3 col-sm-3">
		        								<select class="form-control select2" style="width:100%;" name="EnableOrderBy" id="EnableOrderBy" >
		        									...
		        									<?php
		    											$sqlResult = mysqli_query($appConnect, "SELECT field_name FROM ".TBL_REPORT." WHERE table_name = '".TBL_ATTRIBUTES."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc" );
					...
					<tbody>
						<?php 	
							...
							while($row = @mysqli_fetch_row($QuerySQL)){
								...
								$strLocation = 'editItemTypes.php?eId='.encryptor(encrypt,$Id).'&action=edit&page='.$_REQUEST['page'];
  ...
  <script type="text/javascript">
    ...
    swal({
     ...
     function(isConfirm)
     {
	     ...
	     if (isConfirm)
	     {  
	          self.location='manageItemTypes.php?delId='+sid+'&action=delete&page=<?=$_REQUEST['page']?>';
	     } 
	     else
	     {
	        self.location='manageItemTypes.php';
	      }
	     });
    }
