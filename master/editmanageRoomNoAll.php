...
/////////////////////////////////////////////////////////////////////////////////////
if($_REQUEST['eId'] == ''){
	header("location:editmanageRoomNoAll.php");
}
/////////////////////////////////////////////////////////////////////////////////////
//---------------------------------------------------------------------------------------------------------
...
print_r($record1);
	...
	//echo '<pre>';print_r($arrayOfDates);die;
    ...
    foreach ($arrayOfDates as $date => $count) {
        ...
        echo $queryUpdate = "UPDATE fo_inventory 
		 ...
		 $last_id =  $idd + 1;
			...
			$auditaddSql = "INSERT INTO audit_trail SET
				...
				if(executeSql($addSql)){
					...
					header("location:manageRoomNoAll.php?eId=".$_REQUEST['eId']."&id_mst_room_types=".encryptor('encrypt', $_REQUEST['id_mst_room_types'])."&action=edit&page=".$_REQUEST['page']);
					exit;
				}else{
					$err++;
...
//echo $editSql;die;
			...
			if($_POST['status_active_date'] <= $_POST['active_date']){
				...
				if(executeSql($addSql1)){
					...
					header("location:manageRoomNoAll.php?eId=".$_REQUEST['eId']."&action=edit&page=".$_REQUEST['page']);
					exit;
				}else{
					$err++;
           ...
           <div class="nav-tabs-custom">
			<ul class="nav nav-tabs">
              ...
              <li ><a href="editHotelVideoGallery.php?eId=<?php echo $_GET['eId']; ?>&action=edit&page=<?php echo $_GET['page']; ?>" data-toggle="tab">Video Gallery</a></li>
				<li class="active"><a href="editmanageRoomNoAll.php?eId=<?php echo $_GET['eId']; ?>&action=edit&page=<?php echo $_GET['page']; ?>" data-toggle="tab">Manage Rooms</a></li>     
            ...
            <div class="box-header with-border">
              <h3 class="box-title"><?php echo $_REQUEST['id']==''?'Add':'Edit'?> Room : <a><?php echo selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".addslashes(encryptor('decrypt',$_REQUEST['eId']))."'"); ?> </a></h3>     
			   <a href="manageRoomNoAll.php?eId=<?php echo $_GET['eId']; ?>&id_mst_room_types=<?php echo $_REQUEST['id_mst_room_types']; ?>&action=edit&page=<?php echo $_GET['page']; ?>" class="btn btn-success pull-right"><i class="fa fa-angle-double-left"></i> Back</a>  
			</div> 
            <!-- /.box-header -->
            <!-- form start -->  			        
							         	...
							         	if(isset($_REQUEST['eId']) && $row->blocked_room_dates !=''){
							         		foreach (explode(',',$row->blocked_room_dates) as $index => $range) {
							         				/*echo '<tr id="date_'.$index.'">
							         						<td>'.$range.'</td>
							         						<td><i '.$disabled1.' onclick="removeThisSaved('.$index.','.addslashes(encryptor('decrypt',$_REQUEST['id'])).','.$range.'");" '.$noaction1.' class="fa fa-trash"></i></td>
							         						</tr>';*/
												echo '<tr id="date_'.$index.'">
    <td>'.$range.'</td>
    <td>
        <i '.$disabled1.' 
           onclick="removeThisSaved('.$index.', \''.addslashes(encryptor('decrypt', $_REQUEST['id'])).'\', \''.$range.'\',\''.addslashes(encryptor('decrypt',$_REQUEST['id_mst_room_types'])).'\',\''.addslashes(encryptor('decrypt',$_REQUEST['eId'])).'\');" 
           '.$noaction1.' 
           class="fa fa-trash"></i>
    </td>
</tr>';
...
function toggleBlockDates() {
    ...
    if (managementBlock === 'Yes') {
        // Enable all inputs inside exclusion box
        exclusionBox.style.pointerEvents = 'auto';
        exclusionBox.style.opacity = '1';
        inputs.forEach(function(input) {
        ...
        inputs.forEach(function(input) {
            input.disabled = true;
				...
				<div class="box-footer">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
			   		<input type='button' value='Close' class="btn btn-danger" onclick='location.replace("manageRoomNoAll.php?eId=<?php echo $_GET['eId']; ?>&action=edit&page=<?php echo $_GET['page']; ?>"); '>
...
$(function() {
   // var exeCount = 0;
	...
	/*(function removeThisSaved(val,id){ 
	...
	function removeThisSaved(val, id, daterange, id_mst_room_types, id_mst_hotel) {
    if (!confirm("Are you sure you want to remove this blocked date?")) {
        return; // Exit if user cancels
    }

    // Show loader (ensure you have an element with id 'loader' in your HTML)
    $('#loader').show();

    $.ajax({
        url: 'ajax/ajaxRemoveBlockedDate.php',
        type: 'POST',
        data: {
        ...
        error: function(xhr, status, error) {
            ...
            alert('Failed to remove date on server.');
        },
        complete: function() {
