<?php 
  ...
  if($_REQUEST['action'] == 'change'){
    ...
    if(executeSql($statusSql)){
      $err = 0;
      $_SESSION['successMsg'] = 'User '.selectColumn(TBL_USER_LEVELS,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';
    }else{
      $err = 1;
      $_SESSION['errorMsg'] = 'User '.selectColumn(TBL_USER_LEVELS,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
    }
    ...
    $delSql = "DELETE FROM `".TBL_USER_LEVELS."` WHERE `id` = '".addslashes($deleteIds)."'";
    $sqlDelUsers = selectRow(TBL_USER_LEVELS," WHERE `id` = '".addslashes($deleteIds)."'");
  ...
  if($_REQUEST['searchFormSubmit']==''){
      $sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_USER_LEVELS."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
      ...
      $sqlResultOrder = mysqli_query($appConnect, "SELECT field_name FROM ".TBL_REPORT." WHERE table_name = '".TBL_USER_LEVELS."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc LIMIT 0,1" );
    ...
    $sqlResult1 = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_USER_LEVELS."' AND id_shop = ".$_SESSION['shop'] ." AND field_name IN ('".$fieldname."')  ORDER BY display_order");
  ...
  $sql = local_SelectQuery_Mst_UserLevels($_REQUEST['tableName'],$field_name,$field_label,$EnableOrderBy);
  //echo "hello"; exit;
  $QuerySQL = mysqli_query($connNew,$sql);
  $numRows = @mysqli_num_rows($QuerySQL);

  $TotalCountRecord = selectColumn(TBL_USER_LEVELS,'count(id)',' WHERE id_shop="'.$_SESSION['shop'].'" ');
                    ...
                    <div  id="fieldset" class="<?php echo $viewClass; ?>">
                      ...
                      <div  id="moreReportDiv" style="padding-left:0px;<?php echo $DispalyClass; ?>">
                          ...
                          <div class="row">
                            ...
                            <div class="col-md-3 col-sm-3">
                              <select class="form-control select2" style="width:100%;" name="EnableOrderBy" id="EnableOrderBy" >
                                ...
                                <?php
                                $sqlResult = mysqli_query($appConnect, "SELECT field_name,field_label FROM ".TBL_REPORT." WHERE table_name = '".TBL_USER_LEVELS."'AND enable_order_by = 1 AND id_shop = ".$_SESSION['shop']." order BY display_order asc" );
                        ...
                        is_array($_REQUEST['field_name']) ? '' : $_REQUEST['field_name']=array();
                              $sqlResult = mysqli_query($appConnect, "SELECT * FROM ".TBL_REPORT." WHERE table_name = '".TBL_USER_LEVELS."' AND id_shop = ".$_SESSION['shop'] ." ORDER BY display_order");
                        ...
                        <?php } }?>   
                        <input type="hidden" name="tableName" value="<?php echo TBL_USER_LEVELS;?>" />  
                      </div>
              ...
              while($row = @mysqli_fetch_row($QuerySQL)){
                ...
                //print_r($row) ;
                  ...
                  if($cell=='Active'){            
                    ...
                    $cell='<span onclick="location.href=\'manageUserLevels.php?inactiveId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>';
                  }else if($cell=='Inactive'){
                    $cell='<span onclick="location.href=\'manageUserLevels.php?activeId='.encryptor(encrypt,$Id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';
                  }
                  if($value == 1){
                    $Id = $cell;
                  }else{
                ...
                $strLocation = 'editUserLevels.php?eId='.encryptor(encrypt,$Id).'&action=edit&page='.$_REQUEST['page'];

                
                //echo $Id;
                $table_name = array(TBL_USERS,TBL_MENU_ACCESS);
                $ajaxCheckTransactions = CheckTransactionsUserLevel($Id, $table_name); 
