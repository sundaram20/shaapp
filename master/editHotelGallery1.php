		   ...
		   <div class="col-xs-12 col-md-12">             
              ...
              <div class="form-group">
                ...
                <select class="form-control" name="roomType" required="required">
                	...
                	<?php
                		$fetchRoom ="SELECT * FROM ".TBL_ASSIGN_HOTEL_ROOM." WHERE id_hotel=".encryptor(decrypt,$_REQUEST['eId'])." "; 
                		$resFetch = mysqli_query($connNew,$fetchRoom);
                		while($rowRoom = mysqli_fetch_object($resFetch)){
                			echo '<option value="'.$rowRoom->id_room.'">'.selectColumn(TBL_ROOM_TYPE,'name','where id='.$rowRoom->id_room.' ').'</option>';
                		}
                	?>
                </select>				
        ...
        <div class="col-md-12">
          <!-- general form elements -->
		   <div class="nav-tabs-custom">
            ...
            <div class="box-body" >    
            	<?php 
					   $roomGalSql="SELECT id_room,id_hotel FROM ".TBL_ASSIGN_HOTEL_ROOM." WHERE id_hotel='".encryptor(decrypt,$_REQUEST['eId'])."' ";
					   ...
					   while($rowGal=mysqli_fetch_object($resRoomGal)){
					   	...
					   	echo '<div class="box-header with-border">
              					<h3 class="box-title">'.selectColumn(TBL_ROOM_TYPE,'name','where id='.$rowGal->id_room.' ').'</h3>
			  	            </div>';
				...
				<div  class="row"  >  
				    ...
				    <ul class="sortable-list list-unstyled" >   
			        	...
			        	<?php
			        	$gallerySql="SELECT * FROM ".TBL_ROOM_GALLERY." WHERE id_room=".$rowGal->id_room." AND id_hotel=".$rowGal->id_hotel." ";
	                ...
	                <div  class="col-xs-12 col-md-4 col-lg-4 <?=$activeStatus?>" id="<?=$row->id?>" >
	                  <li class="panel panel-default">
						  ...
						  <?php if(@file_exists($image_path_room.'main-'.$rowGallery->image)){ ?>
		                    <div class="sortable-heading panel-body" >	<img src="<?=$image_display_path_room.'main-'.$rowGallery->image?>" class="img-responsive" style="height:150px; width:300px;"/>
		                    </div>
						  <?php }else {?>
