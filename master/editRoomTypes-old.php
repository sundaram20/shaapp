...
if($_POST['Save']){
	...
	}else if($db->num_rows2(selectSql(TBL_ROOM_TYPE,"WHERE `id` NOT IN('".addslashes(encryptor(decrypt,$_POST[eId]))."') and `id_shop` = '".addslashes($_SESSION['shop'])."' AND `name` = '".addslashes($_POST['name'])."'",''))){
		$err++;
		$err_name = '<font style="color:red;font-weight:normal;" ><br>Room Type name all-ready exists in our database.</font>';
	}
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			if(executeSql($addSql)){
				...
				$err++;
				$_SESSION['errorMsg'] = 'Room Type has not been saved. Please make corrections below.';
			}
        ...
        <div class="col-md-12">
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title"><?php echo $_REQUEST['eId']==''?'Add':'Edit'?> Room Types</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->  			        
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
			   <input type='button' value='Cancel' class="btn btn-default" onclick='location.replace("manageRoomTypes.php"); '>
			 </div>
            </form>			
          </div>
          <!-- /.box -->
