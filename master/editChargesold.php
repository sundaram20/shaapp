...
if($_REQUEST['eId']=='')
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_CHARGES,'edit');
...
if($_POST['Save']){
	...
	if($err == 0){//No error
		...
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			$sql = " SELECT * FROM `".TBL_CHARGES."` WHERE  `id_shop` = '".addslashes($_SESSION['shop'])."' AND `name` = '".addslashes(trim($_POST['name']))."' ";
			$db->query($sql);
			$numRows= $db->num_rows();
			if($numRows == '0'){
...
$sql1 = executeSql("SELECT * FROM `".TBL_CHARGES."` ORDER BY id DESC LIMIT 1");
	 ...
	 if($idd == '0'){
		 ...
		 $last_id =  $idd + 1;
			...
			$auditaddSql = "INSERT INTO audit_trail SET
							`voucher_id` = '".$last_id."',
							`tables_name` = 'mst_charges',
							`form_code` = 'charges_manager_form',
							`changes` = 'No Change',
			...
			if(executeSql($addSql)){
				...
				$err++;
				$_SESSION['errorMsg'] = 'Charges Master Name Already Exist.';
			}
		}
        ...
        checkUserLevelPermission($_SESSION['userLevel'],TBL_CHARGES,'update');
			
			
 $auditquery = "SELECT * From `".TBL_CHARGES."` WHERE id = '".addslashes(encryptor(decrypt,$_POST[eId]))."' ";
   ...
   $auditresSQL = mysqli_query($connNew, $auditquery);	
	while($auditrow = mysqli_fetch_object($auditresSQL)){ 
	  ...
	  $id = $auditrow -> id;
	  $c1 = $auditrow -> name;
	  $c2 = $auditrow -> charges_account;
	  $c3 = $auditrow -> tax_applicable;
	  $c4 = $auditrow -> transaction_type;
	  $c5 = $auditrow -> tax_type;
	   $c55 = $auditrow -> percentage;
	  $c7 = $auditrow -> status; 
	  $c8 = $auditrow -> id_mst_charges_vat; 
	   $c9 = $auditrow -> id_mst_charges_sgst; 
	  $c10 = $auditrow -> id_mst_charges_cgst; 
	  $c11 = $auditrow -> id_mst_charges_igst; 
	  $c12 = $auditrow -> id_mst_charges_cess; 
	  $c13 = $auditrow -> id_mst_charges_surcharge;
 ...
 else
 {
	...
	if($c2 != $_POST['charges_account']){
		
		if($c2 == 1){
			$old='SALES';}else if($c2 == 2){$old='PURCHASE';}else if($c2 == 3){$old='INCOME';}else if($c2 == 4){$old='EXPENSE';}else if($c2 == 5){$old='TAXES';}else if($c2 == 6){$old='DISCOUNT';}else if($c2 == 7){$old='OTHERS';}
		
		if($_POST['charges_account'] == 1){$new='SALES';}else if($_POST['charges_account'] == 2){$new='PURCHASE';}else if($_POST['charges_account'] == 3){$new='INCOME';}else if($_POST['charges_account'] == 4){$new='EXPENSE';}else if($_POST['charges_account'] == 5){$new='TAXES';}else if($_POST['charges_account'] == 6){$new='DISCOUNT';}else if($c2 == 7){$new='OTHERS';}
		$ch2 ="Charges Account Details Changed from " . $old." - to - ".$new;
	}
	
	echo $c3;
	echo $_POST['tax_applicable'];
	
	if($c3 != $_POST['tax_applicable']){
		
			if($c3 == 1){$old='GST';}else if($c3 == 2){$old='VAT';}
			if($_POST['tax_applicable'] == 1){$new='GST';}else if($_POST['tax_applicable'] == 2){$new='VAT';}
			$ch3 ="Tax Applicable Details Changed from " . $old ." - to - " . $new;
		
	} 
	if($c4 != $_POST['transaction_type']){
			if($c4 == 1){$old='Local';}else if($c4 == 2){$old='Interstate';}
			
			if($_POST['transaction_type'] == 1){$new='Local';}else if($_POST['transaction_type'] == 2){$new='Interstate';}
			$ch4 ="Transaction Type  Details Changed from " . $old ." - to - " . $new;
	}
	if($c5 != '0'  && $_POST['tax_type'] != ''){		
			if($c5 == 1){$old='SGST';}else if($c5 == 2){$old='CGST';}else if($c5 == 3){$old='IGST';}else if($c5 == 4){$old='VAT';}else if($c5 == 5){$old='CESS';}else if($c5 == 6){$old='SURCHARGE';}
			
			if($_POST['tax_type'] == 1){$new='SGST';}else if($_POST['tax_type'] == 2){$new='CGST';}else if($_POST['tax_type'] == 3){$new='IGST';}else if($_POST['tax_type'] == 4){$new='VAT';}else if($_POST['tax_type'] == 5){$new='CESS';}else if($_POST['tax_type'] == 6){$new='SURCHARGE';}
			
			$ch5 ="Tax Type Details Changed from " . $old ." - to - " . $new;
	}
	
	if($c6 != $percentage){
		
		
			$ch6 ="PERCENTAGE Details Changed from " . $c6 ." - to - " . $_POST['percentage'];
	}
	
	if($c7 != $_POST['status']){
		if($c7 == 1){$old='Active';}else{$old='Inactive';}
		if( $_POST['status'] == 1){$new='Active';}else{$new='Inactive';}
		$ch7 ="Status Details Changed from " .   $old ." - to - " . $new;
	}
	$vat=$_POST['id_mst_charges_vat'];
	if($vat=='')
	{
	$vatnew='0';	
	}
	...
	if($c8 != $vatnew){
		$old_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$c8."'");
		$new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_vat']."'  ");
		$ch8 ="VAT Details Changed from " . $old_data ." - to - " . $new_data;
	
	}
	$sgst=$_POST['id_mst_charges_sgst'];
	if($sgst=='')
	{
	$sgstnew='0';	
	}
	...
	if($c9 != $sgstnew){
		$old_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$c9."'");
		$new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_sgst']."'  ");
		$ch9 ="SGST Details Changed from " . $old_data ." - to - " . $new_data;
	
	}
	
	$cgst=$_POST['id_mst_charges_cgst'];
	if($cgst=='')
	{
	$cgstnew='0';	
	}
	...
	if($c10 != $cgstnew){
		$old_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$c10."'");
		$new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_cgst']."'  ");
		$ch10 ="CGST Details Changed from " . $old_data ." - to - " . $new_data;
	
	}
	
	$igst=$_POST['id_mst_charges_igst'];
	if($igst=='')
	{
	$igstnew='0';	
	}
	...
	if($c11 != $igstnew){
		...
		$new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_igst']."'  ");
		$ch11 ="IGST Details Changed from " . $old_data ." - to - " . $new_data;
	
	}
	$cess=$_POST['id_mst_charges_cess'];
	if($cess=='')
	{
	$cessnew='0';	
	}
	...
	if($c12 != $cessnew){
		...
		$new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_cess']."'  ");
		$ch12 ="CESS Details Changed from " . $old_data ." - to - " . $new_data;
	
	}
	$such=$_POST['id_mst_charges_surcharge'];
	if($such=='')
	{
	$suchnew='0';	
	}
	...
	if($c13 !=$suchnew){
		...
		$new_data = selectColumn(TBL_CHARGES,'name'," WHERE `id` = '".$_POST['id_mst_charges_surcharge']."'  ");
		$ch13 ="SURCHARGE Details Changed from " . $old_data ." - to - " . $new_data;
	}
			...
			$editSql .= "	,`last_modified` = '".currenDateTime()."'
			 ...
			 echo   $auditeditSql = " INSERT audit_trail SET 
							...
							`tables_name` = 'mst_charges',
							`form_code` = 'charges_manager_form',
							`changes` =  '".addslashes($ch1).",".addslashes($ch2).",".addslashes($ch3).",".addslashes($ch4).",".addslashes($ch5).",".addslashes($ch6).",".addslashes($ch7).",".addslashes($ch8).",".addslashes($ch9).",".addslashes($ch10).",".addslashes($ch11).",".addslashes($ch12).",".addslashes($ch13)." ',
							`date_created` = '".currenDateTime()."',
							`last_modified` = '".currenDateTime()."',
							`id_mst_user_modified_by` = '".$_SESSION['userId']."',
							`id_mst_user_created_by` = '".$_SESSION['userId']."',
...
<!-- End Audit trail Modal -->	
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="manageCharges.php"><i class="fa fa-dashboard"></i> Home</a></li>
        ...
        <div class="col-md-12">
          <!-- general form elements -->
			...
			<div class="box-header with-border">
              <h3 class="box-title"><?php echo $_REQUEST['eId']==''?'Add':'Edit'?> Charges Manager : <a><?php echo selectColumn(TBL_CHARGES,'name'," WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  'id' = '".addslashes(encryptor(decrypt,$_REQUEST['eId']))."'"); ?></a></h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->  			        
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="input-group"> 
                 	...
                 	<?php if($row->id == !''){
                 		if($row->charges_account == '1'){
                 			$categoryDropDown = '<select class="form-control select2" name="charges_account" id="charges_account" onchange="changeFunc()" data-parsley-required>
                ...
                <?php if( $row->charges_account != '6' && $row->charges_account != '7' && $row->charges_account != '4'){  ?>
                 	...
                 	<?php if($row->id == !''){
                 		if($row->tax_applicable == '1'){
							$selected4	='selected="selected"';
                 			//$categoryDropDown = 'option value="1">GST</option>';
							
                 		}elseif($row->tax_applicable == '2'){
                 			//$categoryDropDown = '<option value="2">VAT</option>';
            	...
            	<?php } ?>
            	 <?php if(  $row->charges_account != '4' && $row->charges_account != '6'  && $row->charges_account != '7'){ ?>
                  ...
                  <div class="input-group"> 
                 	...
                 	<?php if($row->id == !''){
                 		if($row->transaction_type == '1'){
                 			$categoryDropDown = '<select class="form-control select2" name="transaction_type" onchange="changetaxtype();">
				 ...
				 <?php } ?>
                  ...
                  <div class="form-group" id="taxtype" name="taxtype"> 
                 	...
                 	<?php	if($row->tax_type == '1'){
                 			$categoryDropDown = '<select class="form-control select2" name="tax_type" id="tax_type" onchange="changeFunc1()" >
             ...
             <?php if($row->charges_account != '1' && $row->charges_account != '2' && $row->charges_account != '4' ){ ?>
            ...
            <div  id="taxtypesectionss" name="taxtypesectionss">
             ...
             if($row->charges_account != '5' || $row->charges_account == '4' ){

             if($id_mst_charges_sgst || $row->id == ''){ ?>
				...
				<?php echo $err_id_mst_charges_sgst;?></div>
                  ...
                  <?php } ?>
                 <?php if($id_mst_charges_cgst || $row->id == ''){ ?>  
            ...
            <?php } ?>
             <?php if($id_mst_charges_igst  || $row->id == ''){ ?>
            ...
            <?php } ?>
             <?php if($id_mst_charges_vat  || $row->id == ''){ ?>
          ...
          <?php  
             	if($id_mst_charges_surcharge  || $row->id == ''){ 
             		?>
...
<script type="text/javascript">
	...
	//    $("#taxtypesection").css("display", "none");
	// });
</script>
<?php } ?>
<script type="text/javascript">
