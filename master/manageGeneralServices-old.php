...
if($_REQUEST['action'] == 'change'){
	...
	$delSql = "DELETE FROM `".TBL_GENERAL_SERVICES."` WHERE `id` = '".addslashes(encryptor(decrypt,$_REQUEST['delId']))."'";
	$sqlDelUserLevel = selectRow(TBL_GENERAL_SERVICES," WHERE `id` = '".addslashes(encryptor(decrypt,$_REQUEST['delId']))."'");
	if(executeSql($delSql)){		
		$err = 0;
		$_SESSION['successMsg'] = 'One General Services '.$sqlDelUserLevel["name"].' has been deleted sucessfully.';
	}else{
		$err = 1;
		$_SESSION['errorMsg'] = 'Unable to delete General Services '.$sqlDelUserLevel["name"];
	}
}
if($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])){
	...
	$delSql = "DELETE FROM `".TBL_GENERAL_SERVICES."` WHERE `id` IN (".addslashes($deleteIds).")";
	if(executeSql($delSql)){		
		$err = 0;
...
// ----------cate---------
$sql = " SELECT * FROM `".TBL_GENERAL_SERVICES."`  WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' ";
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Hotel Manager
        <small>Manage General Services</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="home.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Manage General Services</li>
      </ol>
    </section>
    <!-- Main content -->
        ...
        <div class="box-header with-border">
		  ...
		  <div class="btn-group  pull-right">
							  ...
							  <ul class="dropdown-menu" role="menu">
								<?php ?><li><a title="Export to excel file" href="exportTable.php?fileType=xls&tableName=<?php echo TBL_GENERAL_SERVICES;?>"><img src="images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>
								<li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo TBL_GENERAL_SERVICES;?>"><img  src="images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li>
							  <?php ?>
							  </ul>
							</div>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label>General Services Title</label>				
        ...
        <div class="col-xs-12">		     
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">General Services List</h3>
            </div>
            ...
            <div class="box-body table-responsive">
              <table id="example2" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><?=$row->name;?></td>
                  <td><?=$row->status=='1'?'<span onclick="location.href=\'manageGeneralServices.php?inactiveId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageGeneralServices.php?activeId='.encryptor(encrypt,$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>			 
				  <td><img src="images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editUniqueExperiences.php?eId=<?=encryptor(encrypt,$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/delete.gif" style="cursor:pointer;" title="Delete" name="<?php echo $row->name; ?>" id="<?php echo $row->id;?>" onClick="deleteMe(this.id,this.name);"/></td>
                </tr>
               <?php }?> 
			   <!--<tr>
  		    ...
  		    if (this.readyState == 4 && this.status == 200) {
  		      ...
  		      if(this.responseText == 1){
  		      	...
  		      	if(confirm('Are you sure that you want to delete this record '+name+'?')){
  		      		window.location.href='manageGeneralServices.php?delId='+id+'&action=delete&page=<?=$_REQUEST['page']?>';
  		      	}
  		      }
  		    }
  		  };
  		  xhttp.open("GET", "ajax/ajaxCheckCompanyDomain.php?id_gen_ser="+id, true);
  		  xhttp.send();
  	}
  </script>                                   
