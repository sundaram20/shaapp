<?php 
	...
	function FoPlanDateWiseReportDaily($date,$id_main_group,$id_sub_group,$id_items,$id_report_type,$report_show,$id_order_by,$showItemReport,$res_bookingStatus_new,$hk_status,$shop,$pdfNameReport3,$id_mst_shops,$cronSet){
	global $connNew;
	global $objPHPExcel;
...
$SalesRegisterArray=array();
 $cond = "  where `fo_reservations`.`id_mst_shops` = '".addslashes($id_mst_shops)."' ";
...
while($row	   =	mysqli_fetch_object($querySalesReportPayment)){
		...
		foreach($datewise_array as $checkinDatearr){			
			...
			$hk_statusCondtion=0;	
					...
					}elseif($hkno_showoff=='0' && $row->id_mst_room_no_allocation>0){
							$FHstatusGroup = "Occuiped";//"Checkin/Occuiped";
						    $hk_statusCondtionValue=1;
			...
			//}
					if($hk_status==0 || $hk_status==$hk_statusCondtionValue){
					 ...
					 $room_no =  $row->id_mst_room_no_allocation=='0'?'0':selectColumn('mst_room_no_allocation','room_no'," WHERE `id` = '".$row->id_mst_room_no_allocation."'");
							...
							$datawisearrayFinal[$checkinDatearr][$FHstatusGroup][$row->id_reservations_details][$FHstatusGroup]["id_mst_room_no_allocation"]=$row->id_mst_room_no_allocation; 
...
?>
				...
				foreach($datawisearrayFinal as $dateCheckin=>$dateData){
                  ...
                  <tbody>
                    <tr>
                      <th colspan=6 style="background-color:#01B9F5; color: white;">Date: <?php echo dateformat_date($dateCheckin)?></th>
                    </tr>
					...
					foreach($dateData as $nameA	=>$dateData2){?>
						 ...
						 <tr>
                      <td colspan=12><table class="table"  style="  width:100%;">
                       <tr>
                      <th  colspan=12 style="background-color:#e6e4e4;text-align: center; color: #000;"> <?php echo ($nameA)?></th>
                    </tr>
					...
					?>
                          <tr>
                            ...
                            <td><?php echo selectColumn('fo_booking_status','name'," WHERE `id` = '".$order_data['booking_status']."'");?></td>
					...
					elseif ($order_data['no_showoff']=='1' && $order_data['checkin_user_status']=='') {
						$status =  "No showoff";
					}elseif ($order_data['no_showoff']=='0' && $order_data['id_mst_room_no_allocation']>'0') {
						$status = "Occuiped";
					}
				...
				?> <tr>
									...
									//$dateCheckin
									//debugData($BookingStatusArray[$dateCheckin]);
                      ...
                      <th  colspan=12 > </th>
						 ...
						 <tr>
                      <td colspan="5"><table class="table" >
                       <tr>
                      <th  colspan=4 style="background-color:#c2d69a;text-align: center; color: #000;"> Resevation vs Occupied with Meal Plan</th>
                    </tr>
                          <tr style=" margin-bottom: 0px;border: 1px; width:50%; text-align: center; color: #000;   background-color:#c2d69a;">
                            <th>Name</th>
							  <th>No Of Room</th>
			...
			?>				
				<tr>
                            <td><?php echo $type;?></td>
							  <td><?php echo $order_data['RoomCount']; ?></td>
                            <td><?php echo $order_data['adults']?></td> <td><?php echo $order_data['child']?></td>
					</tr>
                    ...
                    <tr>
                            <td><b>Total</b></td>
							  <td><b><?php echo $TotalRoomCount; ?></b></td>
                            <td><b><?php echo $Totaladults; ?></td> <td><b><?php echo $Totalchild?></b></td>
					</tr>
                    <?php
					}
			...
			?>				
				<tr>
					...
					?>
                           ...
                           <td colspan="8" style="width:300px" ></td>
	...
	//debugdata($BookingSummaryPlanArray);die;
...
$Filename='Resevation_vs_Occupied_with_Meal_Plan_'.$date;	
if($cronSet	=='0'){

	if($report_show==3){ //print_r($_REQUEST);die;
		
		$Filename=	"Flash Summary Report As On  ".date('d-m-Y',strtotime($ReportAsOnDate));//'nightAuditReports'.date('d-M-Y');
...
//$objWriter->save('/var/www/vhosts/app.roomstatushub.in/httpdocs/mailattach/'.$Filename.'.xls');
		...
		$savePath = '/var/www/vhosts/app.roomstatushub.in/httpdocs/mailattach/'.$Filename.'.xls';
