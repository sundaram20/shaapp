...
$channelName = selectColumn(TBL_CHANNEL_MANAGER, 'name', " WHERE `id` = '$channelId'");


// Step 2: Find channel by name
$stmt = $appConnect->prepare("SELECT * FROM `fs_channel_manager` WHERE `name` LIKE ?");
$likeName = "%$channelName%";
$stmt->bind_param("s", $likeName);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows > 0) {
    $rowChannel = $result->fetch_object();
    $id_channel = $rowChannel->id_channel;

    // Step 3: Check if fs_shop_hotel_detail row exists
    $stmtCheck = $appConnect->prepare("SELECT * FROM `fs_shop_hotel_detail` WHERE `id_app_shops` = ? AND `channel_id` = ?");
    $stmtCheck->bind_param("ii", $_SESSION['id_app_shop'], $id_channel);
    $stmtCheck->execute();
    $resultCheck = $stmtCheck->get_result();

    $status = addslashes($_POST['status']); // sanitize
    ...
    if ($resultCheck->num_rows > 0) {
        // Update existing row
        $stmtUpdate = $appConnect->prepare(
            "UPDATE `fs_shop_hotel_detail` SET `auto_sync_inv` = ?, `last_modified` = ?, `status` = ?, `last_modified_by` = ? 
             WHERE `id_app_shops` = ? AND `channel_id` = ?"
        );
        $stmtUpdate->bind_param("ssssii", $auto_sync_inv, $last_modified, $status, $last_modified_by, $_SESSION['id_app_shop'], $id_channel);
        $stmtUpdate->execute();
        //echo "Record updated successfully.";
    } else {
        // Insert new row
        $stmtInsert = $appConnect->prepare(
            "INSERT INTO `fs_shop_hotel_detail` (`id_app_shops`, `channel_id`, `auto_sync_inv`, `last_modified`, `status`, `last_modified_by`)
             VALUES (?, ?, ?, ?, ?, ?)"
        );
        $stmtInsert->bind_param("iisssi", $_SESSION['id_app_shop'], $id_channel, $auto_sync_inv, $last_modified, $status, $last_modified_by);
        $stmtInsert->execute();
        //echo "Record inserted successfully.";
    }
} else {
    //echo "No matching channel found for '$channelName'.";
}


	//print_r($_POST);print_r($_SESSION);die;
/*print_r($_POST);//die;
	echo $appShopsSql = " INSERT INTO `fs_shop_hotel_detail` SET id_app_shops = '".$_SESSION['id_app_shop']."', `auto_sync_inv`='".$_POST['auto_sync_inv']."',
							`last_modified` = '".currenDateTime()."'

							,`status` = '".addslashes($_POST['status'])."'

							,`last_modified_by` = '".$_SESSION['userId']."'";//die;
	mysqli_query($appConnect,$appShopsSql);*/
	$err = 0;
...
<div class="box-body">
  ...
  <label for="shop_id">Shop<font color="#FF0000">*</font></label>
   ...
   <?php $shopDropDown = '<select class="form-control select2" name="shop_id"  id="shop_id" >
...
<?php $channelDropDown = '<select class="form-control select2" name="channel_id" id="channel_id"  >
