...
if($_POST['Save']){
	...
	if($err == 0){//No error
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			$sql = " SELECT * FROM `".TBL_ATTRIBUTES."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' AND  `field_value` = '".addslashes(trim($_POST['field_value']))."' and `table_name` = 'mode_of_travel' ";
			$db->query($sql);
			$numRows= $db->num_rows();
			if($numRows == '0'){
			...
			$addSql = "   	INSERT INTO `".TBL_ATTRIBUTES."` SET
							...
							`table_name` = 'mode_of_travel',
							`field_name` = 'mode_of_travel_name',
			...
			if(executeSql($addSql)){
				...
				$_SESSION['successMsg'] = 'New Mode of Travel details has been added sucessfully.';
				header("location:manageModeofTravel.php?eId=".encryptor(encrypt,$lastInsertId)."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
				$_SESSION['errorMsg'] = 'Mode of Travel details has not been saved. Please make corrections below.';
			}
			}else{
				$err++;
				$_SESSION['errorMsg'] = 'Mode of Travel Master Name Already Exist.';
			}
		}
		...
		//Update Section Here
			...
			if(executeSql($editSql)){
				...
				header("location:manageModeofTravel.php?eId=".$_GET['eId']."&action=edit&page=".$_REQUEST['page']);
				exit;
			}else{
				$err++;
		...
		$err++;
		$_SESSION['errorMsg'] = 'Mode of Travel details has not been saved. Please make corrections.';
	}
}
// ----------cate---------
          ...
          <!-- general form elements -->
            ...
            <!-- form start -->  			        
			 <form name="form1"  method="post" enctype="multipart/form-data" data-parsley-validate autocomplete="off" >
              ...
              <div class="box-body">
				 ...
				 <div class="form-group">
                  <label for="name">Mode of Travel Name<font color="#FF0000">*</font></label>
                  <div class="input-group"> 
              			<div class="input-group-addon">
							<i class="fa fa-delicious"></i> 
					   	</div>
                  <input type="text" class="form-control" placeholder="Enter Mode of Travel Name" id="field_value" name="field_value" value="<?php if($_POST) echo $_POST['field_value'];else echo stripslashes($row->field_value);?>"  data-parsley-required>
			 ...
			 <div class="box-footer">                                       
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
			   <input type='button' value='Close' class="btn btn-danger" onclick='location.replace("manageModeofTravel.php"); '>
			 </div>
            </form>			
          </div>
          <!-- /.box -->
