...
<form id="saveReservationDateform" method="post"  class="saveReservationDateform" data-parsley-validate autocomplete="off">
  ...
  <div class="form-group col-sm-4">
    ...
    <?php 
      ...
      $categoryDropDown = '<select class="form-control select2" name="id_mst_company" id="id_mst_company" >
  ...
  <div class="form-group col-sm-4">
    <label for="checkin" style="float:left;">Booker By</label>
    <?php 
      
      
      $categoryDropDown = '<select class="form-control select2" name="id_mst_company_contacts" id="id_mst_company_contacts" >
	    ...
	    while($resultCat=mysqli_fetch_assoc($query)){
                    ...
                    if($row->id_mst_company_contacts == $resultCat['id']){
  ...
  <div class="form-group col-sm-9">
    <div class="box-body">
      <div class="well table-responsive" style="padding: 0px 0px;">
        <table class="table order-list1 table-hover">
          ...
          <?php 
			...
			$sqlOrderDetail = mysqli_query($connNew,"Select  `".FO_RESERVATIONS_DETAILS."`.* from `".FO_RESERVATIONS_DETAILS."` where `id_fo_reservations` = '".addslashes(encryptor(decrypt,$_REQUEST['id']))."' and  order_by_room ='".addslashes($_REQUEST['order_by_room'])."' ");
		if(mysqli_num_rows($sqlOrderDetail) >0 ){
				...
				$downArrowInc	='<a href="javascript:void(0);" onclick="fillRightInc(\''.date('Y-m-d',strtotime($checkin)).'\',\''.date('Y-m-d',strtotime($checkout)).'\',\''.$rowOrderDetail->dated.'\',\''.$rowOrderDetail->id.'\');" class="text-green input-group-addon"><i  class="arrows fa fa-angle-double-up"  style="margin-left:-4px;"></i></a>';		
					...
					?>
          <tr>
            <td style="width: 140px;"><input type="text" class="form-control parsley-error" style="width: 84px;" name="dated[]" id="dated" value="<?php echo date('d-m-Y',strtotime($rowOrderDetail->dated)); ?>" readonly=""></td>
           <td><select class="form-control parsley-error" name="reservation[<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>][<?php echo $rowOrderDetail->id;?>][roomtype]" id="roomtype_<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>" onchange="roomtypechange(this.id,this.value);" style="width: 100px;" <?php echo $readonly; ?>>
             <?php
...
$dataArr='';
	...
	while($rowneww = mysqli_fetch_object($resneww)){
        ...
        ?>
            ...
            <td><select class="form-control parsley-error" name="reservation[<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>][<?php echo $rowOrderDetail->id;?>][plan]" id="plan_<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>" onchange="getval(this.id)" style="width: 80px;" <?php echo $readonly; ?>>
               <?php 
			    $dataArrplan='';
              ...
              <td>
              <select class="form-control parsley-error" name="reservation[<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>][<?php echo $rowOrderDetail->id;?>][id_mst_room_no_allocation]" id="id_mst_room_no_allocation_<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>" onchange="getval(this.id)" style="width: 80px;" <?php echo $readonly; ?>>
               <?php 
			   $dataArrplan='';
			   $dataArrplan='<option value="">Select one</option>';
			   $selectnewp="SELECT * FROM  ".TBL_ROOMNO."
				where status='1' ";				
				$resnewp = mysqli_query($connNew,$selectnewp);				
				while($rownewp = mysqli_fetch_object($resnewp)){					
					if($rownewp->id==$rowOrderDetail->id_mst_room_no_allocation){
						$selected ='selected="selected"';
					}else{
						$selected ="";
					}					
            ...
            <td><select class="form-control parsley-error" name="reservation[<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>][<?php echo $rowOrderDetail->id;?>][adultperroom]" id="adultperroom_<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>" style="width: 60px;" <?php echo $readonly; ?> <?php echo $readonly; ?>>
              <option value="1">1</option>
              <option selected="selected" class="2">2</option>
              <option class="3">3</option>
              </select></td>
            <td><select class="form-control parsley-error" name="reservation[<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>][<?php echo $rowOrderDetail->id;?>][childbelow]" id="childbelow_<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>" style="width: 60px;" <?php echo $readonly; ?>>
                <option selected="selected" value="0">0</option>
                <option value="1">1</option>
                <option class="2">2</option>
              </select></td>
            <td><select class="form-control parsley-error" name="childabove[]" id="childabove1" style="width: 70px;" <?php echo $readonly; ?>>
                <option selected="selected" value="0">0</option>
                <option value="1">1</option>
                <option class="2">2</option>
              </select></td>
            <td style="display:inline-flex"><?php echo $topArrow;?>
              <input type="text" class="form-control parsley-error" style="width: 70px;" name="reservation[<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>][<?php echo $rowOrderDetail->id;?>][tariffperroom]" id="tariffperroom_<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>" value="<?php echo $rowOrderDetail->tariff_price_per_day_per_room; ?>"
            ...
            <td><input type="text" class="form-control parsley-error" style="width: 70px;" name="reservation[<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>][<?php echo $rowOrderDetail->id;?>][taxes]" id="taxes_<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>" value="<?php echo $rowOrderDetail->tax_per_day_per_room; ?>" readonly=""></td>
            <td style="display:inline-flex"><?php echo $topArrowInc;?>
              <input type="text" class="form-control parsley-error" style="width: 60px" name="reservation[<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>][<?php echo $rowOrderDetail->id;?>][tariffperroomtax]" id="tariffperroomtax_<?php echo date('Y-m-d',strtotime($rowOrderDetail->dated));?>"  
  ...
  <div class="modal-footer">
    <button class="btn n-btn" onclick="saveEditReservationForm();" type="button">Save</button>
    &nbsp;
