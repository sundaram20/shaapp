...
$resRoom = mysqli_query($connNew, "SELECT * FROM  ".TBL_ROOMNO." where  id_mst_room_types = '" . $id_room_type . "' and status = '1' and management_block = 'No' and room_status = '4' ");
...
while ($rowRoom = mysqli_fetch_object($resRoom)) {
 ...
 $room_no_allocation = "select * from mst_room_no_allocation where id='".$room_id."' and id_mst_room_types='".$id_room_type."'";
        ...
        if (mysqli_num_rows($room_no_allocation_query)>0) {
          ...
          $hotelRoomType.=  '<option '.$selected.' value="'.$room_no_allocation_id.'" >'.$room_no_text.'</option>';
        }
