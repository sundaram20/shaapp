...
$nightAuditDate = "'".date('d-m-Y',strtotime('+1 day',strtotime($rowNightAudit->dated)))."'";
...
$all_rooms = [];
echo $sql = "SELECT * FROM " . TBL_ROOMNO . " WHERE id_mst_room_types = '" . $id_mst_room_types . "' and management_block='No' and status = 1 GROUP BY room_no";
$room_no_allocations = mysqli_query($connNew, $sql);

while ($row = mysqli_fetch_object($room_no_allocations)) {
    $all_rooms[$row->id] = [
		'room_id' => $row->id,
		'room_no' => $row->room_no,
	];
...
$allocated_All_room_query = mysqli_query($connNew, "SELECT DISTINCT room.id,room.room_no,room.id_mst_room_types,room.room_status,resdetails.id_fo_reservations, resdetails.id_mst_guest,resdetails.id_fo_folio_to ,resdetails.id_fo_bill, resdetails.order_by_room, resdetails.child_below_5_year,resdetails.child_above_5_year,resdetails.adults_per_room, resdetails.checkin_date, resdetails.checkout_date, resdetails.checkout_status, reservation.booking_status, resdetails.`no_showoff`, resdetails.`dated`, reservation.checkout as reservation_checkout, resdetails.checkin_time FROM `mst_room_no_allocation` as room INNER JOIN fo_reservations_details as resdetails ON room.id=resdetails.id_mst_room_no_allocation INNER JOIN fo_reservations as reservation ON resdetails.id_fo_reservations=reservation.id WHERE resdetails.`no_showoff`='0' and resdetails.`dated`='".$today."' and room.id_mst_room_types = '".$id_mst_room_types."'  and room_availability = 'Reserv'");
...
$start .='
 ...
 echo "<pre>";
 print_r($allocation_room_ids);
 print_r($occupied_based_on_reservation_room_ids);
 print_r($occupied_room_ids);
print_r($all_rooms);echo "</pre>";
// echo "sdf";
// exit;
...
echo "select * from fo_reservations_details where id_mst_room_no_allocation = '".$room['room_id']."' and checkin_status = '1' and checkout_status = '0' and room_availability = 'checkout' and checkout_date is not null";
...
if (strtotime($today) == strtotime($reservation_result->checkin) ) {
...
$returnData['rr'] .='&nbsp;&nbsp;&nbsp;<button class="btn btn-primary pull-right" onClick="RoomAllocationsingleForm('.$reservation_id.','.$id_mst_room_types.');">Allocate </button>';
