...
foreach($_REQUEST['id_pos_detail'] as $porchdetailID =>$dataCode){
		//array_push($item_BillSplit,$_REQUEST['id_pos_detail'][$porchdetailID]['total']);
...
$auditquery = "SELECT * From `".TBL_INV_PURCH_DETAILS."` WHERE id = '".addslashes($_REQUEST['id_inv_detail'])."'  ";
  $auditresSQL = mysqli_query($connNew, $auditquery);	
	while($auditrow = mysqli_fetch_object($auditresSQL)){ 
	
		$c1 = $auditrow -> discount_percent;
...
/* $itemDetailSizeOf1 =	sizeof($_REQUEST['id_inv_detail']);
for($i=0;$i<$itemDetailSizeOf1;$i++){
	
        $id_item_name1 = $_REQUEST['id_inv_detail'][$i];
} */
	
	
	
	 echo   $insertPosKotDetail = "UPDATE `".TBL_PURCH_DETAILS."` SET 
