...
error_reporting(0);
function tablePaceReportfunction($Report_period,$Report_id_hotel,$Report_id_group_master,$Report_reportType,$Report_viewMonthwise,$Report_summaryReportType,$CronSet)
{
...
// if(($_REQUEST['pdf']==1 && $Report_reportType==1) || ($Report_summaryReportType == '9' && $Report_reportType==1)  || ($Report_summaryReportType == '9' && $CronSet=='1' && $Report_reportType=='1') ){
	 
	 //Team Wise  Summary PDF reportType Pickup Report
...
/*  $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_COMPANY."`.id_default_group,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_USERS."`.ids_team,
   ...
   */
	...
	`".TBL_USERS."`.name as name_executive ,`mst_team`.name as TeamName
	   FROM `".TBL_ORDERS."`  
       LEFT JOIN `".TBL_COMPANY."` ON `".TBL_ORDERS."`.id_company = `".TBL_COMPANY."`.id_company
       LEFT JOIN `".TBL_AREAS."` ON `".TBL_COMPANY."`.area = `".TBL_AREAS."`.id
       LEFT JOIN `".TBL_USERS."` ON `fs_areas_assign`.user_id=".TBL_USERS.".id  
	...
	while($rowList = mysqli_fetch_object($resultList)){
	  $companyname= selectColumn(TBL_TEAM,'name'," WHERE `id` = '".$rowList->MyOwnteam."'");
	   ...
	   $GroupName= selectColumn(TBL_GROUP_MASTER,'name'," WHERE `id` = '".$rowList->id_group."'");
		...
		$reportArray['Team Wise'][$GroupName]['confimed_revenue']=$newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue);
...
$startYear = $d["year"];
 ...
 if($Report_summaryReportType == '9'){  //Yearly Graph Conditions
...
if ( date('m') > 6 ) {
    //echo  'first'.$year = date('Y') + 1;
       ...
       /* if ( date($monthNUmers) >= 12 ) {
        ...
        if ((date($monthNUmers)!='01' || date($monthNUmers)!='02' || date($monthNUmers)!='03') && date($monthNUmers)>=04) {
            $startYr = date($checkYear);// + 1;
            $PrevstartYr = (date($checkYear));
...
if($diff==$i){
  ...
  $sqlCurrentYearMonthWise ="SELECT `fs_orders`.*,`fs_users`.name as name_executive,`fs_users`.myownteam_id as MyOwnteam,`mst_team`.id_group,`fs_company_group`.name as name_company_group, `fs_order_detail`.room_id ,
   ...
   // echo '<br><br>'.$sqlCurrentYearMonthWise;
	...
	while($rowListMonth = mysqli_fetch_object($resultCurrentYearMonthWise)){
	   // echo '<br><br><br>'.$GroupName.'=='.date('F', mktime(0, 0, 0, $startMo, 10)).'====>';
	    ...
	    $SplitnewConfirmednewTentative_revenue=($rowListMonth->confimed_revenue+$rowListMonth->tentative_revenue);
	    $SplitnewConfirmednewTentative=($rowListMonth->newConfirmed+$rowListMonth->newTentative);

	    $emptytext7 ='empty_'.$empty7++;
	    //$reportArray['Team Wise'][$GroupName]['confimed_revenue']=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
	    
		$reportArray['Team Wise'][$GroupName][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['Newroomnights']      = $SplitnewConfirmednewTentative==''?'0':$SplitnewConfirmednewTentative;
	    $reportArray['Team Wise'][$GroupName][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['NewconfimedRevenue'] = $SplitnewConfirmednewTentative_revenue==''?0:round($SplitnewConfirmednewTentative_revenue);
...
$monthArray[date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['MonthName'] = $DataMonthname.'-'.$startYr;
	...
	$RoomNightsTotal=array();
     ...
     //=======================================================================================
         if($maintitle=='Team Wise'){ 
        	 ...
        	 $contentGroup .='<tr style="vertical-align:central;text-align:"><th colspan="17" style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important"><b>'.$maintitle0ch.' Pace Report For Period '.$reportPeriod.'</b></th></tr>	';
             $contentGroup .='<tr ><th style="vertical-align:central;text-align:center;border-left:1px solid #000;background-color:#c2d69a; ">Office</th><th style="vertical-align:central;text-align:center;background-color:#c2d69a;border-left:1px solid #000;">Cancelled <br> Room Nights</th><th style="vertical-align:central;text-align:center;background-color:#c2d69a;border-left:1px solid #000;">Room Nights</th>
			 <th style="vertical-align:central;text-align:center;background-color:#c2d69a;border-left:1px solid #000;">ARR</th>
			 <th style="vertical-align:central;text-align:center;border-left:1px solid #000;background-color:#c2d69a;">Revenue (Lacs)</th>';
             
             $contentGroupHeader .='</tr>';
             $contentGroupHeader .='<tr ><th colspan="5" style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000; "></th>
			 <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000;"> Room Nights</th>
			  <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000;"> ARR</th>
			 <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000;">Revenue (Lacs)</th>
			 <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000;"> Room Nights</th>
			  <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000;"> ARR</th>
			 <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000;">Revenue (Lacs)</th>
			 <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000;">Room Nights</th>
			 ...
			 <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;border-left:1px solid #000;">Revenue (Lacs)</th>';
              ...
              $contentGroupHeader .='</tr>';
                 ...
                 foreach($GroupArray as $name => $GroupNameArray){
                     ...
                     if(strtoupper($name)!='UNIT'){
                       ...
                       //$RoomNightsTotal=array();
                        foreach($GroupNameArray  as $k1=>  $monthdata){
                            $k12=$monthdata['MonthName'];
							//echo $k1;
                           if($monthdata['MonthName']!=''){
                            
                            $SubData .= '
							<td style="text-align:center;" >'.$monthdata['Newroomnights'].'</td>
							<td style="text-align:center;" >'.round($monthdata['NewconfimedRevenue']/$monthdata['Newroomnights'],0).'</td>
							<td style="text-align:center;" >'.round($monthdata['NewconfimedRevenue']/100000,2).'</td>';
                            
                            $RoomNightsTotal[$k12]['Newroomnights']    	+=   $monthdata['Newroomnights']==''?'0':$monthdata['Newroomnights'];
                            $RoomNightsTotal[$k12]['NewconfimedRevenue']   +=   $monthdata['NewconfimedRevenue']==''?'0':$monthdata['NewconfimedRevenue'];
                            //print_r($monthdata);
                    ...
                    $contentGroupHeader .='<tr ><td style="text-align:left;">'.strtoupper($name).'</td>
					<td style="text-align:center;">'.($GroupNameArray['CancelledRoomNights']==''?'0':$GroupNameArray['CancelledRoomNights']).'</td>
					<td style="text-align:center;">'.($GroupNameArray['roomnights']==''?'0':$GroupNameArray['roomnights']).'</td>
					<td style="text-align:center;">'.round($GroupNameArray['confimed_revenue']/$GroupNameArray['roomnights'],0).'</td>
					<td style="text-align:center;">'.round($GroupNameArray['confimed_revenue']/100000,2).'</td>';
                    ...
                    $TotalTeamWisCancelledRoomNights1+=$GroupNameArray['CancelledRoomNights'];
                         ...
                         foreach($RoomNightsTotal as $keyval=> $totalValueMonth){
							...
							$arrTotal		 += round($totalValueMonth['NewconfimedRevenue']/$totalValueMonth['Newroomnights'],0);
                            $SubDataTotal .= '<td style="border-left:1px solid #000;background-color:#c2d69a;text-align:center;">'.$RoomNightsTotaldata.'</td>';
							$SubDataTotal .= '<td style="border-left:1px solid #000;background-color:#c2d69a;text-align:center;">'.$arrTotal.'</td>';
							 $SubDataTotal .= '<td style="border-left:1px solid #000;background-color:#c2d69a;text-align:center;">'.$RoomNightsTotaldata2.'</td>';
                            $RoomNightsTotaldata='';$RoomNightsTotaldata2='';
							$arrTotal='';
                             }
              foreach($monthArray as $listMonthName){  
			  
            $dynamicHeader .= '<th colspan="3" style="vertical-align:central;text-align:center;color:#000;border-left:1px solid #000;background-color:#c2d69a;">'.$listMonthName['MonthName'].'</th>';
			  }
            $contentGroup .=$dynamicHeader.$contentGroupHeader;
             ...
             $contentGroup .='<tr style="font-weight:bold;background-color:#c2d69a;color:#000;">
			 <td style="text-align:center;border-left:1px solid #000;background-color:#c2d69a;color:#000;">Total </td>
			 <td style="border-left:1px solid #000;background-color:#c2d69a;text-align:center;color:#000;">'.$TotalTeamWisCancelledRoomNights.'</td>
			 <td style="border-left:1px solid #000;background-color:#c2d69a;text-align:center;color:#000;">'.$TotalTeamWiseRoomNight.'</td>
			 <td style="border-left:1px solid #000;background-color:#c2d69a;text-align:center;color:#000;">'.round($TotalTeamWiseConfirmRoomNight/$TotalTeamWiseRoomNight,0).'</td>
			 <td style="border-left:1px solid #000;background-color:#c2d69a;text-align:center;color:#000;">'.round($TotalTeamWiseConfirmRoomNight/100000,2).'</td>'.$SubDataTotal.'</tr>';
             ...
             $contentGroup .= '</table>';
              ...
              if($UnitValueIs==1){ 
             $contentGroup .='<table class="table table-striped text-center">';  
        	 $contentGroup .='<tr style="vertical-align:central;text-align:"><th colspan="5" style="vertical-align:central;text-align:center;color:#23fff;background-color:#234f6228; font-size:16px !important"><b>UNIT '.$maintitle0ch.' Pace Report  For Period '.$reportPeriod.'</b></th></tr>	';
             $contentGroup .=    '<tr ><th style="vertical-align:central;text-align:center;color:#23000;background-color:#23c2d69a; ">Office</th><th style="vertical-align:central;text-align:center;color:#23000;background-color:#23c2d69a;">Cancelled Room Nights</th><th style="vertical-align:central;text-align:center;color:#23000;background-color:#23c2d69a;">Room Nights</th><th style="vertical-align:central;text-align:center;color:#23000;background-color:#23c2d69a;">Revenue</th><th style="vertical-align:central;text-align:center;color:#23000;background-color:#23c2d69a;">ARR</th>';
             $contentGroup .=    '</tr>';
                 $TotalTeamWiseRoomNightContribution='';
                 foreach($GroupArray as $name => $GroupNameArray){
                     if(strtoupper($name)=='UNIT'){
             ...
             $contentGroup .='<tr style="font-weight:bold;background-color:#233C8DBC;color:white;"><td style="text-align:center;background-color:#23c2d69a;">Total </td><td style="border-left:1px solid %23fff;background-color:#23c2d69a;text-align:center;">'.$TotalTeamWisCancelledRoomNights2.'</td><td style="border-left:1px solid %23fff;background-color:#23c2d69a;text-align:center;">'.$TotalTeamWiseRoomNight.'</td><td style="border-left:1px solid %23fff;background-color:#23c2d69a;text-align:center;">'.round($TotalTeamWiseConfimedRevenue/100000,2).'</td><td style="border-left:1px solid %23fff;background-color:#23c2d69a;text-align:center;">'.$SumTotalTeamWiseArray.'</td></tr>';
             $contentGroup .='<tr style="font-weight:bold;background-color:#233C8DBC;color:white;font-size:13px;"><td style="text-align:center;background-color:#23c2d69a;">Grand Total </td><td style="border-left:1px solid %23fff;background-color:#23c2d69a;text-align:center;">'.($TotalTeamWisCancelledRoomNights2+$TotalTeamWisCancelledRoomNights1).'</td><td style="border-left:1px solid %23fff;background-color:#23c2d69a;text-align:center;">'.($TotalTeamWiseRoomNight2+$TotalTeamWiseRoomNight1).'</td><td style="border-left:1px solid %23fff;background-color:#23c2d69a;text-align:center;">'.round(($TotalTeamWiseConfimedRevenue2+$TotalTeamWiseConfimedRevenue1)/100000,2).'</td><td style="border-left:1px solid %23fff;background-color:#23c2d69a;text-align:center;">'.$SumTotalTeamWiseArrayArr.'</td></tr>';
        	 $SumTotalTeamWiseArray='';
             $TotalTeamWiseConfimedRevenue='';
             $TotalTeamWiseRoomNight='';
...
$Filename=$ReportTypeMainTitle.'PickupReport_'.date("Y-m-d H:i:s");
	...
	$dompdf->stream($Filename.'.pdf', array("Attachment" => true));
}elseif($_REQUEST['pdf']==0 && $_REQUEST['CronSet']==0 && $_REQUEST['excel']==1){
            if($Report_reportType==1){
                       // $ReportTypeMainTitle ='PICKUP ';
        ...
        echo $test;die;
				...
				if($Report_reportType==1){
					// $ReportTypeMainTitle ='PICKUP ';
					$Filename='PaceReport_'.date("Y-m-d");
				}
				if($Report_reportType==2){
						$ReportTypeMainTitle ='BOB ';
						$Filename='PaceReport_'.date("Y-m-d");
