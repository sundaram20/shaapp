...
$editrow1 = mysqli_fetch_object($editSql1);
		...
		//$printgroup['print_BillSplit'][$grouparray]['SAC'][]=$resResult->SAC;
		$SAC = selectColumn(TBL_INV_ITEMS,'SAC','WHERE  id="'.$resResult->id_mst_items.'" ');
		...
		$SAC = selectColumn(TBL_INV_ITEMS,'SAC','WHERE  id="'.$resResult->id_mst_items.'" ');
			...
			//$printgroup['print_BillSplit'][$grouparray]['item_description'][]=$resResult->item_description;
			$printgroup['print_BillSplit'][$grouparray]['SAC'][]=$SAC;
...
function printPreview($printgroup){	
	...
	if($outletMobile!=''){	
				$printer.=('<div style="text-align:center;"><span>Contact:'.$outletMobile.'</span>&nbsp;&nbsp;');
			}
			

	if($outletEmail!=''){	
				$printer.=('<span>Email:'.$outletEmail.'</span>&nbsp;&nbsp;');
			}

if($outletCin!=''){			
			$printer.=('<span>CIN:'.$outletCin.'</span>');			
			}
			if($outletFssai!=''){
...
if($outletRegOfficeAddress!=''){
	...
	$printer.=('<div style="text-align:center;font=size:10px;font-weight:bold;">Reg Off. </div>');
			...
			//$printer.=(str_pad('Bill No : '.$index,25," "));
			 $printer.=("<table style='width:100%;margin:17px 0;margin-left:85px;text-align:left;font-size: 13px;font-weight: bolder;'><tr><td>");
			...
			$printer.=("</td></tr><tr><td '>");
			$printer.=(str_pad("Bill Date : <span style='font-weight:100;'>".date('d-M-Y',strtotime($printgroup[0]['doc_date'])),25,' '));
			$printer .=("</span></td><td>");
			if($details['roomNo']!=''){
            ...
            $printer .=("<span><hr style='margin:6px;border-bottom:1px solid #cccccc;'></span>");
			//$printer.=("-------------------------------------------------------------------------------------------------------------<br>");
            $printer .=("<table style='width:80%;margin-left:85px;text-align:left;font-size: 13px;'><thead><tr style='font-weight: bolder;'><th>");
		 	$printer.=(str_pad('SL ',5," "));
		 
			$printer.=(str_pad('PARTICULARS',19," "));
	       $printer .=("</th><th>");
			$printer.=(str_pad('SAC.',5," "));
           $printer .=("</th><th>");
		...
		//	$printer.=("<br>");
			...
			foreach ($details['item_id'] as $id_index => $id_item) {
				...
				$printer .=("</td><td>");
			  $printer.=(str_pad($details['SAC'][$id_index],8," "));
				$printer .=("</td><td>");
		   ...
		   $printer .=("<span><hr style='margin:6px;border-bottom:1px solid #cccccc;'></span>");
			...
			//$printer->feed();
			//$printer.=("<br>---------------------------------------------<br>");
			//$printer.="<div style='float:right;margin-right: 101px;'>";
			...
			$printer.=("Grand Total : ");
			$printer.=(str_pad(round($details['net_amount'],0),0," "));
			$printer.=("</div><br>");
