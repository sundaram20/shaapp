...
error_reporting(0);
function FunnelAndForecast($Report_period,$Report_id_hotel,$Report_id_group_master,$Report_reportType,$Report_viewMonthwise,$Report_summaryReportType,$CronSet,$ComparePeriodDate,$CurrentFinancialYearDate){
...
/**************** END ***********************/ 
 ...
 $startMo =date('n');//date("n", strtotime("-1 months"));// $d["month"];
//print_r($d);
//echo '---'.
$startYear = $yr;
$d["year"];
 ...
 if($Report_summaryReportType == '20'){  //Yearly Graph Conditions
...
if ( date('m') > 6 ) {
    //echo  'first'.$year = date('Y') + 1;
     ...
     if($i>0){   
      $startYr=date('Y',strtotime('+'.$i.' month'));
					}
//}
...
LEFT JOIN `".TBL_GROUP_MASTER."` ON `".TBL_GROUP_MASTER."`.id = `mst_team`.id_group
  ...
  // echo '<br> <br> <br> '.$sqlBudget;
   ...
   while($rowBudget = mysqli_fetch_object($resultBudget)){
       if($rowBudget->budget_qty>0 || $rowBudget->budget_month_value>0 || $rowBudget->forecast_qty>0 || $rowBudget->forecast_month_value>0  ){
       
        //debugdata($rowBudget);
        //$MyOwnteam  = selectColumn('fs_users','myownteam_id'," WHERE `id` = '".$rowBudget->id_user."'");
        $companynameBudget= selectColumn(TBL_TEAM,'name'," WHERE `id` = '".$rowBudget->MyOwnteam."'");
        
        $id_group  = selectColumn('mst_team','id_group'," WHERE `id` = '".$rowBudget->MyOwnteam."'");
        
	    $GroupNameBudget= selectColumn(TBL_GROUP_MASTER,'name'," WHERE `id` = '".$id_group."'");
	    ...
	    $reportArray['Team Wise'][$GroupNameBudget][strtolower($companynameBudget==''?$emptytext7:$companynameBudget)][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['budget_revenue']=round($rowBudget->budget_month_value,2);
	    ...
	    $reportArray['Team Wise'][$GroupNameBudget][strtolower($companynameBudget==''?$emptytext7:$companynameBudget)][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['forecast_revenue']=round($rowBudget->forecast_month_value,2);
       
        $reportArrayTotal['Team Wise'][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['Totalbudget_roomnights']            +=$rowBudget->budget_qty;
	    $reportArrayTotal['Team Wise'][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['Totalbudget_revenue']            +=round($rowBudget->budget_month_value,2);
	    $reportArrayTotal['Team Wise'][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['Totalforecast_roomnights']                   += $rowBudget->forecast_qty;
	    $reportArrayTotal['Team Wise'][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['Totalforecast_revenue']       += round($rowBudget->forecast_month_value,2);
	    ...
	    $reportArraySubTotal['Team Wise'][$GroupNameBudget][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['Totalbudget_revenue']             +=round($rowBudget->budget_month_value,2);
	    ...
	    $reportArraySubTotal['Team Wise'][$GroupNameBudget][date('F', mktime(0, 0, 0, $startMo, 10)).'-'.$startYr]['Totalforecast_revenue']       +=round($rowBudget->forecast_month_value,2);
...
if($_REQUEST['pdf']==1 || $CronSet==1){
 ...
 $content .= '<table class="table" style="width:100% !important;height:0px !important;">
						<tr>					
						  <th>
...
//debugData($monthArray);

       $contentGroup .='<table class="table table-striped text-center" border="1" style="width:100% !important;height:0px !important;">';  
        	 $contentGroup .='<tr style="vertical-align:central;text-align:">
        	 <th colspan="15"  style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important"><b>'.$maintitle0ch.'Budget And Forecast Report For Period '.$reportPeriod.'</b></th>
 ...
 foreach($monthArray as $ked=>$monthlist){
            ...
            $contentGroup .='<tr >';
                 $contentGroup .='<th colspan="15" style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important">'.$monthName.'</th>';
                  $contentGroup .='<tr >
              ...
              <th  colspan="7" style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">Revenue</th>
    ...
    foreach($reportArraySubTotal as $temaname=>$Temlistdate){
       // debugData($Temlistdate);
        foreach($Temlistdate as $temasubname=>$Temdate){
        if(strtoupper($teamGroup) ==strtoupper($temasubname)){
        
     
        foreach($Temdate as $monthViewName=>$listsubtotaldata){
            if($monthViewName==$monthName){
        ...
        //RoomNights=============================       
         ...
         <td style="text-align:center;color:#000;background-color:#c2d69a;">'.($listsubtotaldata['Totallead_roomnights']!=''?$listsubtotaldata['Totallead_roomnights']:0).'</td>
        ...
        /* $contentGroup .='
         ...
         $contentGroup .='
         <td style="text-align:center;color:#000;background-color:#c2d69a;">'.($listsubtotaldata['Totalbudget_roomnights']!=''?$listsubtotaldata['Totalbudget_roomnights']:0).'</td>
          <td style="text-align:center;color:#000;background-color:#c2d69a;">'.$TotalSubvariance_to_roomnights.'</td>
          <td style="text-align:center;color:#000;background-color:#c2d69a;">'.($listsubtotaldata['Totalforecast_roomnights']!=''?$listsubtotaldata['Totalforecast_roomnights']:0).'</td>
         ';
        //RoomNights============================= 
        //Revenue=================================
         ...
         <td style="text-align:center;color:#000;background-color:#c2d69a;">'.round($listsubtotaldata['Totallead_revenue']!=''?$listsubtotaldata['Totallead_revenue']:0,2).'</td>
         <td style="text-align:center;color:#000;background-color:#c2d69a;">'.round(round(($listsubtotaldata['Totalconfimed_revenue']+$listsubtotaldata['Totaltentative_revenue'])/100000,2)+$listsubtotaldata['Totallead_revenue'],2).'</td>
         ';
         $TotalSubFunnelrevenue=round(round(($listsubtotaldata['Totalconfimed_revenue']+$listsubtotaldata['Totaltentative_revenue'])/100000,2)+$listsubtotaldata['Totallead_revenue'],2);
         $TotalSubvariance_to_revenue=$TotalSubFunnelrevenue-round($listsubtotaldata['Totalbudget_revenue'],2);
        /* $contentGroup .='
         <td style="text-align:center;color:#000;background-color:#c2d69a;">'.$listsubtotaldata['TotalConvertion_Ratio'].'</td>';*/
         $contentGroup .='
         <td style="text-align:center;color:#000;background-color:#c2d69a;">'.($listsubtotaldata['Totalbudget_revenue']!=''?round($listsubtotaldata['Totalbudget_revenue'],2):0).'</td>
          <td style="text-align:center;color:#000;background-color:#c2d69a;">'.$TotalSubvariance_to_revenue.'</td>
          <td style="text-align:center;color:#000;background-color:#c2d69a;">'.($listsubtotaldata['Totalforecast_revenue']!=''?round($listsubtotaldata['Totalforecast_revenue'],2):0).'</td>
         ';
 ...
 foreach($subDataList1 as $TeamName=> $subDataList){
     ...
     $is=0;
      //$k='<br>';
     foreach($subDataList as $subDataMonthName=>  $subData){
           if($subDataMonthName==$monthName){
     ...
     // $sumroomnights     +=$subData['roomnights'];
      ...
      ';*/
	  $ToFunnelroomnights=round(($subData['roomnights']+$subData['newTentative']))+$subData['lead_roomnights'];
      $Tovariance_to_roomnights=$ToFunnelroomnights-$subData['budget_roomnights'];
      $contentSubGroup .='
      <td style="text-align:center;">'.($subData['lead_roomnights']!=''?$subData['lead_roomnights']:0).'</td>
      <td style="text-align:center;">'.($subData['roomnights']+$subData['newTentative']+$subData['lead_roomnights']).'</td>
      
      <td style="text-align:center;">'.($subData['budget_roomnights']!=''?$subData['budget_roomnights']:0).'</td>
      <td style="text-align:center;">'.($Tovariance_to_roomnights).'</td>
      <td style="text-align:center;">'.($subData['forecast_roomnights']!=''?$subData['forecast_roomnights']:0).'</td>';
      ...
      $contentSubGroup .='
      <td style="text-align:center;">'.round($subData['lead_revenue']!=''?$subData['lead_revenue']:0,2).'</td>
      <td style="text-align:center;">'.round($totalFannel+$subData['lead_revenue'],2).'</td>
      
      <td style="text-align:center;">'.($subData['budget_revenue']!=''?$subData['budget_revenue']:0).'</td>
      <td style="text-align:center;">'.round($Tovariance_to_revenue,2).'</td>
      <td style="text-align:center;">'.($subData['forecast_revenue']!=''?$subData['forecast_revenue']:0).'</td>';
      //Revenue=================================
...
foreach($ReTotal as $ReTotal1MonthName=>$ReTotal1){    
  
    if($ReTotal1MonthName==$monthName){
  $contentGroup .='
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.$ReTotal1['Totalroomnights'].'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.$ReTotal1['TotalnewTentative'].'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.($ReTotal1['Totallead_roomnights']!=''?$ReTotal1['Totallead_roomnights']:0).'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.($ReTotal1['Totalroomnights']+$ReTotal1['TotalnewTentative']+$ReTotal1['Totallead_roomnights']).'</th>';
     // $contentGroup .='<td style="text-align:center;background-color:#C2D69A;">'.$ReTotal1['TotalConvertion_Ratio'].'</td>';
      ...
      $contentGroup .='
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.($ReTotal1['Totalbudget_roomnights']!=''?$ReTotal1['Totalbudget_roomnights']:0).'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.$Totalvariance_to_roomnights.'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.($ReTotal1['Totalforecast_roomnights']!=''?$ReTotal1['Totalforecast_roomnights']:0).'</th>
      ';
      
      //Revenue===================================================
      $contentGroup .='
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.round($ReTotal1['Totalconfimed_revenue']/100000,2).'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.round($ReTotal1['Totaltentative_revenue']/100000,2).'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.round($ReTotal1['Totallead_revenue']!=''?$ReTotal1['Totallead_revenue']:0).'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.round(round(($ReTotal1['Totalconfimed_revenue']+$ReTotal1['Totaltentative_revenue'])/100000,2)+$ReTotal1['Totallead_revenue']).'</th>';
     // $contentGroup .='<td style="text-align:center;background-color:#C2D69A;">'.$ReTotal1['TotalRevenueConvertion_Ratio'].'</td>';
      
      $TotalTotalFunnelrevenue=round(round(($ReTotal1['Totalconfimed_revenue']+$ReTotal1['Totaltentative_revenue'])/100000,2)+$ReTotal1['Totallead_revenue']);
      $Totalvariance_to_revenue=$TotalTotalFunnelrevenue-$ReTotal1['Totalbudget_revenue'];
      $contentGroup .='
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.($ReTotal1['Totalbudget_revenue']!=''?$ReTotal1['Totalbudget_revenue']:0).'</th>
      <th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">'.$Totalvariance_to_revenue.'</th>
      <th style="text-align:center;background-color:#C2D69A;font-weight: bold;border-right: 1px solid red">'.($ReTotal1['Totalforecast_revenue']!=''?$ReTotal1['Totalforecast_revenue']:0).'</th>
      ';
      //Revenue==================================================
    }
}
...
$dompdf->get_canvas()->page_text(700, 14, "Page: {PAGE_NUM} of {PAGE_COUNT}", $font, 6, array(0,0,0));


        if($CronSet==1){
            if($Report_reportType==1){
           // $ReportTypeMainTitle ='PICKUP ';
            $Filename='BudgetAndForecast_'.date("Y-m-d");
        }
        
        
        }else{
        
        $Filename='BudgetAndForecast_'.date("Y-m-d H:i:s");
        }
...
if($Report_reportType==2){
   ...
   // echo $content;die;
    pdfGeneratorAttach($content, $Filename);
            ...
            if($Report_reportType==1){
                       // $ReportTypeMainTitle ='PICKUP ';
                        $Filename='ForecastReport-'.date("Y-m-d H:i:s").'.xls';
                    }
                    if($Report_reportType==2){
                        $ReportTypeMainTitle ='BOB ';
                         $Filename='BudgetAndForecast_'.date("Y-m-d H:i:s").'.xls';
                    }
        $test=$content;
