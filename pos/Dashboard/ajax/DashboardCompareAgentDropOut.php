...
$reportSingleArray=array();
$id_mst_hotel=$_REQUEST['id_mst_hotel'];
if($id_mst_hotel!=''){
...
$arrayRoomNightCY=array();
$arrayRevenueCY=array();
$arrayHotelName=array();
$mtdLastValues = array();
$mtdThisValues = array();
...
//$team_data_access_approved	= selectColumn(TBL_USER_LEVELS,'teamdataaccess_approved','WHERE id="'.$_SESSION['userLevel'].'" ');
	...
	if( $_SESSION['userLevel']==1){ //Yes
	
	if($_REQUEST['id_hotel']==0){}else{}	
	...
	$Report_id_group_master=$_REQUEST['id_group_master'];
	
if($Report_id_group_master != '' && $Report_id_group_master != '0' && $Report_id_group_master != '10000' ){}elseif($Report_id_group_master == '10000'){}else{}	
...
if(($_REQUEST['pdf']==1) || ($_REQUEST['summaryReportType'] == '4') || ($_REQUEST['summaryReportType'] == '61')){ //Agentwise Summary
 
 
 if($_REQUEST['reportType']==1){     
   $sql = "select pp.id_mst_country_lang,pp.id_mst_outlet,pp.doc_date,pp.kot_doc_no,ppp.id_mst_items,ppp.item_description, ppp.id as id_purch_detail,inv.item_code, pp.pax,  ppp.item_discount_amount,
SUM(CASE WHEN  DATE(pp.doc_date) BETWEEN '".date('Y-m-d',strtotime($from_book))."' AND '".date('Y-m-d',strtotime($to_book))."' THEN ROUND(pp.pax,0) ELSE 0 END ) AS newConfirmed,
		SUM(CASE WHEN  DATE(pp.doc_date) BETWEEN '".date('Y-m-d',strtotime($from_book))."' AND '".date('Y-m-d',strtotime($to_book))."' THEN ROUND(((ppp.item_amount)-ppp.item_discount_amount),0) ELSE 0 END ) AS confimed_revenue,
ppp.item_amount
FROM pos_purch pp 
LEFT JOIN pos_purch_details ppp ON ppp.id_pos_purch=pp.id 

INNER JOIN inv_items inv ON inv.id=ppp.id_mst_items 
WHERE pp.pos_bill_type='2' and pp.cancelled=0 and pp.doc_type=21 and pp.id_shop= '2'  GROUP BY  pp.id_mst_outlet,pp.id_mst_country_lang order by confimed_revenue asc"  ;
     ...
     //  
	   ...
	   $reportChartArray=array();
	while($rowList = mysqli_fetch_object($resultList)){
	  $companyname= $rowList->id_mst_country_lang=='0'?'Others':ucwords(strtolower(selectColumn(TBL_COUNTRY_LANG,'nationality'," WHERE status = '1' AND `id_lang` = '1' AND nationality!='' AND id_country= '".$rowList->id_mst_country_lang."'") ));
	  
	  $rowList->newConfirmed=  selectColumn('pos_purch','sum(pax)'," WHERE DATE(doc_date) BETWEEN '".date('Y-m-d',strtotime($from_book))."' AND '".date('Y-m-d',strtotime($to_book))."' AND pos_bill_type='2' and cancelled=0 and doc_type=21 and id_shop= '2' AND id_mst_country_lang= '".$rowList->id_mst_country_lang."' and id_mst_outlet= '".$rowList->id_mst_outlet."'");
	  array_push($NationalityNameArr,$companyname.'('.round($rowList->confimed_revenue/100000,2).')');
	  array_push($mtdRoomNationalityReportRevenue,round($rowList->confimed_revenue/100000,2));
	  //$rowList->id_mst_country_lang;//
	  
	  $outletName=  selectColumn(TBL_OUTLETS,'name'," WHERE `id` = '".$rowList->id_mst_outlet."'");
	    ...
	    // $companyname= strtolower(selectColumn(TBL_HOTELS,'name','WHERE id='.$rowList->id_hotel.' '));
	    ...
	    // $GroupName= selectColumn(TBL_GROUP_MASTER,'name'," WHERE `id` = '".$rowList->id_group."'");
		 ...
		 if($newConfirmednewTentative_revenue>0){
	    $reportArray[$outletName][strtolower($companyname==''?$emptytext7:$companyname)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	    $reportArray[$outletName][strtolower($companyname==''?$emptytext7:$companyname)]['confimed_revenue']=$newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue);
	    $reportArray[$outletName][strtolower($companyname==''?$emptytext7:$companyname)]['name_executive']=$rowList->name_executive;
	    $reportArray[$outletName][strtolower($companyname==''?$emptytext7:$companyname)]['CityPinCode']=$companyCity.$companypincode;
	    
		
		$reportSingleArray['all_confimed_revenue'] +=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
		
		
		
		
		$reportChartArray[strtolower($companyname==''?$emptytext7:$companyname)]['confimed_revenue']=$newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue);
		$reportChartArray[strtolower($companyname==''?$emptytext7:$companyname)]['nationality']=$companyname;
		$reportChartArray[strtolower($companyname==''?$emptytext7:$companyname)]['piechartName']=strtolower($companyname==''?$emptytext7:$companyname).'('.($newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue/100000,2)).')';
...
//LASTYEAR SQL START >>================================================================================================================	
 ...
 if(($_REQUEST['pdf']==1) || ($_REQUEST['summaryReportType'] == '4') || ($_REQUEST['summaryReportType'] == '61')){ 
...
if($_REQUEST['reportType']==1){
$lastYearPickUpsql ="select pp.doc_date,pp.kot_doc_no,ppp.id_mst_items,ppp.item_description, ppp.id as id_purch_detail,inv.item_code, pp.pax,  ppp.item_discount_amount,
SUM(CASE WHEN  DATE(pp.doc_date) BETWEEN '".date('Y-m-d',strtotime($Diffrence.' years',strtotime($from_book)))."' AND '".date('Y-m-d',strtotime($Diffrence.' years',strtotime($to_book)))."' THEN ROUND(pp.pax,0) ELSE 0 END ) AS newConfirmed,
		SUM(CASE WHEN  DATE(pp.doc_date) BETWEEN '".date('Y-m-d',strtotime($Diffrence.' years',strtotime($from_book)))."' AND '".date('Y-m-d',strtotime($Diffrence.' years',strtotime($to_book)))."' THEN ROUND(((ppp.item_amount)-ppp.item_discount_amount),0) ELSE 0 END ) AS confimed_revenue,
ppp.item_amount
FROM pos_purch pp 
LEFT JOIN pos_purch_details ppp ON ppp.id_pos_purch=pp.id 

INNER JOIN inv_items inv ON inv.id=ppp.id_mst_items 
WHERE pp.pos_bill_type='2' and pp.cancelled=0 and pp.doc_type=21 and pp.id_shop= '2'   GROUP BY  ppp.id_mst_items order by confimed_revenue asc" ;
...
if(($_REQUEST['pdf']==1 && $_REQUEST['summaryReportType'] == '3') || ($_REQUEST['summaryReportType'] == '61' )){   //Hotel Wise  Summary PDF PICKUP Report
   ...
   // $companyname= strtolower(selectColumn(TBL_HOTELS,'name','WHERE id='.$rowlastYearList->id_hotel.' '));
    ...
    $TaleName='Hotel Wise Source';
	  ...
	  $companyname=   strtolower(selectColumn('inv_items','name','WHERE id='.$rowlastYearList->id_mst_items.' '));//selectColumn(TBL_COMPANY,'name'," WHERE `id_company` = '".$rowlastYearList->id_company."'");
	    $exeNameArr[]=ucwords(strtolower($companyname));
    ...
    $reportArray['Top Item'][strtolower($companyname==''?$emptytext7:$companyname)]['lastYearroomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
    $reportArray['Top Item'][strtolower($companyname==''?$emptytext7:$companyname)]['lastYearconfimed_revenue']=$newConfirmednewTentative_revenue==''?0:round($newConfirmednewTentative_revenue);
    $reportArray['Top Item'][strtolower($companyname==''?$emptytext7:$companyname)]['name_executive']=$rowlastYearList->name_executive;
    $reportArray['Top Item'][strtolower($companyname==''?$emptytext7:$companyname)]['CityPinCode']=$companyCity.$companypincode;
 ...
 $reportSingleArray['all_confimed_revenue']= round($reportSingleArray['all_confimed_revenue']/100000,2);         
...
$content .= '</tr>	
	...
	$content .='<tr><th colspan="10" style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important"><b>Compare Nationality Wise For Period '.$reportPeriod.'</b></th></tr>';
    $content .='<tr>
    <th   colspan="1" style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;"> Nationality</th>
    <th colspan="3" style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;">Pax</th>
    <th colspan="3"  style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">APC</th>
    <th colspan="3"  style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">Amount</th>
    </tr>';
...
foreach($reportArray as $maintitle=>$mainDatalist){
	$content .='<tr><th colspan="10" style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important"><b> '.$maintitle.'</b></th></tr>';
...
if($subDataList['roomnights']>0 || $subDataList['lastYearroomnights']>0){
    if($subDataList['confimed_revenue']>0 || $subDataList['lastYearconfimed_revenue']>0){
    ...
    if($teamGroup!='name'){
		...
		$arrayHotelName[]=ucwords($teamGroup);
		$arrayRoomNightCY[]=$subDataList['roomnights'];
		$arrayRevenueCY[]=round($TotalConfimedRevenue/100000,2);
		 $reportSingleArray['top_confimed_revenue']+= round($TotalConfimedRevenue/100000,2);
    $content .='<tr style="">
    <td style="border:1px solid #000;text-align:left;width:30%;">'.ucwords($teamGroup).$subDataList['CityPinCode'].' </td>';
   
    $content .='<td style="border:1px solid #000;text-align:center;">'.$TotalRoomNightlastYear.'</td>
	...
	$k++;
	 ...
	 $mtdRoomNationalityReportRevenue=array();
	 $NationalityNameArr=array();
foreach($reportChartArray as $NationalityName=>$data){
	
	$arrayHotelName[]=ucwords($NationalityName);
	$arrayRevenueCY[]=round($data['confimed_revenue']/100000,2);
	$mtdRoomNationalityReportRevenue[]=round($data['confimed_revenue']/100000,2);
	$NationalityNameArr[]=$data['piechartName'];
	}
//debugData($arrayHotelName2);
if($_REQUEST['pdf']==1){
//echo $content;
//die;
    $dompdf = new DOMPDF();
...
$Filename='TopItem'.$ReportTypeMainTitle.'_Report_'.date("Y-m-d H:i:s");
	...
	$dompdf->stream($Filename.'.pdf', array("Attachment" => true));
            ...
            if($_REQUEST['reportType']==1){
                       // $ReportTypeMainTitle ='PICKUP ';
                        $Filename='TopItemCompareView-PickupRepor_'.date("Y-m-d H:i:s").'.xls';
                    }
                    if($_REQUEST['reportType']==2){
                        $ReportTypeMainTitle ='BOB ';
                         $Filename='TopItemCompareView-BobReport_'.date("Y-m-d H:i:s").'.xls';
                    }
        $test=$content;
...
function sortBySize($arrayHotelName, $arrayRevenueCY)
    {
        if ($a['size'] == $b['size']) return 0;
        return ($a['size'] < $b['size']) ? -1 : 1;

    }
$reportSingleArray['pie_top_hotel_lable'][]='To 10 Item ('.$reportSingleArray['top_confimed_revenue'].')';
$reportSingleArray['pie_top_hotel_lable'][]='Other Item ('.($reportSingleArray['all_confimed_revenue']-$reportSingleArray['top_confimed_revenue']).')';

$reportSingleArray['pie_top_hotel_data'][]=$reportSingleArray['top_confimed_revenue'];
$reportSingleArray['pie_top_hotel_data'][]=round($reportSingleArray['all_confimed_revenue']-$reportSingleArray['top_confimed_revenue'],2);

$jsonData=array();	
$jsonData['content']=$content;
$jsonData['TopHotelNameData']=$arrayHotelName;
$jsonData['TopHotelRoomNightsData']=$arrayRoomNightCY;
$jsonData['TopHotelRevenueDataCY']=$arrayRevenueCY;

//$jsonData['all_confimed_revenue']=$reportArray['all_confimed_revenue'];
//$jsonData['top_confimed_revenue']=$reportArray['top_confimed_revenue'];
$jsonData['pie_top_hotel_lable']=$NationalityNameArr;
$jsonData['pie_top_hotel_data']=$mtdRoomNationalityReportRevenue;
