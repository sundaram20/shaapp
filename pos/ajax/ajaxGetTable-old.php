...
// $is_table = true;
// $table_group = selectSql(TBL_ATTRIBUTES," where id_shop='".$_SESSION['shop']."'  AND id='".$table_group."' AND  status = '1' AND table_name ='".'table_group'."' ",'');
// if($db->num_rows2($table_group) > 0) {
//     $firstRecord = $table_group->fetch_assoc();
//     if ($firstRecord->id_table_group) {
//       $is_table = false;
//     }
// }
...
<div class="col-md-12 mt-10">
  <div class="form-group">
	...
	// PreviousOrder(this.id); ?>
                <td style="" class="btn tableviewBlockedbtn" onclick="TabeleSelect(this.id); PreviousOrderPax(this.id);" id="TableGroup_<?php echo $rowContact->id ?>_<?php echo $rowContact->field_value;?>"><?php echo $rowContact->field_value;?><?php echo $GuestName!=''?$Title.' '.$GuestName.' '.$lastName:'' ?></td>
                <?php }else{ ?>
                <td style=" background-color:#c6574b;" class="btn tableviewBlockedbtn"   id="TableGroup_<?php echo $rowContact->id ?>_<?php echo $rowContact->field_value;?>"><?php echo $rowContact->field_value;?><?php echo $GuestName!=''?$Title.' '.$GuestName.' '.$lastName:'' ?></td>
                <?php }
...

  {
    ?>
                ...
                <td style="flex-direction : column;" class="btn tableviewbtn" onclick="TabeleSelect(this.id); PreviousOrderPax(this.id);" id="TableGroup_<?php echo $rowContact->id ?>_<?php echo $rowContact->field_value;?>"><div style="font-size : 1.4rem!important; white-space : break-spaces; "><?php echo $rowContact->field_value;?></div><div style="font-size : 1.2rem!important; white-space : break-spaces; font-weight : 400;"><?php echo $GuestName!=''?$Title.' '.$GuestName.' '.$lastName:'' ?></div></td>                                
                <?php }
    ...
    <form name="listingForm" action="" method="post">
      ...
      <div class="box-body table-responsive" style="padding:0px;">
        <table id="myTableTest" class="table table-fixed table-striped table-bordered dataTabletest no-footer" cellspacing="0"  >
          <tbody>
            <tr>
              <td style="width:100%;"> No data Available </td>
            </tr>
          </tbody>
        </table>
      </div>
