 ...
 $auditquery = "SELECT * From `".TBL_PURCH_DETAILS."` WHERE id = '".addslashes($_REQUEST['id_pos_detail'][$porchdetailID]['id_inv_detail'])."'  ";
  $auditresSQL = mysqli_query($connNew, $auditquery);	
	while($auditrow = mysqli_fetch_object($auditresSQL)){
		...
		$c2 = $auditrow -> id_pos_purch;
		 $c3 = $auditrow -> id_mst_items_details;
		 $c4 = $auditrow -> id_mst_items;
		 $c5 = $auditrow -> qty;
		
		
		 $auditquery1 = "SELECT * From `".TBL_PURCH."` WHERE id = '".$_REQUEST['id_posbilling']."'  ";
		  $auditresSQL1 = mysqli_query($connNew, $auditquery1);	
			$auditrow1 = mysqli_fetch_object($auditresSQL1);
				$bill = $auditrow1 -> mdoc_no;
				$sc = $auditrow1 -> sc_reverse;
				$sc_amonut = $auditrow1 -> sc_charges_net_amount;
	 
		if($c1 != $_REQUEST['id_pos_detail'][$porchdetailID]['discount']){
			if($_REQUEST['id_pos_detail'][$porchdetailID]['discount']!=''){
				
				if($c3 == '0'){
					$item_name = selectColumn(TBL_INV_ITEMS,'name','WHERE  id="'.$c4.'" ');
					$ch1 = $item_name." Disc% Details Changed from " .   $c1 ." - to - " . $_REQUEST['id_pos_detail'][$porchdetailID]['discount'] . " in Bill no ". $bill ;
				}else{
					$item_name = selectColumn(TBL_INV_ITEMS_DETAILS,'name','WHERE  id="'.$c3.'" ').'/';
					$item_name1 = selectColumn(TBL_INV_ITEMS,'name','WHERE  id="'.$c4.'" ');
					$ch1 = $item_name.$item_name1 ." Disc% Details Changed from " .   $c1 ." - to - " . $_REQUEST['id_pos_detail'][$porchdetailID]['discount'] . " in Bill no ". $bill ;
				}	
		   
			}
		}
		if($c5 != $_REQUEST['id_pos_detail'][$porchdetailID]['item_qty']){
			if($_REQUEST['id_pos_detail'][$porchdetailID]['item_qty']!=''){
				
				if($c3 == '0'){
					$item_name = selectColumn(TBL_INV_ITEMS,'name','WHERE  id="'.$c4.'" ');
					$ch2 = $item_name. " Qty Details Changed from " .   $c5 ." - to - " . $_REQUEST['id_pos_detail'][$porchdetailID]['item_qty'] . " in Bill no ". $bill ;
				}else{
					$item_name = selectColumn(TBL_INV_ITEMS_DETAILS,'name','WHERE  id="'.$c3.'" ').'/';
					$item_name1 = selectColumn(TBL_INV_ITEMS,'name','WHERE  id="'.$c4.'" ');
					$ch2 = $item_name.$item_name1 ." Qty Details Changed from " .   $c5 ." - to - " . $_REQUEST['id_pos_detail'][$porchdetailID]['item_qty'] . " in Bill no ". $bill ;
				}	
		   
			}
		}
		if($sc=='1'){
			$ch3 = "Sc Charges Amount ".$sc_amonut." Added to Bill No ".$bill ;
		}
	}
   ...
   executeSql($insertPosKotDetail);
	 ...
	 $auditeditSql = " INSERT audit_trail SET 
							...
							`changes` =  '".addslashes($ch1).",".addslashes($ch2).",".addslashes($ch3)."',
