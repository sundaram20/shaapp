...
<div class="form-group col-xs-3 col-md-1 col-sm-2 c-box">
	<a onclick="reload" href="kds.php?submenu=<?php echo $_REQUEST['submenu'] ?>&session=<?php echo $_REQUEST['session'] ?> ">
	  <div class="btn c-btn btn-block" style="margin-right:15px" ><i class="fa-solid  fa-arrows-rotate fa-1x"></i> Refresh</div >
	 </a>
</div>
<!--
   <div class="form-group col-xs-3 col-md-1 col-sm-2 c-box">
...
<script>  
	...
	/*function search_form(clicked_id){
		...
		function item_test(clicked_id){ alert('123424');
			if(clicked_id.length==0){
				document.getElementById("search_div").innerHTML='';
				return;
			}else{
...
<form name="searchForm" id="searchForm">
	...
	<div class="kw-filter" >
	 ...
	 ?>
        ...
        <div class="kw-status">
	  <?php $categoryDropDown = '<select class="form-control select2" name="outlet" id="outlet" data-parsley-required data-parsley-errors-container="#outletError" >
...
function viewkds(){
	...
	$.ajax({
			type: "POST",
			url: "ajax/ajaxLoadKotDigitalSystem.php?search_name=" + search_name + "&outlet=" + outlet + "&id_attribute_table_search=" + table + "&order=" + order + "&fstatus=" + fstatus,
...
function listItemWise(id_mst_items){	
	...
	function search_form(clicked_id){
		...
		function ChangeCookStatus(clicked_id, qty, old_qty){
			...
			$.ajax({
			type: "POST",
			url: 'ajax/ajaxChangeCookStatus.php?qty=' + qty + '&id=' + pos_purch_details_id + '&status=' + status + '&old_qty=' + old_qty,			 
			success: function (data) {
...
function CheckServerStatus(mdoc_no,id,id_mst_outlet){
	...
	$.ajax({
			type: "POST",
			url: 'ajax/ajaxUpdateServeStatus.php?mdoc_no='+mdoc_no+'&id='+id+'&id_mst_outlet='+id_mst_outlet,			 
			success: function (data) {
				 result = JSON.parse(data);
				 if(result.status == 0){
						  $("#serve_status_"+mdoc_no).prop("checked",false);
				alert(result.Msg);
				 $("#serve_status_"+mdoc_no).prop("checked",false);
			   }else{
