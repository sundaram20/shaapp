...
$auditquery = "SELECT * From `".TBL_INV_ITEMS."` WHERE id = '".addslashes(encryptor(decrypt,$_POST[eId]))."' ";
   $auditresSQL = mysqli_query($connNew, $auditquery);	
	...
	if($_POST['id_mst_charges_purchase_local'] ==''){
		...
		$pol = $_POST['id_mst_charges_purchase_local'];
	}
	...
	if($_POST['id_mst_charges_purchase_interstate'] ==''){
		$poi = '0';
	}else{
		$poi = $_POST['id_mst_charges_purchase_interstate'];
	}
...
if($_POST['purchase_rate'] ==''){
	$pr = '0';
}else{
	$pr = $_POST['purchase_rate'];
}	
	if($c16 != $pr){
...
if($_POST['display_order'] ==''){
	...
	if($c31 != $_POST['sac']){
		 $ch31 ="sac Details Changed from " .   $c31 ." - to - " . $_POST['sac']." in Item Name ".$c3;
	}
...
$item = $auditrow1 -> name;
				...
				if($enabled[$i] != $_REQUEST['enabled'][$i]){
					...
					while($row12 = $db->fetch_object()){ 
						$books_start_from = $row12->books_start_from;  
						$books_start_from=date('Y-m-d',strtotime($books_start_from . "-1 days"));
					} 
...
/*if($submenu=='88'){
	$sqlResult1 = "SELECT * FROM ".TBL_ATTRIBUTES." WHERE table_name = 'items_type' AND field_category IN ('Menu Items','Both') AND id_shop = ".$_SESSION['shop'] ." ";
	...
	$QuerySQL1	=	mysqli_query($connNew,$sqlResult1);
		...
		$outLetTypeFilter	="AND outlettype='1'";
		
}else if($submenu=='235' || $submenu=='308'){ //Ingredients
	...
	$QuerySQL1	=	mysqli_query($connNew,$sqlResult1);
		...
		$outLetTypeFilter	="AND  outlettype='2'";
	...
	$QuerySQL1	=	mysqli_query($connNew,$sqlResult1);
		...
		$outLetTypeFilter	="AND outlettype='3'";
...
}else if($submenu=='289' || $submenu=='309'){ //Appliances Item
...
<?php }  ?>				
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                 ...
		                 <?php $categoryDropDown = '<select class="form-control select2" name="id_mst_attributes_unit_main" id="id_mst_attributes_unit_main" data-parsley-required onchange="changeFunc()" style="width:100%" '.$disabled1.' >
		            	...
		            	<div class="form-group col-xs-12 col-md-6 col-sm-2">
		                 ...
		                 <?php $categoryDropDown = '<select class="form-control select2" name="id_mst_attributes_unit_alt" id="id_mst_attributes_unit_alt" data-parsley-required onchange="changeFunc()" style="width:100%" '.$disabled1.'>
									...
									if($db->num_rows2($resCat1)){
										while($resultCat1 = $db->fetch_object2($resCat1)){
											...
											$categoryDropDown .= '<option selected="selected" value="'.$resultCat1->id.'">'.$resultCat1->name.'</option>';
											  ...
											  $resCat = selectSql(mst_outlets," where id_shop='".$_SESSION['shop']."' AND  status = '1' and id NOT IN (4,6) $outLetTypeFilter",'');
		        ...
		        <div class="card text-dark bg-light">
	              	...
	              	<div class="row">	 
		                ...
		                <div class="form-group col-xs-12 col-md-6 col-sm-2">
		                 ...
		                 <?php $categoryDropDown = '<select class="form-control select2 parsley-error" name="id_mst_charges_sales_local" id="id_mst_charges_sales_local" onchange="salesaccountlocal()" style="width:100%" '.$validateErr.'>
...
<?php } ?>		
		        ...
		        <div class="card text-dark bg-light">
	              	...
	              	<div class="row">
		                ...
		                <div class="form-group col-md-3 col-sm-3">
		                  <label for="item_enable_desc_billing">Open Item :</label>
                ...
                <div class="row">
						...
						<div class="form-group col-md-4 col-sm-3">
						  	<label for="item_production_item">Food Processed item:</label>
