      ...
      <div class="row">
        <div class="col-xs-12"> 
          ...
          <div class="box">
			...
			$sqll = "SELECT p.id_fo_bill, p.id,p.mdoc_no, p.date_created AS purch_date, d.id AS detail_id, d.item_description, d.qty, d.item_amount, d.item_sgst_percent, d.item_cgst_percent, d.item_sgst_amount, d.item_cgst_amount FROM pos_purch AS p INNER JOIN pos_purch_details AS d ON p.id = d.id_pos_purch WHERE DATE(d.date_created) >= '2025-09-22' AND d.item_sgst_percent = '9.00' and p.pos_bill_type='2' ORDER BY d.id_pos_purch ASC, d.date_created DESC; 
";
			  /*
    SELECT * 
    FROM pos_purch_details 
    WHERE DATE(date_created) >= '2025-09-22' 
      AND item_sgst_percent = '9.00' and id_pos_purch ='1055'
    ORDER BY date_created DESC
*/
$rec = mysqli_query($connNew, $sqll);

while ($query4data = mysqli_fetch_object($rec)) {
    ...
    // Extract fields
    $id                    = $query4data->detail_id; // assuming `id` is primary key
    $qty                   = $query4data->qty;
    $item_amount           = $query4data->item_amount*$query4data->qty;

    // New SGST/CGST values
    $id_mst_charges_sgst   = 1;
    $id_mst_charges_cgst   = 38;

    $item_sgst_percent     = 2.50;
    $item_cgst_percent     = 2.50;

    // Calculate new SGST/CGST amounts
    $item_sgst_amount      = ($item_amount * $item_sgst_percent) / 100;
    $item_cgst_amount      = ($item_amount * $item_cgst_percent) / 100;

    // ✅ Update query
    echo '<br>'.$updateSql = "
        UPDATE pos_purch_details 
        SET 
            id_mst_charges_sgst = '$id_mst_charges_sgst',
            id_mst_charges_cgst = '$id_mst_charges_cgst',
            item_sgst_percent   = '$item_sgst_percent',
            item_sgst_amount    = '$item_sgst_amount',
            item_cgst_percent   = '$item_cgst_percent',
            item_cgst_amount    = '$item_cgst_amount'
        WHERE id = '$id'
    ";
    ...
    if (mysqli_query($connNew, $updateSql)) {
        ...
        echo "❌ Error updating ID $id: " . mysqli_error($connNew) . "<br>";
    }
}
			?>
...
//echo '<br><pre>';print_r($row);echo '</pre>';
                   ...
                   <td><?php echo (($_REQUEST['page']-1)*$setpage)+$counter++;?>.&nbsp;</td>
					...
					if($amount[$purch_id][1]==0){
                      ...
                      <?php }else{ ?>
                      <a href="kotbilling.php?updateid=<?=encryptor(encrypt, $row->id);?>&submenu=<?php echo $_REQUEST['submenu']; ?>"  >
                      <img src="../images/view2.png" style="cursor:pointer;height: 20px;" title=" View / Edit "  /></a> 
                        &nbsp;&nbsp; 
                      <?php } ?>
