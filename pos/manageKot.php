...
<?php include_once("../includes/header.php")?>
 ...
 //  $statuscase = " AND kot_status='Pending'" ;
    ...
    $statuscase .= " AND DATE(`doc_date`) BETWEEN '".date('Y-m-d',strtotime($startDate))."' And '".date('Y-m-d',strtotime($endDate))."'";
		...
		//$DefaultDate = " AND DATE(ppp.date_created)>>'".$POSCurrentDate."'";
		
		
      $DefaultDate .= " AND DATE(pp.`doc_date`) BETWEEN '".date('Y-m-d',strtotime('-1 days'))."' And '".date('Y-m-d')."'";
  }
...
( select pp.*,  
 ...
 $searchDocumentType $DefaultDate
...
cellColor('A1:I1','254061');
 $objPHPExcel->getActiveSheet()->getStyle('A1:I1')->getAlignment()->applyFromArray(
...
$styleArrayWhite = array(
		...
		$objPHPExcel->getActiveSheet()->getStyle('A1:I1')->applyFromArray($styleArrayWhite);
...
$objPHPExcel->getActiveSheet($sheetOneIndex)->getColumnDimension('D')->setWidth(20);								
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue($head_cntr++.'1', 'Status');
					...
					->setCellValue('I' . $counter, $status);
					

                      $objPHPExcel->getActiveSheet()->getStyle('A' .  $counter.':I'. $counter)->applyFromArray($styleThinBlackBorderOutline);
	...
	header('Content-Disposition: attachment;filename="KitchenOrderTicket'.$datep.'.xls"');
...
die;
  ...
  <div class="btn-group"> <a type="button"  title="KOT Table View" class="btn n-btn pull-right" href="pendingkots.php?submenu=178" > <i class="fas fa-table"></i> KOT</a> </div> 
    ...
    <div class="box box-default">
        ...
        <div class="box-header with-border">
          ...
          <div class="btn-group  pull-right"> <a type="button" class="btn n-btn" href="managePosKot.php?submenu=<?php echo $_GET['submenu'] ?>&session=<?php echo $_GET['session'] ?>" >Add <?php echo currentNavigation()['submenu']; ?></a> 
            ...
            <div class="row">
              ...
              elseif($_REQUEST['status'] == 'Billed'){
               ...
               // $selected2 = 'selected="selected"';
				...
				<option   '.$selected3.' value="">All</option>				
				<option  '.$selected2.' value="Pending">Pending</option>
				<option '.$selected0.' value="Billed">Billed</option>
				<option '.$selected1.' value="Cancelled">Cancelled</option>
 ...
 //$resCat = selectSql("pos_purch_details WHERE qty-adj_qty>0 AND id_pos_purch  IN  (SELECT id FROM pos_purch WHERE pos_bill_type= '1')");
                    ...
                    <td>
                      ...
                      <img  height="20px" src="../images/<?php echo $stausicon;?>" style="cursor:pointer;" title="<?php echo $Imgtitle; ?> "/></a>
                      &nbsp;&nbsp;  </td>
                     <?php }else{
					?>
                      ...
                      <img height="20px" src="../images/<?php echo $stausicon;?>" style="cursor:pointer;" title="<?php echo $Imgtitle; ?> "/></a>&nbsp;&nbsp; 
                     <a href="printKotPreview.php?printPreviewid=<?=encryptor(encrypt, $row->id);?>&submenu=<?php echo $_GET['submenu']; ?>&session=<?php echo $_GET['session']; ?>" ><img src="../images/preview.png" style="cursor:pointer;height:20px;" title="Page Preview "  /></a>&nbsp;&nbsp;
                      <?php if($row->kot_status=='Pending'){
						 // echo checkUserLevelPermission($_SESSION['userLevel'], TBL_PURCH,'edit');
						  //if(checkUserLevelPermissionButton($_SESSION['userLevel'],TBL_PURCH,'edit')=='1'){
		 $UserAccessStatus	= checkUserLevelPermissionButton($_SESSION['userLevel'],TBL_PURCH,'status','managekot.php');					  ?>
                       
                       <a class="showSingle" onClick="ajaxKOTcancel(<?php echo $row->id;?>,'<?php echo $row->mdoc_no;?>','<?php echo $UserAccessStatus;?>');" style="cursor:pointer;"><img src="../images/cash-register.png" style="cursor:pointer;width:20px; height:20px;" title=" Cancel KOT "   /></a>&nbsp;&nbsp;
                      <?php //} 
					  }  ?>
...
function ajaxKOTcancel(posid,mdoc_no,UserAccessStatus){
